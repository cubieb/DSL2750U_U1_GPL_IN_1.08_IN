...
static ulong load_serial_bin (ulong offset, unsigned int *file_size)
{
	...
	*file_size = k_recv();
	...
	flush_cache (offset, (unsigned long)(*file_size));
...
int do_save_serial_image (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if((argc == 2) && (strcmp(argv[1],"-b") == 0))
		{
		uppav.tail.img_type = TYPE_BOOTLOADER;
		uppav.tail.image_len = file_size;
		argc = argc - 1;
		}
	offset = CONFIG_LOADADDR;
	printf ("## Ready for image (kermit) download to 0x%08lX...\n", offset);
	addr = load_serial_bin (offset, &file_size);
	if((addr == ~0) || (file_size <= 0))
		{
	...
	ret = flash_update((void *)addr, file_size, -1, &uppav);
	if( ret != ERROR_OK )
		{
		rcode = 0;
		goto out;
