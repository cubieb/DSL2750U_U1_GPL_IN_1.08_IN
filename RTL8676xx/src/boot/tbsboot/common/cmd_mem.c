...
extern void flash_perror (int err);
...
int display_line_data(int *size_p,ulong *addr_p,ulong addr_tmp,ulong *length_p)
{
	ulong nbytes, linebytes;
	u_char	*cp;
	int i,rc=0;
	
	nbytes = (*length_p) * (*size_p);
	...
	do {
		...
		if(addr_tmp>=CONFIG_FLASH_BASE&&addr_tmp<=(CONFIG_FLASH_BASE+info->size))
			printf("%08lx:", addr_tmp);
		else
			printf("%08lx:", (*addr_p));
		linebytes = (nbytes>DISP_LINE_LEN)?DISP_LINE_LEN:nbytes;


		for (i=0; i<linebytes; i+= (*size_p)) {
			if ((*size_p) == 4) {
				printf(" %08x", (*uip++ = *((uint *)(*addr_p))));
			} else if ((*size_p) == 2) {
				printf(" %04x", (*usp++ = *((ushort *)(*addr_p))));
			} else {
				printf(" %02x", (*ucp++ = *((u_char *)(*addr_p))));
			}
			(*addr_p) += (*size_p);
			addr_tmp+=(*size_p);
		}
...
int do_mem_md ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	int rc = 0;
	int *size_p;
	ulong *addr_p,*length_p;
#ifdef SPI_FLASH
	ulong	i;
	unsigned char *p;
	ulong p_tmp,addr_tmp;
#endif
...
#ifdef SPI_FLASH
		if((*addr_p)>=CONFIG_FLASH_BASE&&(*addr_p)<=(CONFIG_FLASH_BASE+info->size))
		{
			if(length>=1024)
			{
				for(i=0;i<=(length/1024);i++)
				{
					flash_read_buf( info , ( unsigned char * ) addr , &p , 1024 );
				}
			}
			else
			{
				...
				p_tmp = ( ulong ) p;
				addr_tmp=(*addr_p);				
				flash_read_buf( info , ( unsigned char * ) addr , &p , length * size );				
				rc=display_line_data(size_p, &p_tmp, addr_tmp,length_p);							
				dp_last_addr = addr+length*size;
				dp_last_length = *length_p;
				dp_last_size = *size_p;
				free(p);
...
#endif	
	rc=display_line_data(size_p, addr_p, 0,length_p);
	dp_last_addr = *addr_p;
	dp_last_length = *length_p;
	dp_last_size = *size_p;
