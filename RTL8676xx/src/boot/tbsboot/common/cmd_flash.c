...
int do_fltest(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	unsigned long val, incr, pattern, block_size, cycles;
	int ret_val = 0;
	unsigned char *buf;
	int i;

	block_size = flash_info[0].start[1] - flash_info[0].start[0];
	...
	if(argc > 2)
		...
		{
		end = (ulong *)(CONFIG_LOADADDR + block_size);
		}
	if(argc > 3)
		{
		...
		{
		pattern = get_timer(0) & 0x0FFFFFFF;
		}
	
	for(cycles = 1; cycles < 0xFFFFFFFF; cycles++)
		{
		incr = get_timer(0) & 0x0FFFFFFF;
		printf("\rWriting and reading Pattern=%#x cycle=%#x\n", pattern, cycles);
		for(addr = start, val = pattern; addr < end; addr++)
			{
			*addr = val;
			val  += incr;
			}
		for(i = 3; i < info->sector_count; i++)
			{
			if(ctrlc())
				{
				printf("\n");
			...
			addr = (volatile unsigned long *)flash_info[0].start[i];
			printf("%03d", i);
			flash_erase(info, i, i);
			flash_write((char *)start, (unsigned long)addr, (unsigned long)block_size);
			printf("\b\b\b\b\b");
			buf = (unsigned char *)(CONFIG_DRAM_BASE + KERNEL_TMP_OFFSET);
			memset(buf, 0, block_size);
			flash_read_buf(info, (void *)addr, &buf, (int)block_size);
			if(0 != memcmp(buf, (char *)start, block_size))
				{
				printf("\nFlash test error @ block %d\n", i);
				ret_val = 1;
				}
			}
		 pattern = get_timer(0) & 0x0FFFFFFF;
...
U_BOOT_CMD(
	fltest, CFG_MAXARGS, CMD_FLAG_REPEAT, do_fltest,
	"fltest  - test FLASH chip by write and read data\n",
	"\n    - Flash chip write and read test\n"
	"fltest N\n    - test FLASH memory bank # N\n"
);
