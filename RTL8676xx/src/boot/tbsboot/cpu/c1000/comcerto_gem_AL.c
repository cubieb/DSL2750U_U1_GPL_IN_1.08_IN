...
{
	*(volatile u32 *)(gemac->registers + GEM_NET_CONTROL) |= (GEM_TX_START | GEM_TX_EN);
	return 0;
}
...
{
	*(volatile u32 *)(gemac->registers + GEM_NET_CONTROL) |= GEM_TX_HALT;
}
...
{
	return ((*(volatile u32 *)(gemac->registers + GEM_TX_STATUS) & GEM_TX_GO) == GEM_TX_GO);
}
...
{
	*(volatile u32 *)(gemac->registers + GEM_NET_CONTROL) |= GEM_RX_EN;
	return 0;
}
...
{
	return ((*(volatile u32 *)(gemac->registers + GEM_NET_CONTROL) & GEM_RX_EN) == GEM_RX_EN);
}
...
void gem_set_loop(struct gemac_dev *gemac,	/* Pointer to the device structure. */
	...
	switch (gem_loop) {
	case LB_LOCAL:
		*(volatile u32 *)(gemac->registers + GEM_NET_CONTROL) &= (~GEM_LB_PHY);
		*(volatile u32 *)(gemac->registers + GEM_NET_CONTROL) |= (GEM_LB_MAC);
		break;
	case LB_EXT:
		*(volatile u32 *)(gemac->registers + GEM_NET_CONTROL) &= (~GEM_LB_MAC);
		*(volatile u32 *)(gemac->registers + GEM_NET_CONTROL) |= (GEM_LB_PHY);
		break;
	default:
		*(volatile u32 *)(gemac->registers + GEM_NET_CONTROL) &= (~(GEM_LB_MAC | GEM_LB_PHY));
	}
}
...
{
	*(volatile u32 *)(gemac->registers + GEM_NET_CONTROL) |= GEM_MDIO_EN;
}
...
{
	*(volatile u32 *)(gemac->registers + GEM_NET_CONTROL) |= GEM_STATS_WR_EN;
}
...
{
	*(volatile u32 *)(gemac->registers + GEM_NET_CONTROL) |= GEM_STATS_INC;
}
...
{
	*(volatile u32 *)(gemac->registers + GEM_NET_CONTROL) |= GEM_STATS_CLR;
}
...
{
	*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) |= GEM_RX_PAUSE_EN;
}
...
{
	*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) &= (~GEM_RX_NO_PAUSE);
}
...
{
	*(volatile u32 *)(gemac->registers + GEM_NET_CONTROL) |= GEM_RX_NO_PAUSE;
}
...
{
	*(volatile u32 *)(gemac->registers + GEM_NET_CONTROL) |= GEM_TX_0Q_PAUSE;
}
...
{
	*(volatile u32 *)(gemac->registers + GEM_NET_CONTROL) |= GEM_TX_PAUSE;
}
...
{
	*(volatile u32 *)(gemac->registers + GEM_TX_PAUSE_QUANT) = gem_pause;
}
...
{
	return *(volatile u32 *)(gemac->registers + GEM_TX_PAUSE_QUANT);
}
...
{
	*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) |= GEM_RETRY_TEST;
}
...
{
	*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) |= GEM_EAM_EN;
}
...
{
	*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) &= (~GEM_RX_NO_FCS);
}
...
{
	*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) |= GEM_FRAME_1536;
}
...
{
	*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) |= GEM_UNICAST_EN;
}
...
{
	*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) |= GEM_MULTICAST_EN;
}
...
{
	*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) &= (~GEM_NO_BROADCAST);
}
...
{
	*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) |= GEM_COPY_ALL;
}
...
{
	*(volatile u32 *)(gemac->baseaddr + GEM_CFG) |= GEM_CONF_DUPLEX_GEM_FULL;
}
...
void gem_set_speed(struct gemac_dev *gemac, MAC_SPEED gem_speed)
{
	switch (gem_speed) {
	case SPEED_10M:
		*(volatile u32 *)(gemac->baseaddr + GEM_CFG) = (*(volatile u32 *)(gemac->baseaddr + GEM_CFG) & ~GEM_CONF_SPEED_MASK) | GEM_CONF_SPEED_SEL_GEM | GEM_CONF_SPEED_GEM_10M;
		*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) &= (~GEM_PCS_SEL);
		break;
	case SPEED_100M:
		*(volatile u32 *)(gemac->baseaddr + GEM_CFG) = (*(volatile u32 *)(gemac->baseaddr + GEM_CFG) & ~GEM_CONF_SPEED_MASK) | GEM_CONF_SPEED_SEL_GEM | GEM_CONF_SPEED_GEM_100M;
		*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) &= (~GEM_PCS_SEL);
		break;
	case SPEED_1000M:
		*(volatile u32 *)(gemac->baseaddr + GEM_CFG) = (*(volatile u32 *)(gemac->baseaddr + GEM_CFG) & ~GEM_CONF_SPEED_MASK) | GEM_CONF_SPEED_SEL_GEM | GEM_CONF_SPEED_GEM_1G;
		*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) &= (~GEM_PCS_SEL);
		break;
...
MAC_SPEED gem_get_speed(struct gemac_dev *gemac)
{
	...
	if (*(volatile u32 *)(gemac->baseaddr + GEM_CFG) & GEM_CONF_SPEED_SEL_GEM) {
		if (*(volatile u32 *)(gemac->baseaddr + GEM_CFG) & GEM_CONF_SPEED_GEM_1G) {
			if (*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) & GEM_PCS_SEL)
				return (SPEED_1000M_PCS);
			else
				return (SPEED_1000M);
		} else {
			if (*(volatile u32 *)(gemac->baseaddr + GEM_CFG) & GEM_CONF_SPEED_GEM_100M)
				return (SPEED_100M);
			else
				return (SPEED_10M);
		}
	} else {
		if (*(volatile u32 *)(gemac->baseaddr + GEM_CFG) & GEM_CONF_SPEED_PHY_1G) {
			if (*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) & GEM_PCS_SEL)
				return (SPEED_1000M_PCS);
			else
				return (SPEED_1000M);
		} else {
			if (*(volatile u32 *)(gemac->baseaddr + GEM_CFG) & GEM_CONF_SPEED_PHY_100M)
				return (SPEED_100M);
			else
				return (SPEED_10M);
		}
...
void gem_set_ahb_width(struct gemac_dev *gemac, MAC_AHB_WIDTH gem_buswidth)
{
	switch (gem_buswidth) {
	case AMBA_AHB_128:
		*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) |= GEM_AHB_WIDTH1;
		break;
	case AMBA_AHB_64:
		*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) &= (~GEM_AHB_WIDTH1);
		*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) |= GEM_AHB_WIDTH0;
		break;
	case AMBA_AHB_32:
	default:
		*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) &= ((~GEM_AHB_WIDTH1) & (~GEM_AHB_WIDTH0));
	}
}
...
{
	if (*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) & GEM_AHB_WIDTH1) {
		return AMBA_AHB_128;
	} else {
		if (*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) & GEM_AHB_WIDTH0) {
			return AMBA_AHB_64;
		} else {
			return AMBA_AHB_32;
		}
...
void gem_set_mdc_div(struct gemac_dev *gemac, MAC_MDC_DIV gem_mdcdiv)
{
	switch (gem_mdcdiv) {
	case MDC_DIV_8:
		*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) &= ((~GEM_MDC_DIV2) & (~GEM_MDC_DIV1) & (~GEM_MDC_DIV0));
		break;
	case MDC_DIV_16:
		*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) &= ((~GEM_MDC_DIV2) & (~GEM_MDC_DIV1));
		*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) |= GEM_MDC_DIV0;
		break;
	case MDC_DIV_32:
		*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) &= ((~GEM_MDC_DIV2) & (~GEM_MDC_DIV0));
		*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) |= GEM_MDC_DIV1;
		break;
	case MDC_DIV_48:
		*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) |= (GEM_MDC_DIV1 | GEM_MDC_DIV0);
		*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) &= (~GEM_MDC_DIV2);
		break;
	case MDC_DIV_64:
		*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) &= ((~GEM_MDC_DIV1) & (~GEM_MDC_DIV0));
		*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) |= GEM_MDC_DIV2;
...
{
	if (*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) & GEM_MDC_DIV2) {
		if (*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) & GEM_MDC_DIV1) {
			if (*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) & GEM_MDC_DIV0) {
				return MDC_DIV_224;
			} else {
				return MDC_DIV_128;
			}
...
{
	*(volatile u32 *)(gemac->registers + GEM_NET_CONTROL) |= GEM_READ_SNAP;
}
...
{
	*(volatile u32 *)(gemac->registers + GEM_NET_CONTROL) |= GEM_TAKE_SNAP;
}
...
{
	return ((*(volatile u32 *)(gemac->registers + GEM_NET_STATUS) & GEM_LINK_STATUS) == GEM_LINK_STATUS);
}
...
{
	return ((*(volatile u32 *)(gemac->phyregisters + GEM_NET_STATUS) & GEM_PHY_IDLE) == GEM_PHY_IDLE);
}
...
{
	return (*(volatile u32 *)(gemac->registers + GEM_TX_STATUS));
}
...
void gem_reset_tx_stat(struct gemac_dev *gemac,	/* Pointer to device structure. */
	...
	*(volatile u32 *)(gemac->registers + GEM_TX_STATUS) |= rst_status;
}
...
{
	return (*(volatile u32 *)(gemac->registers + GEM_RX_STATUS));
}
...
void gem_reset_rx_stat(struct gemac_dev *gemac,	/* Pointer to device structure. */
	...
	*(volatile u32 *)(gemac->registers + GEM_RX_STATUS) |= rst_status;
}
...
{
	*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) |= GEM_RX_JUMBO;
}
...
{
	*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) |= GEM_VLAN_ONLY;
}
...
{
	return (*(volatile u32 *)(gemac->registers + GEM_IRQ_STATUS));
}
...
{
	*(volatile u32 *)(gemac->registers + GEM_IRQ_STATUS) |= irq_status;
}
...
{
	*(volatile u32 *)(gemac->registers + GEM_IRQ_ENABLE) |= irq_en;
}
...
{
	*(volatile u32 *)(gemac->registers + GEM_IRQ_DISABLE) |= irq_mask;
}
...
{
	return (*(volatile u32 *)(gemac->registers + GEM_IRQ_MASK));
}
...
int gem_phy_man_wr(struct gemac_dev *gemac, u8 phy_addr, u8 reg_addr, u32 data) {
	...
	if ((phy_addr > 0x1F) || (reg_addr > 0x1F) || (data > 0xFFFF)) {
		...
		*(volatile u32 *)(gemac->phyregisters + GEM_PHY_MAN) = write_data;
		return 0;
	}
}
...
{
	return (*(volatile u32 *)(gemac->phyregisters + GEM_PHY_MAN) & 0xFFFF);
}
...
{
	return (*(volatile u32 *)(gemac->registers + GEM_RX_PAUSE_TIME));
}
...
void gem_set_id_check1(struct gemac_dev *gemac, u32 id_check) {
	*(volatile u32 *)(gemac->registers + GEM_ID_CHECK1) = (id_check & 0xFFFF);
}

void gem_set_id_check2(struct gemac_dev *gemac, u32 id_check) {
	*(volatile u32 *)(gemac->registers + GEM_ID_CHECK2) = (id_check & 0xFFFF);
}

void gem_set_id_check3(struct gemac_dev *gemac, u32 id_check) {
	*(volatile u32 *)(gemac->registers + GEM_ID_CHECK3) = (id_check & 0xFFFF);
}

void gem_set_id_check4(struct gemac_dev *gemac, u32 id_check) {
	*(volatile u32 *)(gemac->registers + GEM_ID_CHECK4) = (id_check & 0xFFFF);
}
...
{
	return (*(volatile u32 *)(gemac->registers + GEM_ID_CHECK1));
}

u32 gem_get_id_check2(struct gemac_dev *gemac)
{
	return (*(volatile u32 *)(gemac->registers + GEM_ID_CHECK2));
}

u32 gem_get_id_check3(struct gemac_dev *gemac)
{
	return (*(volatile u32 *)(gemac->registers + GEM_ID_CHECK3));
}

u32 gem_get_id_check4(struct gemac_dev *gemac)
{
	return (*(volatile u32 *)(gemac->registers + GEM_ID_CHECK4));
}
...
int gem_register_wr(struct gemac_dev *gemac, u32 reg_addr, u32 data)
{
	if (reg_addr > GEM_REG_TOP) {
		...
		*(volatile u32 *)(gemac->registers + reg_addr) = data;
		return 0;
	}
}
...
{
	return *(volatile u32 *)(gemac->registers + reg_addr);
}
...
int gem_reset(struct gemac_dev *gemac)
{
	...
	*(volatile u32 *)(gemac->registers + GEM_NET_CONTROL) = GEM_STATS_CLR | GEM_MDIO_ENABLED;
#if defined(CONFIG_COMCERTO_100)
	*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) = GEM_DEF_DUPLEX;
#else
	*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) = 0;
#endif
	...
	*(volatile u32 *)(gemac->registers + GEM_TX_STATUS) = 0xFFFFFFFF;
	*(volatile u32 *)(gemac->registers + GEM_RX_QPTR) = 0x00000000;
#if defined(CONFIG_COMCERTO_100)
	*(volatile u32 *)(gemac->registers + GEM_TX_QPTR) = 0x00000000;
#endif
	*(volatile u32 *)(gemac->registers + GEM_RX_STATUS) = 0xFFFFFFFF;
	*(volatile u32 *)(gemac->registers + GEM_IRQ_DISABLE) = 0xFFFFFFFF;
	*(volatile u32 *)(gemac->registers + GEM_IRQ_STATUS) = 0x00000000;
	...
	*(volatile u32 *)(gemac->registers + GEM_ID_CHECK1) = 0x00000000;
	*(volatile u32 *)(gemac->registers + GEM_ID_CHECK2) = 0x00000000;
	*(volatile u32 *)(gemac->registers + GEM_ID_CHECK3) = 0x00000000;
	*(volatile u32 *)(gemac->registers + GEM_ID_CHECK4) = 0x00000000;

	/* Now read back values and return if not correct. */
	if ((*(volatile u32 *)(gemac->registers + GEM_ID_CHECK1) != 0x00000000) ||
	    (*(volatile u32 *)(gemac->registers + GEM_ID_CHECK2) != 0x00000000) ||
	    (*(volatile u32 *)(gemac->registers + GEM_ID_CHECK3) != 0x00000000) ||
	    (*(volatile u32 *)(gemac->registers + GEM_ID_CHECK4) != 0x00000000) ||
	    ...
	    (*(volatile u32 *)(gemac->registers + GEM_IRQ_STATUS) != 0x00000000) ||
	    (*(volatile u32 *)(gemac->registers + GEM_IRQ_MASK) != 0x0003FFFF) ||
	    (*(volatile u32 *)(gemac->registers + GEM_RX_STATUS) != 0x00000000) ||
#if defined(CONFIG_COMCERTO_100)
	    (*(volatile u32 *)(gemac->registers + GEM_TX_QPTR) != 0x00000000) ||
#endif
	    (*(volatile u32 *)(gemac->registers + GEM_RX_QPTR) != 0x00000000) ||
	    (*(volatile u32 *)(gemac->registers + GEM_TX_STATUS) != 0x00000000)
	    ) {
		return -1;
	} else if (((*(volatile u32 *)(gemac->registers + GEM_NET_CONTROL) & GEM_MDIO_EN) != (0x00000000 | GEM_MDIO_ENABLED)) ||
		...
		for (loop_i = 0; loop_i < stats_length; loop_i++) {
			if (*(volatile u32 *)(gemac->registers + GEM_OCT_TX_BOT + 4 * loop_i) != 0x00000000) {
				return -1;
			}
		}
...
{
	*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) |= GEM_RX_LEN_CHK;
}
...
int gem_enet_addr_byte_mac(u8 *enet_byte_addr, MAC_ADDR *enet_addr)
{
	if ((enet_byte_addr == NULL) || (enet_addr == NULL)) {
		...
		enet_addr->bottom = enet_byte_addr[0] | (enet_byte_addr[1] << 8) | (enet_byte_addr[2] << 16) | (enet_byte_addr[3] << 24);
