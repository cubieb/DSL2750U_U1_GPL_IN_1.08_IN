...
#define	regbase 0xB8010000	//0xB8018000

#define NIC_ID0				(VPint(regbase+0x0000))		/* Interrupt Mitigation register */
#define NIC_ID1				(VPint(regbase+0x0004))		/* Interrupt Mask Register */
#define MAR0				(VPint(regbase+0x0008))
#define MAR4				(VPint(regbase+0x000C))
#define CR					(VPchar(regbase+0x003B))		/* Control register */
#define EIMR				(VPint16(regbase+0x003C))			/* Interrupt Status */
#define EISR				(VPint16(regbase+0x003E))				/* Interrupt Mask of MAC */
#define TCR					(VPint(regbase+0x0040))		/* Type Match 0 Register */
#define RCR					(VPint(regbase+0x0044))		/* Interrupt Status Register */
#define MSR					(VPchar(regbase+0x0058))		/* Media Independent Interface Access Register */
#define MIIAR 				(VPint(regbase+0x005C))
#define TxFDP1				(VPint(regbase+0x1300))			/* Interrupt Routing */
#define TxCDO1				(VPint16(regbase+0x1304))		/* Ethernet ID */
#define RxFDP				(VPint(regbase+0x13F0))		/* Starting Address of Tx Descriptor */
#define RxCDO				(VPint16(regbase+0x13F4))		/* Starting Address of Rx Descriptor */
#define RxRingSize			(VPchar(regbase+0x13F6))		/* Type Match 3 Regsiter */
#define EthrntRxCPU_Des_Num	(VPchar(regbase+0x1430))		/* Type Match 2 Register */
#define Rx_Pse_Des_Thres	(VPchar(regbase+0x1432))		/* Type Match 1 Register */
#define IO_CMD				(VPint(regbase+0x1434))		/* Multicast register */
