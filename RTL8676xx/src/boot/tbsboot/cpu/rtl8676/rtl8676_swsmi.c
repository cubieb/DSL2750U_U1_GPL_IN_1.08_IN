...
unsigned int smi_SCK;        /* GPIO used for SMI Clock Generation */
unsigned int smi_SDA;        /* GPIO used for SMI Data signal */
unsigned int smi_RST;     /* GPIO used for reset swtich */
...
void _smi_writeBit(u16 signal, u32 bitLen)
{
    for( ; bitLen > 0; bitLen--)
    {
        CLK_DURATION(DELAY);
...
void _smi_readBit(u32 bitLen, u32 *rData)
{
    u32 u;
...
//smi implement for rtl8367b
extern int rtl8651_setAsicEthernetPHYReg(unsigned int phyId, unsigned int regId, unsigned int wData);
extern int rtl8651_getAsicEthernetPHYReg(unsigned int phyId, unsigned int regId, unsigned int *rData);
int RL6000_read(unsigned int addr, unsigned int *data)
{
	/* Write address control code to register 31 */
	rtl8651_setAsicEthernetPHYReg(0,MDC_MDIO_CTRL0_REG,MDC_MDIO_ADDR_OP);
	/* Write address to register 23 */
	rtl8651_setAsicEthernetPHYReg(0,MDC_MDIO_ADDRESS_REG,(addr&0xffff));
	/* Write read control code to register 21 */
	rtl8651_setAsicEthernetPHYReg(0,MDC_MDIO_CTRL1_REG,0001);
	//Read data from register 25
	rtl8651_getAsicEthernetPHYReg(0,MDC_MDIO_DATA_READ_REG,data);
	return 0;
}
int RL6000_write(unsigned int addr, unsigned int data)
{
	/* Write address control code to register 31 */
	rtl8651_setAsicEthernetPHYReg(0,MDC_MDIO_CTRL0_REG,MDC_MDIO_ADDR_OP);
	/* Write address to register 23 */
	...
	rtl8651_setAsicEthernetPHYReg(0,MDC_MDIO_DATA_WRITE_REG,(data&0xffff));
	/* Write data control code to register 21 */
	rtl8651_setAsicEthernetPHYReg(0,MDC_MDIO_CTRL1_REG,0x0003);
	return 0;
}
...
{
#if 1
	RL6000_read(mAddrs, rData);
	return RT_ERR_OK;
#elif defined(MDC_MDIO_OPERATION)
