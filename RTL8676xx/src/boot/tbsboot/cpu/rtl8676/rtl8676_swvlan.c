...
#include "rtl8676_swtable.h"

extern void tableAccessForeword(u32, u32, void *);
extern int swTable_addEntry(u32 tableType, u32 eidx, void *entryContent_P);
extern int swTable_modifyEntry(u32 tableType, u32 eidx, void *entryContent_P);
extern int swTable_forceAddEntry(u32 tableType, u32 eidx, void *entryContent_P);
extern int swTable_readEntry(u32 tableType, u32 eidx, void *entryContent_P);
 ...
 */
#if 0
int swCore_aclCreate(u32 idx, rtl_acl_param_t * rule)
{
	acl_table_t    entry;
...
int swCore_netifCreate(u32 idx, rtl_netif_param_t * param)
{
    ...
    // disable interrupt
    // if you want to use "-O" flag, must disable interrupt before swTable_readEntry();
    ...
    if ( swTable_addEntry(TYPE_NETINTERFACE_TABLE, idx, &entryContent) == 0 )
        return 0;
	else        /* There might be something wrong */
		return -1;
}
...
int vlanTable_create(u32 vid, rtl_vlan_param_t * param)
{
    ...
    // disable interrupt
    // if you want to use "-O" flag, must disalbe interrupt before swTable_readEntry();
    temp = lx4180_ReadStatus();
    if (0 != (temp&0x1)){
	    temp2 = temp&0xfffffffe;
	    lx4180_WriteStatus(temp2);
...
int vlanTable_destroy(u32 vid)
{
    ...
    memset(&entryContent, 0, sizeof(vlan_table_t));
...
int vlanTable_setStpStatusOfAllPorts(u32 vid, u32 STPStatus)
{
