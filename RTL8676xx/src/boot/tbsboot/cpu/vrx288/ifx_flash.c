...
#include <common.h>
#include "../../bootstart/cpu/vrx288/ifx_mips.h"

#ifdef CFG_VRX288_FLASH_CFI_DRIVER
/*
#ifdef CONFIG_AMAZON
	#define FLASH_DELAY 	{int i; \
				for(i=0;i<800;i++) \
					*((volatile u32 *)CONFIG_DRAM_BASE_UNCACHE); \
				}
#else
	#define FLASH_DELAY
#endif
...
#if 0// CFG_MONITOR_BASE >= CONFIG_FLASH_BASE    // TEXT_BASE >= 0xB3000000
...
//	printf("type is %08lx\n", addr[1] & 0xff); 	//joelin 10/06/2004 flash type
//	printf("type is %08lx\n ", addr[0] & 0xff); 	//joelin 10/06/2004 flash type
//		asm("SYNC");	 
	switch (addr[1] & 0xff) {
...
int	flash_erase (flash_info_t *info, int s_first, int s_last)
{
	...
	for (sect=s_first; sect<=s_last; ++sect) {
		if (info->protect[sect]) {
			printf("Skipping protected sector %d\n", sect);
			prot++;
		}
	}
