...
debug("phy setup\n");
    ...
    if (!foundPhy) {
        debug("No PHY's configured for this ethUnit\n");
        return 0; /* No PHY's configured for this ethUnit */
    }
    ...
    for (phyUnit=0; (phyUnit < ATHR_PHY_MAX) /*&& (timeout > 0) */; phyUnit++) {
        ...
        for (;;) {
            ...
            if (ATHR_RESET_DONE(phyHwStatus)) {
               debug("Port %d, Neg Success\n", phyUnit);
                break;
            }
            if (timeout == 0) {
                debug("Port %d, Negogiation timeout\n", phyUnit);
                break;
            }
            if (--timeout == 0) {
               debug("Port %d, Negogiation timeout\n", phyUnit);
    ...
    for (phyUnit=0; phyUnit < ATHR_PHY_MAX; phyUnit++) {
        ...
        if (athrs26_phy_is_link_alive(phyUnit)) {
            liveLinks++;
            ATHR_IS_PHY_ALIVE(phyUnit) = 1;
        } else {
            ATHR_IS_PHY_ALIVE(phyUnit) = 0;
        }

        debug("eth%d: Phy Specific Status=%4.4x\n",
            ethUnit, 
