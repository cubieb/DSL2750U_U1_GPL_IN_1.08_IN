...
#ifdef	CFG_FLASH_CFI_DRIVER_RT3052
...
static int flash_toggle (flash_info_t * info, flash_sect_t sect, uint offset, uchar cmd,char *prompt);
...
unsigned long flash_init (void)
{
	...
	if(size)
	{
		i=find_sector(&flash_info[0],0xbe200000);
		printf("will erase sector from %d to %d",i,i+2);
		flash_write(0xbe000000, 0xbe200000,0x30000);
		flash_erase(&flash_info[0],i,i+2);
	}
#endif
	info=&flash_info[0];
	return (size);
 ...
 */
static int flash_is_busy (flash_info_t * info, flash_sect_t sect,char *prompt)
{
	int retval;
	...
	case CFI_CMDSET_AMD_STANDARD:
	case CFI_CMDSET_AMD_EXTENDED:
		retval = flash_toggle (info, sect, 0, AMD_STATUS_TOGGLE,prompt);
		break;
	default:
		retval = 0;
	}
...
#endif
#else
tout  = CFG_FLASH_STATE_DISPLAY_TOUT;
#endif
	/* Wait for command completion */

	start = get_timer (0);
	while (flash_is_busy (info, sector, prompt)) {
				...
				cfiword_t cword)
{
	...
	{
		case FLASH_CFI_8BIT:
			while( cptr.cp[0] != cword.c );					
				break;
		case FLASH_CFI_16BIT:
			while( cptr.wp[0] != cword.w );
				break;
		case FLASH_CFI_32BIT:
			while( cptr.lp[0] != cword.l );
				break;
		case FLASH_CFI_64BIT:
			while( cptr.llp[0] != cword.ll );
				break;
	}
