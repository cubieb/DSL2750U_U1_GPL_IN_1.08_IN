...
#include "rtl8196x/rtl_errno.h"
#include "rtl8196x/loader.h"  //wei edit
#include "rtl8196x/config.h"
...
#define tick_Delay10ms(x) { int i=x; while(i--) delay_msec(1); }
...
int32 rtl8651_getAsicEthernetPHYReg(uint32 phyId, uint32 regId, uint32 *rData)
{
	...
	tick_Delay10ms(1);   //wei add, for 8196C_test chip patch. mdio data read will delay 1 mdc clock.
#endif
...
*/
	...
	printf("Set GPHY_ROM_BIST_LOOP OK\n");
}
#endif
...
void Setting_RTL8196C_PHY_REV_B(void)
{
	int i=0;
	for(i=0; i<5; i++)
		REG32(PCRP0+i*4) |= (EnForceMode);
...
//	ProbeP5GigaPHYChip();
	...
	REG32(PITCR) = REG32(PITCR) & 0xFFFFF3FF; //configure port 5 to be a MII interface
	 	...
	 	printf("Set 8198 Switch Port5 as MII PHY mode OK\n");
		...
		REG32(PCRP5) = 0 | (0x10<<ExtPHYID_OFFSET) |
	 	...
	 	printf("swcore.c:Set 8198 Switch Port5 as GMII MAC mode OK\n");
