...
*       None
*************************************************************************/
int32 swNic_send(volatile void *output, uint32 len)
{
    struct pktHdr * pPkthdr;
    //uint8 pktbuf[2048];
    ...
    /* Copy Packet Content */
    memcpy(pktbuf_alligned, (void *)output, len);
	
    ASSERT_CSP( ((int32) txPkthdrRing[0][currTxPkthdrDescIndex] & DESC_OWNED_BIT) == DESC_RISC_OWNED );
    /* Fetch packet header from Tx ring */
    pPkthdr = (struct pktHdr *) ((int32) txPkthdrRing[0][currTxPkthdrDescIndex] & ~(DESC_OWNED_BIT | DESC_WRAP));
...
void swNic_txDone(void)
{
	...
	while (txPktDoneDescIndex != currTxPkthdrDescIndex)
		{
	    if((*(volatile uint32 *)&txPkthdrRing[0][txPktDoneDescIndex] & DESC_OWNED_BIT) == DESC_RISC_OWNED )
			{										
#ifdef CONFIG_AUTO_IDENTIFY
			if (REG32(REVR) == RTL8196C_REVISION_A)
           		txPkthdrRing[0][txPktDoneDescIndex]=txPkthdrRing_BAK[0][txPktDoneDescIndex];	    
#endif		    
		    pPkthdr = (struct pktHdr *)((int32) txPkthdrRing[0][txPktDoneDescIndex] & ~(DESC_OWNED_BIT | DESC_WRAP));
                 ...
                 uint32 clusterSize)
{
    ...
    ASSERT_CSP( (uint32) pPkthdrList & 0x0fffffff );
     memset((void *)pPkthdrList,0,((totalRxPkthdrRingCnt + totalTxPkthdrRingCnt) * sizeof(struct pktHdr))) ;             
    /* Allocate mbufs */
    ...
    memset((void *)pMbufList,0,((rxMbufRingCnt + totalTxPkthdrRingCnt) * sizeof(struct mBuf))) ;             
#if 1                  
    /* Allocate clusters */
	...
	memset((void *)pClusterList,0,(rxMbufRingCnt * size_of_cluster + 8 - 1+2*rxMbufRingCnt));	
