...
#define RC_TEST { if (Buffer == BufferLim) \
  { SizeT size; InCallback->Read(InCallback, &Buffer, &size); \
...
void LzmaReadCompressed(void *object, const unsigned char **buffer, SizeT *size)
{
...
int uncompressLZMA (Bytef *ptrDest, Bytef *ptrSrc)
{/* We use two 32-bit integers to construct 64-bit integer for file size.
	...
	int waitEOS = 1; 
	int i;
	/* waitEOS = 1, if there is no uncompressed size in headers, so decoder will wait EOS (End of Stream Marker) in compressed stream */
  ...
  if(LzmaDecodeProperties(&state.Properties, properties) != LZMA_RESULT_OK)
    return Z_DATA_ERROR;
#if 0
  state.Probs = (CProb *)malloc(LzmaGetNumProbs(&state.Properties) * sizeof(CProb));
#else
	state.Probs = (CProb *)(CONFIG_TEXT_BASE + HEAP_OFFSET);
#endif
  #ifdef _LZMA_OUT_READ
