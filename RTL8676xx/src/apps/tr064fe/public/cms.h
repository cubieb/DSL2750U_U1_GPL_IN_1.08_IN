...
typedef enum
{
   ...
   CMSRET_TW_VLAN_ID_REPEAT    = 1,     /**Vlan id repeat. */
   CMSRET_TW_VLAN_ID_MAX       = 2,     /**The max vlan number. */
   CMSRET_TW_VLAN_ID_NOTEQUAL  = 3,     /**The PVID of access port is not equal vlan id. */ 
   CMSRET_TW_VLAN_DHCP_CONFLICT  = 4,   /** dhcp address pool conflict */
...
typedef enum
{
   LOCAL_RESET_TO_DEFAULT=1,
   REMOTE_RESET_TO_DEFAULT=2,
   OLD_REMOTE_RESET_TO_DEFAULT=3, /* for CT old criterion */
} ResetToDefaultType;
