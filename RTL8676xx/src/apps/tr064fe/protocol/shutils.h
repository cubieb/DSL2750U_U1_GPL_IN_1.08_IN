 ...
 *
 * $Id: shutils.h,v 1.15.20.1 2003/10/16 21:26:01 mthawani Exp $
 */
...
{
	cmsUtl_strcpy(buf, s1);
	cmsUtl_strcat(buf, s2);
	return buf;
}	
...
#define foreach(word, wordlist, next) \
	for (next = &wordlist[strspn(wordlist, " ")], \
	     cmsUtl_strncpy(word, next, sizeof(word)), \
	     word[strcspn(word, " ")] = '\0', \
...
extern int cmsUtl_strcasecmp(const char *s1, const char *s2); 
