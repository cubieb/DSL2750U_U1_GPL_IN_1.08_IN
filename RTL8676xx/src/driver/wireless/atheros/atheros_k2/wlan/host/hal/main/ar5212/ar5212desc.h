 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/carriersrc/branches/magpie_host_dev/split_src/host/hal/main/ar5212/ar5212desc.h#5 $
 */
 ...
 */
adf_os_packed_pre struct ar5212_desc {
	...
	union {
		struct {		/* xmit format */
			u_int32_t	ctl2;	/* DMA control 2 */
			u_int32_t	ctl3;	/* DMA control 3 */
			u_int32_t	status0;/* DMA status 0 */
			u_int32_t	status1;/* DMA status 1 */
		} tx;
		struct {		/* recv format */
			u_int32_t	status0;/* DMA status 0 */
			u_int32_t	status1;/* DMA status 1 */
...
#define TXCTL_NUMWORDS(ah)    4
#define TXSTATUS_OFFSET(ah)   6
#define TXSTATUS_NUMWORDS(ah) 2

#define RXCTL_OFFSET(ah)      2
#define RXCTL_NUMWORDS(ah)    2
#define RXSTATUS_OFFSET(ah)   4
#define RXSTATUS_NUMWORDS(ah) 2  
