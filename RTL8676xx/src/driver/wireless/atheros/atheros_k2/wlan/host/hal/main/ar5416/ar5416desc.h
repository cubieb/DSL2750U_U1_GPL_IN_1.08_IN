...
#define TXCTL_NUMWORDS(ah)    (AR_SREV_OWL_20_OR_LATER(ah) ? 12 : 8)
#define TXSTATUS_OFFSET(ah)   (AR_SREV_OWL_20_OR_LATER(ah) ? 14 : 10)
...
#define RXSTATUS_RATE(ah, ads)  (AR_SREV_OWL_20_OR_LATER(ah) ?  		\
                                            MS(ads->ds_rxstatus0, AR_RxRate) :		\
					    (ads->ds_rxstatus3 >> 2) & 0xFF)
#define RXSTATUS_DUPLICATE(ah, ads)  (AR_SREV_OWL_20_OR_LATER(ah) ?	\
...
extern  HAL_BOOL ar5416SetTxQueueProps(struct ath_hal *ah, a_int32_t q,
        const HAL_TXQ_INFO *qInfo);
extern  HAL_BOOL ar5416GetTxQueueProps(struct ath_hal *ah, a_int32_t q,
        HAL_TXQ_INFO *qInfo);
extern  a_int32_t ar5416SetupTxQueue(struct ath_hal *ah, HAL_TX_QUEUE type,
        const HAL_TXQ_INFO *qInfo);
extern  HAL_BOOL ar5416ReleaseTxQueue(struct ath_hal *ah, a_uint32_t q);
extern  HAL_BOOL ar5416ResetTxQueue(struct ath_hal *ah, a_uint32_t q);
...
extern void ar5416Set11nRifsBurstMiddle(struct ath_hal *ah, struct ath_desc *ds);
extern void ar5416Set11nRifsBurstLast(struct ath_hal *ah, struct ath_desc *ds);
extern void ar5416Clr11nRifsBurst(struct ath_hal *ah, struct ath_desc *ds);
extern void ar5416Set11nAggrRifsBurst(struct ath_hal *ah, struct ath_desc *ds);	
/* RX common functions */
