 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/carriersrc/release/1.3.1/src/madwifi/hal/main/ar5416/ar5416.h#3 $
 */
...
typedef struct ModalEepHeader {
    ...
    u_int8_t   db_ch1;                                // 1 
    u_int8_t   lna_cntl_0 : 5,                        // 1 -> xlna cntl + fem band select pin cntl + localbias bit (merlin)
               local_bias : 1,                        // enable local bias
               lna_cntl_1 : 2;                              
    u_int8_t   futureModalMerlin;                     // 1
    u_int16_t  xpaBiasLvlFreq[3];                     // 3
    u_int8_t   futureModal[6];                        // 6 , old value:22
...
struct ath_hal_5416 {
    ...
    u_int32_t   ah_totalPowerMeasI[AR5416_MAX_CHAINS];
    u_int32_t   ah_totalPowerMeasQ[AR5416_MAX_CHAINS];
    int32_t     ah_totalIqCorrMeas[AR5416_MAX_CHAINS];
    u_int16_t   ah_iqCalSamples;
...
extern  HAL_BOOL ar5416SetEifsMask(struct ath_hal *, u_int);
extern  u_int ar5416GetEifsMask(struct ath_hal *);
extern  HAL_BOOL ar5416SetEifsDur(struct ath_hal *, u_int);
extern  u_int ar5416GetEifsDur(struct ath_hal *);
...
extern  const int16_t ar5416GetRateTxPower(struct ath_hal *ah, u_int mode,
                                           u_int8_t rate_index, u_int8_t chainmask);
extern  void ar5416InitRateTxPower(struct ath_hal *ah, u_int mode, HAL_HT *ht, 
                                   int ht40Correction, int16_t powerPerRate[], u_int8_t chainmask);
...
extern void ar5416XrEnable(struct ath_hal *ah);
extern void ar5416XrDisable(struct ath_hal *ah);
extern HAL_RFGAIN ar5416GetRfgain(struct ath_hal *ah);
extern HAL_BOOL ar5416SetDecompMask(struct ath_hal *ah, u_int16_t keyidx, int en);
...
extern HAL_DFS_PULSE * ar5416GetDfsRadars(struct ath_hal *ah,
                                          ...
                                          int *numb5radars,
                                          HAL_PHYERR_PARAM *pe);
extern HAL_BOOL ar5416HaveRadarEvent(struct ath_hal *ah);
extern HAL_BOOL ar5416ProcessRadarEvent(struct ath_hal *ah, HAL_CHANNEL *chan);
extern u_int32_t ar5416CheckRadarNol(struct ath_hal *ah, HAL_CHANNEL *chans, 
                                     u_int32_t nchans);
extern HAL_CHANNEL* ar5416GetExtensionChannel(struct ath_hal *ah, void *ht);
