 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/carriersrc/release/1.3.1/src/madwifi/hal/main/ah_internal.h#1 $
 */
...
struct ath_hal_private {
    ...
    HAL_BOOL	(*ah_eepromRead)(struct ath_hal *, u_int, u_int16_t *);
	HAL_BOOL	(*ah_eepromWrite)(struct ath_hal *, u_int, u_int16_t);
	...
	HAL_BOOL 	(*ah_setTransmitPower)(struct ath_hal *, 
					HAL_CHANNEL_INTERNAL *, u_int16_t *);
	HAL_BOOL	(*ah_getEepromCapabilityInfo)(struct ath_hal *, u_int16_t *);
	HAL_BOOL	(*ah_getEepromNoiseFloorThresh)(struct ath_hal *,
					const HAL_CHANNEL_INTERNAL *, int16_t *);
    HAL_BOOL    (*ah_gpioCfgOutput)(struct ath_hal *, u_int32_t gpio, HAL_GPIO_OUTPUT_MUX_TYPE halSignalType);
...
extern  int ath_hal_desc_tpc;
extern  int ath_hal_rifs_enable;
...
/* Math interfaces */
extern u_int32_t __ahdecl ath_hal_sqrt(u_int32_t x);

/* allocate and free memory */
extern  void * __ahdecl ath_hal_malloc(size_t);
extern  void __ahdecl ath_hal_free(void *);

/* common debugging interfaces */
#ifdef AH_DEBUG
extern  int ath_hal_debug;
extern  void __ahdecl HALDEBUG(struct ath_hal *ah, const char* fmt, ...)
    __printflike(2,3);
extern  void __ahdecl HALDEBUGn(struct ath_hal *ah, u_int level, const char* fmt, ...)
    __printflike(3,4);
#else
