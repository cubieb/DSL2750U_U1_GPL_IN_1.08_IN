...
#define	DFS_MAX_RADAR_OVERLAP		6		/* Max number of overlapping filters */
...
struct dfs_nolelem {
    ...
    struct timer_list   nol_timer;      /* per element NOL timer */
};
...
struct dfs_bin5radars {
	...
	HAL_DFS_BIN5PULSE br_pulse;		/* Original info about bin5 pulse */
};
...
struct ath_dfs {
    ...
    struct timer_list sc_dfs_task_timer;/* dfs wait timer */
...
void        ath_process_phyerr(struct ath_softc *sc, struct ath_desc *ds,
                               u_int64_t fulltsf, struct ath_rx_status *rx_stats);
