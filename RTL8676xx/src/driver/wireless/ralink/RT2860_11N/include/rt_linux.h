...
#ifdef IKANOS_VX_1X0
extern void (*vx1x0_cache_ptr)(unsigned long val, int len);
#endif
...
{																	\
	...
	if(vx1x0_cache_ptr)                              \
           vx1x0_cache_ptr((unsigned long)((_A)->CSRBaseAddress + (_R)), 4);\
...
{																	\
	...
	writel(_Val, (void *)((_A)->CSRBaseAddress + (_R)));			\
        if(vx1x0_cache_ptr)               \	
           vx1x0_cache_ptr((unsigned long)(&_Val), 4);	\
    }                                                                   \
}
...
#define RTMP_APQuickResponeForRateUpExec	QuickResponeForRateUpExecEx
#else
/* Modified by Wu Xi-Kun 4/21/2006 */
...
DECLARE_TIMER_FUNCTION(QuickResponeForRateUpExecEx);
