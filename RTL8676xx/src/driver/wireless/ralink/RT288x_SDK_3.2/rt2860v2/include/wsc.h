...
#define WSC_HDR_BTN_GPIO_3			0x0008	/* bit 3 */

/* bit7: WPS PBC (0:off, 1:on) */
#define WSC_HDR_BTN_MR_HDR_SUPPORT_SET(_pAd, _flg_sup)	\
		(_pAd)->WscHdrPshBtnFlag = _flg_sup;

/* check if hardware push button is supported */
#define WSC_HDR_BTN_MR_IS_HDR_SUPPORT(_pAd)				\
		((_pAd)->WscHdrPshBtnFlag)

/* run hardware push button handler */
#define WSC_HDR_BTN_MR_HANDLE(_pAd)						\
		if ((_pAd)->WscHdrPshBtnFlag) WSC_HDR_BTN_CheckHandler(_pAd);

/* bit3: WPS PBC function is controlled through GPIO[3] */
#define WSC_HDR_BTN_MR_PRESS_FLG_GET(_flg_pressed)		\
	{	UINT32 _gpio_value;									\
		RTMP_IO_READ32(pAd, GPIO_CTRL_CFG, &_gpio_value);	\
		_flg_pressed = !(_gpio_value & WSC_HDR_BTN_GPIO_3); }
// WSC HDR PSH BTN FUNC //


#define WSC_TIMER_INIT(pAd, pData, time_var, time_flg, time_fn) \
{ \
        RTMPInitTimer(pAd, time_var, GET_TIMER_FUNCTION(time_fn), pData, FALSE); \
    	time_flg = FALSE; \
} \
...
typedef struct PACKED _RTMP_WSC_NLMSG_HDR{
	...
	UCHAR	devAddr[MAC_ADDR_LEN];		// MAC address who send this netlink msg.
