...
struct DSL_TimeoutElement_s
{
   ...
   /**
      Time on which the timeout event has been started [s] */
   DSL_TimeVal_t nStartTime;
   /**
      Timout value [s] */
   DSL_uint32_t nTimeout;
   /**
      Time on which the element will timeout [s] */
   DSL_TimeVal_t nStopTime;
   /**
...
do{ \
   if( DSL_MUTEX_LOCK(pContext->bspMutex) ) \
   { \
      DSL_DEBUG_SET_ERROR(DSL_ERROR); \
      DSL_DEBUG(DSL_DBG_ERR, (pContext, "DSL: ERROR: failed to lock access to the timeouts list!" DSL_CRLF)); \
      return DSL_ERROR; \
   } \
}while(0)
...
DSL_Error_t DSL_Timeout_Shutdown
(
   DSL_Context_t *pContext
);

DSL_uint32_t DSL_Timeout_GetTotalSizeOfLists
(
   DSL_Context_t *pContext
);

DSL_Error_t DSL_Timeout_GetNextActiveEvent
(
   DSL_Context_t *pContext,
   DSL_int_t *nEventType,
...
DSL_uint32_t DSL_Timeout_AddEvent
(
   DSL_Context_t *pContext,
   DSL_int_t nEventType,
   DSL_uint32_t nTimeout
...
DSL_Error_t DSL_Timeout_RemoveEvent
(
   DSL_Context_t *pContext,
   DSL_uint32_t nTimeoutID
);

DSL_Error_t DSL_Timeout_RemoveAllEvents
(
   DSL_Context_t *pContext
);

DSL_Error_t DSL_Timeout_Debug_PrintTimeoutList
(
   DSL_Context_t *pContext
);
