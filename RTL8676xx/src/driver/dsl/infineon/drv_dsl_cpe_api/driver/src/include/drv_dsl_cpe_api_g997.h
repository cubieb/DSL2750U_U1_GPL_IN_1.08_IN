...
/**
   ...
   subcarriers. Regarding to the choosen ADSL standard, the used Annex type and
...
typedef enum
{
   ...
   dignostic measurement as follows:
   ...
   - all following line activations will be done without dignostic mode */
...
/**
   This configuration parameter defines the behaviour of the line after
   activation.
*/
typedef struct
{
   ...
   /**
   This parameter is defined in order to improve testing of the performance of
   XTUs supporting automode when it is enabled in the MIB. The valid values are
   0 and 1. A change in value of this parameter indicates a change in loop
   conditions applied to the devices under test. The XTUs shall reset any
   historical information used for automode and for shortening G.994.1 handshake
   and initialization.
   Automode is defined as the case where multiple operation-modes are enabled
   in the MIB in the G.997.1 "xTU Transmission System Enabling (XTSE)" table
   and where the selection of the operation-mode to be used for transmission
   does not only depend on the common capabilities of both XTUs (as exchanged
   in G.994.1), but depends also on achievable data rates under given loop
   conditions.
   This parameter is mandatory at the Q interface for modems supporting
   automode.
   \todo TD, 2006-12-01, Check Danube implementation.
   \attention If using the GEMINAX-D device the Implementation of this feature
              is supported by a different handling as follows:
              A line deactivate and a following line activate will automatically
              lead in an automode cold start forced!
              Thus the configuration of this value does NOT have any influence!
   (default: DSL_G997_INHIBIT_ACSF) */
...
/*
   xTUTransmission System Enabling - Octet 3
*/
/**
   Reserved. */
...
/*
   xTUTransmission System Enabling - Octet 4
*/
/**
...
typedef struct
{
   ...
   The interpretation of the other octets is vendor discretionary and can b
...
typedef struct
{
   ...
   It is coded in a bit-map representation (0 if not allowed, 1 if allowed). */
...
/**
   ...
   state in the downstream/upstream direction in the last full intialization
...
/**
   Gain Allocation per Subcarrier (see chapter 7.5.1.21.3 and 7.5.1.21.4 of G.997.1)
...
/**
   ...
   measurement could be done for this subcarrier group because it is out of the
   passband or that the SNR is out of range to be represented.
*/
typedef struct
{
   /**
...
/**
   ...
   \note This structure will be used for downstream (Far-End, xTU-R) and
         ...
         Therefore the parameter commets are done for both access possibilities.
...
/**
   ...
   \note This structure will be used for downstrean and upstream handling.
         Therefore the parameter commets are done for both access possibilities.
   \note All bits within this bit field have the following meaning:
         0: Line failure event is NOT signalled
         1: Line failure event will be signalled by callback function
...
typedef enum
{
   ...
   line is in the L3 state, attemps shall be made to transition to the L0 state
   until it is forced into another state through this configuration parameter. */
   DSL_G997_PMSF_L3_TO_L0 = 0,
   /**
   Force the line to transition from L0 full on to L2 low power state.
   This transition requires the entry into L2 mode. This is a out-of-service
   test value for triggering the L2 mode. */
...
typedef struct
{
   ...
   PMS-TC between the alfa and beta reference points excluding delay in L1 and
   ...
   This parameter reports the actual impulse noise protection on the bearer
   channel in L0 state. In L1 or L2 states, the parameter contains the impulse
   noise protection in the previous L0 state. This value is computed according
   to the formula specified in the relevant recommendation based on the actual
   framing parameters. The values is coded in fraction of DMT symbols.
   The value is coded as unsigned integer with steps of 0.5.
   Valid values are:
...
typedef struct
{
   ...
   PMS-TC between the alfa and the beta reference points, in the direction of
   ...
   The xTUs shall choose the S and D values such that the actual one-way
   ...
   \note    A single Maximum Delay value is configured. As a consequence, xTUs
            ...
            line initialization.
   \attention For Geminax only:
            The delay is coded in ms. For the interleaved path a typical
            value is 20H (30 + 2 ms) but can also be set to 01H (1 + 0 ms) if
            two fast paths shall be realized.
            Handshake supports a range of 01H... 3FH. If no delay bounds are
            to be given, this parameter has to be set to 0402H.
   \note    For Geminax only:
            If the Maximum Delay is configured to 4 msec, then the Fast Latency
            Path will be used in the G.992.1 and G.992.2 operating modes. In
            the G.992.3 and G.992.4 operating modes, S and D will be selected
            such that S*D 1. Configuring a lower value will cause the line to
            be unable to initialize. */
   ...
   \note
   ATUs supporting multiple ADSL Recommendations may use or ignore the
   configured value depending on the operating mode actually being selected at
   line initialization. In G.992.3, the ATUs will use the configured value.
...
/**
   This structure has to be used for ioctl
   \todo Clarify: Is a bearer channel configuration for CPE really necessary?
*/
typedef struct
{
   /**
   ...
   DSL_IN DSL_AccessDir_t nDirection;
   /**
   Structure that contains all necessary confiruration data */
...
/**
   ...
   (see chapter 7.3.2.6.1/7.3.2.6.2 of G.997.1)
...
/**
   More detailled Initialization Success/Failure cause.
   This is not standardised in G.997.1.
*/
typedef enum
{
   /**
...
/**
   ...
   an attempt to reach the L0 state (or to succesfully complete the loop
...
typedef struct
{
   ...
   \note The downstream actual power spectrum dessity is the sum (in dB) of
         the REFPSDds and RMSGIds.
   \note This value is only available in showtime. In DELT mode the value is
         always 0.
   \note For ADSL systems the value that will be reported if the line is
         disabled is 0. */
   ...
   steps.
   A special value (-311) indicates the parameter is out of range to be
   represented.
   ...
   \note For ADSL systems and normal initialization the value that will be
         returned depends on the line state as follows:
         \code
         t(idle) < t <  t(training)     : 8.4 dBm/Hz will be reported
         t(training) <= t < t(showtime) : Approximated value will be reported
                                          (Gi=0, NOMATPds=MaxNomATPds)
         t >= t(showtime)               : Showtime value will be reported

         with:
         t(x) = time where line is in special line state x
         t    = time where value is requested from DS API
         \endcode
         The value will set to 0 if leaving showtime whether automatically
         (e.g. exception) or manually (e.g. LineDeactivate).
         Also note that the showtime value will be directly requested from the
         firmware each time it will be requested from DSL API.
         If operating in DELT mode according DELT values will be reported. */
...
typedef struct
{
   ...
   /**
   Downstream/Ustream Signal-to-Noise Ratio Margin per band
   (see chapter 7.5.1.10/7.5.1.11 of G.997.1)
