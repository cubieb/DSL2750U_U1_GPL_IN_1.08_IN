			...
			size_t len)
{
...
#ifndef WSC_AKM_NO_1X // Jerry, even PSK, still process eapol
	...
	case IEEE802_1X_TYPE_EAPOL_START:
		hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE8021X,
			       ...
			       "from STA");
                if(hapd->conf->wsc_auth_mode !=0)
                {
                    sta->flags |= WLAN_STA_WSC_START_RCVD;

                    if(!(sta->flags & WLAN_STA_WSC_ASSOC))
                    {
                        if(strncmp(buf+ sizeof(struct ieee802_1x_hdr),
                                   "WiFi_Protected_Setup", 
                                    strlen("WiFi_Protected_Setup"))==0) 
                        {          
                             sta->flags |= WLAN_STA_WSC_START_PATCH_RCVD;
                        } else {
                            /* ignore EAPOL_START for WPS */
                            sta->flags &= ~WLAN_STA_WSC_START_RCVD;
                            printf("******ignore EAPOL_START\n");
                            break;
                        }
                    }   
                }
...
{
#ifndef WSC_AKM_NO_1X  // Jerry even PSK, still init eapol SM
	if (!hapd->conf->ieee802_1x || sta->wpa_key_mgmt == WPA_KEY_MGMT_PSK)
#else
