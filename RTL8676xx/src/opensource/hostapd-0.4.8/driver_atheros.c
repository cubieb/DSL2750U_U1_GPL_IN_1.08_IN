...
set80211priv(struct atheros_driver_data *drv, int op, void *data, int len)
{
	...
	if (ioctl(drv->ioctl_sock, op, &iwr) < 0) {
		...
		if (first <= op && op <= last){
			...
			if(op == IEEE80211_IOCTL_SETMLME){
				struct ieee80211req_mlme *mlme;
				mlme = (struct ieee80211req_mlme *)data;

				printf("driver_madwifi: mlme->im_op = %d\n", mlme->im_op);
				printf("driver_madwifi: mlme->im_macaddr = %02x:%02x:%02x:%02x:%02x:%02x\n", 
...
/* modified by hansel 20080722 for WPS use this function */
 void
