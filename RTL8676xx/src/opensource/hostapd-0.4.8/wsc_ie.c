...
static WSC_IE_DATA * g_wsc_ie_data = NULL;
...
/* added by hansel 20080806 */	
#ifdef SIMPLE_CONFIG	
	char sendBuf[48];//modified by hansel 20080709, to send IFNAME command
	int len;
#else
	char sendBuf[5];
#endif
...
/* added by hansel 20080806 */		
#ifdef SIMPLE_CONFIG	
	snprintf(sendBuf, 48, "IFNAME %s", hapd->conf->iface);
	len = strlen(sendBuf) + 1;
	if (udp_write(g_wsc_ie_data->udpFdCom, sendBuf, len, &to) < len) {
		wpa_printf(MSG_ERROR, "WSC_IE: Sending IFNAME message to "
				"upper Layer failed");
		return -1;
	} else {
		wpa_printf(MSG_DEBUG, "WSC_IE: send ifname = %s", hapd->conf->iface );
	}	
#endif	
	return 0;
}
...
{
    if(g_wsc_ie_data == NULL){//already clean up
        return 0;
    }
