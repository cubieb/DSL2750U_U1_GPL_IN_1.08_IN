...
/* added by hansel 20080613 for WPS use this function */
extern void atheros_push_button_notify();
/* end by hansel */
				       ...
				       void *sock_ctx)
{
	...
	if (strcmp(buf, "PING") == 0) {
		...
		if (hostapd_ctrl_iface_level(hapd, &from, fromlen,
						    buf + 6))
			reply_len = -1;
/* added by hansel 20080613 for send WPS PBC message to wsccmd */
#ifdef SIMPLE_CONFIG
	}else if(strcmp(buf, "WSC_PBC") == 0){
		//only for madwifi driver, other driver may cause compile error
...
int hostapd_ctrl_iface_init(struct hostapd_data *hapd)
{
	...
	/* Jerry:soc nok. if (mkdir(hapd->conf->ctrl_interface, S_IRWXU | S_IRWXG) < 0) {*/
    if (mkdir(hapd->conf->ctrl_interface, 777) < 0) {
		if (errno == EEXIST) {
...
/* added by hansel, 20080826, try to unlink it */
        perror("bind(PF_UNIX)");        
        printf("try unlink %s...\n", addr.sun_path);
