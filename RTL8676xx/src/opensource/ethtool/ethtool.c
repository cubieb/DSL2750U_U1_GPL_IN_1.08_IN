...
static int do_svport(int fd, struct ifreq *ifr);
static int do_svlan(int fd, struct ifreq *ifr);
static int do_gvlan(int fd, struct ifreq *ifr);
static int do_sigmp(int fd, struct ifreq *ifr);
static int do_sqinq(int fd, struct ifreq *ifr);
...
static struct option {
    ...
    { "-m", "--vport", MODE_SVPORT, "Enable/Disable Vport or Enable/Disable tag", 
                "               [ enable | disable | enable_tag | disable_tag ]\n" },
    { "-V", "--vlan", MODE_SVLAN, "Configure VLAN ",
		"		[ enable | disable ]\n"
		"		[ addvlan VID ]\n"
		"		[ delvlan VID ]\n"
		"		[ addport VID PortNum ]\n"
		"		[ delport VID PortNum ]\n"
		"		[ setport PortNum tag|untag ]\n" 
		"		[ setprio VID Prio]\n" 
		"		[ settrunk enable|disable PortNum]\n" },
    { "-v", "--show-vlan", MODE_GVLAN, "Show VLAN configuration",
		"		[ show-vlan ]\n"
		"		[ show-member VID ]\n"
		"		[ show-port PortNum ]\n" 
		"		[ show-trunk ]\n" 
		"		[ show-prio VID ]\n" },
    { "-I", "--igmp", MODE_SIGMP, "Configure IGMP",
		"		[ enable | disable ]\n"
		"		[ enport  PortNum ]\n"
		"		[ disport PortNum ]\n"
		"		[ getport ]\n" },
    { "-Q", "--qinq", MODE_SQINQ, "Configure QinQ",
		"		[ enable | disable ]\n"
		"		[ stag VID ]\n" },
    {}
};
...
static int vport_enable = 0;
static int vlan_cmd = 0;
static int vlan_enable = 0;
static int vlan_vid = 0;
static int vlan_prio = 0;
static int vlan_pvid = 0;
static int vlan_port = 0;
static int vlan_tag = 0;
static int igmp_cmd = 0;
static int igmp_enable = 0;
static int igmp_port = 0;
static int qinq_cmd = 0;
static int qinq_enable = 0;
static int qinq_stag = 0;
...
static void parse_cmdline(int argc, char **argp)
{
	...
	for (i = 1; i < argc; i++) {
		...
		case 2:
			...
			if (mode == MODE_GSET) {   /* add by Zhang Yu */
				if (!strcmp(argp[i], "phyad")) {
					i += 1;
		...
		case 3:
			if (mode == MODE_TEST) {
				...
				break;
			} else if (mode == MODE_SVPORT) {
				if (!strcmp(argp[i], "enable")) {
					vport_enable = 1;
				} else if (!strcmp(argp[i], "disable")) {
					vport_enable = 0;
				} else if (!strcmp(argp[i], "enable_tag")) {
					vport_enable = 3;
				} else if (!strcmp(argp[i], "disable_tag")) {
					vport_enable = 2;
				} else {
					show_usage(1);
				}
		...
		default:
			...
			if (mode == MODE_SVLAN){
				if (!strcmp(argp[i], "enable")) {
					...
					vlan_cmd = VLAN_CMD_DISABLE;
					vlan_enable = 0;
					i += 1;
					if (argc > i)
						show_usage(1);
					break;
				} else if (!strcmp(argp[i], "addvlan")) {
					vlan_cmd = VLAN_CMD_ADDVLAN;
					i += 1;
					if (i >= argc)
						show_usage(1);
					vlan_vid = strtol(argp[i], NULL, 0);
					if (vlan_vid <= 0)
						show_usage(1);
					i += 1;
					if ( argc>i )
						show_usage(1);
					break;
				} else if (!strcmp(argp[i], "delvlan")) {
					vlan_cmd = VLAN_CMD_DELVLAN;
					i += 1;
					if (i >= argc)
						show_usage(1);
					vlan_vid = strtol(argp[i], NULL, 0);
					...
					if (argc >i)
						show_usage(1);
					break;
				} else if (!strcmp(argp[i], "addport")) {
					vlan_cmd = VLAN_CMD_ADDPORT;
					i += 1;
					if (i >= argc)
						show_usage(1);
					vlan_vid = strtol(argp[i], NULL, 0);
					...
					if (i >= argc)
						show_usage(1);
					vlan_port= strtol(argp[i], NULL, 0);
					if (vlan_port < 0)
						show_usage(1);
					i += 1;
					if (argc > i)
						show_usage(1);
					break;
				} else if (!strcmp(argp[i], "delport")) {
					vlan_cmd = VLAN_CMD_DELPORT;
					i += 1;
					if (i >= argc)
						show_usage(1);
					vlan_vid = strtol(argp[i], NULL, 0);
					...
					if (argc > i)
						show_usage(1);
					break;
				}else if (!strcmp(argp[i], "setport")) {
					vlan_cmd = VLAN_CMD_SETPORT;
					i += 1;
					if (i >= argc)
						show_usage(1);
					vlan_port= strtol(argp[i], NULL, 0);
					...
					if (i >= argc)
						show_usage(1);
					if (!strcmp(argp[i], "tag")) {
						vlan_tag = 1;
					}else if (!strcmp(argp[i], "untag")) {
							vlan_tag = 0;
					} else {
						show_usage(1);
					}
					...
					if (argc>i)
						show_usage(1);
					break;
				}else if (!strcmp(argp[i], "setprio")) {
					vlan_cmd = VLAN_CMD_SETPRIO;
					i += 1;
					if (i >= argc)
						show_usage(1);
					vlan_vid = strtol(argp[i], NULL, 0);
					...
					if (i >= argc)
						show_usage(1);
					vlan_prio= strtol(argp[i], NULL, 0);
					if (vlan_prio< 0)
						show_usage(1);
					i += 1;
					if (argc > i)
						show_usage(1);
					break;
				}else if (!strcmp(argp[i], "settrunk")) {
					vlan_cmd = VLAN_CMD_SETTRUNK;
					i += 1;
					if (i >= argc)
						show_usage(1);
			...
			if (mode == MODE_GVLAN){
				if (!strcmp(argp[i], "show-vlan")) {
					vlan_cmd = VLAN_CMD_GETGROUP;
					i += 1;
					if ( argc > i )
						show_usage(1);
					break;
				} else if (!strcmp(argp[i], "show-member")) {
					vlan_cmd = VLAN_CMD_GETMEMBER;
					i += 1;
					if (i >= argc)
						show_usage(1);
					vlan_vid = strtol(argp[i], NULL, 0);
					...
					if (argc > i)
						show_usage(1);
					break;
				} else if (!strcmp(argp[i], "show-port")) {
					vlan_cmd = VLAN_CMD_GETPORT;
					i += 1;
					if (i >= argc)
						show_usage(1);
					vlan_port= strtol(argp[i], NULL, 0);
					...
					if (argc > i)
						show_usage(1);
					break;
				}  else if (!strcmp(argp[i], "show-prio")) {
					vlan_cmd = VLAN_CMD_GETPRIO;
					i += 1;
					if (i >= argc)
						show_usage(1);
					vlan_vid = strtol(argp[i], NULL, 0);
					...
					if (argc > i)
						show_usage(1);
					break;
				} else 	if (!strcmp(argp[i], "show-trunk")) {
					vlan_cmd = VLAN_CMD_GETTRUNK;
					i += 1;
					if ( argc > i )
						show_usage(1);
					break;
			...
			if (mode == MODE_SIGMP){
				if (!strcmp(argp[i], "enable")) {
					...
					igmp_cmd = IGMP_CMD_DISABLE;
					igmp_enable = 0;
					i += 1;
					if ( argc > i )
						show_usage(1);
					break;
				} else if (!strcmp(argp[i], "enport")) {
					igmp_cmd = IGMP_CMD_ENPORT;
					i += 1;
					if (i >= argc)
						show_usage(1);
					igmp_port = strtol(argp[i], NULL, 0);
					if (igmp_port < 0)
						show_usage(1);
					i += 1;
					if ( argc>i )
						show_usage(1);
					break;
				} else if (!strcmp(argp[i], "disport")) {
					igmp_cmd = IGMP_CMD_DISPORT;
					i += 1;
					if (i >= argc)
						show_usage(1);
					igmp_port = strtol(argp[i], NULL, 0);
					...
					if ( argc>i )
						show_usage(1);
					break;
				} else if (!strcmp(argp[i], "getport")) {
					igmp_cmd = IGMP_CMD_GETPORT;
					igmp_port = 0;
					i += 1;
					if ( argc > i )
						show_usage(1);
			...
			if (mode == MODE_SQINQ){
				if (!strcmp(argp[i], "enable")) {
					...
					qinq_cmd = QINQ_CMD_DISABLE;
					qinq_enable = 0;
					i += 1;
					if ( argc > i )
						show_usage(1);
					break;
				} else if (!strcmp(argp[i], "stag")) {
					qinq_cmd = QINQ_CMD_SET_STAG;
					i += 1;
					if (i >= argc)
						show_usage(1);
					qinq_stag = strtol(argp[i], NULL, 0);
					if (qinq_stag < 0)
						show_usage(1);
					i += 1;
					if ( argc>i )
...
static int dump_ecmd(struct ethtool_cmd *ep)
{
	...
	fprintf(stdout, "	Link State: %s\n", ep->link_state == LINK_UP? "Up":"Down");
...
static int dump_vlan(struct ethtool_gvlan*evlan)
{
	int i;
	
	if(evlan->enable == 0){
		fprintf(stdout, "The VLAN function is disable!\n");
		return 0;
	}

	if (evlan->vlan_cmd == VLAN_CMD_GETGROUP) {
		fprintf(stdout, "Existed VLAN: ");
		for (i = 0; i < MAX_VLAN_GROUP; i++) {
			if(evlan->group[i] != 0)
				fprintf(stdout, " %d  ", evlan->group[i] );
		}
		fprintf(stdout, "\n");
	}

	if (evlan->vlan_cmd == VLAN_CMD_GETMEMBER) {
		fprintf(stdout, "VLAN ID: %d\n        Member ship: ", evlan->vid);
		for (i = 0; i < 32; i++) {
			if((evlan->member) >> i & 0x1)
				fprintf(stdout, "%d    ", i);
		}
		fprintf(stdout, "\n");
	}
	
	if (evlan->vlan_cmd == VLAN_CMD_GETPORT) {
		if(evlan->tag == 1)
			fprintf(stdout, "Port %d is tag.\n", evlan->port);
		else
			fprintf(stdout, "Port %d is untag. \n", evlan->port);

	}

	if (evlan->vlan_cmd == VLAN_CMD_GETPRIO) {
			fprintf(stdout, "The priority of VLAN %d is %d.\n", evlan->vid, evlan->prio);
	}


	if (evlan->vlan_cmd == VLAN_CMD_GETTRUNK) {
		if(evlan->enable == 1){
			fprintf(stdout, "VLAN TRUNK is enable.\n");
			fprintf(stdout, "TRUNK port is %d.\n", evlan->port);
		}
		else
			fprintf(stdout, "VLAN TRUNK is disable.\n");
	}

	return 0;
}

static int dump_igmp(struct ethtool_sigmp *eigmp)
{
	int i;
	
	if(eigmp->enable == 0){
		fprintf(stdout, "The IGMP function is disable!\n");
		return 0;
	}

	if (eigmp->igmp_cmd == IGMP_CMD_GETPORT) {
		fprintf(stdout, "IGMP port enable: ");
		for (i = 0; i < 32; i++) {
			if((eigmp->port) >> i & 0x1)
				fprintf(stdout, "%d    ", i);
		}
...
static int doit(void)
{
	...
	if (mode == MODE_GDRV) {
		...
		return do_svport(fd, &ifr);
	}else if (mode == MODE_SVLAN) {
		return do_svlan(fd, &ifr);
	}else if (mode == MODE_GVLAN) {
		return do_gvlan(fd, &ifr);
	}else if (mode == MODE_SIGMP) {
		return do_sigmp(fd, &ifr);
	}else if (mode == MODE_SQINQ) {
		return do_sqinq(fd, &ifr);
	}
...
{
	int err;
	struct ethtool_svport evport;

	evport.cmd = ETHTOOL_SVPORT;
	evport.enable = vport_enable;
	ifr->ifr_data = (caddr_t)&evport;
	err = ioctl(fd, SIOCETHTOOL, ifr);
	if (err < 0)
		perror("Cannot set VPORT");
...
{
	int err;
	struct ethtool_svlan evlan;

	evlan.cmd		= ETHTOOL_SVLAN;
	evlan.vlan_cmd	= vlan_cmd;
	evlan.enable		= vlan_enable;
	evlan.vid			= vlan_vid;
	evlan.prio		= vlan_prio;
	evlan.port		= vlan_port;
	evlan.tag			= vlan_tag;
	
	ifr->ifr_data = (caddr_t)&evlan;
	err = ioctl(fd, SIOCETHTOOL, ifr);
	if (err < 0)
		perror("Cannot set VLAN");
...
{
	int err;
	struct ethtool_gvlan evlan;
	memset(&evlan,0,sizeof(evlan));
	
	evlan.cmd		= ETHTOOL_GVLAN;
	evlan.vlan_cmd	= vlan_cmd;
	evlan.enable		= 0;
	evlan.vid			= vlan_vid;
	evlan.port		= vlan_port;
	...
	if (err < 0){
		perror("Cannot get VLAN configuration");
		return err;
	}
...
{
	int err;
	struct ethtool_sigmp eigmp;
	memset(&eigmp,0,sizeof(eigmp));
	
	eigmp.cmd	= ETHTOOL_SIGMP;
	eigmp.igmp_cmd	= igmp_cmd;
	eigmp.enable	= igmp_enable;
	eigmp.port	= igmp_port;
	
	ifr->ifr_data = (caddr_t)&eigmp;
	err = ioctl(fd, SIOCETHTOOL, ifr);
	
	if (err < 0){
		perror("Cannot configure IGMP");
		return err;
	}
	
	if(igmp_cmd == IGMP_CMD_GETPORT)
		dump_igmp(&eigmp);
...
{
	int err;
	struct ethtool_sqinq eqinq;
	memset(&eqinq,0,sizeof(eqinq));
	
	eqinq.cmd	= ETHTOOL_SQINQ;
	eqinq.qinq_cmd	= qinq_cmd;
	eqinq.enable	= qinq_enable;
	eqinq.stag	= qinq_stag;
	
	ifr->ifr_data = (caddr_t)&eqinq;
	err = ioctl(fd, SIOCETHTOOL, ifr);
	
	if (err < 0){
		perror("Cannot configure QinQ");
		return err;
	}
