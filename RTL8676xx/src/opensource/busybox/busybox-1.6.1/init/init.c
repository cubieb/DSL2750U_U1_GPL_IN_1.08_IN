...
/* Added by yujinshi send reboot's trap to snmpd */
void SendSingleToSnmpd( void )
{
    FILE *fp_pid = NULL;
    int iPid = 0;
	
    if( ( fp_pid = fopen( "/var/run/snmpd_pid", "r" ) ) != NULL )
    {
        fscanf( fp_pid, "%d", &iPid );
        fclose( fp_pid );
...
static void shutdown_system(void)
{
	...
	message(L_CONSOLE | L_LOG, "Reboot, undial 3g");
	system("3g-mngr undial --force");
	sleep(3);
#endif
