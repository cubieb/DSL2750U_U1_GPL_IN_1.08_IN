...
int klogd_main(int argc, char **argv)
{
	...
	while (1) {
		...
		while (i < n) {
			...
			if (log_buffer[i] == '<') {
				i++;
#if !(ENABLE_FEATURE_RECORD_SYSEVENTS) /* Added by zj, kernel ganerates multi-digit prios to combine with facility information */
#error "We should have ENABLE_FEATURE_RECORD_SYSEVENTS defined"
...
#else
                /* Added by zj, kernel ganerates multi-digit prios to combine with facility information */
				priority = 0;
				while (log_buffer[i] >= '0' && log_buffer[i] <= '9') {
                    priority = priority * 10 + (log_buffer[i] - '0');
                    i++;
