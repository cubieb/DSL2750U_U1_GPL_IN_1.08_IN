...
BUPnPDeviceHandle BUPnPDevice_Create(BUPnPDeviceInfo* pDeviceInfo)
{
    ...
    {
        pos = cmsUtl_strstr(pDeviceInfo->udn, "uuid:");
        assert(pos);
...
void BUPnPDevice_Data(nxml_t handle, const char *tag_name, unsigned len)
{
    ...
    else if (IMATCH_PREFIX(handle->lastStartTag, "url"))
    {
        ...
        cmsUtl_strncpy(temp, tag_name, len);
        temp[len] = '\0';
        if (temp[0] != '/')
        ...
        pIcon->iconInfo->url = BUPnPStrndup(url, cmsUtl_strlen(url));
    }
    else if (IMATCH_PREFIX(handle->lastStartTag, "serviceType"))
    {
        ...
        {
            serviceName = tag_name + cmsUtl_strlen(UPNP_HEADER);
    ...
    else if (IMATCH_PREFIX(handle->lastStartTag, "SCPDURL"))
    {
            ...
            ((tag_name[0] != '/') && (!IMATCH_PREFIX(tag_name, "http://"))))
        {
            ...
            cmsUtl_strncpy(temp, tag_name, len);
            temp[len] = '\0';
            pService->serviceInfo->scpdUrl = (char*)BUPnPCalloc(1, cmsUtl_strlen(pDevice->urlBase)+len+1);
    ...
    else if (IMATCH_PREFIX(handle->lastStartTag, "controlURL"))
    {
            ...
            ((tag_name[0] != '/') && (!IMATCH_PREFIX(tag_name, "http://"))))
        {
            ...
            cmsUtl_strncpy(temp, tag_name, len);
            temp[len] = '\0';
            pService->serviceInfo->controlUrl = (char*)BUPnPCalloc(1, cmsUtl_strlen(pDevice->urlBase)+len+1);
    ...
    else if (IMATCH_PREFIX(handle->lastStartTag, "eventSubURL"))
    {
            ...
            ((tag_name[0] != '/') && (!IMATCH_PREFIX(tag_name, "http://"))))
        {
            ...
            cmsUtl_strncpy(temp, tag_name, len);
            temp[len] = '\0';
            pService->serviceInfo->eventSubUrl = (char*)BUPnPCalloc(1, cmsUtl_strlen(pDevice->urlBase)+len+1);
