...
struct nxml {
    ...
    int error;
    int dataCount;
    const char* lastStartTag;
    int lastStartTagLen;
    void* context; /* general purpose void pointer */
};
...
/* 
Summary:
Removed escaped xml tag

Description:
Takes a escaped xml string and replace all escaped characters with 
*/
void nxml_unescape(const char* input, char* out);

/* 
Summary:
Change xml to escaped format

Description:
Takes a normal xml string and converted to a "escaped" string so that it can be
embedded in another xml string
*/
