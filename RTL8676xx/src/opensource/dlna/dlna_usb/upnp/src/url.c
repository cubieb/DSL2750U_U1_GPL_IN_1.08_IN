...
int Url_Create (Url *pUrl, const char *pszUrl)
{
    ...
    cmsUtl_strcpy(pUrl->scheme, "http");
    pUrl->port = 80;
...
const char* Url_ToString(Url *pUrl)
{
    ...
    length = cmsUtl_strlen(pUrl->scheme) + cmsUtl_strlen(pUrl->host) + cmsUtl_strlen(pUrl->path);
    if ( pUrl->query != NULL )
        length += cmsUtl_strlen(pUrl->query);
    length+=16;/* pad for extra chars ://, :, portlength */

    if ( pUrl->buf == NULL )
        pUrl->buf = (char*)BUPnPCalloc(length, 1);
    else if ( cmsUtl_strlen(pUrl->buf) > length )
    {
    ...
    if ( pUrl->query )
    {
        cmsUtl_strcat(pUrl->buf, "?");
        cmsUtl_strcat(pUrl->buf, pUrl->query);
    }
...
static int Url_Parse (Url *pUrl, const char *pszUrl)
{
    ...
    if ( *end != '/' )
    {
        ...
        cmsUtl_strncpy(pUrl->scheme, start, end-start);
    ...
    end = pUrl->path + cmsUtl_strlen(pUrl->path);
    if ( *end == '/' )
        *end = '\0';
