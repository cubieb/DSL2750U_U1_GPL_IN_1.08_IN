...
*  ANY LIMITED REMEDY.
 *
 * $brcm_Workfile: debug.c $
 * $brcm_Revision: 2 $
 * $brcm_Date: 7/23/09 8:19a $
 * 
 * Module Description:
 * 
 * Revision History:
 * 
 * $brcm_Log: /AppLibs/upnp/src/debug.c $
 * 
 * 2   7/23/09 8:19a dliu
 * PR55165: Default debug message to errors for all category
* 
***************************************************************************/
#include "upnp_priv.h"
#include "debug.h"
...
static uint32_t _debuglevel = BUPnPDebugLevel_Error | BUPnPDebugLevel_Warning;
static uint32_t _debugCategory = BUPnPDebugCategory_All;
...
void BUPnPDebug_Print( BUPnPDebugLevel level, BUPnPDebugCategory category, const char* fileName, int32_t lineNumber, const char *format, ... )
{
    ...
    {
        if ( (level & BUPnPDebugLevel_Error) == BUPnPDebugLevel_Error )
            pstrLevel = DEBUG_ERROR_STRING;
        else if ( (level & BUPnPDebugLevel_Warning) == BUPnPDebugLevel_Warning )
