...
void BDlna_Didl_ParseProtocolInfo(char* protocolInfo, BDlna_ProtocolInfo* dlnaProtocol)
{
    ...
    if (temp != NULL)
    {
        temp += cmsUtl_strlen("DLNA.ORG_FLAGS=");
        cmsUtl_strncpy(flag, temp, 8); /* grab first 32-bits only for now */
        flag[8] = '\0';
    ...
    if (temp != NULL)
    {
        temp += cmsUtl_strlen("DLNA.ORG_CI=");
    ...
    if (temp != NULL)
    {
        temp += cmsUtl_strlen("DLNA.ORG_PS=");
    ...
    if (temp != NULL)
    {
        temp += cmsUtl_strlen("DLNA.ORG_OP=");
    ...
    if (temp != NULL)
    {
        temp += cmsUtl_strlen("DLNA.ORG_PN=");
...
static void BDlna_Didl_XmlTagBegin(nxml_t handle, const char *tag_name, unsigned len)
{
    ...
    {
        didlIndex++;
        end = cmsUtl_strstr(tag_name, "</item>");
        pMedia = ((struct BDlna_DmpMediaItem*)handle->context);
        pMedia[didlIndex].metaData = strndup(tag_name-1, end-tag_name+1+cmsUtl_strlen("</item>"));
