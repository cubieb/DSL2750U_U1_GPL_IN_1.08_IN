...
char* String_Copy(const char *inString, int length)
{
	char *RetVal;
	if(length<0)
	{
...
{ 
    ...
    result=String_Copy(pszOutputBuffer,iOutputPos);
...
errout:
	return(String_Copy(mbcsStr, (int)strlen(mbcsStr)));
} 
...
errout:
	return(String_Copy(utf8Str, (int)strlen(utf8Str)));
} 
