...
char ContentDir_fortimer[80];
extern char DirPathToIno[100][50];
...
#else
#define DEVICE_INFO_BRCM_NAME "Broadcom Digital Media Server"
#endif
...
BUPnPDeviceInfo BcmMediaServerDevice = {
    ...
    "http://www.broadcom.com",    /* manufacturerUrl    */
#ifdef DMP_X_CT_COM_COMMON_1
    "Telecom Digital Media Server Ver 1.0",    /* modelDescription    */
#else
    "Broadcom Digital Media Server Ver 1.0",    /* modelDescription    */
#endif
    "Settop Box Revision",    /* modelName    */
    "BcmXXXX",    /* modelNumber    */
    "http://www.broadcom.com",    /* modelUrl    */
    "12345678",    /* serialNumber        */
    NULL,    /* udn    */
    "upc code",    /* upc    */
    NULL,        /* iconList    */
    "http://www.broadcom.com",    /*presentationUrl*/
};
#endif
...
static void BDlna_Dms_ReCreate(TimerHandle timer, void* arg)
{ 
    printf("---------this is for the timeupdate\n");
}
...
#endif
//#if 0 /* content_dir has been fixed */
    if ( argc < 3 )
    {
       printf("Usage: bcmmserver mediaPath\n");
       return 1;
    }
//#endif
    ...
    strncpy(ContentDir, argv[1], MAX_MEDIA_PATH);
    strncpy(ContentDir_fortimer, argv[1], MAX_MEDIA_PATH);
    //strncpy(ContentDir, "/mnt", MAX_MEDIA_PATH);
...
/*    
    if(updatetime>0)
    {
        timerSettingss.period = updatetime*1000;
        timerSettingss.callback =BDlna_Dms_ReCreate;
        timerSettingss.param = (void*)DmsHandle;
        timerSettingss.executeOnce = false;
        DmsHandle->expirationTimer= Timer_Create(&timerSettingss);
    }   */
...
*/
        while(DirPathToIno[j][0]!= 0)
        {
           wd_dlna[j]=inotify_add_watch(fd_dlna, DirPathToIno[j], (IN_CREATE | IN_DELETE |IN_MOVE) );
           j++;
        }
        ...
        if(len=read(fd_dlna,buffer,BUF_LEN))
        {
/*          printf("some event happens,len=%d\n",len); */
            while(i < len)
            {
                ...
                if(fileevent->len)
                {
                    ...
                    {
                        sleep(30);
                        AddDlna_File(fileevent->name,DirPathToIno[fileevent->wd-1]);
                        
                    }else if(fileevent ->mask & IN_DELETE)
                    {
                        DleteDlna_File(fileevent->name,DirPathToIno[fileevent->wd-1]);
                    }else if(fileevent ->mask & IN_MOVE)
                    {  
                       if(flag_modify==0)
                       {
                            strcpy(oldname,fileevent->name) ;
                            strcpy(oldpath,DirPathToIno[fileevent->wd-1]);
                            flag_modify=1;
                       }else
                       {
                            ModifyDlna_File(oldname,fileevent->name,oldpath,DirPathToIno[fileevent->wd-1]);
                            flag_modify=0;
                       }                                  
                    }                    
                }
            ...
            while(j)
            {
              inotify_rm_watch( fd_dlna, wd_dlna[j-1] ); 
              j--;
            }            
            close( fd_dlna );
            i=j=0;
