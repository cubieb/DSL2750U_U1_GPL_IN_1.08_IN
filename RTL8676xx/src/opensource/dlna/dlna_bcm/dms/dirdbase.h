...
#define ITEM_GET_ARTIST(_object_) _object_->u.item_info.u.file.artist
#define ITEM_GET_DATE(_object_) _object_->u.item_info.u.file.date
#define ITEM_GET_GENRE(_object_) _object_->u.item_info.u.file.genre
#define ITEM_GET_ALBUM(_object_) _object_->u.item_info.u.file.album
#define ITEM_GET_TRACK(_object_) _object_->u.item_info.u.file.track
...
void * 
init_database(char*  cds_url,char *ContentDir,unsigned int ObjectID);
...
struct mime_type_t *
GetMimeFromTable(char *Extension, CONTENT_TYPE MType);

BOOL
ScanFileCallback(pcb_context pContext, PFOUND_FILE_INFO pFoundFile);
