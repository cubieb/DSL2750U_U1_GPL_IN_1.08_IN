...
cms_update_state_variables(pcms_core_state_variables sVars)
{
    ...
    //strcat(&g_cms_core_state_variables.currconnectionIds[0],",5");

    memcpy(&sVars->dms_protocol_info,CMS_Context->pDmsPrototocolInfo, sizeof(cms_protocolinfo_params));
    strcpy(sVars->currconnectionIds,CMS_Context->pCurrConnectionIds);
    return TRUE;
...
BOOL cms_get_current_connection_IDs(char *pId)
{
    ...
    if(CMS_Context->NumOfConnections == 0)
        ...
        strcpy(pId,CMS_Context->pCurrConnectionIds);
...
cms_get_current_connection_info( unsigned int connection_id,pcms_connectioninfo_outparams pConInfo)
{
    ...
    if((CMS_Context->NumOfConnections == 0)||((CMS_Context->ConnectionListPtrs[connection_id]==NULL)&&(connection_id==0))){
        ...
        strcpy(pConInfo->connectioninfo_params.ConnectionStatus,"Unknown");
        strcpy(pConInfo->connectioninfo_params.Direction,"Output");
        pConInfo->connectioninfo_params.PeerConnectionID =-1;
        strcpy(pConInfo->connectioninfo_params.PeerConnectionManager,"");
        strcpy(pConInfo->connectioninfo_params.ProtocolInfo,"");
...
BC_STATUS cms_prepare_for_connection(pcms_prepare_connection_params pPrepInfo)
{
    ...
    if(!strstr(CMS_Context->pDmsPrototocolInfo->SourceProtocolInfo,pPrepInfo->inparams.rProtocolInfo))
    ...
    ret = strcpy(pNewConnectCtx->Connection.ConnectionStatus,(const char *)"unknown");
    strcpy(pNewConnectCtx->Connection.ProtocolInfo,pPrepInfo->inparams.rProtocolInfo);
    strcpy(pNewConnectCtx->Connection.PeerConnectionManager,pPrepInfo->inparams.PeerConnectionManager);
    strcpy(pNewConnectCtx->Connection.Direction,pPrepInfo->inparams.Direction);
...
cms_init()
{
    ...
    strcpy(pstr,g_cms_core_state_variables.currconnectionIds);
...
void CmsUpdateSrcProtocolInfo(char * SrcProtocolInfo)
{
    ...
    //strcpy(g_cms_core_state_variables.dms_protocol_info.SourceProtocolInfo,SrcProtocolInfo);
