                          ...
                          const char *deliveryPath, const char *sid, uint32_t seq, const char* body)
{
    ...
    snprintf(headers, sizeof(headers)-1, 
        ...
        "SID: %s\r\n"
        "CONNECTION: close\r\n"
        "SEQ: %d\r\n\r\n", ipAddress, port, strlen(body), sid, seq);
...
BUPnPError Gena_Subscribe(struct BUPnPDevice *pDevice, struct BUPnPService* pService)
{
    ...
    assert(strlen(pDevice->url.path) + strlen(pService->serviceInfo->eventSubUrl) < sizeof(uri) - 1);

    strncpy(uri, pService->serviceInfo->eventSubUrl, sizeof(uri)-1);
...
void Gena_XmlData(nxml_t handle, const char *tag_name, unsigned len)
{
    ...
    strncpy(name, handle->lastStartTag, handle->lastStartTagLen);
