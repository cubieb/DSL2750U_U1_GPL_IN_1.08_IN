...
const char* CDS_STORAGEMEDIUM_FORMATS[] =	{"UNKNOWN", "DV", "MINI-DV", "VHS", "W-VHS", "S-VHS", "D-VHS", "VHSC", "VIDEO8", "HI8", "CD-ROM", "CD-DA", "CD-R", "CD-RW", "VIDEO-CD", "DV", "SACD", "MD-AUDIO", "MD-PICTURE", "DVD-ROM", "DVD-VIDEO", "DVD-R", "DVD+RW", "DVD-RW", "DVD-RAM", "DVD-AUDIO", "DAT", "LD", "HDD"};
#define CDS_STORAGEMEDIUM_FORMATS_LEN		28
...
int _CdsToDidl_Helper_GetSizeForAudioItem(CdsToDidl_Fn_XmlEscapeLength fn_escape_length, struct CdsObject *cds_obj, unsigned int filter, unsigned int *print_these)
{
	...
	retval += _CdsToDidl_Helper_GetSizeForNonNegativeInt(
		filter, 
		CdsFilter_Track,
		CDS_DIDL_TRACK_ESCAPED_LEN,
		cds_obj->TypeMajor.AudioItem.Track,
		print_these);
	...
	retval += _CdsToDidl_Helper_GetSizeForSingleString(
		...
		filter,
		CdsFilter_Artist,
		CDS_DIDL_ARTIST_ESCAPED_LEN,
		cds_obj->TypeMajor.AudioItem.Artist,
		print_these);
			...
			while (filter[i] != '\0')
			{
				if (nextComma == 0)
				{
					...
					case 'd':
						...
						else if (strncasecmp(filter+i, CDS_STRING_DATE, CDS_STRING_DATE_LEN) == 0)
						{
					...
					case 'u':
						...
						else if (strncasecmp(filter+i, CDS_STRING_CREATECLASS, CDS_STRING_CREATECLASS_LEN) == 0)
						{
							retVal |= CdsFilter_CreateClass;
						}
						else if (strncasecmp(filter+i, CDS_STRING_SEARCHCLASS, CDS_STRING_SEARCHCLASS_LEN) == 0)
						{
							retVal |= CdsFilter_SearchClass;
						}
						nextComma = 1;
						break;
...
int CDS_GetCsvLengthFromBitString(unsigned int bitstring)
{
	...
	if (bitstring & CdsFilter_CreateClass)			{ length += CDS_STRING_CREATECLASS_LEN;			numFields++; }
	if (bitstring & CdsFilter_SearchClass)			{ length += CDS_STRING_SEARCHCLASS_LEN;			numFields++; }
...
char* CDS_ConvertBitStringToCsvString(char *csvString, unsigned int bitstring)
{
	...
	if (bitstring & CdsFilter_CreateClass) { WRITE_CSV_VALUE(csv, CDS_STRING_CREATECLASS, numElements); }
	if (bitstring & CdsFilter_SearchClass) { WRITE_CSV_VALUE(csv, CDS_STRING_SEARCHCLASS, numElements); }
...
char* CDS_SerializeObjectToDidl (struct CdsObject *mediaObj, int cds_obj_metadata_escaped, unsigned int filter, int includeHeaderFooter, int *outErrorOrDidlLen)
{
		...
		else
		{
			...
			*/
			...
			if ((mediaObj->MediaClass & CDS_CLASS_MASK_CONTAINER) != 0)
			{
				...
				/* upnp:createClass */
				if((mediaObj->TypeObject.Container.CreateClass != NULL) && (filter & CdsFilter_CreateClass))
				{
					printThese |= CdsFilter_CreateClass;
				...
				/* upnp:searchClass */
				if((mediaObj->TypeObject.Container.SearchClass != NULL) && (filter & CdsFilter_SearchClass))
				{	
					printThese |= CdsFilter_CreateClass;
			...
			if (includeHeaderFooter != 0)
			{
				...
				{
					size += CDS_DIDL_HEADER_ESCAPED_NO_DLNA_NAMESPACES_LEN;
				}
	...
	if (size > 0)
	{
		...
		if (includeHeaderFooter != 0)
		{
			...
			{
				cp += sprintf(cp, CDS_DIDL_HEADER_ESCAPED_NO_DLNA_NAMESPACES);				
			}
		}
		...
		/* print dlna:dlnaManaged */
		if(printThese & CdsFilter_CreateClass)
		{
			createClass = mediaObj->TypeObject.Container.CreateClass;
		...
		if(printThese & CdsFilter_SearchClass)
		{
			searchClass = mediaObj->TypeObject.Container.SearchClass;
		...
		case CDS_CLASS_MASK_MAJOR_AUDIOITEM:
			...
			if(printThese & CdsFilter_Artist && mediaObj->TypeMajor.AudioItem.Artist)
			{			
				cp += sprintf(cp, CDS_DIDL_ARTIST1_ESCAPED);
				cp += fnEscape(cp, mediaObj->TypeMajor.AudioItem.Artist);
				cp += sprintf(cp, CDS_DIDL_ARTIST2_ESCAPED);
			}			
			...
			if(printThese & CdsFilter_Track)
			{			
				cp += sprintf(cp, CDS_DIDL_TRACK1_ESCAPED);
				cp += sprintf(cp, "%d", mediaObj->TypeMajor.AudioItem.Track);
				cp += sprintf(cp, CDS_DIDL_TRACK2_ESCAPED);
			}
			break;
		case CDS_CLASS_MASK_MAJOR_IMAGEITEM:
...
{
	return CDS_DeserializeDidlToObjectEx(node, attribs, isItem, rangeStart, rangeEnd, 0);
}

struct CdsObject* CDS_DeserializeDidlToObjectEx(struct ILibXMLNode *node, struct ILibXMLAttribute *attribs, int isItem, const char *rangeStart, const char *rangeEnd, int isDlna)
{
	...
	startNode = node;
	node = startNode->Next;
	while (node != startNode->ClosingTag && error == 0)
	{
		if (node->StartTag != 0)
		{
...
//								c = att->Value[i];
			...
			else if (strcmp(node->Name, CDS_TAG_MEDIACLASS) == 0)
			{
				...
				if(strncmp(prefixNS, CDS_XML_NAMESPACE_UPNP, CDS_XML_NAMESPACE_UPNP_LEN)==0)
				{
					...
					if(newObj->MediaClass == CDS_CLASS_MASK_BADCLASS)
					{
						error = 1;
						break;
					}
			...
			else if (strcmp(node->Name, CDS_TAG_STORAGEMEDIUM) == 0)
			{
				prefixNS = ILibXML_LookupNamespace(node,node->NSTag,node->NSLength);
				if(strncmp(prefixNS, CDS_XML_NAMESPACE_UPNP, CDS_XML_NAMESPACE_UPNP_LEN)==0)
				{
					...
					if(_DidlToCds_Helper_FindStringInArray(innerXml, CDS_STORAGEMEDIUM_FORMATS, CDS_STORAGEMEDIUM_FORMATS_LEN) < 0)
					{
						error = 1;
						break;
					}
...
{
	if(
		(bit_string & CdsFilter_DlnaManaged) |
		(bit_string & CdsFilter_ResIfoFileUri) |
		(bit_string & CdsFilter_ResResumeUpload) |
		(bit_string & CdsFilter_ResUploadedSize) |
		(bit_string& CdsFilter_ResTrackTotal) |
		(bit_string & CdsFilter_TakeOut)
	)
	{
		return 1;
	}
