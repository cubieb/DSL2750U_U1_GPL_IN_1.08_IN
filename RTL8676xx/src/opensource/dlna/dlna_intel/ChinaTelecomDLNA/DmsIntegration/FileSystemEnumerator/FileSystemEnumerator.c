...
int _FSE_ExposeEntry(struct FSE_PathInfo *pi, struct FSE_State *fse, struct FSE_QueryState *qs)
{
	...
	if (
		...
		(strcmpi(pi->Ext,EXTENSION_VIDEO_AVI)==0) ||	/* AVI */
		(strcmpi(pi->Ext,EXTENSION_VIDEO_RM)==0) ||		/* RM */
		(strcmpi(pi->Ext,EXTENSION_VIDEO_RMVB)==0) ||	/* RMVB */
		(strcmpi(pi->Ext,EXTENSION_VIDEO_MP4)==0) ||	/* MP4 */
		(strcmpi(pi->Ext,EXTENSION_VIDEO_3GP)==0) ||	/* 3GP */
		(strcmpi(pi->Ext,EXTENSION_VIDEO_MKV)==0) ||	/* MKV */
		(strcmpi(pi->Ext,EXTENSION_VIDEO_MOV)==0) ||	/* MOV */
		(strcmpi(pi->Ext,EXTENSION_VIDEO_MPEG)==0) ||	/* MPEG */
...
//		(strcmpi(pi->Ext,EXTENSION_AUDIO_3GPP)==0) ||
//		(strcmpi(pi->Ext,EXTENSION_AUDIO_AAC)==0) ||
	...
	if ((qs->Flags & FSE_FLAG_Done) == 0)
	{
		...
		*/
		...
		if (expose != 0)
		{
			...
			{
				/* this is a file, use the file name for its title */
				cdsObj->Title = (char*) malloc((int) strlen(pi.Name)+1);
				strcpy(cdsObj->Title, pi.Name);
					...
					*/
					switch (cdsObj->MediaClass & CDS_CLASS_MASK_MAJOR)
					{
						case CDS_CLASS_MASK_MAJOR_AUDIOITEM:
						{
							...
							if (!InitAudioTagInfo(cdsObj->Source))
							{
							char *tmp_audio[1];
							char *temp_ptr = NULL;
							char **ptr = NULL;

							/* get creator/artist info */
							temp_ptr = GetAudioTagInfo("TPE1");
							if (temp_ptr)						
								cdsObj->Creator = temp_ptr;
							else
								cdsObj->Creator = "Unkown";

							/* get audio track info */
							temp_ptr = GetAudioTagInfo("TRCK");
							if (temp_ptr)
								cdsObj->TypeMajor.AudioItem.Track = atoi(temp_ptr);
							else
								cdsObj->TypeMajor.AudioItem.Track = 0;

							/* get Album info */
							cdsObj->TypeMajor.AudioItem.NumAlbums = 1;
							temp_ptr = GetAudioTagInfo("TALB");
							if (temp_ptr) {
								tmp_audio[0] = temp_ptr;
							} else 
								tmp_audio[0] = "Unkown";					
							
							ptr = tmp_audio;
							_CDS_Clone_MultipleStrings(&cdsObj->TypeMajor.AudioItem.Albums, 1, ptr);
							...
							ptr = tmp_audio;
							_CDS_Clone_MultipleStrings(&cdsObj->TypeMajor.AudioItem.Genres, 1, ptr);							
					                FinishAudioTagInfo();
							}		
#endif		
						}
						...
						case CDS_CLASS_MASK_MAJOR_VIDEOITEM:
						{
							...
							char *tmp_video[1] = {"Unkown"};
							char **ptr = tmp_video;							
							cdsObj->TypeMajor.VideoItem.NumGenres = 1;
							_CDS_Clone_MultipleStrings(&cdsObj->TypeMajor.VideoItem.Genres, 1, ptr);
						}
							break;
					}
				}
...
int FSE_LocalFilePathToRelativePath(void *fseObj, const char* localFilePath, int localFilePathLen, char **relativePath)
{
	...
	*relativePath = (char*)malloc(RetVal+1);
	memcpy(*relativePath,localFilePath+fse->RootPathLen,localFilePathLen-fse->RootPathLen);
...
int FSE_HandleCreateObject(void *fseObj, struct MSA_CdsCreateObj *createObjArg, struct DLNAProtocolInfo* protocolInfo, struct CdsObject *newCdsObject)
{
		...
		else
		{
			...
			switch(newCdsObject->MediaClass & CDS_CLASS_MASK_MAJOR)
			{
				...
				case CDS_CLASS_MASK_MAJOR_VIDEOITEM:
					...
					else if(strcmp(protocolInfo->Profile, DLNAPROFILE_MP4)==0)
					{
						extension = EXTENSION_VIDEO_MP4;
					}
					else if(strcmp(protocolInfo->Profile, DLNAPROFILE_3GP)==0)
					{
						extension = EXTENSION_VIDEO_3GP;
					}
					else if(strcmp(protocolInfo->Profile, DLNAPROFILE_MKV)==0)
					{
						extension = EXTENSION_VIDEO_MKV;
					}
					else if(strcmp(protocolInfo->Profile, DLNAPROFILE_MOV)==0)
					{
						extension = EXTENSION_VIDEO_MOV;
					}
					else if(strcmp(protocolInfo->Profile, DLNAPROFILE_MPEG)==0)
					{
						extension = EXTENSION_VIDEO_MPEG;
					}
					else
					{
						/* not supported */
	...
	// if parent id doesn't end with '\', add it
	if(ILibString_EndsWith(parentId, (int) strlen(parentId), "\\", 1)==0)
	{
		strcat(id, "\\");
	}
	strcat(id, title);
