...
#define DMS_UPLOAD_INITIATE_TIMEOUT 30
...
void _DMS_IncrementContainerUpdateID(struct DMS_State* dms, char* containerID)
{
	...
	length = (int) strlen(containerID);

	if(dms != NULL && dms->ContainderUpdateIDTable != NULL)
	{
		if(ILibHasEntry(dms->ContainderUpdateIDTable, containerID, length)!=0)
		{
			updateID = (int*)ILibGetEntry(dms->ContainderUpdateIDTable, containerID, length);
			(*updateID)++;
		}
		else
		{
			updateID = (int*) malloc(sizeof(unsigned int));
			*updateID = 1;
			ILibAddEntry(dms->ContainderUpdateIDTable, containerID, length, (void*) updateID);
		}
		MSA_UpdateContainerID(dms->Token_Msa, containerID, *updateID);
	}
}

void _DMS_UploadRequestExpired(void* uploadData)
{
	struct DMS_UploadData* data;
	struct DMS_UploadRequestObj* request;

	if(uploadData== NULL) return;
	data = (struct DMS_UploadData*) uploadData;

	ILibHashTree_Lock(data->UploadTable);

	request = (struct DMS_UploadRequestObj*) ILibGetEntry(data->UploadTable, data->UploadId, (int) strlen(data->UploadId));

	if(request != NULL && request->TransferStatus == NULL)
	{
		printf("HTTP-POST request expired after 30 secs.\r\n");
		...
		free(request);
		ILibDeleteEntry(data->UploadTable, data->UploadId, (int) strlen(data->UploadId));
	}
	ILibHashTree_UnLock(data->UploadTable);
	free(data->UploadId);
	free(data);
}
...
void AddHTTPResources(struct DMS_State *dms, unsigned int filter, struct MSA_CdsQuery * cds_query, struct CdsObject *cds_obj)
{
	...
	int tempLen;
/*
 * TBS_TAG: by zhaihaichen 2010-2-5
 * Desc:这里需要进行UTF8转换，因为经过HTTPEscape后路径就不是UTF8的了
 */  
    char *temp2;
	int IsTempFile = 0;
	...
	if(cds_obj->MediaClass & CDS_CLASS_MASK_ITEM)
	{
		...
		for(i=0;((CdsFilter_DlnaAllIp & filter) && i < cds_query->IpAddrListLen) || (i==0);++i)
		{
			...
			if(IsTempFile!=0)
			{
				...
				uploadId = (char*) malloc(DMS_UPLOAD_VIRTUAL_DIRNAME_LEN + tempLen + 2);
				sprintf(uploadId,"%s/%s",DMS_UPLOAD_VIRTUAL_DIRNAME,temp);
				r->ImportUri = _DMS_SetImportUriValue(cds_query->IpAddrList[i],ILibWebServer_GetPortNumber(dms->Token_WebServer),uploadId);
				r->Allocated |= CDS_RES_ALLOC_ImportUri;

				ILibHashTree_Lock(dms->UploadRequests);

				request = (struct DMS_UploadRequestObj*) ILibGetEntry(dms->UploadRequests, uploadId, (int) strlen(uploadId));
		
				if(request!=NULL && request->Resumable == 1)
				{
					r->ResumeUpload=1;
			...
			else
			{
				...
				//
				tempLen = FSE_LocalFilePathToRelativePath(dms->Token_BackEnd,cds_obj->Source,(int)strlen(cds_obj->Source),&temp);
				r->Size = (int)ILibFileDir_GetFileSize(cds_obj->Source); //ToDo: This needs to be a long
/*
 * TBS_TAG: by zhaihaichen 2010-2-5
 * Desc:这里需要进行UTF8转换，因为经过HTTPEscape后路径就不是UTF8的了
 */  
...
#else
                temp2 = EncodeToUTF8(temp);
                tempLen = strlen(temp2);
				r->Value = (char*)malloc(tempLen + 30 + DMS_VIRTUAL_DIRNAME_LEN + 1);	// http://255.255.255.255:65535/
				sprintf(r->Value,"http://%s:%u/%s/%s",ILibIPAddress_ToDottedQuad(cds_query->IpAddrList[i]),ILibWebServer_GetPortNumber(dms->Token_WebServer),DMS_VIRTUAL_DIRNAME,temp2);
                free(temp2);
#endif
/*
 * TBS_END_TAG
...
static void _DMS_MsaHandler_OnBrowse(MSA msa_obj, void* upnp_session, struct MSA_CdsQuery* cds_query)
{
	...
	*/
	...
	filter = filter | CdsFilter_Creator | CdsFilter_Date | CdsFilter_Album | CdsFilter_Genre |
			CdsFilter_Track;
...
static void _DMS_MsaHandler_OnGetCurrentConnectionInfo(MSA msa_obj, void* upnp_session, int connection_id)
{
	...
	{
		MSA_RespondGetCurrentConnectionInfo(msa_obj, upnp_session, -1, -1, "", "", -1, "Output", "Unknown");
	}
}
...
static void _DMS_WebServerHandler_OnContentRequest(struct ILibWebServer_Session *session, struct packetheader *header, char *bodyBuffer, int *beginPointer, int endPointer, int done, void *user)
{
	...
	char* profile;
	char* protocolStr;
	char* directiveObj=NULL;
	unsigned int transferMode = DH_TransferMode_Unspecified;
	int len = 0;
	...
	*/
	...
	if(strncasecmp(header->Directive, "GET", pdl) == 0 || strncasecmp(header->Directive, "HEAD", pdl) == 0)
	{
		...
		{
			protocolInfo->TM_S=1;
			transferMode |= DH_TransferMode_Streaming;
		}
			...
			if(request!=NULL)
			{
				...
				if (request != NULL)
				{
					...
					else if(dhs_error_code == DHS_ERRORS_PEER_ABORTED_CONNECTION)
					{
						...
						if(request->Resumable == 0)
						{
							/* if upload is aborted but client will not resume, then server can remove this request */
							ILibFileDir_DeleteFile(request->UploadLocalFilePath);
							request->TransferStatus = NULL;
							free(request->UploadId);
							free(request->ImportURI);
...
static void _DMS_MsaHandler_OnCreateObjectAndUpload(MSA msa_obj,  void* upnp_session, void* create_obj_arg, struct CdsObject* cds_obj)
{
	...
	*/
	...
	if(((cds_obj->MediaClass & CDS_CLASS_MASK_OBJECT_TYPE) == CDS_CLASS_MASK_ITEM) && cds_obj->ID!=NULL)
	{
		...
		if(
			(cds_obj->Title != NULL) &&
			((int) strlen(cds_obj->Title) > 0) &&
			(cds_obj->ParentID != NULL) &&
			((int) strlen(cds_obj->ParentID) > 0) &&
			(cds_obj->Res != NULL) && 
			(cds_obj->Res->ProtocolInfo != NULL) &&
			((int) strlen(cds_obj->Res->ProtocolInfo) > 0) &&
			...
			)
		{
			...
			while(prf!=NULL)
			{
				...
				if(
					(protocolInfo->Protocol==sourceProtocolInfo->Protocol || protocolInfo->Protocol==DLNAProtocolInfo_ProtocolType_ANY || sourceProtocolInfo->Protocol==DLNAProtocolInfo_ProtocolType_ANY) &&
					(ILibString_EndsWithEx(sourceProtocolInfo->Profile,
										  (int) strlen(sourceProtocolInfo->Profile),
										  protocolInfo->Profile,
										  (int) strlen(protocolInfo->Profile),
										  1)!=0)
				)
				{
					// there's a match of the protocol and profile, then DMS will support it
					reject=0;
			...
			/* we only support HTTP now and fixing up any unsupported features */
			protocolInfo->DLNA_Major_Version=1;
			protocolInfo->DLNA_Minor_Version=5;
			protocolInfo->DLNAPlayContainer = 0;
			protocolInfo->HTTP_Stalling = 1;
			protocolInfo->IsConvertedContent = 0;
			protocolInfo->IsPlaySingleUri = 0;
			protocolInfo->Protocol = DLNAProtocolInfo_ProtocolType_HTTP;
			protocolInfo->LimitedOperations_ByteBasedSeek = 0;
			protocolInfo->LimitedOperations_TimeBasedSeek = 0;
			protocolInfo->MaxSpeed_Major = 0;
			protocolInfo->MaxSpeed_Minor = 0;
			protocolInfo->RTSP_Pause = 0;
			protocolInfo->S0_Increasing = 0;
			protocolInfo->SenderPaced = 0;
			protocolInfo->SN_Increasing = 0;
			protocolInfo->SupportedPlaySpeeds = 0;
			protocolInfo->SupportedPlaySpeedsLength = 0;
			protocolInfo->SupportsByteBasedSeek = 1;
			protocolInfo->SupportsTimeBasedSeek = 0;

			protocolInfo->TM_B = 1;
			if((cds_obj->MediaClass & CDS_CLASS_MASK_MAJOR) == CDS_CLASS_MASK_MAJOR_IMAGEITEM)
			{
				protocolInfo->TM_I=1;
			}
			else if(((cds_obj->MediaClass & CDS_CLASS_MASK_MAJOR) == CDS_CLASS_MASK_MAJOR_AUDIOITEM) ||
					((cds_obj->MediaClass & CDS_CLASS_MASK_MAJOR) == CDS_CLASS_MASK_MAJOR_VIDEOITEM)
					)
			{
				protocolInfo->TM_S=1;
			}
			...
			else if(protocolInfo->NameValueTable != NULL)
			{
				char *key;
				int keyLength;
				void* data;

				/* remove any optional unsupported vendor-specified metadata by serializing back the protocolInfo */
				void* en = ILibHashTree_GetEnumerator(protocolInfo->NameValueTable);
				while(ILibHashTree_MoveNext(en)==0)
				{
					ILibHashTree_GetValue(en,&key,&keyLength,&data);
					if(strcmp(key, "DLNA.ORG_PN")!=0)
					{
						ILibDeleteEntry(protocolInfo->NameValueTable, key, keyLength);
						free(data);
				...
				free(cds_obj->Res->ProtocolInfo);
				cds_obj->Res->ProtocolInfo = DLNAProtocolInfo_Serialize(protocolInfo);
			}
			if(!reject)
			{
				if(
					(cds_obj->DlnaManaged & CDS_DlnaManaged_CreateChildContainer) ||
					(cds_obj->DlnaManaged & CDS_DlnaManaged_UploadContent)
					)
				{
					reject = 1;
				}
			}
			if(!reject && FSE_HandleCreateObject(dms->Token_BackEnd, createObjArg, protocolInfo, cds_obj)!=0)
			{
			...
			else if(!reject)
			{
				...
				*/
				...
				cds_obj->DlnaManaged = CDS_DlnaManaged_DestroyItem;
				...
				/* create a 0-byte file as a place holder */
				f = fopen(request->UploadLocalFilePath, "wb");
				if(f!=NULL)
				{
					fclose(f);
				}
				...
				if(MSA_ForCreateObjectResponse_AcceptUpload(msa_obj, upnp_session, createObjArg, cds_obj)==0)
				{
					...
					*/
					...
					/* add a 30-sec timer that if no HTTP-POST request, then remove this upload request */
					data = (struct DMS_UploadData*) malloc(sizeof(struct DMS_UploadData));
					data->UploadTable = dms->UploadRequests;
					data->UploadId = ILibString_Copy(request->UploadId, -1);
					ILibLifeTime_Add(dms->Token_Ltm, (void*) data, DMS_UPLOAD_INITIATE_TIMEOUT, _DMS_UploadRequestExpired, NULL);

					/* increment upnp event update IDs */
					MSA_IncrementSystemUpdateID(msa_obj);
					if(memcmp(cds_obj->ParentID, "0\\", strlen(cds_obj->ParentID)) == 0)
					{
						_DMS_IncrementContainerUpdateID(dms, "0");
					}
					else
					{
						_DMS_IncrementContainerUpdateID(dms, cds_obj->ParentID);
					}
				}
				else
				{
	...
	else if(((cds_obj->MediaClass & CDS_CLASS_MASK_OBJECT_TYPE) == CDS_CLASS_MASK_CONTAINER) && cds_obj->ID!=NULL)
	{
		...
		if(!reject)
		{
			if(cds_obj->DlnaManaged & CDS_DlnaManaged_DestroyItem)
			{
				reject = 1;
			}
		}
	...
	if(reject)
	{
		...
		{
			MSA_ForCreateObjectResponse_Reject(msa_obj, upnp_session, createObjArg, cds_obj, 712, "CreateObject Failed");
			printf("CreateObject Failed - Error: 712\r\n");
		}
	}
...
static void _DMS_MsaHandler_OnDestroyObject(MSA msa_obj, void* upnp_session, void* destroy_obj_arg)
{
	...
	*/
	...
	if (status == 0)
	{
		...
		if(ILibString_EndsWith(destroyObj->ID, (int) strlen(destroyObj->ID), "\\", 1) != 0)
		{
			/* remove any trailing '\' because the object id could a container */
			destroyObj->ID[(int) strlen(destroyObj->ID) -1] = '\0';
		}
		slashPos = ILibString_LastIndexOf(destroyObj->ID, (int) strlen(destroyObj->ID), "\\", 1);
		if(slashPos > 0 && slashPos < (int) strlen(destroyObj->ID) -1)
		{
			/* find the parent Id */
			destroyObj->ID[slashPos+1] = '\0';
			if(memcmp(destroyObj->ID, "0\\", strlen(destroyObj->ID)) == 0)
			{
				_DMS_IncrementContainerUpdateID(dms, "0");
			}
			else
			{
				_DMS_IncrementContainerUpdateID(dms, destroyObj->ID);
			}
		}
		MSA_IncrementSystemUpdateID(msa_obj);
	}
...
void DMS_DestroyMediaServer(DMS_StateObj dms_obj)
{
	...
	if(dms->ContainderUpdateIDTable != NULL) 
	{
		ILibHashTree_Lock(dms->ContainderUpdateIDTable);
		en = ILibHashTree_GetEnumerator(dms->ContainderUpdateIDTable);
		while(ILibHashTree_MoveNext(en)==0)
		{
			ILibHashTree_GetValue(en,&key,&keyLen,&val);
			free((int*) val);
		...
		ILibHashTree_DestroyEnumerator(en);
		ILibHashTree_UnLock(dms->ContainderUpdateIDTable);
	}
	ILibDestroyHashTree(dms->ContainderUpdateIDTable);
						...
						void* dms_user_obj)
{
	...
	*/
	...
	/* intialize the container update id table for storing the list modified containers */
	retval->ContainderUpdateIDTable = ILibInitHashTree();
	...
	/* TODO: we disable CDS:Search action in the service description document
	 * If vendor decides to implement CDS:Search, just remove the following line, and implement the search callback 
	 * handler in DmsIntegration layer
	 */
	MediaServer_GetConfiguration()->ContentDirectory->Search = NULL;
	MediaServer_GetConfiguration()->ContentDirectory->StateVar_A_ARG_TYPE_SearchCriteria = NULL;
