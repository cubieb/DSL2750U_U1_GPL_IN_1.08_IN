...
{
	/* walk around, using big buffer to avoid socket error in some cases. */
#define WALKAROUND_BIG_BUF_FOR_IP_MREQ  100             /* 100 is big enough for struct ip_mreq */
	/* struct ip_mreq mreq; */
	char mreq[WALKAROUND_BIG_BUF_FOR_IP_MREQ] = "";     /* avoids socket error in some cases. */
#if defined(WIN32) || defined(_WIN32_WCE)
	...
	((struct ip_mreq *)mreq)->imr_multiaddr.s_addr = remoteInterface;
	((struct ip_mreq *)mreq)->imr_interface.s_addr = localInterface;
