...
int delete_temp_file()
{
        char cmd[64];
        memset(cmd, 0, 64);
        sprintf(cmd, "rm -rf %s/%s %s/%s", FILE_PATH, FIRMWARE_FILE, FILE_PATH, CONFIG_FILE);
        tbsSystem(cmd, 1);
        return 0;
}

int do_update(int action)
{
    char szCmd[100];
    ST_MSG *pstMsg;
    ...
    {
        sprintf(szCmd, "upgrader -c %d -p "FILE_PATH"/"CONFIG_FILE" -u FTPUPG", CONFIG_READ);
    }
    else if (action == CONFIG_WRITE)
    {
        sprintf(szCmd, "upgrader -c %d -p "FILE_PATH"/"CONFIG_FILE" -u FTPUPG", CONFIG_WRITE);
    }
    else if (action == IMG_WRITE)
    {
        sprintf(szCmd, "upgrader -c %d -p "FILE_PATH"/"FIRMWARE_FILE" -u FTPUPG", IMG_WRITE);
    }
    else
    {
        return TBS_FAILED;
...
#ifdef TBS_FTPUPG_DEBUG
    if (nRet != 0)
    {
        FILE *fd;
        fd = fopen("/var/bftpd.log", "w");
        fprintf(fd, "err = %d\n", nRet);
        fprintf(fd, "errno = %d\n", errno);
        pstMsg = strerror(errno);
        fprintf(fd, "Mesg: %s\n", pstMsg);
        fclose(fd);
    }
    else
    ...
    usMsgType = MSG_ReceiveMessage(&pstMsg, 5);

    switch (usMsgType)
    {
        case MSG_OK:
            bftpd_log("Receive msg success.\n");
            break;
        default:
            bftpd_log("Receive msg failed.\n");
            return TBS_FAILED;
    }
    ...
    if (ulResult != 0)
    {
        bftpd_log("do_update failed.\n");
        safe_free_msg(pstMsg);
        return TBS_FAILED;
    }

    bftpd_log("do_update success.\n");
    safe_free_msg(pstMsg);
    return TBS_SUCCESS;
}
#endif
...
void do_stor(char *filename, int flags)
{
    ...
    char *ptmp = NULL;
       ...
       {
           acttype = IMG_WRITE;
	   /* Release memory to receive the image */
           tbsSystem("killall -16 pc", 1);
       }
       else if (!strcmp(filename, CONFIG_FILE))
       {
           acttype = CONFIG_WRITE;
       }
       else
       {
            control_printf(SL_FAILURE, "214 Useage: \"put "CONFIG_FILE"\" or \"put image.img\"");
            return;
       }
       
        sprintf(mapped, "%s/%s", FILE_PATH, filename);
       
#endif
    //判断文件是否是.img文件或.xml文件，并设置相应的保存路径
    if((ptmp = strchr(filename, '.')) != NULL && ptmp != filename)
    {
        if (!strcmp(ptmp, FIRMWARE_FILE_EXTEND))
        {
            acttype = IMG_WRITE;
            /* Release memory to receive the image */
            tbsSystem("killall -16 pc", 1);

            sprintf(mapped, "%s/%s", FILE_PATH, FIRMWARE_FILE);
        }
        else if (!strcmp(ptmp, CONFIG_FILE_EXTEND))
        {
            acttype = CONFIG_WRITE;
            sprintf(mapped, "%s/%s", FILE_PATH, CONFIG_FILE);
        }
        else
        {
            control_printf(SL_FAILURE, "214 Useage: \"put xxx"CONFIG_FILE_EXTEND"\" file or \"put xxx"FIRMWARE_FILE_EXTEND"\" file");
            return;
        }
    }
    else
    {
	  ...
	  if (fd == -1) {
		control_printf(SL_FAILURE, "553 Error: %s.%s", strerror(errno), mapped);
	...
	if (!((i = recv(sock, buffer, my_buffer_size - 1, 0))))
        ...
        file_size += i;
        if (((acttype == IMG_WRITE) && (file_size > TBS_MAX_IMG_SIZE)) ||
        	((acttype == CONFIG_WRITE) && (file_size > TBS_MAX_CONFIG_SIZE)))
        {
            if(buffer)
                free(buffer);
            close(fd);
            delete_temp_file();
            control_printf(SL_FAILURE, "552 Error: File oversize or Insufficient storage space in system, Aborted.");
            close(sock);
    	     return;
        }
	...
	close(fd);
       ...
       if (TBS_SUCCESS == do_update(acttype))
       {
           control_printf(SL_SUCCESS, "221 Restart system now, See you later.");
          close(sock);
           exit(0);
       }
       else
       {
           delete_temp_file();
           control_printf(SL_SUCCESS, "553 Error: Upgrader failed.");
       }
...
void command_retr(char *filename)
{
	...
	struct stat statbuf;
        ...
        if (!strcmp(filename, CONFIG_FILE))
        {
            if (0 != do_update(CONFIG_READ))
            {
                control_printf(SL_FAILURE, "553 Error: Unknown error occured, Download config failed.");
                return;
            }
        }
        else
        {
            control_printf(SL_FAILURE, "214 Useage: \"get "CONFIG_FILE".\"");
            return;
        }
       ...
       xfer_delay = 0;
	sprintf(mapped, "%s/%s", FILE_PATH, "cfg.xml");
...
#ifdef TBS_FTPUPG
	{"PWD", "(returns cwd)", command_list, STATE_AUTHENTICATED, 0},
	{"NLST", "[<sp> pathname]", command_list, STATE_AUTHENTICATED, 0},
#endif
...
int parsecmd(char *str)
{
	...
	char *p, *pp;
#ifndef TBS_FTPUPG
	char confstr[64]; /* strlen("ALLOWCOMMAND_XXXX") + 1 == 18 */
#endif
...
#ifdef TBS_FTPUPG
	control_printf(SL_FAILURE, "202 Command not implemented, superfluous at this site.", str);
#else
