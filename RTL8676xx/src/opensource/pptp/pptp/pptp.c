...
int pptp_openpty(int *master, int *slave, char *name, void *unused1, void *unused2)
{
...
int main(int argc, char **argv, char **envp)
{
    ...
    if(launchpppd){
        rc = pptp_openpty (&pty_fd, &tty_fd, ttydev, NULL, NULL);
...
void launch_pppd(char *ttydev, int argc, char **argv)
{
    char *new_argv[argc + 5];/* XXX if not using GCC, hard code a limit here. */
    int i = 0, j;
    new_argv[i++] = PPPD_BINARY;

#ifdef PPPD_OPTIONS
    new_argv[i++] = "file";
    new_argv[i++] = PPPD_OPTIONS;
#endif
//#ifdef USER_PPP
#if 0
    new_argv[i++] = "-direct";
