...
#ifdef RADAR_IPP_DEBUG
	printf("ippd [fd_%03d](%s) print. \n", conn->fd, inet_ntoa(conn->addr));
#endif
#ifdef IPP_DEBUG
...
static IPP *ipp_get_jobs( IPP *ipp )
{
  ...
  if( ( printer = ipp_get_printer( ipp ) ) == NULL ) { // Get printer from printer-uri
    ...
    val = 9;
    val = htonl(val);
    ipp_add_tag( response, IPP_TAG_ENUM, "job-state", &val, 4, 0); /* 9 = COMPLETE */
...
IPP *ipp_print_job( Connection *conn, int len )
{
  ...
  if( ( printer = ipp_get_printer( ipp ) ) == NULL ) { // Get printer from printer-uri
    ...
    //penguin 20050915, discard this Job if printer doesn't exist on system
    struct stat p_stat;
    if (stat(printer->device, &p_stat) != 0)
    {
