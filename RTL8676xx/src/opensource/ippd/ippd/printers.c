...
/* append by radar. 20070619 */
#define	FOR_PRINTER_INFO
...
/* Reason: if ippd open printer device at the startup, the cfm can't open for getting printer info. */
#ifdef	FOR_PRINTER_INFO	/* by radar. 20070619 */
	new_printer->state = PRINTER_CLOSED;
#else
...
Job *printer_list_job( Printer *printer, Job *prev )
{
  ...
  return (Job *)printer->jobs; //add by radar 20090427
}
