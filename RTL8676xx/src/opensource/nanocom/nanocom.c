...
fprintf(stderr, "Usage: nanocom device [options]\n\n"
            "\tdevice\t\tSpecifies which serial port device to use, on most systems this will be /dev/ttyS0 or /dev/ttyS1\n\n" 
            "\t[options] All options are required\n\n"
            "\t n save result to /var/pppd3g/atresult_n \n\n"); 
#endif
  exit(1);
}
...
fd=open(argv[1], 0);
	  	if (fd<0)
		{
	 	...
	 	rs=ioctl(fd, CDROMEJECT,0);
	  	if (rs!=0)
		    {
			      fprintf(stderr, "CDROMEJECT faile (error %d).\n", rs);
			      //exit (-1);
		    }
		  else
		    fprintf(stdout, "CDROMEJECT ok");
#endif
