...
static char const ident[]="$Id: sock.c,v 1.36 2009/05/21 09:07:34 hb9xar Exp $";
...
int sipsock_listen (void) {
   ...
   sip_udp_socket=sockbind(ipaddr, configuration.sip_listen_port, 1, NON_RAW);//modified by baumann, add last arg 0 to indicate not raw flag
 ...
 */
int sockbind(struct in_addr ipaddr, int localport, int errflg, int rawflag) {/*add last arg rawflag to indicate raw by baumann*/
