...
/* This formats the essential NTP data, as a debugging aid. */
    fprintf(stdout,"=================================================\n");
    fprintf(stderr,"status=%d, version=%d, mode=%d, stratum=%d, poll=%d, precision=%d\n", 
                    data->status,data->version,data->mode,data->stratum,data->polling, data->precision);
    fprintf(stderr, "dispersion=%.6f, \n",data->dispersion);
    fprintf(stderr, "reference=%.6f, \n",data->reference);
    fprintf(stderr, "originate=%.6f, \n",data->originate);
    fprintf(stderr, "receive=%.6f, \n",data->receive);
    fprintf(stderr, "transmit=%.6f, \n",data->transmit);
    fprintf(stderr, "current=%.6f, \n",data->current);
    fprintf(stdout,"==================================================\n");
}
...
/* Read the packet and deal with diagnostics. */
    ...
    if (verbose > 2) {
        fprintf(stderr,"read_packet: Incoming packet on socket descriptors[%d]:%d\n",which, descriptors[which]);
...
/*************************************************************************
 修订记录:
            ...
            日期: 2008-9-4
            内容:
                  原处理检查status为非0时表示出错，根据协议文档status为非0
            是警告信息，我们测试环境中的ntp服务器经常有警告信息，所以定义一
            个宏来弱化检查
 *************************************************************************/
#define TBS_WEAK_CHECK 1
#ifdef TBS_WEAK_CHECK
...
systems do not set the return value from (s)printf. */
    fprintf(stdout,"the offset value is : %d \n", offset);
    
    now = convert_time(current_time(offset),&milli);
    
    printf("utc new time is :%d\n", (long)now);
...
/* If the correction is large, ask for confirmation before proceeding. */
    ...
    if (absoff > prompt) {
        if (! daemon && ftty(stdin) && ftty(stdout)) {
            fprintf(stdout,"The time correction is %.3f +/- %.3f+%.3f seconds\n",
                offset,dispersion,error);
            fprintf(stdout,"Do you want to correct the time anyway? ");
            fflush(stdout);
...
void run_client (char *hostnames[], int nhosts) {
    ...
    /* 如果所有的sntp server都不能连接上，则退出程序 */
    for (k = 0; k < nhosts; ++k)
    {
        open_socket(k,hostnames[k],delay);
    ...
    attempts = 0;
    deadline = current_time(JAN_1970)+delay+5;
...
continue1:  ;
	...
	else 
	{
		...
		{
            /* 对于socket建立失败的服务器，不试图去连接 */
            if (!valid_socket(cycle))
            {
                if (++cycle >= nhosts)
                    cycle = 0;
                continue;
            ...
            if (read_packet(cycle,&data,&x,&y)) 
			{
                if (++rejects >= count)
                {
                    printf("too many bad or lost packets\n");
                    break;
    	        }
		        else
		        {
		            if (++cycle >= nhosts) cycle = 0;  //重置cycle,重新连接NTP Servers
                    	continue;
		        }
            } 
			else
            ...
            if (verbose > 2)
                fprintf(stderr,"Offset=%.6f+/-%.6f, Dispersion=%.6f\n",x,y,dispersion);
            ...
            if (a > b) 
			{
                ...
                //fatal(0,"inconsistent times got from NTP server on socket %s", text);
                // 获取的时间不一致时，不需要让进程退出，再获取一次就可以了
                continue;
            }
            if (error <= minerr)
				break;
    ...
    {
        fprintf(stderr,"error > NTP_INSANITY\n");
    ...
    if (locked) set_lock(0);
#endif
    if (verbose > 2) fprintf(stdout,"Stopped normally\n");
    exit(EXIT_SUCCESS);
}
...
int main (int argc, char *argv[]) {
	...
	//fprintf(pFile, %s, "Decode the arguments.\n");
    ...
    while (argc > 1) 
	{
        ...
        if (strcmp(argv[1],"-B") == 0 && action == 0) 
		{
			...
			{
                period = 60*60;  //default period = 60 minutes
			}
        } 
		...
		else if (strcmp(argv[1],"-P") == 0 && prompt == 0.0 && argc > 2) 
		{
			...
			{
                if (sscanf(argv[2],"%lf%c",&prompt,&c) != 1)
					syntax(1);
	...
	else
	{
		...
		{ /*指定了NTP server的地址*/
            ...
            for (k = 1; k < argc; ++k) 
			{
                ...
                hostnames[k-1] = argv[k];  //NTP servers
            }
            nhosts = argc-1; //NTP servers数量
        }
		else 
		{
            operation = op_listen;
		...
		else 
		{
            ...
            if (delay == 0) /*set delay default value*/
                delay = (operation == op_listen ? 300 : (2*count >= 15 ? 2*count+1 :15));
    ...
    if (verbose) 
	{
        ...
        {
            if (hostnames[k] != NULL)
				fprintf(stderr," %s",hostnames[k]);
        }
		
        fprintf(stderr,"\n");
    }
    if (nhosts == 0) 
		nhosts = 1;    /* Kludge for broadcasts */

	#if defined(_SUPPORT_SERVER_MODE)
	
   		if (operation == op_server || operation == op_broadcast)
        	run_server();
	#elif defined(_SUPPORT_DAEMON_MODE)
