...
int enable_igmp_passthrough = 0;
int enable_fast_leaving	= 0;

int general_query_interval = GENERAL_QUERY_INTERVAL;
int general_query_response_interval = GENERAL_QUERY_RESPONSE_INTERVAL;

int group_query_interval = MEMBER_QUERY_INTERVAL;
int group_query_response_interval = MEMBER_QUERY_RESPONSE_INTERVAL;
int group_query_count = MEMBER_QUERY_COUNT;

int lm_query_interval = LAST_MEMBER_QUERY_INTERVAL;
int lm_query_count = LAST_MEMBER_QUERY_COUNT;

#ifdef IGMP_GENERAL_QUERY
#ifdef _SUPPORT_IGMPV3_
int gq_lefttime;
void handle_general_timer()
{
	if (general_query_interval == 0) return;
	
	gq_lefttime--;
	if (gq_lefttime <= 0)
	{
		igmp_query(ALL_SYSTEMS, 0, general_query_response_interval);
		gq_lefttime = general_query_interval;
	}
}
#else
struct callout gq_timer;		//general query timer
void igmp_general_timer_expired(void *arg)
{
	timeout(igmp_general_timer_expired , NULL, general_query_interval, &gq_timer);
	igmp_query(ALL_SYSTEMS, 0, general_query_response_interval);
}
...
struct mcft_entry * add_mcft(__u32 grp_addr, __u32 src_addr)
{
	...
	mcp->srclist = NULL;
	mcp->timer.lefttime = group_query_interval;
	mcp->timer.retry_left = group_query_count;
	mcp->mrt_state = 0;
...
//#endif
		...
		//printf( "<t>\n" );
		timeout(igmp_specific_timer_expired , mcp, lm_query_interval, &mcp->timer.ch);
		igmp_query(ALL_SYSTEMS, mcp->grp_addr, lm_query_interval);
	}
...
int add_group_and_src( __u32 group, __u32 src )
{
	...
	//	struct IfDesc *up_dp = getIfByName(igmp_up_if_name);
		...
		timeout(igmp_specific_timer_expired , mymcp, group_query_interval, &mymcp->timer.ch);
#endif
	}
	else {
...
int del_group_and_src( __u32 group, __u32 src )
{
	...
	if(chk_mcft(group)) {
		...
		if(mymcp) {
			...
			{
#endif
			mymcp->timer.retry_left = lm_query_count -1; //bellow have send once query packet
			timeout(igmp_specific_timer_expired , mymcp, lm_query_interval, &mymcp->timer.ch);
			igmp_query(ALL_SYSTEMS, mymcp->grp_addr, lm_query_interval);
#ifdef KEEP_GROUP_MEMBER
			}
#endif
		}
...
int igmp_leave(__u32 grp, int if_idx)
{
    ...
    {
        dp = getIfByName(igmp_up_if_name[i]);
        if(NULL == dp)
            continue;
...
int main(int argc, char **argv)
{
	...
	int index;
       ...
       if (argc >= 12 + ADVANCED_PARAMETERS_COUNT) {
#else
	if (argc >= 12) {
#endif
#else
#ifdef CONFIG_ADVANCED_PARAMETERS
       if (argc >= 5 + ADVANCED_PARAMETERS_COUNT) {
#else
	if (argc >= 5) {
#endif
#endif
...
#ifdef CONFIG_ADVANCED_PARAMETERS
	for(index=1 + ADVANCED_PARAMETERS_COUNT;index<argc-1;index++)
		strcpy(igmp_up_if_name[index-ADVANCED_PARAMETERS_COUNT-1], argv[index]);
	igmp_up_if_num=argc-2-ADVANCED_PARAMETERS_COUNT;
	
	enable_igmp_passthrough = atoi(argv[1]);
	enable_fast_leaving	= atoi(argv[2]);

	general_query_interval = atoi(argv[3]);
	general_query_response_interval = atoi(argv[4]);

	group_query_interval = atoi(argv[5]);
	group_query_response_interval = atoi(argv[6]);
	group_query_count = atoi(argv[7]);

	lm_query_interval = atoi(argv[8]);
	lm_query_count = atoi(argv[9]);
	...
	{
		strcpy(igmp_down_if_name, argv[argc-1]);
		strcpy(igmp_up_if_name, argv[argc-2]);
	}
	
#ifdef CONFIG_ADVANCED_PARAMETERS
	strcpy(igmp_up_if_name, argv[1 + ADVANCED_PARAMETERS_COUNT]);
	...
	signal(SIGUSR2, get_static_group_file);
#endif
	while (!initMRouter())
	{
...
#ifdef IGMP_GENERAL_QUERY
	init_gq_timer();	//init general query timer
#endif


#ifdef CONFIG_APPS_LOGIC_STATIC_IGMP
	get_static_group_file(0);
#endif
	/* process loop */
	while(1)
	{
