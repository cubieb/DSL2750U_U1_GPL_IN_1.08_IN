...
system_store(int a, int b, void *c, void *d)
{
    ...
    if(CMM_SetStrColumn(SNMPC_PATH_PREFIX, NODE_SystemContact, sysContact, strlen(sysContact), NULL, 0) != SUCCESS)
    {
        snmp_log(LOG_WARNING,"Error returned after calling CMM_SetStr. node:%s%s, value:%s\n", SNMPC_PATH_PREFIX, NODE_SystemContact, sysContact);
        return SNMP_ERR_GENERR;
    }
    if(CMM_SetStrColumn(SNMPC_PATH_PREFIX, NODE_SystemLocations, sysLocation, strlen(sysLocation), NULL, 0) != SUCCESS)
    {
        snmp_log(LOG_WARNING,"Error returned after calling CMM_SetStr. node:%s%s, value:%s\n", SNMPC_PATH_PREFIX, NODE_SystemLocations, sysLocation);
        return SNMP_ERR_GENERR;
    }		
    return 0;
}
