...
#include "mibgroup/tbs-private/common/cmmif.h"
...
/*
 * ZTE hot AP linkup trap
 * Return: 0 - successful, non-zero fail
 */
static int ZTE_launchLinkupTrap()
{
    netsnmp_variable_list *notification_vars = NULL;
    char **ref = NULL;
/* 这些鸟东西来自中兴"第三方AP接入中兴网管接口规范V1.0"，如果它有变化，注意修改这里就是了............
ID	            OID	              类型	         说明
sysUpTime  	.1.3.6.1.2.1.1.3	TimeTicks	上电时间,是协议规定需实现的    <这个值定义在snmptrap.c文件中了>
snmpTrapOID	.1.3.6.1.6.3.1.1.4.1	SnmpOid	TrapOid,其值必须为.1.3.6.1.4.1.3902.1.8.1.8.10.0.8
nodeId 	.1.3.6.1.4.1.3902.1.8.1.8.1.1	DisplayString	设备标识，在全网中必须唯一，可引用Mac地址，也可在开通时设定
alarmIpAddr	.1.3.6.1.4.1.3902.1.8.1.8.10.2	IpAddress	IP地址
alarmCause	.1.3.6.1.4.1.3902.1.8.1.8.10.5	Integer	告警原因码，本Trap中必须为1001
productType	.1.3.6.1.4.1.3902.1.8.1.8.1.3	DisplayString	设备型号，如W800A、W640等
productVersion	.1.3.6.1.4.1.3902.1.8.1.8.1.6	DisplayString	当前版本号
*/
    oid zte_snmptrap_oid[] = {1, 3, 6, 1, 6, 3, 1, 1, 4, 1, 0};
    oid zte_snmptrap_val[] = {IGD_OID_PREFIX, 8, 1, 8, 10, 0, 8};
    oid zte_alarm_cause[] = {IGD_OID_PREFIX, 8, 1, 8, 10, 5}; //alarmCause
    oid zte_ref_oid[][13] = {{IGD_OID_PREFIX, 8, 1, 8, 10, 2}, //IpAddress
                                          {IGD_OID_PREFIX, 8, 1, 8, 1, 1}, //nodeId
                                          {IGD_OID_PREFIX, 8, 1, 8, 1, 3}, //productType
                                          {IGD_OID_PREFIX, 8, 1, 8, 1, 6}}; //productVersion
    unsigned long ulalarmCause = 1001;
    int i = 0;

    ref = (char**)malloc(LINKUP_REF_NUM * sizeof(char*));
    for(; i<LINKUP_REF_NUM; i++)
    {
        ref[i] = (char*)malloc(64); /*64 is defined in cmmif.c with variable <iPathLen>*/
    }
    if(CMM_GetZTE_LinkupRef(ref) != 0) return -1;

    /*
   * add in the trap definition object
   */
    snmp_varlist_add_variable(&notification_vars, zte_snmptrap_oid, OID_LENGTH(zte_snmptrap_oid), ASN_OBJECT_ID, 
                                                  (u_char *)zte_snmptrap_val, OID_LENGTH(zte_snmptrap_val) * sizeof(oid));

    /*nodeId 	.1.3.6.1.4.1.3902.1.8.1.8.1.1	DisplayString	设备标识，
                                                            在全网中必须唯一，
                                                            可引用Mac地址，也可在开通时设定*/
    snmp_varlist_add_variable(&notification_vars, zte_ref_oid[1], OID_LENGTH(zte_ref_oid[1]),
    	                                            ASN_OCTET_STR, (u_char *)ref[1], strlen(ref[1]));


    /*alarmIpAddr	.1.3.6.1.4.1.3902.1.8.1.8.10.2	IpAddress	IP地址*/
    snmp_varlist_add_variable(&notification_vars, zte_ref_oid[0], OID_LENGTH(zte_ref_oid[0]),
    	                                            ASN_IPADDRESS, CHARS_TO_intIPADDR(ref[0]), sizeof(unsigned long));
    
    /*alarmCause	.1.3.6.1.4.1.3902.1.8.1.8.10.5	Integer	告警原因码，本Trap中必须为1001*/
    snmp_varlist_add_variable(&notification_vars, zte_alarm_cause, OID_LENGTH(zte_alarm_cause),
    	                                            ASN_INTEGER, (u_char *)&ulalarmCause, sizeof(ulalarmCause));  

    /*productType	.1.3.6.1.4.1.3902.1.8.1.8.1.3	DisplayString	设备型号，如W800A、W640等*/
    snmp_varlist_add_variable(&notification_vars, zte_ref_oid[2], OID_LENGTH(zte_ref_oid[2]),
    	                                            ASN_OCTET_STR, (u_char *)ref[2], strlen(ref[2]));

    /*productVersion	.1.3.6.1.4.1.3902.1.8.1.8.1.6	DisplayString	当前版本号*/
    snmp_varlist_add_variable(&notification_vars, zte_ref_oid[3], OID_LENGTH(zte_ref_oid[3]),
    	                                            ASN_OCTET_STR, (u_char *)ref[3], strlen(ref[3]));
    send_v2trap(notification_vars);
    snmp_free_varbind(notification_vars);

    /*free the variable for reference values*/
    for(i=0; i<LINKUP_REF_NUM; i++)
    {
        free((char*)ref[i]);
    }
    free(ref);
 ...
 */
 #ifdef ZTE_HOT_AP_SNMP_IF
 #include "mibgroup/tbs-private/common/snmpProcess.h"
 #endif
static int
receive(void)
{
...
//#ifdef ZTE_HOT_AP_SNMP_IF
  //  if(SNMPA_GetTrapEventSavedInFlash() == TBS_SUCCESS)
   // {
     //   SNMPA_TRACE("One trap about download/upload must be sent to manager when start snmpa...\n");
     //   SNMPA_SendDl_UlSuccTrap(NULL, "1"); /*1 marks this is a start trap for download after reboot*/
   // }
//#endif
    /*
    ...
    while (netsnmp_running)
	{
        ...
        //send Start trap info
      //  SNMPA_Send_StartTrap();
    
#ifdef ZTE_HOT_AP_SNMP_IF
  #ifndef TBS_SNMP_APP
        ZTE_launchLinkupTrap();
  #endif
after_recieving_logic_msg:
#else 
      //  send_easy_trap(SNMP_TRAP_ENTERPRISESPECIFIC, 3);
#endif
...
#ifdef ZTE_HOT_AP_SNMP_IF
        FD_SET(MSG_GetMsgSocket(), &readfds);
        if(MSG_GetMsgSocket() > numfds) numfds = MSG_GetMsgSocket() + 1;
#endif
    ...
    reselect:
	    ...
	    {
#ifdef ZTE_HOT_AP_SNMP_IF
                if (FD_ISSET(MSG_GetMsgSocket(), &readfds))
                {
                    snmpa_handleMessage();
                    goto after_recieving_logic_msg;
                }
#endif
