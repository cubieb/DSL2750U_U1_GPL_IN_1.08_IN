...
int br_set_static_mac(int enable,const char* brname,const char* staticmac)
{
	int ret;
	struct ifreq ifr;
	
	unsigned int arg[4] = {0,0,0,0};
		
	strncpy(ifr.ifr_name, brname, IFNAMSIZ);
	
	arg[0] = BRCTL_SET_STATIC_MAC;
	unsigned short *pEnable = (unsigned short *)(arg+1);
	unsigned char *pMac = ((unsigned char *)arg)+6;
		
	*pEnable = enable;
	if(staticmac)
		memcpy(pMac,staticmac,6);
...
int br_mld_set_port_snooping(const char *br, const char *port, const char *addr)
{
	unsigned int iaddr[8];
	...
	/* mode, 1 byte */
	sscanf(addr+26, "%02x", iaddr);
	dest[12] = iaddr[0];

	/* src_in6, high-8 byte */
	sscanf(addr+28, "%02x%02x%02x%02x%02x%02x%02x%02x", iaddr, iaddr+1, iaddr+2, iaddr+3, iaddr+4, iaddr+5, iaddr+6, iaddr+7);
	for (i=0; i < 8; i++)
		dest[i+13] = iaddr[i];

	/* src_in6, low-8 byte */
	sscanf(addr+36, "%02x%02x%02x%02x%02x%02x%02x%02x", iaddr, iaddr+1, iaddr+2, iaddr+3, iaddr+4, iaddr+5, iaddr+6, iaddr+7);
	for (i=0; i < 8; i++)
		dest[i+21] = iaddr[i];

	return port_set(br, port, "port_snooping", (unsigned long)dest, BRCTL_MLD_SET_PORT_SNOOPING);
}

int br_mld_clear_port_snooping(const char *br, const char *port, const char *addr)
{
	unsigned int iaddr[8];
	unsigned char dest[32];
	int i = 0;
	...
	return port_set(br, port, "port_snooping", (unsigned long)dest, BRCTL_MLD_CLEAR_PORT_SNOOPING);
}

int br_mld_enable_port_snooping(int enable)
{
	int ret;
	unsigned long arg[3] = {BRCTL_MLD_ENABLE_SNOOPING, 0, 0};
...
int br_mld_enable_proxy_mode(const char *br, int enable)
{
	int ret;
	char _br[IFNAMSIZ];
	unsigned long arg[3] = {BRCTL_MLD_ENABLE_PROXY_MODE, (unsigned long)_br, 0};
