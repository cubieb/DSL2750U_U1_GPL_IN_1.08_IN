...
void acceptIgmp(int recvlen) {
    ...
    if (ip->ip_p == 0) {
        if (src == 0 || dst == 0) {
            ...
            else if(!IN_MULTICAST(ntohl(dst))) {
                log(LOG_WARNING, 0, "The destination address %s is not a multicast.",
                    inetFmt(dst, s2));
                return;
            }
            else if(dst == ALL_PRINTER) {
                log(LOG_NOTICE, 0, "It's MS-Windows UPNP all printers notify. Ignoring.",
                    inetFmt(dst, s2));
                return;
            }
