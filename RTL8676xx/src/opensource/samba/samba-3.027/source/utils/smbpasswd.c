...
static fstring mypasswd;
static BOOL passwd_from_line = False;
...
static int process_options(int argc, char **argv, int local_flags)
{
	int ch;
	pstring configfile;
	//pstrcpy(configfile, "/var/smb.conf");
	...
	while ((ch = getopt(argc, argv, "c:axdehminjr:sw:R:D:p:U:LW")) != EOF) {
		switch(ch) {
		case 'L':
		...
		case 'D':
			...
			break;
		case 'p'://hjp
			fstrcpy(mypasswd , optarg);
			passwd_from_line =True;
			break;
		case 'U': {
			got_username = True;
...
static void clean() __attribute__((destructor));
static void clean()
{
    gfree_all();
}
