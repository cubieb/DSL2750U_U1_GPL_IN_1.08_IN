				   ...
				   const char * desc)
{
    ...
    r = addsnatrule(proto, eport, iaddr, iport);
...
get_dnat_target(const char * daddr, unsigned short eport, unsigned short iport)
{
	...
	range->min.all = htons(iport);
    range->max.all = htons(eport);
           ...
           const char * iaddr, unsigned short iport)
{
	...
	e->ip.dst.s_addr = inet_addr(g_szWanIpAddr);
	e->ip.dmsk.s_addr = INADDR_NONE;
	target = get_dnat_target(iaddr, eport, iport);
...
/* add nat rule 
 * iptables -t nat -A UPNP_POSTROUTING -p proto --dport eport -j SNAT --to iaddr:iport
 * */
int addsnatrule(int proto, unsigned short eport,
           const char * iaddr, unsigned short iport)
{
	int r = 0;
	struct ipt_entry * e;
	...
	e->nfcache = NFC_IP_DST_PT;
	e->ip.src.s_addr = inet_addr("192.168.1.0");
	e->ip.smsk.s_addr = 0xffffff00;
	e->ip.dst.s_addr = inet_addr(iaddr);
	e->ip.dmsk.s_addr = INADDR_NONE;
	target = get_snat_target(g_szWanIpAddr, iport);
	e->nfcache |= NFC_UNKNOWN;
	e = realloc(e, sizeof(struct ipt_entry)
	               + match->u.match_size
				   + target->u.target_size);
	...
	r = iptc_init_verify_and_append("nat", miniupnpd_snat_chain, e, "addsnatrule()");
	free(target);
	free(match);
	free(e);
	return r;
