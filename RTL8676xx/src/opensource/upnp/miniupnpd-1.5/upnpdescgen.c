...
/*The follow global variables store the device information.
 *Only function readoptionsfile() in options.c  modify them.
 *And only function genXML() visit them*/
char g_szManufacturer[128] = {0};
char g_szModelName[128] = {0};
char g_szModelDescription[128] = {0};
char g_szProductClass[128] = {0};
char g_szWanIpAddr[16] = {0};
		...
		if(eltname[0] == '/')
		{
			if(p[i].data && p[i].data[0])
			{
				...
				switch (i)
				{
				    /*friendlyName*/
				    case 6 : 
				        str = strcat_str(str, len, tmplen, g_szProductClass);
				        break;

				    /*manufacturer*/    
				    case 7 : 
				        str = strcat_str(str, len, tmplen, g_szManufacturer);
				        break;

				    /*modelDescription*/    
				    case 9 : 
				        str = strcat_str(str, len, tmplen, g_szModelDescription);
				        break;
			        
				    /*modelName*/    
				    case 10 : 
				        str = strcat_str(str, len, tmplen, g_szModelName);
				        break;
