...
//lyl add wildcard match to -i and -o
...
static int parse_iface(char *iface, char *option)
{
	...
	if ((c = strchr(iface, '+'))) {
		if (*(c + 1) != '\0') {
			printf("Spurious characters after '+' wildcard for '%s'", option);
			return -1;
		} else
			*c = IF_WILDCARD;
	}
...
/* BRCM end */
{
	 ...
	 */
		 for(i=0;i<sizeof(ebt_options)/sizeof(struct option);i++)
	 {
	    printf("ebt is %s\n",ebt_options[i].name);   
	 }
