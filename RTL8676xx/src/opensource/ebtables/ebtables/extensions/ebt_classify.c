   ...
   struct ebt_entry_target **target)
{
	...
	switch (c) {
	case CLASS_SETVAL:
		check_option(flags, OPT_SET_CLASS);
		if (string_to_priority(optarg, &clinfo->priority))
			print_error("Bad class value `%s'", optarg);		
		class_supplied = 1;
		break;
	case CLASS_TARGET:
		check_option(flags, OPT_CLASS_TARGET);
		if (FILL_TARGET(optarg, clinfo->target))
			print_error("Illegal --class-target target");
		break;	
	 default:
		return 0;
	}
   ...
   unsigned int hookmask, unsigned int time)
{
	...
	if (time == 0 && class_supplied == 0)
		print_error("No class value supplied");
		
	if (BASE_CHAIN && clinfo->target == EBT_RETURN)
		print_error("--class-target RETURN not allowed on base chain");		
}
