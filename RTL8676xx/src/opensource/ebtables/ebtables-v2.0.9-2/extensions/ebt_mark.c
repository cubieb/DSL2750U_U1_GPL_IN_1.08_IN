   ...
   struct ebt_entry_target **target)
{
	...
	case MARK_SETMARK:
		...
		break;
	case MARK_SETRMARK:
		ebt_check_option(flags, OPT_MARK_SETRMARK);
		markinfo->rmark = strtoul(optarg, &end, 0);
		if (*end != '\0' || end == optarg)
			ebt_print_error("Bad RMARK value '%s'", optarg);
   ...
   const struct ebt_entry_target *target)
{
	...
	if (markinfo->flag & OPT_MARK_SETRMARK)
		printf(" --set-rmark 0x%lx", markinfo->rmark);		
   ...
   const struct ebt_entry_target *t2)
{
	...
	return markinfo1->target == markinfo2->target &&
			...
			markinfo1->rmark == markinfo2->rmark;
}
