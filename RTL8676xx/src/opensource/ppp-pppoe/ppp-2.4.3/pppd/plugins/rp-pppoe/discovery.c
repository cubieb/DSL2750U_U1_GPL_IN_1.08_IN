...
#include <signal.h>
 ...
 UINT16_t  backup_session = 0;		/* Session ID */
...
waitForPADS(PPPoEConnection *conn, int timeout)
{
    ...
    /* Add by wuguoxiang
      * 2008.05.09
      * print PPPOE session id
      */
    warn("PPP session is %d", (int) ntohs(conn->session));
    /* Add end */
...
discovery(PPPoEConnection *conn)
{
    ...
    int padrAttempts = 0;
	int timeout;
	/*如果是特殊pvc，做个特殊处理。主要针对阿根廷版本*/
	if(g_flag_value == 1)
	{
		timeout =OTHER_PADI_TIMEOUT;
	}
	else if(g_flag_value == 2)
	{
		timeout =PERU_PADI_TIMEOUT;
	}
	else if(g_flag_value == 3)
	{
	...
	/* telefornica spain request
          Sequence of messages BRAS->CPE:
          g_flag_value == 3
          PADS--->

         <---PADT

        <---PADI (5 consecutive PADIS, time between PADIs must be 5 seconds)

        <---PADT

        <---PADI (5 PADIs, 5 seconds between each PADI)

        ...repeat
        */
	if(g_flag_value == 3)
	{
         	if (padiAttempts > MAX_SPAIN_PADI_ATTEMPTS) {

                      if(existingSession)
			    conn->session =  backup_session;
			 else
			    conn->session = 0;
    ...
    /* if PVC search,the g_flag_value is 4,when received PADO,the PPPD will die */
    if ((g_flag_value == 4) && (conn->discoveryState == STATE_RECEIVED_PADO)){
        FILE *fp;
        if ((fp=fopen("/var/TRY_PPPOE_SEARCH", "w")) != NULL)
        {
            warn("create TRY_PPPOE_SEARCH successed! \n");
            fprintf( fp, "1\n");
            fclose(fp);
        }
        die(0);
