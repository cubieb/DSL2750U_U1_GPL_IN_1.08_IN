...
extern 	   int need_holdoff;		/* need holdoff period before restarting */
extern int 	idle_time_limit;	/* Disconnect if idle for this many seconds */
char *pppd_pppoe_service = NULL;
static char *acName = NULL;

/* Modified bu wuguoxiang
  * 2008.05.09
  * change existingSession to extern
  */
//static char *existingSession = NULL;
extern char *existingSession;
/* Modified end*/
extern void lcp_close(unit, reason);
...
PPPOEConnectDevice(void)
{
    ...
    {
        pid_t ppp = getpid();
        ioctl(conn->sessionSocket, PPPIOCSPID, &ppp);	
    }
...
PPPOEDisconnectDevice(void)
{
    ...
    /* Modified by wuguoxiang 
      * 2008.04.18
      * Add sending PADT when disconnect
      */
    sendPADT(conn, NULL);
...
PPPoEDevnameHook(char *cmd, char **argv, int doit)
{
	...
	cmd += 4;
		   ...
		   && strncmp(cmd, "tap", 3) && strncmp(cmd, "br", 2) && strncmp(cmd, "vport", 4))) {
	return 0;
    }
