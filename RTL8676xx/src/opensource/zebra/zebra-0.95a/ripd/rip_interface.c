...
rip_request_interface_send (struct interface *ifp, u_char version)
{
  ...
  if (version == RIPv1 || if_is_pointopoint (ifp) || if_is_broadcast (ifp))
    {
      listnode cnode;
#ifdef TBS_RIP_DEBUG
      if (IS_RIP_DEBUG_EVENT)
...
#if defined(TBS_AUTH_SUPPORT) && defined(TBS_CMD_SUPPORT)
...
#endif /* defined(TBS_AUTH_SUPPORT) && defined(TBS_CMD_SUPPORT) */
...
#if defined(TBS_CMD_SUPPORT) && defined(TBS_AUTH_SUPPORT)
