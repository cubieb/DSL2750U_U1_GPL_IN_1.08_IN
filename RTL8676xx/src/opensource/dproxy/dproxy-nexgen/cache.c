...
void cache_purge(int older_than)
{
    ...
    if( (fp_default = fopen("/var/cache/default.cache", "r")) != NULL)
    {
        sprintf( old_cache, "%s.old", "/var/cache/default.cache");
        if( rename( "/var/cache/default.cache", old_cache ) < 0 )
        {
            debug_perror("Could not move cache file");
            fclose(fp_default);
            return;
        ...
        out_fp = fopen( "/var/cache/default.cache" , "w");
        if(!out_fp)
        {
            if( fp_default )
            {
