...
int send_renew(unsigned long xid, unsigned long server, unsigned long ciaddr)
{
	...
	if (server)
		//The "kernel_packet"  can't send the Unicast packets
		//ret = kernel_packet(&packet, ciaddr, CLIENT_PORT, server, SERVER_PORT);
		ret = raw_packet(&packet, ciaddr, CLIENT_PORT, server,
				SERVER_PORT, server_mac_addr, client_config.ifindex);
...
int get_raw_packet(struct dhcpMessage *payload, int fd)
{
		...
		//Skip baotou
		memcpy(&packet,buff+14,bytes-14);
		//get Destination MAC
		memcpy(server_mac_addr_tmp,buff+6,6);
