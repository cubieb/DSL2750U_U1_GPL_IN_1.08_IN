...
/* root write-able */
char g_flgRootChangeAble = 0;
char pasvcmd[MAXCMD] = {0};
extern void remove_pasv_ports();
...
#define	SET_PSTR(str)	(pstr = (str))
	if (WIFSIGNALED(status))
		SET_PSTR("WIFSIGNALED");
	else if ( WIFEXITED(status) )
		SET_PSTR("WIFEXITED");
	else if ( WEXITSTATUS(status) )
		SET_PSTR("WEXITSTATUS");
		...
		SET_PSTR("UNKNOWN");
	sprintf(cmd, "echo %s > /var/ftpdchild", pstr);
	system(cmd);
}
#endif
...
//		check(status);
	...
	for (i = 0; i < bftpd_list_count(child_list); i++) { /* this line has a bug. */
...
#if 1	/* by radar. for kill all child */
static void init_pid(void)
{
	FILE *fppidfile = NULL;
	fppidfile = fopen(PATH_FTPD_PID, "w");
	if (fppidfile){
		fprintf(fppidfile, "%d", getpid());
		fclose(fppidfile);
	}
}

static void destroy_pid(void)
{
	remove(PATH_FTPD_PID);
}

static void handler_sigquit(int signum)
{
	struct bftpd_childpid *childpid;
	struct bftpd_list_element *list = child_list;

	while ( list ) {
		childpid = bftpd_list_get(list, 0);
		list = list->next;
		if (childpid) {
...
/* a safe method for quiting daemon */
/* send SIGUSR1 -> main process send SIGTERM ->-> all child process */
static void quit_daemon(void)
{
	FILE *fppidfile = NULL;
	pid_t pid;
	fppidfile = fopen(PATH_FTPD_PID, "r");
	if (fppidfile) {
...
int main(int argc, char **argv)
{
	...
	while (((retval = getopt(argc, argv, "p:c:hdDink"))) > -1) {
		switch (retval) {
			case 'h':
				printf(
					...
					"-p specify daemon listen port. Default:21\n"
					"-k destroy daemon\n", argv[0]);
				return 0;
			...
			case 'p': port = atoi(optarg); break; /* radar */
			case 'k': quit_daemon(); return(0); /* radar */
		}
	}
	if (daemonmode) {
		...
		signal(SIGUSR1, handler_sigquit); /* radar */
	...
	/* if root dir is a tmpfs mountpoint, please disable this option to avoid OOM or other panic. */
	if ( !strcasecmp(config_getoption("ROOTDIR_CHANGEABLE"), "yes") )
		g_flgRootChangeAble = 1;
	else
		g_flgRootChangeAble = 0;
