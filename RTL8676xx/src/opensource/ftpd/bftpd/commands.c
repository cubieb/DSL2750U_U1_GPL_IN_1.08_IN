...
extern char g_flgRootChangeAble; /* define in main.c */

int pasv_port = 0;
extern pasvcmd[MAXCMD];

void remove_pasv_ports()
{
	if ((pasv == 1) && (pasv_port != 0) )/* first remove the old port */ 
	{
		snprintf(pasvcmd, sizeof(pasvcmd), "iptables -t filter -D INPUT -p tcp --dport %d -j ACCEPT 2>/dev/null", pasv_port);
    	system(pasvcmd);
	}
...
void control_printf(char success, char *format, ...)
{
    ...
    /* TBS_TAG: sync files when return infomation to client,by Zhangyu */
    sync();
}
...
void command_pwd(char *params)
{
     ...
     if (strncmp(my_cwd,"/mnt",4) == 0)
     {
         control_printf(SL_SUCCESS, "257 \"%s\" is the current working directory.", &my_cwd[4]);
         free(my_cwd);
     }
}
...
void command_pasv(char *foo)
{
	...
	pasv_port = ntohs(localsock.sin_port);
	memset(pasvcmd,0,sizeof(pasvcmd));
	sprintf(pasvcmd, "iptables -t filter -I INPUT -p tcp --dport %d -j ACCEPT 2>/dev/null", pasv_port);
	system(pasvcmd);
...
void do_stor(char *filename, int flags)
{
    ...
    char transfer_failed = FALSE; /* radar */
    ...
    */
	...
	close(sock);
        alarm(control_timeout);
        offset = 0;
	if (TRUE == transfer_failed) {/* radar */
		control_printf(SL_FAILURE, "553 File transmission failed.");
	}else{
...
void command_stor(char *filename)
{
    if(strcmp(cwd,"/mnt/") == 0)
	{
		control_printf(SL_FAILURE, "550 Error: %s.", "prohibit");
		return;
	}
...
{
	int err = bftpd_cwd_chdir(dir);
	if (err == NOT_ALLOWED_PATH) 
	{
		control_printf(SL_FAILURE, "451 Error: %s.", "prohibit");
	}
	else if(err == 0)
	{
...
{
	if (bftpd_cwd_chdir("..") == NOT_ALLOWED_PATH)
		control_printf(SL_FAILURE, "550 Error: %s.", "prohibit");
	else
		control_printf(SL_SUCCESS, "250 OK");
}
...
#if 1
/* avoid tmpfs/ramfs crack.
root dir usually is mount as tmpfs/ramfs, allow user changing will be damaged */
#define	ROOTDIR_ALLOW	1
#define	ROOTDIR_DISALLOW	0
#endif

/* Command parsing */
const struct command commands[] = {
	{"USER", "<sp> username", command_user, STATE_CONNECTED, 0, ROOTDIR_ALLOW},
	{"PASS", "<sp> password", command_pass, STATE_USER, 0, ROOTDIR_ALLOW},
    {"XPWD", "(returns cwd)", command_pwd, STATE_AUTHENTICATED, 1, ROOTDIR_ALLOW},
	{"PWD", "(returns cwd)", command_pwd, STATE_AUTHENTICATED, 0, ROOTDIR_ALLOW},
	{"TYPE", "<sp> type-code (A or I)", command_type, STATE_AUTHENTICATED, 0, ROOTDIR_ALLOW},
	{"PORT", "<sp> h1,h2,h3,h4,p1,p2", command_port, STATE_AUTHENTICATED, 0, ROOTDIR_ALLOW},
    {"EPRT", "<sp><d><net-prt><d><ip><d><tcp-prt><d>", command_eprt, STATE_AUTHENTICATED, 1, ROOTDIR_DISALLOW},
	{"PASV", "(returns address/port)", command_pasv, STATE_AUTHENTICATED, 0, ROOTDIR_ALLOW},
    {"EPSV", "(returns address/post)", command_epsv, STATE_AUTHENTICATED, 1, ROOTDIR_ALLOW},
    {"ALLO", "<sp> size", command_allo, STATE_AUTHENTICATED, 1, ROOTDIR_ALLOW},
	{"STOR", "<sp> pathname", command_stor, STATE_AUTHENTICATED, 0, ROOTDIR_DISALLOW},
    {"APPE", "<sp> pathname", command_appe, STATE_AUTHENTICATED, 1, ROOTDIR_DISALLOW},
	{"RETR", "<sp> pathname", command_retr, STATE_AUTHENTICATED, 0, ROOTDIR_DISALLOW},
	{"LIST", "[<sp> pathname]", command_list, STATE_AUTHENTICATED, 0, ROOTDIR_ALLOW},
	{"NLST", "[<sp> pathname]", command_nlst, STATE_AUTHENTICATED, 0, ROOTDIR_ALLOW},
	{"SYST", "(returns system type)", command_syst, STATE_CONNECTED, 0, ROOTDIR_ALLOW},
	{"MDTM", "<sp> pathname", command_mdtm, STATE_AUTHENTICATED, 1, ROOTDIR_ALLOW},
    {"XCWD", "<sp> pathname", command_cwd, STATE_AUTHENTICATED, 1, ROOTDIR_ALLOW},
	{"CWD", "<sp> pathname", command_cwd, STATE_AUTHENTICATED, 0, ROOTDIR_ALLOW},
    {"XCUP", "(up one directory)", command_cdup, STATE_AUTHENTICATED, 1, ROOTDIR_DISALLOW},
	{"CDUP", "(up one directory)", command_cdup, STATE_AUTHENTICATED, 0, ROOTDIR_DISALLOW},
	{"DELE", "<sp> pathname", command_dele, STATE_AUTHENTICATED, 0, ROOTDIR_DISALLOW},
    {"XMKD", "<sp> pathname", command_mkd, STATE_AUTHENTICATED, 1, ROOTDIR_DISALLOW},
	{"MKD", "<sp> pathname", command_mkd, STATE_AUTHENTICATED, 0, ROOTDIR_DISALLOW},
    {"XRMD", "<sp> pathname", command_rmd, STATE_AUTHENTICATED, 1, ROOTDIR_DISALLOW},
	{"RMD", "<sp> pathname", command_rmd, STATE_AUTHENTICATED, 0, ROOTDIR_DISALLOW},
	{"NOOP", "(no operation)", command_noop, STATE_AUTHENTICATED, 0, ROOTDIR_ALLOW},
	{"RNFR", "<sp> pathname", command_rnfr, STATE_AUTHENTICATED, 0, ROOTDIR_DISALLOW},
	{"RNTO", "<sp> pathname", command_rnto, STATE_RENAME, 0, ROOTDIR_DISALLOW},
	{"REST", "<sp> byte-count", command_rest, STATE_AUTHENTICATED, 1, ROOTDIR_ALLOW},
	{"SIZE", "<sp> pathname", command_size, STATE_AUTHENTICATED, 1, ROOTDIR_ALLOW},
	{"QUIT", "(close control connection)", command_quit, STATE_CONNECTED, 0, ROOTDIR_ALLOW},
	{"HELP", "[<sp> command]", command_help, STATE_AUTHENTICATED, 0, ROOTDIR_ALLOW},
    {"STAT", "<sp> pathname", command_stat, STATE_AUTHENTICATED, 0, ROOTDIR_ALLOW},
	{"SITE", "<sp> string", command_site, STATE_AUTHENTICATED, 0, ROOTDIR_DISALLOW},
    {"FEAT", "(returns list of extensions)", command_feat, STATE_AUTHENTICATED, 0, ROOTDIR_ALLOW},
/*    {"AUTH", "<sp> authtype", command_auth, STATE_CONNECTED, 0},
*/    {"ADMIN_LOGIN", "(admin)", command_adminlogin, STATE_CONNECTED, 0, ROOTDIR_ALLOW},
      {"MGET", "<sp> pathname", command_mget, STATE_AUTHENTICATED, 0, ROOTDIR_DISALLOW},
      {"MPUT", "<sp> pathname", command_mput, STATE_AUTHENTICATED, 0, ROOTDIR_DISALLOW},
	{NULL, NULL, NULL, 0, 0, ROOTDIR_DISALLOW}
};

#if 1
static int is_rootdir_disable_command(int cmd_idx)
{
	char *local_cwd = NULL;

	if ( !g_flgRootChangeAble ) {
		local_cwd = bftpd_cwd_getcwd();
		if (NULL == local_cwd)
			return 0;

		if ( 0 == strcmp(local_cwd, "/") ) {
			free(local_cwd);
			if ( ROOTDIR_DISALLOW == commands[cmd_idx].allow_in_rootdir )
				return 1;
		}
	}
	return 0;
...
#if 1 /* by radar */
			if ( is_rootdir_disable_command(i) ){
				control_printf(SL_FAILURE, "550 The command '%s' is not allowed in current directory.",
					commands[i].name);
				return 1;
			}
#endif
