...
//to avoid chroot jail, otherwise we can't not use iptables rule
#if 0		
        if (chroot(foo) == -1) {
...
char *check_file_password(char *my_filename, char *my_username, char *my_password)
{
...
#ifdef	SUPPORT_FTPD_BASE64
	pbase64_username = base64_encode(my_username, strlen(my_username));
#endif

   return_value = fscanf(my_file, "%32s %32s %32s %32s", user, password, group, home_dir);
#ifdef	SUPPORT_FTPD_BASE64
   if (! strcmp(user, pbase64_username) )
      found_user = 1;
#else
   ...
   {
       return_value = fscanf(my_file, "%32s %32s %32s %32s", user, password, group, home_dir);
	if ( return_value == EOF)	/* radar append */
		break;

#ifdef	SUPPORT_FTPD_BASE64
	if (! strcmp(user, pbase64_username) ) {
		found_user = 1;
		break; /* found it, may leave now. radar append. */
	}
#else
       if (! strcmp(user, my_username) ){
          found_user = 1;
...
#if 1 /* using crypted password. by radar. */
	else if ( strcmp(password, crypt(my_password, FTPD_CRYPT_SALT)) )
		return NULL;
#else
