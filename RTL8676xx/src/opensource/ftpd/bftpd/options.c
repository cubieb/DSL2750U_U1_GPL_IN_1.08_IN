...
#define	FTPD_MALLOC(size)	calloc(1, size)
/*
	...
	while (!strchr(str, '}')) {
  		if (str[0] != '\n') {
            if ((strstr(str, "directory")) && (strchr(str, '{')) && (directories)) {
                char *tmp;
                if (dir) {
                    dir = dir->next = FTPD_MALLOC(sizeof(struct directory));
                } else {
                    *directories = dir = FTPD_MALLOC(sizeof(struct directory));
                }
                /* avoid memory bug */
                if (! dir) return;
                ...
                create_options(configfile, &(dir->options), NULL);
       			...
       			if (opt) {
       				opt = opt->next = FTPD_MALLOC(sizeof(struct bftpd_option));
       			} else {
       				*options = opt = FTPD_MALLOC(sizeof(struct bftpd_option));
       			}
                        /* bail out on memory error */
                        if (! opt)
                           return;

       			opt->name = (char *) FTPD_MALLOC( strlen(str) + 2 );
       			// opt->value = (char *) FTPD_MALLOC(strlen(str));
                        opt->value = (char *) FTPD_MALLOC( strlen(str) + 256);
       			sscanf(str, "%[^=]=\"%[^\n\"]", opt->name, opt->value);
            }
   		}
		str = config_read_line(configfile);
            ...
            while (strchr(grp->temp_members, ',')) {
                ...
                if (foo[0] == '@') {
                    if (sscanf(foo + 1, "%i", &uid)) {
                        ...
                        if (!((grpinfo = getgrnam(foo + 1))))
                            continue;
                    if (grp->groups)
                        endg = endg->next = FTPD_MALLOC(sizeof(struct list_of_struct_group));
                    else
                        grp->groups = endg = FTPD_MALLOC(sizeof(struct list_of_struct_group));
                    ...
                    endg->grp.gr_mem = FTPD_MALLOC((i + 1) * sizeof(char *));
                    for (i = 0; grpinfo->gr_mem[i]; i++)
                ...
                if (sscanf(foo, "%i", &uid)) {
                    ...
                    if (!((temp = getpwnam(foo))))
                        continue;
                if (grp->users)
                    endp = endp->next = FTPD_MALLOC(sizeof(struct list_of_struct_passwd));
                else
                    grp->users = endp = FTPD_MALLOC(sizeof(struct list_of_struct_passwd));
...
void config_user_init(void)
{
	...
	char *str = NULL;
	char *pbase64_username = NULL;
	char *pstrend = NULL;
	char *pstrbegin = NULL;

	pbase64_username = base64_encode(user, strlen(user));
	if (NULL == pbase64_username){
		control_printf(SL_FAILURE, "421 OOM before open configuration");
		return;
	}

	configfile = fopen(PATH_VIRTUAL_RIGHT_DB, "r");
	if (!configfile){
		//control_printf(SL_FAILURE, "421 Unable to open configuration file:%s.", PATH_VIRTUAL_RIGHT_DB);
		return;
	}
...
#if 1
				pstrbegin = str + 5;
				pstrend = strrchr(pstrbegin, '{');
				if (!pstrend) /* line format is unknown */
					continue;

				if (0 != strncmp(pstrbegin, pbase64_username, pstrend - pstrbegin) ) {
					/* not the login user */
					continue;
				}
#endif
				if (usr) {
					usr = usr->next = FTPD_MALLOC(sizeof(struct user));
				} else {
					config_users = usr = FTPD_MALLOC(sizeof(struct user));
				}
				/* avoid memory errors */
				if (! usr)
				{
		...
		if (strchr(str, '{')) {
            ...
            if (!strcasecmp(str, "global{\n")) {
                ...
                *strchr(usr->name, '{') = 0;
                ...
                if (grp) {
                    grp = grp->next = FTPD_MALLOC(sizeof(struct group_of_users));
                } else {
                    config_groups = grp = FTPD_MALLOC(sizeof(struct group_of_users));
                }
                if (! grp)
                {
                    fclose(configfile);
...
char *getoption_directories(struct directory *dir, char *name) {
    ...
    strcat(curpath, "/");
    do {
        bar = FTPD_MALLOC(strlen(dir->path) + 2);
        if (! bar)
           return NULL;
...
char *getoption_user(char *name)
{
	...
	if ( (usr = config_users) ) {
		...
		do {
			if (!strcmp(pbase64_username, usr->name)) {
