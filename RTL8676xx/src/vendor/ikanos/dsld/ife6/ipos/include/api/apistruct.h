...
typedef	struct
{
	...
	U32	LineType;					/* line type: VDSL_AUTO_DETECT	for DEFAULT */
...
#endif
	U8  disableMicroCut;
    U8	LineSelectionCriterion;
    U8  PriorityTable[32];
}	ipos_port_provision;	
														 

#define HDE_FRAMING_MODE_HDLC 0x80
#define HDE_FRAMING_MODE_EFM 0x10
#define HDE_FRAMING_MODE_FRAME_BYPASS 0
...
typedef	struct
{
    ...
    U32     FeFecEs_S;             /* Far-end Forward Error Correction Errored Seconds (FEC ES) Slow count    */
    U32     FeEs_S;                  /* Far-end Errored Seconds (ES) Slow count    */
    U32     FeSEs_S;                /* Far-end Severely Errored Seconds (ES) Slow count    */
    U32     FeLosEs_S;             /* Far-end LOS Errored Seconds (LOS ES) Slow count    */
    U32     FeUnAvlEs_S;         /* Far-end Un Available Errored Seconds (UNAVL ES) Slow count    */
    U32     FeBER_S;                /* Near-end Bit Error Rate (BER) Slow count             */
...
typedef struct
{
  ...
  U32	numIDLEcell_F; //	Number of Idle Cells (IDLE) Fast count
    U32 numIDLEcellBe_S;    //Number of Idle Cells Bit Error Slow count
    U32 numIDLEcellBe_F;    //Number of Idle Cells Bit Error Fast count

    U32 FeNumUSRcell_S;     //Far-end   Number of User Cells (USR) Slow count
    U32 FeNumUSRcell_F;     //Far-end   Number of User Cells (USR) Fast count
    U32 FeNumIDLEcell_S;    //Far-end   Number of Idle Cells (IDLE) Slow count
    U32 FeNumIDLEcell_F;     //Far-end   Number of Idle Cells (IDLE) Fast count
    U32 FeNumIDLEcellBe_S;  //Far-end   Number of Idle Cells Bit Error Slow count
    U32 FeNumIDLEcellBe_F;  //Far-end   Number of Idle Cells Bit Error Fast count
...
typedef struct tag_send_ack_info
{
    ...
    int socketfd; 
    int acked;
    void (*pSendAck)(struct tag_send_ack_info *psend_ack_info, char *pBuf, U32 len); 
    int (*send_error_ack)(struct tag_send_ack_info *psend_ack_info, unsigned long errorno);
