...
typedef struct
{
  ...
  long			k1_hi[3];
  tMaxPsd maxPsd;
  unsigned char boardType; //0 -> POTS 1 -> ISDN
  unsigned char sixPtLineHoldEnable;
} blk2Struct;

#ifdef INI_SUPPORT
typedef struct _tsAal5Cfg
{
  U8        bMuxType ;  /* 0 - LLC-based 
                                               1 - VC Based  */
  U8        bFcsEnable; /* 0 -Fcs Included
                                              1- Enable or disable*/
  U8        bCorruptCrc; /* 0 - AAL5 CRC not corrupted
                                                1 - AAL5 CRC corrupted */
   
}tsAal5Cfg,*ptsAal5Cfg ;


typedef struct _tsPortAal5Cfg
{
   U8        bGfc[MAX_QOS_CLASS] ;  /* Generic flow control  */
   U8        bVpi[MAX_QOS_CLASS] ;  /* VPI value 0-32 */
   U16       usVci[MAX_QOS_CLASS];  /* VCI value 0-65535 */
     
}tsPortAal5Cfg,*ptsPortAal5Cfg ;

typedef struct _tsPortAal5ErrorStatus
{
   U8        bCrcError[MAX_QOS_CLASS] ;/* CRC error control 
                                                    0 = ignore CRC error, 
                                                    1 = check CRC error */
   U8        bRfcError[MAX_QOS_CLASS] ;/* RFC header error control  
                                                      0 = ignore RFC error, 
                                                      1 = check RFC error */
   U8        bCpiError[MAX_QOS_CLASS] ;/* CPI error control 
                                                      0 = ignore CPI error, 
                                                      1 = check CPI error */
   U8        bPadError[MAX_QOS_CLASS] ;/* PAD length Error control 
                                                      0 = ignore PAD error, 
                                                      1 = check PAD error */
   U8        bLenError[MAX_QOS_CLASS] ;/* Length error control 
                                                      0 = ignore length error, 
                                                      1 = check length error */
} tsPortAal5ErrorStatus,*ptsPortAal5ErrorStatus ;

typedef struct _tsAal5ConfigParams
{
  tsAal5Cfg                   Aal5Cfg;  
  tsPortAal5Cfg               PortAal5CfgRx;
  tsPortAal5Cfg               PortAal5CfgTx;
  tsPortAal5ErrorStatus       PortAal5ErrorCfg;

}tsAal5ConfigParams, *ptsAal5configParams;
#endif
...
#define oampGetSixPtLineHoldEn         (blk2Shadow.sixPtLineHoldEnable)
...
#define oampSetSixPtLineHoldEn(a)     blk2Shadow.sixPtLineHoldEnable = a; \
                                      nvramUpdateBlk(2)
...
extern void PrRestoreAal5CfgParams(void);
#endif
