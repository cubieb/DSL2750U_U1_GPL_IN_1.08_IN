...
#define DSL_CLI_LOCAL static
	...
	DSL_Error_t CLI_DSL_CheckHelp(const DSL_char_t *pCommands, const DSL_char_t *pUsage, DSL_File_t *out)
	{
		if ( pCommands &amp;&amp; ( strstr ( pCommands, "-h" ) || strstr ( pCommands, "--help" ) || strstr ( pCommands, "/h" ) || strstr ( pCommands, "-?" ) ) )
		{
		   DSL_FPrintf ( out, "%s", pUsage );
		   return DSL_ERROR;
	     }
		return DSL_SUCCESS;
	}
	   ...
	   else
	   {
		...
		<xsl:choose>
			<xsl:when test="1=1">
				...
				<xsl:text>
				{DSL_NULL, DSL_NULL, DSL_NULL}
				};
				</xsl:text>
			</xsl:when>
			<xsl:otherwise>
				<xsl:apply-templates select="//cdecl[attributelist/attribute/@name='decl' and attributelist/attribute/@value='DSL_VersionInformationGet']" mode="definition"/>
				<xsl:apply-templates select="//cdecl[attributelist/attribute/@name='decl' and attributelist/attribute/@value='DSL_AutobootControlSet']" mode="definition"/>
				<xsl:apply-templates select="//cdecl[attributelist/attribute/@name='decl' and attributelist/attribute/@value='DSL_G997_DeltSNRGet']" mode="definition"/>
				<xsl:text>
				DSL_CliCommand_t cliCommand[] = {
				</xsl:text>
				<xsl:for-each select="//cdecl[attributelist/attribute/@name='decl' and attributelist/attribute/@value='DSL_VersionInformationGet']">
					<xsl:sort select="attributelist/attribute[@name='name']/@value" data-type="text" order="ascending"/>
						<xsl:apply-templates select="." mode="register"/>
				</xsl:for-each>
				<xsl:text>
...
<xsl:text>
	...
	<xsl:template match="cdecl" mode="definition">
		...
		<xsl:if test="starts-with(attributelist/attribute[@name='decl']/@value,'f')">
			...
			<xsl:text>(DSL_int32_t fd, DSL_char_t *pCommands, DSL_File_t *out)
			{ </xsl:text>
	...
	<xsl:template match="cdecl" mode="register">
		...
		<xsl:if test="starts-with(attributelist/attribute[@name='decl']/@value,'f')">
			...
			<xsl:text>{"</xsl:text><xsl:value-of select="$SHORTCMD"/><xsl:text>", "</xsl:text><xsl:value-of select="$NOPREFIX"/><xsl:text>", CLI_</xsl:text><xsl:value-of select="$NAME"/><xsl:text>},</xsl:text>
		</xsl:if>
	</xsl:template>
	<xsl:template match="cdecl" mode="function">
		...
		<xsl:apply-templates select="attributelist/parmlist/parm[attributelist/attribute[@name='name']/@value!='pContext']" mode="declaration"/>
		<xsl:variable name="SCAN_1">
			<xsl:message terminate="no"> scan1</xsl:message>
			<xsl:apply-templates select="attributelist/parmlist/parm[attributelist/attribute[@name='name']/@value!='pContext']" mode="scan_1"/>
		</xsl:variable>
		<xsl:variable name="SCAN_2">
			<xsl:message terminate="no"> scan2</xsl:message>
			<xsl:apply-templates select="attributelist/parmlist/parm[attributelist/attribute[@name='name']/@value!='pContext']" mode="scan_2"/>
		</xsl:variable>
		<xsl:variable name="PRINT_RETURNS_1">
			<xsl:message terminate="no"> print1</xsl:message>
			<xsl:apply-templates select="attributelist/parmlist/parm[attributelist/attribute[@name='name']/@value!='pContext']" mode="print_returns_1"/>
		</xsl:variable>
		<xsl:variable name="PRINT_RETURNS_2">
			<xsl:message terminate="no"> print2</xsl:message>
			<xsl:apply-templates select="attributelist/parmlist/parm[attributelist/attribute[@name='name']/@value!='pContext']" mode="print_returns_2"/>
		</xsl:variable>
		...
		<xsl:variable name="USAGE">
			...
			<xsl:variable name="INPUT_PAR">
				...
				<xsl:apply-templates select="attributelist/parmlist/parm[attributelist/attribute[@name='name']/@value!='pContext']" mode="help_in"/>
			</xsl:variable>
			<xsl:variable name="OUTPUT_PAR">
				<xsl:text>"- DSL_Error_t nReturn" DSL_CRLF \
</xsl:text>
				<xsl:message terminate="no"> help_out</xsl:message>
				<xsl:apply-templates select="attributelist/parmlist/parm[attributelist/attribute[@name='name']/@value!='pContext']" mode="help_out"/>
			</xsl:variable>
...
#define USAGE ""
		...
		<xsl:text>
		if(CLI_DSL_CheckHelp(pCommands, USAGE, out) == DSL_ERROR)
		{
			return;
		}
		</xsl:text>
		...
		<xsl:text>if((ret = DSL_Ioctl(fd, </xsl:text><xsl:call-template name="create_fio">
               <xsl:with-param name="NAME" select="$NAME"/>
         </xsl:call-template><xsl:text>, </xsl:text>
		(int)<xsl:apply-templates select="attributelist/parmlist/parm[attributelist/attribute[@name='name']/@value!='pContext']"/>
		<xsl:text> )) &lt; 0)
		     DSL_FPrintf(out, sFailureReturn, ret);
		else
			DSL_FPrintf(out, "</xsl:text>
	...
	<xsl:template match="parm">
		...
		<xsl:if test="starts-with($TYPE,'p.') and $NAME!='pContext'">
			<xsl:text>&amp;</xsl:text>
		</xsl:if>
		<xsl:value-of select="$NAME"/>
	</xsl:template>
		...
		<xsl:if test="$NAME!='pContext'">
   		...
   		<xsl:variable name="ARRAYTYPE">
			...
			<xsl:choose>
				...
				<xsl:when test="starts-with($TYPE,'p.')">
					<!-- do not ignore non const pointer, this might be inputs -->
					<xsl:call-template name="complex_uid_scan">
						<xsl:with-param name="TYPE" select="substring-after($TYPE,'p.')"/>
					</xsl:call-template>
				</xsl:when>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:choose>
					...
					<!-- if it is an string, do not mention the element zero here -->
					<xsl:when test="contains($TYPE,'_char_t')"> , <xsl:value-of select="$AMP"/>
						<xsl:value-of select="$NAME"/>
					</xsl:when>
					<xsl:when test="contains($TYPE,'_boolean_t')"> , <xsl:value-of select="$AMP"/>
		...
		<xsl:if test="$X">
			<xsl:for-each select="$X/attributelist">
				...
				<xsl:variable name="NEW_TYPE">
      			...
      			<xsl:choose>
      				<xsl:when test="starts-with($NEW_TYPE,'q(const).')">
       				   <xsl:text> </xsl:text>
     					<xsl:call-template name="uid_scan">
      						<xsl:with-param name="TYPE" select="substring-after($NEW_TYPE,'q(const).')"/>
      						<xsl:with-param name="ARRAY" select="$ARRAY"/>
      					</xsl:call-template>
      				</xsl:when>
      				<xsl:otherwise>
<!--
       				   <xsl:text> </xsl:text>
            				<xsl:call-template name="uid_scan">
            					<xsl:with-param name="TYPE" select="$NEW_TYPE"/>
            					<xsl:with-param name="ARRAY" select="$ARRAY"/>
            				</xsl:call-template>
-->
      				</xsl:otherwise>
...
-->
	...
	<xsl:template name="create_fio">
        <xsl:param name="NAME"/>
        <xsl:variable name="X1">
           <xsl:choose>
               <xsl:when test="starts-with($NAME,'DSL_G997_')">
                  <xsl:value-of select="substring-after($NAME, 'DSL_G997_')"/>
               </xsl:when>
               <xsl:when test="starts-with($NAME,'DSL_DBG_')">
                  <xsl:value-of select="substring-after($NAME, 'DSL_DBG_')"/>
               </xsl:when>
               <xsl:otherwise>
                  <xsl:value-of select="substring-after($NAME, 'DSL_')"/>
               </xsl:otherwise>
           </xsl:choose>
        </xsl:variable>
        <xsl:choose>
            <xsl:when test="starts-with($NAME,'DSL_G997_')">
              <xsl:text>DSL_FIO_G997_</xsl:text>
              </xsl:when>
            <xsl:when test="starts-with($NAME,'DSL_DBG_')">
              <xsl:text>DSL_FIO_DBG_</xsl:text>
              </xsl:when>
            <xsl:otherwise>
              <xsl:text>DSL_FIO_</xsl:text>
            </xsl:otherwise>
        </xsl:choose>
        <xsl:call-template name="insert_underscore">
         <xsl:with-param name="NAME" select="$X1"/>
         <xsl:with-param name="LENGTH" select="string-length($X1)"/>
      </xsl:call-template>
	</xsl:template>
	<xsl:template name="insert_underscore">
        <xsl:param name="NAME"/>
        <xsl:param name="LENGTH"/>
        <xsl:variable name="X1" select="translate(substring($NAME, 1, 1), 'abcdefghijklmnopqrstuvwxyz','')"/>
        <xsl:variable name="X2" select="translate(substring($NAME, 2, 1), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ','')"/>
        <xsl:variable name="X3" select="translate(substring($NAME, 3, 1), 'abcdefghijklmnopqrstuvwxyz','')"/>
        <xsl:value-of select=" translate(substring($NAME, 1, 1),'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ')"/>
        <xsl:if test="$LENGTH &gt; 1">
           <xsl:if test="string-length($X1) = 0">
              <xsl:if test="string-length($X2) = 0">
                 <xsl:text>_</xsl:text>
              </xsl:if>
           </xsl:if>
           <xsl:if test="string-length($X1) = 1">
              <xsl:if test="string-length($X2) = 0">
                 <xsl:if test="string-length($X3) = 0">
                    <xsl:text>_</xsl:text>
                 </xsl:if>
              </xsl:if>
           </xsl:if>
