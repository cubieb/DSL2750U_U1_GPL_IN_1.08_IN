...
usage()
{
   ...
   echo "Remove from dsl_cpe_control-2.3.0.tar.gz the danube parts"
   echo "and will generate a file dsl_cpe_control_danube-2.3.0.tar.gz"
   echo
...
if [ -e $ORIG_DIST.tar.gz ]; then
    ...
    tar chof - $ORIG_DIST | GZIP=--best gzip -c > dsl_cpe_control_$2-$1.tar.gz
    check_return
