...
{"GetAttainableRate"       ,1,0,        RLCM_GET_GET_ATT_RATE	},
	...
	if(cmdstr==pCmd->cmd){	//tylo, cmdstr compare with pCmd->cmd
		...
		ptr32=ptr=malloc(pCmd->argsize);
	    	if (pCmd->setFlag==2) {  //need set arg from console
				...
				ptr32_2=ptr32+4;
	    	    		*ptr32 = atoi(argv[2]); //small arguments, only WriteCfg can not work
	    	    		*ptr32_2=atoi(argv[3]);
	    	    		myarg.arg = (int)ptr32;
	    	    	}
	    	    	else{
...
int main(int argc, char *argv[])
{
	...
	if (strcmp(argv[1],"start")==0) {
	    if (argc!=3) {
	    	...
	    	arg = atoi(argv[2]);
	    	myarg.arg = (int)&arg;
	        if (ioctl(fileno(fp), RLCM_PHY_START_MODEM, &myarg) < 0) {
	    	    printf("ADSL start failed %x %x!\n", RLCM_PHY_START_MODEM, myarg.arg);
	    	};
	    };
	    ...
	    if (ioctl(fileno(fp), RLCM_CMD_API, NULL) < 0) {
