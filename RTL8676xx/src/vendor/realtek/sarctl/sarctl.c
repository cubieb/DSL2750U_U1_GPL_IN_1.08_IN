...
{
	//char	*usage_desc; // = malloc(0xFFF);	
	//char	*usage_desc_now; // = malloc(0xFFF);	
	char	usage_desc[] = 
"Usage:\n\
  [getstats]\n\
  [getstats <ch_no>]\n\
  ...
  [oamstop <vpi> <vci> <tag>]\n\
   ...
   : MBR        = 0 ~ 255\n\
   : CDVT       = 0 ~ 4294967295\n\
   : tag        = number of start OAM cell\n\
   : scope      = function for segment to segment (0) or end to end (1)\n\
   : channel    = fast (0) or interleave (1)\n\
   : locationID = 16-byte pattern based on ITU-T I.610\n\
   : sourceID   = 16-byte pattern based on ITU-T I.610 (optional)\n\
  [slow]\n\
  [fast]\n\
  [loopback <1/0>]\n\
  [pvcnumber <vc_num>]\n\
