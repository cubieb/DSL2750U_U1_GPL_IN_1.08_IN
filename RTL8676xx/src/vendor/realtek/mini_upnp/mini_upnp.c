...
static int parse_argument(daemon_CTX_Tp pCtx, int argc, char *argv[])
{
	int argNum=1;
	IPCon ipcon=NULL;
	char *dlna_file=NULL, *wsc_file=NULL, *igd_file=NULL;
	int wait_time=5, is_ok;
	char dlna_file_old[200], wsc_file_old[200], igd_file_old[200];
	...
	while (argNum < argc) {
		if (!strcmp(argv[argNum], "-interface")) {
		...
		else if (!strcmp(argv[argNum], "-dlna")) {
			if (++argNum >= argc)
				break;
			...
			dlna_file = argv[argNum];
			strcpy(dlna_file_old, dlna_file);
			strcat(dlna_file_old, ".old");
		}
	...
	while (wait_time-- > 0 && (dlna_file || wsc_file || igd_file)) {
		if (dlna_file) {
			is_ok = 0;
			if (!get_file(dlna_file, &pCtx->device[pCtx->num_device])) {
				if (get_file(dlna_file_old, &pCtx->device[pCtx->num_device]))
					is_ok = 2;
			}
			else {
				is_ok = 1;
				rename(dlna_file, dlna_file_old);
			}
			if (is_ok) {
				pCtx->device[pCtx->num_device].used = DEVICE_DLNA;
				strcpy(pCtx->device[pCtx->num_device].SSDP_file_name, DLNA_BYEBYE_FILE);
				strcpy(pCtx->device[pCtx->num_device].input_file_name, dlna_file);
				DEBUG_PRINT("Mini_upnpd: Read file [%s] success!\n", ((is_ok == 1) ? dlna_file : dlna_file_old));
				pCtx->num_device++;
				dlna_file = NULL;
			}
		}
...
static void sigHandler_alarm(int signo) {
	...
	for (device_num=0; device_num<MAX_NUMBER_OF_DEVICE; device_num++) {
		...
		if (get_file(pCtx->device[device_num].input_file_name, &pCtx->device[device_num])) {
			...
			SendSSDPNotifies(pCtx->snotify, pCtx->lan_ip_address, pCtx->device[device_num].port, //cathy
...
int main(int argc, char *argv[])
{
	...
	f=fopen("/var/run/mini_upnpd.pid","w");	//cathy
   	fprintf(f,"%d",getpid());
   	fclose(f);
...
struct ip_mreq_upnp 
{
	...
	struct in_addr imr_source;	/* local IP address of interface */    
};

static int AddMulticastMembership(int s, const char * ifaddr)
{
	struct ip_mreq_upnp imr;	/* Ip multicast membership */
	...
	if (setsockopt(s, IPPROTO_IP, IP_ADD_MEMBERSHIP, (void *)&imr, sizeof(struct ip_mreq_upnp)) < 0)
	{
...
//TR_064
char DeviceOrService(const char *str)
{
  if(strstr(str, ":device:") != NULL)
  	return 'D';
 	else if(strstr(str, ":service:") != NULL)
 		return 'S';
 	else if(strstr(str, "rootdevice") != NULL)
 		return 'R';
	else
		return 'U'; // Unknown type;
}
#define DELAY_INTERVAL 150000 //  ms
static inline void rate_limit(unsigned int usec) {
...
#define SEND_SSDP_UUID \
		do { \
			...
			"NTS:ssdp:%s\r\n" \
			"\r\n", \
			UPNP_MCAST_ADDR, PORT, max_age, \
			host, port, SSDP->root_desc_name, \
			SSDP->uuid, \
			SSDP->uuid, \
			(type==0?"alive":"byebye")); \
		rate_limit(DELAY_INTERVAL); \
...
#define SEND_SSDP_SERVICE(service_type, num) \
	do{ \
		sprintf(bufr, \
			"NOTIFY * HTTP/1.1\r\n" \
			"Host:%s:%d\r\n" \
	...
	sockname.sin_port = htons(PORT);
	sockname.sin_addr.s_addr = inet_addr(UPNP_MCAST_ADDR);
	if(!strcmp(SSDP->root_desc_name, "devicedesc")) {	//cathy, TR_064
		i = 0;
		while(SSDP->known_service_types[i]) {
			switch(DeviceOrService(SSDP->known_service_types[i])) {
				case 'R':	//Root device
					//Announce itself as a UPnP root device.
					SEND_SSDP_SERVICE(SSDP->known_service_types[i], (i==0?"":"1"));
					//Announce itself via its UUID
					SEND_SSDP_UUID;
					//Announce itself as a TR-064 compliant CPE device
					SEND_SSDP_SERVICE("urn:dslforum-org:device:InternetGatewayDevice:", "1");
					break;
				case 'D': //Embedded device
					//Annouce itself as a embedded device
					SEND_SSDP_SERVICE(SSDP->known_service_types[i], (i==0?"":"1"));
					//Annouce itself as a via its UUID
					SEND_SSDP_UUID;
					break;
				case 'S':	//Service
					SEND_SSDP_SERVICE(SSDP->known_service_types[i], (i==0?"":"1"));
