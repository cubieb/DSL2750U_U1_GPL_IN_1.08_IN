	...
	else if(strcmp("GET", HttpCommand) == 0)
	{
		...
		if (h->sendDesc) {
			...
			while(h->sendDesc[i].DescName)
			{
				...
				{
					if( !h->sendDesc[i].type )
						sendXMLdesc(h, h->sendDesc[i].Impl.sendDescImpl);
					else
						h->sendDesc[i].Impl.sendDescImpl_h(h);
					return;
				}
				i++;
			}
//			if ( h->sendDesc[i].Impl.sendDescImpl_h )	//cathy, for igd
//			{
//				h->sendDesc[i].Impl.sendDescImpl_h(h);
//					return;
//			}
...
void Process_upnphttp(struct upnphttp * h)
{
	...
	case 0:
		...
		else
		{
			...
			h->req_buf = (char *)realloc(h->req_buf, 2048);
