...
int bridge_pppoe(struct mpoa_cfg *cfg, char *arg)
{     
	int err, errno;
	struct atm_backend_br2684 be;
	
	if(cfg==NULL) {
		printf("bridge_pppoe: cfg=NULL\n");
		return -1;
	}
    ...
    err=ioctl (cfg->sock, ATM_SETBRIDGEPPPOE, &be);   
    if (err == 0)
        printf("bridge_pppoe():Interface's brpppoe flag configured\n");
    else {
        printf("bridge_pppoe():Could not configure interface:%s\n",strerror(errno));
        exit(2);
    }
...
/*-----------------------------------------------------------------
 * usage:
 * mpoactl add vc0 pvc 0.33 encaps 1 qos ubr:pcr=7600 brpppoe 1
...
static struct command commands[] = {
	...
	{1, 1, "qos", set_qos},
	{1, 1, "pvc", assign_vcc},
	{1, 1, "brpppoe", bridge_pppoe},   // Added by Mason Yu for PPPoE PassThrough
//#ifdef CONFIG_RE8305
	// vlan tagging
	{1, 1, "vid", set_vid},
	{1, 1, "vprio", set_vprio},
...
#ifdef TR068_INTERNET_LED
#ifndef CONFIG_RTL8672
static void led_check()
{
	static struct timeval newtime,oldtime;
	static int flag=0;

	if(flag==0){
		memset(&newtime,0,sizeof(struct timeval));
		memset(&oldtime,0,sizeof(struct timeval));
		flag = 1;
	}
	
	gettimeofday(&newtime, NULL);

	if(newtime.tv_sec-oldtime.tv_sec>=5){
#if defined (_LINUX_2_6_)&&(!defined(CONFIG_GPIO_LED_CHT_E8B))
