...
//宏传进来有问题，因只有8192CD用，写死
//#ifdef CONFIG_RTL8192CD
#if 1
	//lhj 20100428设备驱动工作方式改变,关掉以解决wpsconfig替换iwcontrol wscd占用率高的问题
...
// Local variables
//----------------------------------------------------------
//static char *pidfile="/var/run/wpsconfig.pid";
...
#endif
#ifdef WIFI_SIMPLE_CONFIG
		strncpy(wscd[link_wscd++].wlanName, wlan_tbl[0], 30);
		iw_message(MESS_DBG_CONTROL,"wscd[%d].wlanName=%s\n", link_wscd-1, wscd[link_wscd-1].wlanName);
...
//-------------------------------------------------------------
	...
	snprintf(pidfile, 64, "/var/run/iwcontrol.pid", wlan_tbl[0]);
