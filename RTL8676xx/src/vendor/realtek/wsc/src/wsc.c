 ...
 *
 *	$Id: wsc.c,v 1.8 2010/02/05 13:26:39 cathy Exp $
 */
...
//	memcpy(MACAddr, msg->MACAddr, ETHER_ADDRLEN);
	...
	if (pCtx->debug) {
		...
		if (pCtx->is_ap && pCtx->blocked_expired_time) {
			int idx;
			idx = search_blocked_list(pCtx, MACAddr);
			if (idx != BLOCKED_ENTRY_NOT_FOUND  && (pCtx->blocked_sta_list[idx].error_count >= 10)){
				DEBUG_PRINT(" pCtx->blocked_sta_list[%d].error_count=%d\n", idx, pCtx->blocked_sta_list[idx].error_count);
				return -1;
			}
	}
#endif
...
static int init_wlan(CTX_Tp pCtx, int reinit)
{
	...
	if (!pCtx->is_ap) {
		...
		if (pCtx->use_ie) {
			...
			//if (pCtx->encrypt_type == ENCRYPT_WEP) // add provisioning service ie
			if (((!pCtx->fix_wzc_wep || pCtx->config_by_ext_reg) && pCtx->fix_wzc_wep != 2) //add by peteryu for WZC in WEP
	...
	for (i=0; i<MAX_STA_NUM; i++) {
		if (pCtx->sta[i] && pCtx->sta[i]->used) {
			...
			if (pCtx->is_ap && pCtx->upnp && pCtx->TotalSubscriptions){
				for(i2=0 ; i2<MAX_SUBSCRIPTION_NUM ; i2++){
					if(pCtx->upnp_subscription_info[i2].used == 1){
						for(e = pCtx->upnp_info.subscribe_list.subscription_head.lh_first; e != NULL; ){
							next = e->entries.le_next;
							if(strcmp(e->sid , pCtx->upnp_subscription_info[i2].Sid)==0){
								LIST_REMOVE(e, entries);
				...
				if (pCtx->is_ap && !(pCtx->sta[i]->used & IS_UPNP_CONTROL_POINT) &&
					...
					if (pCtx->blocked_expired_time &&
						(pCtx->sta[i]->state >= ST_WAIT_M4 && pCtx->sta[i]->state <= ST_WAIT_M7) &&
...
*/		
			...
			pCtx->blocked_expired_time = (int)atoi(value);
		}
#endif
#ifdef MUL_PBC_DETECTTION
