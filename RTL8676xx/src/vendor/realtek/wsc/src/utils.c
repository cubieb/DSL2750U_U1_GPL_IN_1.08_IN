 ...
 *
 *	$Id: utils.c,v 1.11 2010/02/05 13:26:39 cathy Exp $
 */
...
{
/* hide by zhujianwen, not need wps led in wscd */
#if 0
...
int output_the_config(CTX_Tp pCtx, int is_local)
{
	char beanconType[32] = {0};
	int auth, encryp;
 	FILE *fp=NULL;
	fp = fopen("/var/tmp/wscoutput", "w+");
	if(fp == NULL)
	{
	...
	{
		printf("Local SSID:%s\n", pCtx->SSID);
		fprintf(fp, "Local SSID:%s\n", pCtx->SSID);
		
		printf("Local Psk:%s\n", pCtx->network_key);
		fprintf(fp, "Local Psk:%s\n", pCtx->network_key);
		fclose(fp);
		return REINIT_SYS;
	}

	auth = pCtx->assigned_auth_type;
	encryp = pCtx->assigned_encrypt_type;

	if((auth == AUTH_OPEN) && (encryp == ENCRYPT_NONE))
	{
		strcpy(beanconType, "None");
		printf("Remote BenconType:%s\n", beanconType);
		fprintf(fp, "Remote BenconType:%s\n", beanconType);

//#ifdef TBS_E8_TELECOM
#if 0

		printf("Remote Auth:OpenSystem\n");

		printf("Remote Encryp:None\n");
#else
		printf("Remote Auth:None\n");
		fprintf(fp,"Remote Auth:None\n");

		printf("Remote Encryp:None\n");
		fprintf(fp,"Remote Auth:None\n");
#endif
	}
	else if(encryp == ENCRYPT_WEP)
	{
...
//#ifdef TBS_E8_TELECOM
		...
		else if(auth == AUTH_SHARED)
		{
			printf("Remote Auth:ShareKey\n");
		}
#else
		if(auth == AUTH_OPEN)
		{
		...
		{
			printf("Remote Auth:SharedAuthentication\n");
			fprintf(fp, "Remote Auth:SharedAuthentication\n");
		}
#endif
		
		printf("Remote Encryp:WEPEncryption\n");
		fprintf(fp, "Remote Encryp:WEPEncryption\n");

		printf("Remote KeyIndex:%d\n", pCtx->assigned_wep_transmit_key);
		fprintf(fp,"Remote KeyIndex:%d\n", pCtx->assigned_wep_transmit_key);

		if(strlen(pCtx->assigned_wep_key_1) != 0)
		{
			printf("Remote Wep1:%s\n", pCtx->assigned_wep_key_1);
			fprintf(fp,"Remote Wep1:%s\n", pCtx->assigned_wep_key_1);
		}
		if(strlen(pCtx->assigned_wep_key_2) != 0)
		{
			printf("Remote Wep2:%s\n", pCtx->assigned_wep_key_2);
			fprintf(fp,"Remote Wep2:%s\n", pCtx->assigned_wep_key_2);
		}
		if(strlen(pCtx->assigned_wep_key_3) != 0)
		{
			printf("Remote Wep3:%s\n", pCtx->assigned_wep_key_3);
			fprintf(fp,"Remote Wep3:%s\n", pCtx->assigned_wep_key_3);
		}
		if(strlen(pCtx->assigned_wep_key_4) != 0)
		{
			printf("Remote Wep4:%s\n", pCtx->assigned_wep_key_4);
			fprintf(fp,"Remote Wep4:%s\n", pCtx->assigned_wep_key_4);
		}
	}
	else if((auth == AUTH_WPAPSK) || (auth == AUTH_WPA2PSK) || (auth == AUTH_WPA2PSKMIXED))
	{
		if(auth == AUTH_WPAPSK)
		{
		...
		printf("Remote Auth:PSKAuthentication\n");
		fprintf(fp,"Remote Auth:PSKAuthentication\n");

		if(encryp == ENCRYPT_TKIP)
		{
			printf("Remote Encryp:TKIPEncryption\n");
			fprintf(fp,"Remote Encryp:TKIPEncryption\n");
		}
		else if(encryp == ENCRYPT_AES)
		{
			printf("Remote Encryp:AESEncryption\n");
			fprintf(fp,"Remote Encryp:AESEncryption\n");
		}
		else if(encryp == ENCRYPT_TKIPAES)
		{
			printf("Remote Encryp:TKIPandAESEncryption\n");
			fprintf(fp,"Remote Encryp:TKIPandAESEncryption\n");
		}
		else
		{
			fclose(fp);
...
void reset_ctx_state(CTX_Tp pCtx)
{
	...
	if (!pCtx->is_ap) {
		...
		pCtx->connect_fail = 0;
	} else {//add by peteryu for WZC in WEP
#endif	
...
#ifdef BLOCKED_ROGUE_STA
void update_blocked_list(CTX_Tp pCtx, STA_CTX_Tp pSta)
{
	int i;
	for (i=0; i<MAX_BLOCKED_STA_NUM; i++) {
		if (!memcmp(pCtx->blocked_sta_list[i].addr, pSta->addr, ETHER_ADDRLEN)) {
			
			_DEBUG_PRINT("Update the sta [%02x:%02x:%02x:%02x:%02x:%02x]\n",
			...
			if(pCtx->blocked_sta_list[i].error_count<= 10){
				pCtx->blocked_sta_list[i].error_count++;
				DEBUG_PRINT("pCtx->blocked_sta_list[%d].error_count=%d\n",i,  pCtx->blocked_sta_list[i].error_count);
...
/**/
int search_blocked_list(CTX_Tp pCtx, unsigned char *addr)
{
	int i;
	...
	if (addr) {
		...
		return BLOCKED_ENTRY_NOT_FOUND;
	}
	else
		return BLOCKED_ENTRY_NOT_FOUND;
}
	...
	if (pSta->addr) {
		for (i=0; i<MAX_BLOCKED_STA_NUM; i++) {
			if (pCtx->blocked_sta_list[i].used == 0) {
				...
				pCtx->blocked_sta_list[i].error_count = 1;
				pCtx->blocked_sta_list[i].expired_time = pCtx->blocked_expired_time;				
...
void countdown_blocked_list(CTX_Tp pCtx)
{
	...
	for (i=0; i<MAX_BLOCKED_STA_NUM; i++) {
		if (pCtx->blocked_sta_list[i].used && pCtx->blocked_sta_list[i].expired_time > 0) {
			DEBUG_PRINT("pCtx->blocked_sta_list[%d].expired_time=%d\n", i, pCtx->blocked_sta_list[i].expired_time);
...
{
	/* mode by zhujianwen, output the status of wps */
#if 0
	char tmp[40];
	...
	system(tmp);
#endif
	char wpsStatus[4][20] = {"INPROCESS", "OVERLAP", "ERROR", "SUCCESS"};

	if(status == PROTOCOL_START)
	{
		printf("WPS Session Status:%s\n", wpsStatus[status]);
		return;
	}
#if 0
	system("ifconfig wlan0 down");//; ifconfig wlan0-vap0 down");
	system("ifconfig wlan0 up");//; sleep 1; ifconfig wlan0-vap0 up");
	sleep(1);
#endif	
	printf("WPS Session Status:%s\n", wpsStatus[status]);
}
