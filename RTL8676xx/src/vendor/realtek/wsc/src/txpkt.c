 ...
 *
 *	$Id: txpkt.c,v 1.11 2010/02/05 13:43:01 cathy Exp $
 */
...
static int add_credential_attr(CTX_Tp pCtx, STA_CTX_Tp pSta, char *out, int include_cred)
{
	...
	unsigned short shortVal, shortVal2;
	int size, idx=0;
	...
	if (pCtx->config_state == CONFIG_STATE_UNCONFIGURED) {
		...
		//Brad modify for Belkin requirement 2007-12-03		
...
//--------------------------- david+2007-01-26
	...
	pMsg = add_tlv(pMsg, TAG_AUTH_TYPE, 2, &shortVal);
#endif
	shortVal2 = (pSta->encrypt_type_flags & pCtx->encrypt_type_flash);
	if (shortVal2) {
		if (shortVal2 & ENCRYPT_AES)
			shortVal2 = ENCRYPT_AES;
		else if (shortVal2 & ENCRYPT_TKIP)
			shortVal2 = ENCRYPT_TKIP;
		else if (shortVal2 & ENCRYPT_NONE)
			shortVal2 = ENCRYPT_NONE;
		else if (shortVal2 & ENCRYPT_WEP)
			shortVal2 = ENCRYPT_WEP;
		else {
			shortVal2 = pCtx->encrypt_type;
			DEBUG_ERR("%s %d assign encrypt_type shortVal2 = %d\n", __FUNCTION__, __LINE__, shortVal2);
		}
	}
	else {
		shortVal2 = pCtx->encrypt_type;
...
#ifndef WPA2_MIXED_2MODE_ONLY 	
	pMsg = add_tlv(pMsg, TAG_ENCRYPT_TYPE, 2, &shortVal2);
#else
	if((shortVal == AUTH_WPA2PSK) && (shortVal2== ENCRYPT_TKIP)) {
	...
	else if((shortVal == AUTH_WPAPSK) && (shortVal2== ENCRYPT_AES)) {
	...
	pMsg = add_tlv(pMsg, TAG_MAC_ADDRESS, ETHER_ADDRLEN, pSta->msg_addr);
#endif
	if (shortVal2 == ENCRYPT_WEP && pCtx->wep_transmit_key)
...
int send_eap_fail(CTX_Tp pCtx, STA_CTX_Tp pSta)
{
	...
	if (pSta->invoke_security_gen) {
		pCtx->wait_reinit = output_the_config(pCtx, 1);
...
int send_wsc_M7(CTX_Tp pCtx, STA_CTX_Tp pSta)
{
	...
	unsigned char tmpbuf[1024], tmp1[256];
	unsigned short shortVal;
