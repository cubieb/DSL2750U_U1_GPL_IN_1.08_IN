...
struct atm_trafprm {
	...
	/*linux-2.6.19*/
	/* extra params for VBR */ //shlee 2.6
	int		scr;
	int		mbs;
...
typedef struct ch_stat{                                                                                                                                                                               
	...
	int	rcv_cnt;			/* count of Receive Functiont calls */                                                                                                                
	int	rcv_ok;			/* packet return count */                                                                                                                                     
	                                                                                                                                                                                              
	/* RX data statistics related */                                                                                                                                                              
	int	rx_desc_cnt;	/* count of descriptors that receive Functiont walks through */                                                                                                       
	int	rx_byte_cnt;	/* received byte count */                                                                                                                                             
	int	rx_pkt_cnt;                                                                                                                                                                           
	int	rx_pkt_fail;                                                                                                                                                                          
                                                                                                                                                                                                      
	int	rx_FS_cnt;		/* count of FS desc */                                                                                                                                        
	int	rx_LS_cnt;		/* count of LS desc */                                                                                                                                        
	int	rx_desc_ok_cnt;	/* rx ok descriptors */                                                                                                                                               
	int 	rx_oam_count;	/* rx oam cell count */                                                                                                                                               
	int	rx_buf_alloc;	/* rx buffer allocated */                                                                                                                                             
	int	rx_buf_lack;		/* lack of rx buffer */                                                                                                                                       
	int	rx_desc_fail;	/* rx desc errors */                                                                                                                                                  
	int	rx_crc_error;	/* rx AAL5 CRC error count */                                                                                                                                         
	int	rx_lenb_error;	/* rx packet length too large */                                                                                                                                      
	int	rx_lens_error;	/* rx packet length too small */                                                                                                                                      
                                                                                                                                                                                                      
	int	rx_netif_cnt;	/* count for netif calls */                                                                                                                                           
	//int	rx_cell_cnt;                                                                                                                                                                          
                                                                                                                                                                                                      
	/* TX program flow related */                                                                                                                                                                 
	int	send_cnt;		/* count of Send function calls */                                                                                                                            
	int	send_ok;		/* count of Send function ok */                                                                                                                               
	int	send_fail;		/* count of Send function errors */                                                                                                                           
	int	send_desc_full;	/* descriptors full */                                                                                                                                                
	int	send_desc_lack;	/* not enough free descriptors */                                                                                                                                     
                                                                                                                                                                                                      
	/* TX data statistics related */                                                                                                                                                              
	int	tx_desc_ok_cnt;	/* tx ok descriptors (successfully transmitted) */                                                                                                                    
	int	tx_buf_free;		/* tx buffer freed */                                                                                                                                         
	int	tx_pkt_ok_cnt;	/* tx ok packets (successfully transmitted)*/                                                                                                                         
	int	tx_pkt_fail_cnt;	/* tx fail packets (send into descritprtor ring failed) */                                                                                                    
	int	tx_byte_cnt;		/* send byte count (send into descriptor ring) */                                                                                                             
	//int	tx_cell_cnt;                                                                                                                                                                          
...
struct SAR_IOCTL_CFG {                                                                       
	char			ch_no;                                                       
	unsigned char		vpi;					//vpi number         
	unsigned short		vci;					//vci number         
	int			rfc;                                                         
	int			framing;                                                     
	int			created;                                                     
	int			loopback;	                                             
...
struct SAR_IOCTL_DEBUG {
	int enable;
	int chno;
	int etype;
	int ipproto;
	int hr;	// human readable
	struct {
		unsigned long ip;
		unsigned short port;
	} src;
...
#define SAR_SET_ENCAPS			(SIOCDEVPRIVATE+24)                                         
                                                                                                    
#ifdef AUTO_PVC_SEARCH	                                                                            
#define	SENT_STARTUP_PID		(SIOCDEVPRIVATE+25)                                         
#define SENT_BOA_PID			SENT_STARTUP_PID
#endif                                                                                              
                                                                                              
#define	SENT_SNTP_PID			(SIOCDEVPRIVATE+26)                                         
#define SENT_TR069_PID			(SIOCDEVPRIVATE+27)                                         
#define SENT_TERM_REQ			(SIOCDEVPRIVATE+28)
...
//#endif
// for debug                                                                                        
#define SAR_DEBUG_PVC_ENABLE		(SIOCDEVPRIVATE+50)                                         
#define SAR_DEBUG_OBAAL5_ENABLE		(SIOCDEVPRIVATE+51)                                         
#define SAR_DEBUG_OBCELL_ENABLE		(SIOCDEVPRIVATE+52)                                         
#define SAR_DEBUG_NUM2PRINT		(SIOCDEVPRIVATE+55)                                         
#define SAR_DEBUG_STATUS		(SIOCDEVPRIVATE+56)                                         
                                                                                                    
//#ifdef CONFIG_PORT_MIRROR
#define SAR_PORT_MIRROR		(SIOCDEVPRIVATE+57)
//#endif
#define SAR_UPGRADE_FW			(SIOCDEVPRIVATE+58)
#define SAR_RESET_STATS			(SIOCDEVPRIVATE+59)
