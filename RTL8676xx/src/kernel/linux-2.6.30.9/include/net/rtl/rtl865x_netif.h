...
#define RTL865X_ACL_PREPROCESS  				-10001
...
typedef struct _rtl865x_AclRule_s
{
	...
	/* Kevin, the following fileds is used for translating filter chain in iptables rule to acl  ( translate_rule2Acl() @ ip_tables.c )  */
	#if defined (CONFIG_RTL_IPTABLES_RULE_2_ACL)
	uint8 	toACL_;				/*  if this bit is set , the iptables rule will be added into acl !  */
	#ifndef CONFIG_RTL8676_Dynamic_ACL 
	uint8 	forceTrapToCPU_;	/*  if this bit is set , whatever the action(permit/drop) in the orignal iptables rule is	
	                                                                  , force the pkts matching this rule to CPU  (becuase 0412 acl cannot handle)*/
	uint8 	macFilter_;			/*  the bit will be set when this rule use mac filter  */
	#endif
	char     inNetif[IFNAMSIZ];	 /*  According to the in_netif in iptables rule, the acl rule type may be different decided by the match module  */								 
	#endif
...
/*force flag*/
#define RTL865X_ACL_NOT_FORCE	0 
#define RTL865X_ACL_FORCE		1 
...
#else
#define RTL_DRV_WAN0_NETIF_NAME RTL_DEV_NAME_NUM(ALIASNAME_MWNAS,_0)//"nas0_0"
#endif
#else
