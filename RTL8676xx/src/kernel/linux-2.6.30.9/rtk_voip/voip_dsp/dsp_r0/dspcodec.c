...
uint32 nRxSilencePacket[MAX_SESS_NUM];
...
#define INIT_LIST_SESS_NUM6		\
	INIT_LIST_SESS_NUM4,		\
	&Dsp_data[ 4 ].m_nPosDec, 	\
	&Dsp_data[ 5 ].m_nPosDec 	
#define INIT_LIST_SESS_NUM8		\
	INIT_LIST_SESS_NUM6,		\
	&Dsp_data[ 6 ].m_nPosDec, 	\
	&Dsp_data[ 7 ].m_nPosDec 	
#define INIT_LIST_SESS_NUM10	\
	INIT_LIST_SESS_NUM8,		\
	&Dsp_data[ 8 ].m_nPosDec, 	\
	&Dsp_data[ 9 ].m_nPosDec 	
#define INIT_LIST_SESS_NUM12	\
	INIT_LIST_SESS_NUM10,		\
	&Dsp_data[ 10 ].m_nPosDec, 	\
	&Dsp_data[ 11 ].m_nPosDec 	
#define INIT_LIST_SESS_NUM14	\
	INIT_LIST_SESS_NUM12,		\
	&Dsp_data[ 12 ].m_nPosDec,	\
	&Dsp_data[ 13 ].m_nPosDec
#define INIT_LIST_SESS_NUM16	\
	INIT_LIST_SESS_NUM14,		\
	&Dsp_data[ 14 ].m_nPosDec,	\
	&Dsp_data[ 15 ].m_nPosDec
#define INIT_LIST_SESS_NUM32	\
	INIT_LIST_SESS_NUM16,		\
	&Dsp_data[ 16 ].m_nPosDec,	\
	&Dsp_data[ 17 ].m_nPosDec,	\
	&Dsp_data[ 18 ].m_nPosDec,	\
	&Dsp_data[ 19 ].m_nPosDec,	\
	&Dsp_data[ 20 ].m_nPosDec,	\
	&Dsp_data[ 21 ].m_nPosDec,	\
	&Dsp_data[ 22 ].m_nPosDec,	\
	&Dsp_data[ 23 ].m_nPosDec,	\
	&Dsp_data[ 24 ].m_nPosDec,	\
	&Dsp_data[ 25 ].m_nPosDec,	\
	&Dsp_data[ 26 ].m_nPosDec,	\
	&Dsp_data[ 27 ].m_nPosDec,	\
	&Dsp_data[ 28 ].m_nPosDec,	\
	&Dsp_data[ 29 ].m_nPosDec,	\
	&Dsp_data[ 30 ].m_nPosDec,	\
	&Dsp_data[ 31 ].m_nPosDec	
...
const uint32 * const p_DspDecWrite[MAX_SESS_NUM] = {
 ...
 #elif MAX_SESS_NUM == 8
	INIT_LIST_SESS_NUM8,
 #elif MAX_SESS_NUM == 10
	INIT_LIST_SESS_NUM10,
 #elif MAX_SESS_NUM == 12
	INIT_LIST_SESS_NUM12,
 #elif MAX_SESS_NUM == 14
	INIT_LIST_SESS_NUM14,
 #elif MAX_SESS_NUM == 16
	INIT_LIST_SESS_NUM16,
 #elif MAX_SESS_NUM == 32
	INIT_LIST_SESS_NUM32,
#else
#error "Need implememt for pointer p_DspDecWrite for configured SESS_NUM"
 #endif
};
#endif
...
/*
 ...
 dtmf_mode = 2 : Inband
*/
char dtmf_mode[MAX_VOIP_CH_NUM] = {[0 ... MAX_VOIP_CH_NUM-1] = 2};
...
int32 DspcodecWrite(uint32 chid, uint32 sid, uint8* pBuf, int32 nSize, uint32 nSeq, uint32 nTimestamp)
{
...
#endif
	{
  ...
  #ifdef NEW_JITTER_BUFFER_WI_DESIGN
			jbuf_load = Dsp_data[sid].m_nPosDec + BUFFER_NUM_DEC - jbuf_ri;
  #else
			jbuf_load = Dsp_data[sid].m_nPosDec + 1 + BUFFER_NUM_DEC - jbuf_ri;
  #endif 
		} else {
  #ifdef NEW_JITTER_BUFFER_WI_DESIGN
  			jbuf_load = Dsp_data[sid].m_nPosDec - jbuf_ri;
  #else
		...
		{			
			...
			pSendParm[sid].xDecBuffer[n].nArrive = jiffies;
...
#ifdef NEW_JITTER_BUFFER_WI_DESIGN
			Dsp_data[sid].m_nPosDec = (n+1)%BUFFER_NUM_DEC;
#endif
  ...
  #endif 
		{
			...
			for( i = 0; i < tx_jit_buf_high_threshold[ sid ]; i ++ ) {
				...
				if( pSendParm[sid].xDecBuffer[n].nSeqNo == nSeq ) {
					if( pSendParm[sid].xDecBuffer[n].nOwner == 0 ) {
...
int32 DPDspcodecWrite(uint32 chid, uint32 sid, uint8* pBuf, int32 nSize, uint32 nSeq, uint32 nTimestamp)
{
	...
	extern uint32 RtpRedundantTimestamp_remote[ SESS_NUM ];
 #endif
 ...
 #ifdef SUPPORT_RTP_REDUNDANT
	if( RtpRedundantTimestamp_remote[ sid ] == 0 ) {
		RtpRedundantTimestamp_remote[ sid ] = i * TempStamp;
	}
 #endif
	/*
...
//	debug_dev("lost?\n");
	...
	//  seq       [io] seq_no to jitter buffer (based on previous writing)
...
label_start_write_frames:
	...
	for(k=0;k<i;k++)
	{
		...
		if ( bFrameSizeinbit )
		{
			...
			INC_DISCARD_IF0
			DspcodecWrite(chid, sid, Temp_buf, TempLen, seq, timestamp);
			seq ++;
			timestamp +=TempStamp;
			pSrcBuf += TempLen;
