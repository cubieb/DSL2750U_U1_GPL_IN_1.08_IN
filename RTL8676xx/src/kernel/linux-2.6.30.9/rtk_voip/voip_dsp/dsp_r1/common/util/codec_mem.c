...
#if !defined(CONFIG_RTK_VOIP_DRIVERS_PCM865xC) && !defined(CONFIG_RTK_VOIP_DRIVERS_PCM8651) && !defined(CONFIG_RTK_VOIP_DRIVERS_PCM8672)  && !defined(CONFIG_RTK_VOIP_DRIVERS_PCM8972B_FAMILY) && !defined(CONFIG_RTK_VOIP_DRIVERS_PCM89xxC)
...
int set_system_imem(void)
{
	...
	if(imem_data_internal!=SYSTEM_INSIDE)
	{
		...
		*(volatile unsigned long *) IM_CTL;
		...
		restore_flags(flags);
    #else
	#if !defined(CONFIG_RTK_VOIP_DRIVERS_PCM8672) && !defined(CONFIG_RTK_VOIP_DRIVERS_PCM89xxC)
		save_flags(flags);
		cli();
