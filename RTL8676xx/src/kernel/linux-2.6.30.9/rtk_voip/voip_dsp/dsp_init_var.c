...
uint32_t dtmf_data1[VOIP_CH_NUM*480] __attribute__((aligned(8)));
...
TstVoipbusytone_det_data stVoipbusytone_det_data[VOIP_CH_NUM];
...
TstVoip_dtmf_det_data stVoip_dtmf_det_data[VOIP_CH_NUM];

extern void* pstVoip_dtmf_det[];

int dsp_init_var(void)
{
	int chid;
	extern void dtmf_det_init(int chid);
	...
	for (chid=0 ; chid<VOIP_CH_NUM ; chid++)
	{
		pstVoip_dtmf_det[chid] = &stVoip_dtmf_det_data[chid];
		dtmf_det_init(chid);
...
#if defined(SUPPORT_RESAMPLER_G722) || defined(CONFIG_RTK_VOIP_DRIVERS_IIS) || defined(CONFIG_RTK_VOIP_WIDEBAND_SUPPORT)
...
#else	
	resampler_init(SESS_NUM, 8000, 16000, 1, &err_up);
	resampler_init(SESS_NUM, 16000, 8000, 1, &err_down);
#endif
