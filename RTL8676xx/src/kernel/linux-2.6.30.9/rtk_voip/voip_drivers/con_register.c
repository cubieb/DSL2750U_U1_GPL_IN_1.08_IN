	...
	case REG_PROC_COOKIE_SND:
		{
			...
			n += sprintf( buf + n, "sch name    TS1/2 T\n--- ------- ----- -\n" );
			
			p_snd = ( const voip_snd_t * )voip_snd_link.head;
			
			while( p_snd ) {
				n += sprintf( buf + n, "%3d %-7s %2d/%2d %d\n", 
					p_snd ->sch,
...
{
	remove_proc_entry( PROC_VOIP_DIR "/" PROC_VOIP_CH_MAPS_DIR "/snd", NULL );
	remove_proc_entry( PROC_VOIP_DIR "/" PROC_VOIP_CH_MAPS_DIR "/bus", NULL );
	remove_proc_entry( PROC_VOIP_DIR "/" PROC_VOIP_CH_MAPS_DIR "/dsp", NULL );
}
