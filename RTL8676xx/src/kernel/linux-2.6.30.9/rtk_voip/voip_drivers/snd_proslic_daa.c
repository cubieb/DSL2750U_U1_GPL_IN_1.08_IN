...
unsigned char daa_polarity_reversal_det(vdaaChanType *daas)
{
	ctrl_S * const ctrl = ( ctrl_S * )daas ->deviceId ->ctrlInterface ->hCtrl;
	rtl_spi_dev_t * const pdev = &ctrl ->spi_dev;
	const int daa_ch = daas ->channel;
	unsigned char ret;
	static unsigned long timeOut[VOIP_CH_NUM];
	...
	if (ret && (need_to_clear[daa_ch]==0))
	{
		timeOut[daa_ch] = jiffies + HZ*(1000/1000);
		need_to_clear[daa_ch] = 1;

		return 1;
	}

    if ( ret && (time_after(jiffies, timeOut[daa_ch])) ) // wait 0.5s, then clean 
	{
		writeDAAReg(pdev, daa_ch, 4,  readDAAReg(pdev, daa_ch, 4)&0xFE);	//clear the interrup
		need_to_clear[daa_ch] = 0;
...
unsigned char daa_drop_out_det(vdaaChanType *daas)
{
	ctrl_S * const ctrl = ( ctrl_S * )daas ->deviceId ->ctrlInterface ->hCtrl;
	rtl_spi_dev_t * const pdev = &ctrl ->spi_dev;
	const int daa_ch = daas ->channel;
	...
	if (ret && (need_to_clear[daa_ch]==0))
	{
		timeOut[daa_ch] = jiffies + HZ*(500/1000);
		need_to_clear[daa_ch] = 1;
