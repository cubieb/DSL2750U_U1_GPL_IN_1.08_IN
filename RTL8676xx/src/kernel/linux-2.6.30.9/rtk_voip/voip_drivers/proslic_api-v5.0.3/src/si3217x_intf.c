...
**
** $Id: si3217x_intf.c,v 1.1 2011/03/15 08:13:29 yachang Exp $
**
** SI3217X_Intf.c
...
//#include "si3217x_BKBT_constants.c"
//#include "si3217x_patch_B_BB_2009MAY22.c"
...
int Si3217x_PowerUpConverter(proslicChanType_ptr pProslic)
{
    ...
    */
    ...
    if(vbat < (vbath / 2)) {
        ...
        if(pProslic->debugMode)
        {
            LOGPRINT ("Si3217x DCDC Short Circuit Failure %d - disabling channel\n",pProslic->channel);
        }
        #endif      
...
int Si3217x_Init (proslicChanType_ptr *pProslic, int size){
    ...
    */
    for (k=0;k<size;k++)
    {
        if (pProslic[k]->channelEnable)
        {
			if (pProslic[k]->deviceId->chipRev == 1 )
			{
				if (pProslic[k]->debugMode)
