...
static int proslic_initializeDAA( const voip_snd_t p_snds[], 
		vdaaChanType* port[], proslicChanType* slics[], int pcm_mode)
{
		...
		int pcm_mode)
{
	...
	for (i=0; i<prochan_num; i++)
	{
		...
		if( p_snds[ i ].snd_type == SND_TYPE_FXS )
			p_snds[ i ].fxs_ops ->SLIC_show_ID( &p_snds[ i ] );
	}
...
//#define SLIC_LOOP_TEST
	...
	if (proslic_initializeDAA( &p_snds[ daaIdx ], &container[ daaIdx ].daas, pSlic, pcm_mode))
	{
...
static void Set_SLIC_Tx_Gain_proslic(voip_snd_t *this, unsigned char tx_gain)
{
	printk("Not implemented! Support unity gain only!\n");
}

static void Set_SLIC_Rx_Gain_proslic(voip_snd_t *this, unsigned char rx_gain)
{
	printk("Not implemented! Support unity gain only!\n");
}
