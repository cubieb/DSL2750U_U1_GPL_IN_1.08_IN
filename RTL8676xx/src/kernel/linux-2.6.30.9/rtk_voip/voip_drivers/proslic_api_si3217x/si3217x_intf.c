...
#include "si3217x_constants.c"
#include "si3217x_patch.c"
...
static int isVerifiedProslic(proslicChanType *pProslic){
	uInt8 data = ReadReg(pProHW,pProslic->channel,PCMTXHI);
...
static int isBroadcastPossible(proslicChanType *pProslic){
	...
	pProslic->deviceId->chipType= ((data&0x38)>>3) + SI32171;
	/*check for nonDAA version - otherwise we can't broadcast*/
	if (pProslic->deviceId->chipType == SI32172)
	{
        pProslic->error = RC_BROADCAST_FAIL;
        return RC_BROADCAST_FAIL;
	}
	if (pProslic->deviceId->chipType >= SI32177 && pProslic->deviceId->chipType <= SI32179)
	{
        pProslic->error = RC_BROADCAST_FAIL;
        return RC_BROADCAST_FAIL;
	}
...
/* HERE CDP - THIS MIGHT BE REMOVED */
/*
** Function: cal_iteration
**
** Description: 
** Perform ProSLIC calibration on all channels in parallel (si3217x only)
**
** Input Parameters: 
** pProslic: pointer to PROSLIC object
**
...
static int cal_iteration (proslicChanType *pProslic,int iter,uInt8 *data2,uInt8 numChan){
	int i;
    uInt8 data,calStatus;
	int millisecs=0;
    for (i=0;i<numChan;i++){ /*1/1/1 chan/madc/dsp ratio for Si3217x, ie. 1 iteration*/
	...
	do{
		...
		if (millisecs++ > 3000){
			if (pProslic->debugMode)
				PRINT_R("Calibration timeout channel %d\n",iter);
		}
    }while ((calStatus&0x80)&&(millisecs<3000));
    return calStatus;
}
/*
...
int Si3217x_PowerUpConverter(proslicChanType_ptr pProslic)
{
    ...
    */
    ...
    if(pProslic->debugMode)
    {
        LOGPRINT ("VBAT Up = %d.%d v\n",(vbat/SCALE_V_MADC)/1000, abs((vbat/SCALE_V_MADC) - (vbat/SCALE_V_MADC)/1000*1000));
    }
#endif
    if(timer > 200)
    {
        ...
        if(pProslic->debugMode)
        {
            LOGPRINT ("Si3217x DCDC Power up timeout channel %d - disabling channel\n",pProslic->channel);
        }
#endif      
    }
...
int Si3217x_PowerDownConverter(proslicChanType_ptr pProslic)
{
    ...
    */
    ...
    if(timer > 20)
    {
        ...
        if(pProslic->debugMode)
        {
            LOGPRINT ("Si3217x DCDC Power Down timeout channel %d\n",pProslic->channel);
        }
#endif      
    }
...
/* HERE - CDP  THESE SHOULD BE REMOVED */
...
int Si3217x_Init (proslicChanType_ptr *pProslic, int size){
	...
	*/
    ...
    /*
    ** Identify channel type (ProSLIC or DAA) 
    */
	for (k=0;k<size;k++){
		if (isVerifiedProslic(pProslic[k]) == RC_CHANNEL_TYPE_ERR){
			if (pProslic[k]->debugMode)
				PRINT_R("si3217x: Channel %d appears to be DAA\n",pProslic[k]->channel);
			pProslic[k]->error = RC_CHANNEL_TYPE_ERR;
    ...
    */
	for (k=0;k<size;k++){
		...
		pProslic[k]->deviceId->chipType= ((data&0x31)>>3) + SI32171;

		// Added by Ted Liu
		if (pProslic[k]->debugMode)
			PRINT_MSG("si3217x chip version: %d.\n", pProslic[k]->deviceId->chipRev);
    ...
    */
    for (k=0;k<size;k++){
		if (pProslic[k]->channelEnable){
			if (pProslic[k]->deviceId->chipRev == 1 )   // This driver only works with REV B Si3217.
			{/*rev B*/

				// Added by Ted Liu
				if (pProslic[k]->debugMode)
					PRINT_MSG("Loading patch for SLIC%d... ...\n", k);
				...
				if (data){
					...
					PRINT_R("Si3217x load patch error for SLIC%d!\n", k);
					return 1;
				} else {
				...
				// Added by Ted Liu
				if (pProslic[k]->debugMode)
					PRINT_MSG("Patch is verified. (%d)\n", k);

	}
#endif
			}
			else   // Not REV B Si3217
			{
				PRINT_R("[Error]: This driver only supports REV B Si3217x!\n");
				return 1;
			}
...
int Si3217x_Init_GUI (proslicChanType_ptr *pProslic, int size){
	/*
	** This function will initialize the chipRev and chipType members in pProslic
	** as well as load the initialization structures.
	*/
    ...
    */
	...
	//Register 1 = 1			#Reset SLIC Channel
	pProslic[k]->WriteRegX(pProslic[k]->pProHWX,pProslic[k]->channel, 1, 1);

	//WAIT 100
	pProslic[k]->DelayX(pProslic[k]->pProTimerX, 100);
	
	//Register 74 = 20		#Reset DAA Channel
	pProslic[k]->WriteRegX(pProslic[k]->pProHWX,pProslic[k]->channel, 74, 0x20);
		...
		if (pProslic[k]->channelEnable){
			...
			{/*rev B*/
				...
				if (data){
					pProslic[k]->channelEnable=0;
					pProslic[k]->error = RC_PATCH_ERR;  
					PRINT_R("Si3217x load patch error!\n");
					return 1;
				} else {
					setUserMode(pProslic[k],TRUE);
					pProslic[k]->WriteRegX (pProslic[k]->pProHWX, pProslic[k]->channel,JMPEN,1);  
...
int Si3217x_InitBroadcast(proslicChanType_ptr *pProslic)
{
    ...
    */
    if (isBroadcastPossible(pProslic[0]) == RC_BROADCAST_FAIL)
        return RC_BROADCAST_FAIL;
    ...
    */
    ...
    /* Hardcoded mods to default settings */
    pProslic[0]->WriteRegX(pProslic[0]->pProHWX,BROADCAST,GPIO_CFG1,0xF0); /* coarse sensors analog mode */
...
/*
** Function: Si3217x_LBCalBroadcast
**
** Description: 
** Run canned longitudinal balance calibration in broadcast
**
** Input Parameters: 
** pProslic: pointer to PROSLIC channel object
**
...
** 0
*/
int Si3217x_LBCalBroadcast (proslicChanType *pProslic)
{
    uInt8 data;
    int16 k,i;
    int16 numOfChan;
    ...
    for (k=0;k<numOfChan;k++){
        WriteReg(pProHW, (uInt8)k,CALR3,0x80); /*start cal*/
    }
    i=0;
    do {
        data = 0;
        for (k=0;k<numOfChan;k++){
            data |= ReadReg(pProHW,(uInt8)k,CALR3);
        }       
        Delay(pProTimer,10);
        if (i++ >= TIMEOUT_LB_CAL){
#ifdef ENABLE_DEBUG
            if (pProslic->debugMode)
                LOGPRINT("Si3217x LB Calibration timeout\n");
#endif
...
int Si3217x_EnableInterrupts (proslicChanType_ptr pProslic){
    if (pProslic->deviceId->chipRev == 0) /*madc_vbat broken in rev a*/
...
/*  Freq rtper rtdb rtacthCR12 rtacthCR13 rtacthCR14 rtacthCR15 */
        {
            {15, 0x6A000L,  0x4000L,  {14369375L, 12933127L, 11793508L, 10874121L}},
            {16, 0x64000L,  0x4000L,  {13471289L, 12124806L, 11056414L, 10194489L}},
            {20, 0x50000L,  0x6000L,  {10777031L, 9699845L, 8845131L, 8155591L}},
            {22, 0x48000L,  0x6000L,  {9797301L, 8818041L, 8041028L, 7414174L}},
            {23, 0x46000L,  0x6000L,  {9371331L, 8434648L, 7691418L, 7091818L}},
            {25, 0x40000L,  0x8000L,  {8621625L, 7759876L, 7076105L, 6524473L}},
            {30, 0x36000L,  0x8000L,  {7184687L, 6466563L, 5896754L, 5437061L}},
            {34, 0x60000L,  0x6000L,  {6466865L, 5820489L, 5307609L, 4893844L}},
            {35, 0x5C000L,  0x6000L,  {6158303L, 5542769L, 5054361L, 4660338L}},
            {40, 0x50000L,  0x6000L,  {5388516L, 4849923L, 4422565L, 4077796L}},
            {45, 0x48000L,  0x6000L,  {4789792L, 4311042L, 3931169L, 3624707L}},
            {50, 0x40000L,  0x8000L,  {4310812L, 3879938L, 3538052L, 3262236L}},
    ...
    case ProSLIC_RING_SINE:
        ...
        Si3217x_Ring_Presets[preset].ringcon &= 0xE2; 
#else
        Si3217x_Ring_Presets[preset].ringcon &= 0xFA; 
#endif
    ...
    case ProSLIC_RING_TRAP_CF15:  
        ...
        Si3217x_Ring_Presets[preset].ringcon &= 0xE7; 
#else
        Si3217x_Ring_Presets[preset].ringcon |= 0x19; 
#endif
...
void Si3217x_Set_Impendance_Silicon(proslicChanType *pProslic, unsigned short country, unsigned short impd /*reserve*/)
{
	switch(country)
	{
		...
		case COUNTRY_JP:
			Si3217x_ZsynthSetup(pProslic, 1); 	// 600 ohm + 1uF
			//Si3226_ZsynthSetup(pProslic, 0); 	// 600 ohm
			break;

		
		case COUNTRY_AUSTRALIA:	
			Si3217x_ZsynthSetup(pProslic, 5); 	// 220 onm + (600 omh || 100 nF)	
			break;
			
		case COUNTRY_CN:
			//Si3217x_ZsynthSetup(pProslic, 2); 	// 200 ohm + (560 ohm || 100 nF)
			Si3217x_ZsynthSetup(pProslic, 3);	// 200 ohm + (680 ohm || 100 nF)
			break;
		
		case COUNTRY_GR:
			Si3217x_ZsynthSetup(pProslic, 6); 	// 220 ohm + (820 ohm || 115 nF)
			break;
			
		case COUNTRY_UK:
			Si3217x_ZsynthSetup(pProslic, 4); 	// CTR21
			break;
			
		case COUNTRY_SE:
			Si3217x_ZsynthSetup(pProslic, 8); 	// 200 ohm + (1000 ohm || 100 nF)
			break;
			
		case COUNTRY_FR:
			Si3217x_ZsynthSetup(pProslic, 10); 	// 215 ohm + (1000 ohm || 137 nF)
			break;
			
		case COUNTRY_BE:
			Si3217x_ZsynthSetup(pProslic, 9); 	// 150 ohm + (830 ohm || 72 nF)
			break;
		...
		case COUNTRY_IT:
			Si3217x_ZsynthSetup(pProslic, 7); 	// 400 ohm + (700 ohm || 200 nF)
			break;
...
void Si3217x_Set_Impendance(proslicChanType *pProslic, unsigned short preset)
{
	...
	{
		case 0:
			Si3217x_ZsynthSetup(pProslic, 18);
			PRINT_MSG("Set SLIC impedance = 600 ohm\n");
			break;
		case 1:
			Si3217x_ZsynthSetup(pProslic, 19);
		...
		case 3:
			...
			break;
		case 4:
			Si3217x_ZsynthSetup(pProslic, 17);
