...
#define USE_MEM64_OP
#include "gpio/gpio.h"
#elif defined (CONFIG_RTK_VOIP_DRIVERS_PCM8671) || defined (CONFIG_RTK_VOIP_DRIVERS_PCM8672)
#include "gpio/gpio.h"
...
#include "dsp_main.h"
voip_callback_t toRegister[MAX_VOIP_CH_NUM];
#endif
...
int law = PCMMODE_LINEAR;
#else
#ifdef CONFIG_RTK_VOIP_WIDEBAND_APP
int law = PCMMODE_WIDEBAND_LINEAR;
#else
int law = PCMMODE_ALAW; // linear: 0 , a-law: 1, u-law: 2. It set the pcm controller and slic to desired mode.
#endif
#endif
...
#if !defined(CONFIG_RTK_VOIP_DRIVERS_PCM865xC) && !defined(CONFIG_RTK_VOIP_DRIVERS_PCM8672) && !defined(CONFIG_RTK_VOIP_DRIVERS_PCM8972B_FAMILY) && !defined(CONFIG_RTK_VOIP_DRIVERS_PCM89xxC)
	switch(type)
	{
		case P0OK:
...
#ifdef RTK_VOICE_PLAY
static void voice_play_mix(int chid, Word16* poutput)
{
...
#endif
			{
				...
				if (chanEnabled[chid] == TRUE)	{
								...
								p_bus, ( uint16 * )txbuf ) == NULL )
					{
						...
						if( bch == 0 )
							ddinst_write( VOIP_DEV_PCM0_TX, ( char * )txbuf, 160 );
#ifdef SUPPORT_VOIP_DBG_COUNTER
...
static irq_handler_t pcm_interrupt(int32 irq, void *dev_instance)
...
#endif // CONFIG_RTK_VOIP_WIDEBAND_SUPPORT
...
{
	remove_proc_entry( PROC_VOIP_DIR "/" PROC_VOIP_PCM_DIR "/regs", NULL );

#ifdef PCM_DBG_PROC
	remove_proc_entry( PROC_VOIP_DIR "/" PROC_VOIP_PCM_DIR "/dbg", NULL );
#endif
