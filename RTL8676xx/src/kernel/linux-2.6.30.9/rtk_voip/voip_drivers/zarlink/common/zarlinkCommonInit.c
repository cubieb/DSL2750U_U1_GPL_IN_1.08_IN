...
** local variable 
*/
static struct timer_list ZarlinkTimer;
...
int zarlinkRegDevForEvHandle(RTKDevObj * pDev)
{
	...
	if (gDevNum == 0) {
		...
		init_timer( &ZarlinkTimer ); 			/* init once */
		ZarlinkTimer.expires = jiffies + 1; 	/* 10ms      */
		ZarlinkTimer.function = zarlinkAlarmHandle;
		add_timer( &ZarlinkTimer );
	}
