...
uint32* pTxBufTmp;
#else
uint32 RxBufTmp[MAX_VOIP_CH_NUM][RX_BUF_SIZE/4];
uint32 TxBufTmp[MAX_VOIP_CH_NUM][TX_BUF_SIZE/4];
#endif

#ifdef PCM_LOOP_MODE_DRIVER
#define LOOP_FIFO_SIZE	6
uint32 RxLoopBuf[MAX_VOIP_CH_NUM][LOOP_FIFO_SIZE*RX_BUF_SIZE/4*MAX_BAND_FACTOR];
static uint32 rx_loop_r[MAX_VOIP_CH_NUM]={0}, rx_loop_w[MAX_VOIP_CH_NUM]={0};
uint32 TxLoopBuf[MAX_VOIP_CH_NUM][LOOP_FIFO_SIZE*RX_BUF_SIZE/4*MAX_BAND_FACTOR];
static uint32 tx_loop_r[MAX_VOIP_CH_NUM]={0}, tx_loop_w[MAX_VOIP_CH_NUM]={0};
#endif

#ifdef PCM_LOOP_MODE_DRIVER
#define TX_NO_TONE_FIFO_SIZE	6
static uint32 TxNoToneBuf[ MAX_VOIP_CH_NUM ][TX_NO_TONE_FIFO_SIZE*RX_BUF_SIZE/4*MAX_BAND_FACTOR];
static uint32 tx_no_tone_r[ MAX_VOIP_CH_NUM ], tx_no_tone_w[ MAX_VOIP_CH_NUM ];
#endif
...
#if ( defined( NEW_REMOTE_TONE_ENTRY ) && !defined( DISABLE_NEW_REMOTE_TONE ) ) || defined( NEW_TONE_ENTRY_ARCH )
#define TONE_FIFO_SIZE	6
static uint32 RxToneBuf[ MAX_VOIP_CH_NUM ][TONE_FIFO_SIZE*RX_BUF_SIZE/4*MAX_BAND_FACTOR];
static uint32 rx_tone_r[ MAX_VOIP_CH_NUM ], rx_tone_w[ MAX_VOIP_CH_NUM ];
static uint32 rx_tone_sid[ MAX_VOIP_CH_NUM ];
#ifdef NEW_TONE_ENTRY_ARCH
static uint32 TxToneBuf[ MAX_VOIP_CH_NUM ][TONE_FIFO_SIZE*TX_BUF_SIZE/4*MAX_BAND_FACTOR];
static uint32 tx_tone_r[ MAX_VOIP_CH_NUM ], tx_tone_w[ MAX_VOIP_CH_NUM ];
static uint32 tx_tone_sid[ MAX_VOIP_CH_NUM ];
#endif
#endif

static void MixRxToneBuffer( uint32 chid, uint32 *pRxBuffer );
								...
								voip_con_t * const p_con )
{
	 ...
	 */
	if (dtmf_removal_flag[ dch ] == 1) // RFC2833, SIP_INFO 
	{ 
...
#if defined( NEW_REMOTE_TONE_ENTRY ) || defined( NEW_TONE_ENTRY_ARCH )
...
{
 #ifndef DISABLE_NEW_REMOTE_TONE
...
void MixTxToneBuffer( uint32 chid, uint32 *pTxBuffer )
{
 ...
 #endif /* !DISABLE_NEW_REMOTE_TONE */
#endif /* NEW_REMOTE_TONE_ENTRY || NEW_TONE_ENTRY_ARCH */
...
void dsp_rtk_bus_handler( void )
{
	...
	dch = (++last_dch)%VOIP_CH_NUM;
	//for (f_cnt = 0; f_cnt < (VOIP_CH_NUM*PCM_FIFO_SIZE*2); f_cnt++, dch = (++dch)%VOIP_CH_NUM)
	for (f_cnt = 0; f_cnt < (VOIP_CH_NUM*PCM_PERIOD*2); f_cnt++, dch = (++dch)%DSP_RTK_CH_NUM)
	{
...
#if defined( NEW_REMOTE_TONE_ENTRY ) && !defined( DISABLE_NEW_REMOTE_TONE )
