...
static char fsk_cid_enable[MAX_VOIP_CH_NUM]={0};
#endif
...
static void Pluse_Dial_Det_Handle_Hook( 
						...
						voip_snd_t * const p_snd )
{
	...
	if ( (&hook_res)->hook_status == PHONE_ON_HOOK)
	{
		...
		if (Is_DAA_snd(p_con ->snd_ptr))
			hook_in(cch, FXO_ON_HOOK);
		else
			hook_in(cch, PHONE_ON_HOOK);
		//printk("hook_in: ON\n");
	}
	else if ( (&hook_res)->hook_status == PHONE_OFF_HOOK)
	{
		//if (Is_DAA_Channel(i) == 1)
		if (Is_DAA_snd(p_con ->snd_ptr))
			hook_in(cch, FXO_OFF_HOOK);
		else
		{
			hook_in(cch, PHONE_OFF_HOOK);
			//printk("hook_in: OFF\n");

#ifndef CONFIG_AUDIOCODES_VOIP
#if 1	// stop type-1 fsk cid gen when phone off-hook
			voip_dsp_t * const p_dsp = p_con ->dsp_ptr;
	...
	else
	{
		//if (Is_DAA_Channel(i) == 1)
		if (Is_DAA_snd(p_con ->snd_ptr))
			hook_in(cch, FXO_UNKNOWN);
		else
			hook_in(cch, PHONE_UNKNOWN);
	}
...
void ring_det_cad_set( unsigned int ring_on_msec, unsigned int first_ringoff_msec, unsigned int ring_off_msec)
{
	...
	extern int slic_ch_num, voip_ch_num;
						...
						voip_snd_t * const p_snd )
{
	...
	if (temp_ring_tone_flag_get & 0x5)	//should add daa chid
	{
		...
		if(1 == temp_ring_tone_flag_get)
			hook_in(cch, FXO_RING_TONE_ON);
		else
			hook_in(cch, FXO_RING_TONE_OFF);
	}
#endif
...
{
	//printk("*");
	extern unsigned long gFirstRingOffTimeOut[];
	...
	const uint32 dch = p_con ->dsp_ptr ->dch;
	 ...
	 {
	     //printk("=>%d, J=%d\n", gFirstRingOffTimeOut[chid], jiffies);
	     if (time_after(jiffies, gFirstRingOffTimeOut[dch]))    // after 1st Ring Off cadence time out, gen 2nd Ring.
	     {
	         extern unsigned char ioctrl_ring_set[];
			 //PRINT_R("ioctrl_ring_set[%d] = %d\n", chid, ioctrl_ring_set[chid]);
	
			 if ((ioctrl_ring_set[dch]&0x2) && (!(ioctrl_ring_set[dch]&0x1)) && (!(fsk_spec_areas[dch]&0x08)))
	         {
	         ...
	         {
			 	  if ((ioctrl_ring_set[dch]&0x2) && (!(ioctrl_ring_set[dch]&0x1))) // CID prior 1st Ring
				  {
	              		/* App set Ring disable, so DSP no need to Ring atuomatically. */
						...
						voip_snd_t * const p_snd )
{
	...
	if ((slic_cpc[cch].cpc_stop == 0) && (time_after(jiffies, slic_cpc[cch].cpc_timeout)))
	{
	...
	if ((slic_cpc[cch].cpc_stop == 1) && (time_after(jiffies, slic_cpc[cch].cpc_timeout2)))
	{
		...
		else
		{
			if (connect_cnt[cch] > pulse_dial_pause_time[cch])
			{
				if (pulse_cnt[cch] != 0)
				{
					if (pulse_cnt[cch] == 10)
						pulse_cnt[cch] = 0;
					dtmf_in(cch, pulse_cnt[cch]+48/*char*/);
...
static void START_Event_Polling_Use_Timer(unsigned long data)
{
	int i;
...
void Reinit_Event_Polling_Use_Timer( void )
{
	...
	unsigned long data = event_timer.data;
...
void Init_Event_Polling_Use_Timer( unsigned long data )
{
	...
	event_timer.expires = jiffies + 1;
	event_timer.data = data;
	event_timer.function = START_Event_Polling_Use_Timer;
	add_timer(&event_timer);
