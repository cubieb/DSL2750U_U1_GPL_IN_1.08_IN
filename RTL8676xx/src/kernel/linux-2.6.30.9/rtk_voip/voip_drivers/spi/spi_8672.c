...
void rtl8672_share_spi_write(unsigned int ch, unsigned int uses_daisy, unsigned int control, unsigned int address,unsigned int data)
{
	unsigned int		spicnr;
...
void __rtl867x_spi_init(int ch_spi)
{
	...
	*(volatile int*) BSP_IP_SEL &= ~(BSP_EN_XSI_VOIP_SPI);
	...
	spi_reg->SPICKDIV = 0x40000000;  //must > 32
	spi_reg->SPITCR = SPI_SEL_CS(0) | SPI_CTL_EN(1) | SPI_ADD_EN(1) | SPI_D0_EN(1) | 0x5045;
	//printk("SPITCR:[%08x]=%08x\n", RTL8672_SPI_BASE + 0x10 , spi_reg->SPITCR);
...
int32 __rtl867x_spi_init( rtl_spi_dev_t* pDev, uint32 SPI_SEL_CS)
{
	...
	*(volatile int*) BSP_IP_SEL &= ~(BSP_EN_XSI_VOIP_SPI);
	...
	//chip select
	pDev->SPI_SEL_CS = SPI_SEL_CS? 0:1;
