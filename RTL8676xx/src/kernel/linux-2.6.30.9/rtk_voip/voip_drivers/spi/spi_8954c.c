...
/************************************* Set GPIO Pin to SPI ***********************************************************/
/*
@func int32 | __rtl8954C_spi_init | Initialize SPI device
...
int32 __rtl8954C_spi_init( rtl_spi_dev_t* pDev, uint32 gpioSCLK, uint32 gpioCS_, uint32 gpioSDO, uint32 gpioSDI, int32 gpioINT )
//#else
//static int32 _rtl8954C_spi_init( rtl_spi_dev_t* pDev, uint32 gpioSCLK, uint32 gpioCS_, uint32 gpioSDO, uint32 gpioSDI)
//#endif
{
...
#ifdef CONFIG_RTK_VOIP_DECT_SPI_SUPPORT
	_rtl8954C_initGpioPin( gpioINT, GPIO_CONT_GPIO, GPIO_DIR_IN, GPIO_INT_BOTH_EDGE ); //Enable both falling or rising edge interrupt
#endif
...
@comm
*/
int32 _rtl8954C_spi_exit( rtl_spi_dev_t* pDev )
{
	return SUCCESS;
}
...
void _rtl8954C_init_spi_channels(int size, rtl_spi_dev_t* pDev[], uint32 pin_cs[], uint32 pin_reset, uint32 pin_clk, uint32 pin_do, uint32 pin_di)
{
	int i;
	...
	for( i = 0; i < size; i ++ ) {
		_rtl8954C_initGpioPin(pin_cs[i], GPIO_CONT_GPIO, GPIO_DIR_OUT, GPIO_INT_DISABLE);
		__rtl8954C_spi_init(pDev[i], pin_clk, pin_cs[i], pin_do, pin_di, GPIO_INT_DISABLE);
	}
	
	for(i=0;i<BUSY_LOOP;i++);

	for( i = 0; i < size; i ++ )
		_rtl8954C_setGpioDataBit(pin_cs[i], 1); 	/* CS_ preset to high state*/

	_rtl8954C_setGpioDataBit(pin_clk, 1); 	/* SCLK preset to high state*/
