...
TstVoipLoopBcakInfo LoopBackInfo[SESS_NUM]={0};
int loop_3way[VOIP_CH_NUM]={0};
#endif
...
unsigned char remote_is_rfc2833[MAX_VOIP_CH_NUM]; /* 0: remote is not rfc2833, don't do dtmf removal, else is rfc2833, do dtmf removal */
...
extern uint32 rtpConfigOK[MAX_SESS_NUM];
...
char dtmf_mode[MAX_VOIP_CH_NUM] = {0};
unsigned char rfc2833_payload_type_local[MAX_SESS_NUM];
unsigned char rfc2833_payload_type_remote[MAX_SESS_NUM];
static int IsMode3[MAX_VOIP_CH_NUM] = {0};
extern struct RTK_TRAP_profile *filter[];
...
int do_mgr_VOIP_MGR_SET_SESSION( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	ret = ethernetDspSentL2ControlPacket(cmd, mgr_chid, &stVoipMgrSession, sizeof(TstVoipMgrSession));
	stVoipMgrSession.ch_id = mgr_chid;

#else //! CONFIG_RTK_VOIP_ETHERNET_DSP_IS_HOST
	...
	RtpSession_renew(s_id, 
				stVoipMgrSession.rtp_redundant_payload_type_local,
				stVoipMgrSession.rtp_redundant_payload_type_remote,
				stVoipMgrSession.SID_payload_type_local,
				stVoipMgrSession.SID_payload_type_remote);
...
int do_mgr_VOIP_MGR_UNSET_SESSION( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	{
		if(stVoipCfg.ch_id < voip_ch_num)
		{
...
{
	extern int hook_det_start(uint32 ch_id);
	unsigned long flags;
	TstVoipCfg stVoipCfg;
	int ret;
	...
	if (stVoipMgr3WayCfg.enable == 1)
	{
		...
		for(sid0=0; sid0<SESS_NUM; sid0++)
		{
			...
			if(LoopBackInfo[sid0].isLoopBack == 1)
			{
				...
				for (sid1=sid0+1; sid1<SESS_NUM; sid1++)
				{
...
int do_mgr_VOIP_MGR_GET_RTP_STATISTICS( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	extern uint32 nRxRtpStatsCountByte[MAX_VOIP_CH_NUM];
	extern uint32 nRxRtpStatsCountPacket[MAX_VOIP_CH_NUM];
	...
	if( ( ch_id = stVoipRtpStatistics.chid ) > VOIP_CH_NUM )
	{
	...
	// Send Control Packet and wait Response Packet
	mgr_chid = stVoipRtpStatistics.chid;
	stVoipRtpStatistics.chid = API_get_DSP_CH(cmd, stVoipRtpStatistics.chid);	// convert to DSP chid
	ret = ethernetDspSentL2ControlPacket(cmd, mgr_chid, &stVoipRtpStatistics, sizeof(TstVoipRtpStatistics));
	
	// Ckeck Response Packet (need for copy_to_user)
	unsigned short dsp_id;
	ethernetDspCheckL2RespPacket(cmd, &stVoipRtpStatistics, &dsp_id);
	stVoipRtpStatistics.chid = API_get_Host_CH( dsp_id, stVoipRtpStatistics.chid);/* Get Host chid */
...
int do_mgr_VOIP_MGR_GET_SESSION_STATISTICS( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	extern uint32 nRxSilencePacket[MAX_SESS_NUM];
	extern uint32 nTxSilencePacket[MAX_SESS_NUM];
	...
	sid = API_GetSid(stVoipSessionStatistics.chid, stVoipSessionStatistics.sid);
...
int do_mgr_VOIP_MGR_SET_RTCP_SESSION( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	ret = ethernetDspSentL2ControlPacket(cmd, mgr_chid, &stVoipRtcpSession, sizeof(TstVoipRtcpSession));
	...
	RtpSession_renew(s_id+ RTCP_SID_OFFSET, 0 /*ignore*/, 0 /*ignore*/, 0 /*ignore*/, 0 /*ignore*/);
