...
TstDtmfClid dtmf_cid_info[MAX_VOIP_CH_NUM];
...
int eng_det_flag[MAX_VOIP_CH_NUM] = {0};
...
long auto_cid_det[MAX_VOIP_CH_NUM] = {0};
long cid_type[MAX_VOIP_CH_NUM] = {0};
...
int do_mgr_VOIP_MGR_SETRTPPAYLOADTYPE( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	ret = ethernetDspSentL2ControlPacket(cmd, mgr_chid, &stVoipPayLoadTypeConfig, sizeof(TstVoipPayLoadTypeConfig));
...
int do_mgr_VOIP_MGR_DSPCODECSTOP( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	ret = ethernetDspSentL2ControlPacket(cmd, mgr_chid, &stVoipValue, sizeof(TstVoipValue));
 ...
 */
#if ! defined (AUDIOCODES_VOIP)
int do_mgr_VOIP_MGR_FAX_MODEM_PASS_CFG( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	extern unsigned int Fax_Modem_Event_from_pkt[];
        /* 1: fax, 2: local-modem, 3: remote-modem */
        stVoipCfg.enable = Fax_Modem_Event_from_pkt[stVoipCfg.ch_id];
        Fax_Modem_Event_from_pkt[stVoipCfg.ch_id] = 0;	// clean flag
#else

	/* 1: low-speed fax, 2: high-speed fax, 3: local-modem, 4: remote-modem */
	//stVoipCfg.enable = CED_routine(stVoipCfg.ch_id);
	switch (fax_modem_out(stVoipCfg.ch_id))
	{
		case FAX_CED:
			stVoipCfg.enable = 1;
	...
	if (fax_modem_det_mode[stVoipCfg.ch_id] == 1) //force Fax
	{
		if (stVoipCfg.enable > 0)
			stVoipCfg.enable = 1;
	}
	else if (fax_modem_det_mode[stVoipCfg.ch_id] == 2) //force modem
	{
		if ((stVoipCfg.enable == 1) || (stVoipCfg.enable == 2))
			stVoipCfg.enable = 0;
	}
#endif
...
int do_mgr_VOIP_MGR_FAX_MODEM_PASS_CFG( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	ethernetDspCheckL2RespPacket(cmd, &stVoipCfg, &dsp_id);
...
int do_mgr_VOIP_MGR_GET_EC_DEBUG( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	} else if (stVoipEcDebug.mode&0x2) {//stVoipEcDebug.mode = 1, do aec init
#ifdef AEC_TEST
		AEC_g168_set_TailLength(3, 16);
		AEC_g168_init(3, 1);
#endif
		ec_test_mode = 2;
	} else {//stVoipEcDebug.mode = 0, do ec init
		LEC_g168_set_TailLength(3, 16);
		LEC_g168_init( 3, LEC|LEC_NLP);
		ec_test_mode = 0;
	}
#endif
	return 0;
 ...
 */
#if ! defined (AUDIOCODES_VOIP)
int do_mgr_VOIP_MGR_DTMF_DET_THRESHOLD( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	TstVoipCfg stVoipCfg;
	int ret;
 ...
 */
int do_mgr_VOIP_MGR_DTMF_GET( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
...
int do_mgr_VOIP_MGR_SET_CID_DTMF_MODE( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	dtmf_cid_info[stCIDstr.ch_id].bBefore1stRing = (stCIDstr.cid_mode&0x08)>>3; 	/* set the before ring or after ring send cid */
...
#ifdef CONFIG_RTK_VOIP_ETHERNET_DSP_IS_HOST
	PRINT_R("Not support for Host, %s\n", __FUNCTION__);
#else
