...
#endif // CONFIG_RTK_VOIP_ETHERNET_DSP
...
int do_mgr_VOIP_MGR_DEBUG( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	ret = ethernetDspSentL2ControlPacketNoChannel(cmd, &stVoipValue, sizeof(TstVoipValue));
	...
	if (g_voip_debug == 11)
		g_force_codec = 8;
	else if (g_voip_debug == 23)
		g_force_codec = 4;
	else if (g_voip_debug == 22)
		g_force_codec = 9;
	else if (g_voip_debug == 29)
		g_force_codec = 18;
	else if (g_voip_debug == 100)
		g_force_codec = 35;
	else if (g_voip_debug == 38)
		g_force_codec = 110;
	
	//else if (g_voip_debug == 110)
	//	disablePLC = 0;
...
int do_mgr_VOIP_MGR_SET_GETDATA_MODE( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	ret = ethernetDspSentL2ControlPacket(cmd, mgr_chid, &stVoipdataget_o, sizeof(TstVoipdataget_o));
	
	// Ckeck Response Packet (need for copy_to_user)
	unsigned short dsp_id;
	ethernetDspCheckL2RespPacket(cmd, &stVoipdataget_o, &dsp_id);
