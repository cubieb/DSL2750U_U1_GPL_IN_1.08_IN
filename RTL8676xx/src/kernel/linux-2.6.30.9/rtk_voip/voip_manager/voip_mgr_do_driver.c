...
#ifdef FXO_CALLER_ID_DET
int fsk_cid_valid[MAX_VOIP_CH_NUM] = {0};
int dtmf_cid_valid[MAX_VOIP_CH_NUM] = {0};
#endif
...
int gRingCadOff[VOIP_CH_NUM] = {[0 ... VOIP_CH_NUM-1] = 4000 };
unsigned char ioctrl_ring_set[MAX_VOIP_CH_NUM] = {0};	// dch
...
volatile unsigned int sip_registed[MAX_VOIP_CH_NUM]={0};     //register ok:1, no register:0
volatile unsigned int daa_hook_status[MAX_VOIP_CH_NUM]={0};  //off-hook:1 ,on-hook:0
volatile unsigned int slic_hook_status[MAX_VOIP_CH_NUM]={0}; //off-hook:1 ,on-hook:0 ,flash-hook:2
volatile unsigned int fxs_ringing[MAX_VOIP_CH_NUM]={0};                      //no ring:0 ,voip incoming ring:1
...
extern char relay_2_PSTN_flag[MAX_VOIP_CH_NUM]; /* 1: relay to PSTN , 0: relay to SLIC */
#elif defined CONFIG_RTK_VOIP_DRIVERS_DAA_SUPPORT
unsigned int pstn_ringing[MAX_VOIP_CH_NUM] = {0};
#endif
#endif
...
/**
 * @ingroup VOIP_DRIVER_SLIC
 * @brief Polling SLIC hook status  
 * @see VOIP_MGR_SLIC_HOOK TstVoipSlicHook 
 */
int do_mgr_VOIP_MGR_SLIC_HOOK( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
#ifdef CONFIG_RTK_VOIP_ETHERNET_DSP_IS_DSP
	...
	switch (stVoipSlicHook.hook_status)
	{
...
#ifndef CONFIG_RTK_VOIP_ETHERNET_DSP_IS_HOST
#ifdef SW_DTMF_CID
			cid_dtmf_gen_init(stVoipSlicHook.ch_id);
#endif
#endif
#endif
			break;
...
#ifdef CONFIG_RTK_VOIP_LED
			daa_hook_status[stVoipSlicHook.ch_id] = 0;
#endif
			break;
...
#ifdef CONFIG_RTK_VOIP_LED
			daa_hook_status[stVoipSlicHook.ch_id] = 1;
#endif
			break;
		...
		case FXO_CURRENT_DROP:
			PRINT_MSG("FXO_CURRENT_DROP(%d)\n", stVoipSlicHook.ch_id);
			break;
		
		case OUTBAND_FLASH_EVENT:
...
#endif
#endif
	return COPY_TO_USER(user, &stVoipSlicHook, sizeof(TstVoipSlicHook), cmd, seq_no);
#endif // CONFIG_RTK_VOIP_ETHERNET_DSP_IS_DSP
	return 0;
}
...
#endif
	PRINT_MSG("Set SLIC%d Tx Gain = %ddB\n", stVoipValue.ch_id, stVoipValue.value-6);
#else
...
#endif
	PRINT_MSG("Set SLIC%d Rx Gain = %ddB\n", stVoipValue.ch_id, stVoipValue.value-6);
#else
	return NO_COPY_TO_USER( cmd, seq_no );
#endif
	return 0;
...
/**
 * @ingroup VOIP_DRIVER_SLIC
 * @brief Flush hook and dtmf fifo 
 * @param TstVoipValue.ch_id Channel ID
 * @see VOIP_MGR_FLUSH_FIFO TstVoipValue 
 */
int do_mgr_VOIP_MGR_FLUSH_FIFO( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	extern void flush_hook_fifo(uint32 ch_id);
	extern void flush_dtmf_fifo(uint32 ch_id);
...
#else
	save_flags(flags); cli();
	flush_hook_fifo(stVoipValue.ch_id);
	flush_dtmf_fifo(stVoipValue.ch_id);
	restore_flags(flags);
#endif
	return 0;
}
...
int do_mgr_VOIP_MGR_HOOK_FIFO_IN( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	// Host auto forward 
#else
	if ((Is_DAA_Channel(stVoipCfg.ch_id) == 1) && (stVoipCfg.cfg== FXO_BUSY_TONE))
	{
	...
	hook_in(stVoipCfg.ch_id, stVoipCfg.cfg);//write hook event to hook FIFO.
#endif
#else
	return NO_COPY_TO_USER( cmd, seq_no );
#endif
...
int do_mgr_VOIP_MGR_FXO_OFF_HOOK( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	// Host auto forward 
#else
	flush_dtmf_fifo(stVoipCfg.ch_id);//To avoid error events from other phone of the same PSTN line
...
int do_mgr_VOIP_MGR_SET_DSCP_PRIORITY( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	extern int32 rtl8651_reset_dscp_priority();
	extern int voip_qos;
	int value;
	int dscp, priority;
...
{
#if !defined(CONFIG_RTK_VOIP_DRIVERS_PCM89xxC) && !defined (CONFIG_RTK_VOIP_DRIVERS_PCM8672)
...
/**
 * @ingroup VOIP_DRIVER_DECT
 * @brief Retrieve detected DECT button event 
 * @param [out] TstVoipValue.value A DECT button event 
 * @see VOIP_MGR_DECT_BUTTON_GET TstVoipValue 
 */
int do_mgr_VOIP_MGR_DECT_BUTTON_GET( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
#ifdef CONFIG_RTK_VOIP_DRIVERS_ATA_DECT
#ifdef CONFIG_RTK_VOIP_DECT_DSPG_SUPPORT
	extern int dect_out(void);
	...
	copy_from_user(&stVoipValue, (TstVoipValue *)user, sizeof(TstVoipValue));
	save_flags(flags); cli();
	stVoipValue.value = dect_out();
	restore_flags(flags);
	//PRINT_MSG("Get dect button event:0x%x ", stVoipValue.value);
