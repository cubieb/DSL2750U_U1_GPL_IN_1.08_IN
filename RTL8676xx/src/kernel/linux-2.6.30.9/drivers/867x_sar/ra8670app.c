...
void FreeVcBuff(sar_private *cp, int8 ch_no){
	...
	for(i=0;i<SAR_RX_DESC_NUM;i++){
		...
		if(ch_no==OAM_CH_NO){
			...
			#ifdef CONFIG_RTL8676  // andrew, free memory on OAM ch1 as well
...
// Mason Yu. Recieve packet from OAM ch1(not cho) for AutoHunt
#ifdef CONFIG_RTL8676   //8672  for ro1 ch, allocate SAR_RX_Buffer_Size length buffer for normal packet			
			ptr = (UINT32*)kmalloc(SAR_OAM_Buffer_Size, GFP_KERNEL);
			if(ptr==(u32)NULL)		goto free_desc;			
			...
			if ((pLBInfo = OAMF5FindLBInfo(LBTag, pOAMF5)) != NULL) {
				...
				if (pLBInfo->status == FAULT_LB_WAITING) {
					...
					if (pLBInfo == &pOAMF5->OAMF5LBList[0])
					{
						...
						pLBInfo->count++;
						pLBInfo->rtt = OAMF5GetTimeStamp() - pLBInfo->timestamp;// timestamp
						OAMF5StopTimer(&(pLBInfo->timer));// stop timer
						memcpy(pLBInfo[i].locationID, &pOamCell[OAM_FORMAT_LB_LLID],  OAM_LB_LLID_SIZE);// save LLID
						pLBInfo->status = FAULT_LB_STOP;// return to idle state
						OAMF5ChangeState(pOAMF5);
					}
					// Should tell TMN that LB successes.// TODO ... 
