...
#include "fastskb.h"
  ...
  #endif
#endif
unsigned int sar_rx_bh_state = 0;
#ifdef CONFIG_IGMP_FASTPATH
#include <linux/in.h>
#include <linux/ip.h>
#include <linux/netfilter.h>
#include <net/arp.h>
#include <linux/if_pppox.h>
atomic_t igmp_fastpath_enable;
extern atomic_t ipMulticastFastFwd;
unsigned int igmp_fastpath_count = 0;
...
#if 0
#define LYL_DEBUG printk

static void lyl_skb_debug(const struct sk_buff *skb)
{
#define NUM2PRINT 50
	...
	printk(KERN_NOTICE "skb->data: %s\n", buf);
}
#else
#define LYL_DEBUG(format, args...)
#endif
...
static unsigned int autohunt_pkt_type = PT_START;
#define VM_DHCP_OFFSET	8
#define VM_PPPOE_OFFSET	8
#define VM_PPPOA_OFFSET	4
...
// Mason Yu. 20130304
// PPPoED Active Discovery Initiation on LLC
char PPPoED[]={
	0xaa, 0xaa, 0x03, 0x00, 0x80, 0xc2, 0x00, 0x07, 0x00, 0x00,
	0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x00, 0xe0, 0x4c, 0x04, 0x05, 0x07, 0x88, 0x63, 0x11, 0x09,
	0x00, 0x00, 0x00, 0x1c, 0x01, 0x01, 0x00, 0x00, 0x01, 0x03, 0x00, 0x10, 0x00, 0xe0, 0x4c, 0x04,
	0x05, 0x06, 0x70, 0x70, 0x70, 0x6f, 0x65, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x08, 0x00, 0x00, 0x00, 0x08, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00
};

// Mason Yu. 20130304
// PPPoA(LCP) on LLC
#define LCP_MN_OFFSET 12
char PPPoA_LCP[]={
	0xfe, 0xfe, 0x03, 0xcf,
	0xc0, 0x21, 0x01, 0x01, 0x00, 0x0a, 0x05, 0x06, 0x6b, 0x8b, 0x45, 0x70
};
...
//	printk(KERN_INFO "\nprocfile_write received string %s(count %d) \n", proc_buffer, count);
	...
	if(proc_buffer[0] == '1') {
		...
		switch(autohunt_pkt_type)
		{
		case PT_DHCP_LLC:
			autohunt(vcc, test_packet2, sizeof(test_packet2));
			printk("Send DHCP LLC packet\n");
			break;
		case PT_DHCP_VCMUX:
			autohunt(vcc, &test_packet2[VM_DHCP_OFFSET], sizeof(test_packet2)-VM_DHCP_OFFSET );
			printk("Send DHCP VCMUX packet\n");
			break;
		case PT_PPPOED_LLC:
			autohunt(vcc, PPPoED, sizeof(PPPoED));
			printk("Send PPPoED LLC packet\n");
			break;
		case PT_PPPOED_VCMUX:
			autohunt(vcc, &PPPoED[VM_PPPOE_OFFSET], sizeof(PPPoED)-VM_PPPOE_OFFSET );
			printk("Send PPPoED VCMUX packet\n");
			break;
		case PT_PPPOA_LCP_LLC:
			{
				unsigned int *num=(unsigned int*)&PPPoA_LCP[LCP_MN_OFFSET];
				(*num)=(*num)+jiffies+1;
				autohunt(vcc, PPPoA_LCP, sizeof(PPPoA_LCP));
				printk("Send PPPOA_LLC packet, *num=0x%08x\n", *num );
			}
			break;
		case PT_PPPOA_LCP_VCMUX:
			{
				unsigned int *num=(unsigned int*)&PPPoA_LCP[LCP_MN_OFFSET];
				(*num)=(*num)+jiffies+1;
				autohunt(vcc, &PPPoA_LCP[VM_PPPOA_OFFSET], sizeof(PPPoA_LCP)-VM_PPPOA_OFFSET );
				printk("Send PPPOA_VCMUX packet, *num=0x%08x\n", *num );
			}
			break;
		}
		autohunt_pkt_type++;
		if(autohunt_pkt_type>=PT_END)
			autohunt_pkt_type=PT_START;		
...
void foundVC(uint8 vpi,uint16 vci,int mode)
{
	...
	found_pvc = 1;
	SetVpiVci(sar_dev,vpi,vci,0);
	sar_dev->vcc[0].framing=mode;
	//Enable_rx_ch(sar_dev,0);
//	for (i = 0; i<Enable_VC_CNT; i++)
//		Enable_rx_ch(sar_dev,i);
	Enable_Drop_NonOAM(sar_dev,OAM_CH_NO);
#ifdef CONFIG_RTL8676
	Disable_rx_ch(sar_dev, OAM_CH_NO+1);
#endif
	if(mode==0) 
...
void searchOamAutoPvc(sar_private *cp, uint8 ch_no)
{
	...
	{	
		...
		//if (isPVCActiveExist(rx_vpi, rx_vci)) 
		{  //pvc match, check packet type	
#if 0		
		for(i=0;i<0x30;i++){
			if(i%0x10==0)
				printk("\n");
		...
		*/
		// bridged protocol
			...
			else if( outband_cell[4]==0x00 && outband_cell[5]==0x00 &&
				((outband_cell[18]==0x08 && (outband_cell[19]==0x00 || outband_cell[19]==0x06)) ||
				 (outband_cell[18]==0x88 && outband_cell[19]==0x63) ||
				 (outband_cell[18]==0x80 && outband_cell[19]==0x35)))
			{
				foundVC(rx_vpi, rx_vci,1);//VCMuxHdr802_3
			}else if(outband_cell[4]==0xfe && outband_cell[5]==0xfe && 
					outband_cell[6]==0x03 && outband_cell[7]==0xcf &&
					outband_cell[8]==0xc0 && outband_cell[9]==0x21 &&
					(outband_cell[10]==0x01 || outband_cell[10]==0x02) ) 
			{
				foundVC(rx_vpi, rx_vci,0);
			}else if(outband_cell[4]==0xc0 && outband_cell[5]==0x21 && 
						(outband_cell[6]==0x01 || outband_cell[6]==0x02) ) 
			{				
				foundVC(rx_vpi, rx_vci,1);
			}
		}// is pvc active exist 	
		RVDesc=NULL;
	} //->RODesc[j].STS&OWN)
...
static int sar_igmp_fastpath(sar_private *cp, int ch_no, struct sk_buff *skb)
{
	...
	int i = 0;
	int hh_len = 0;
	struct iphdr iphdr, *iph = NULL;
	struct ethhdr eth_head, *eth_hdr = NULL;
	struct net_device *dst_dev = NULL;
	char dst_mac[6];
	struct pppoe_hdr2  pppdr, *ppphd = NULL;
	struct udphdr udpdr, *udphd = NULL;
	
	if (atomic_read(&igmp_fastpath_enable) == 0) 
		goto pass;
	//lyl_skb_debug(skb);
	switch (cp->vcc[ch_no].rfc)
	{
		case RFC1483_BRIDGED:
			...
			LYL_DEBUG("it is a RFC1483_BRIDGED packet, with encap_mode=%d\n", encap_mode);
			eth_hdr = skb_header_pointer(skb, cut_len, sizeof(eth_head), &eth_head);
			if (eth_hdr == NULL)
				goto pass;
			if (eth_hdr->h_proto == htons(ETH_P_PPP_SES)) {
			/*如果是桥模式下的pppoe组播，则跳过不往下走 否则skb_make_writable函数check不过，影响包转发效率*/
            if(atomic_read(&ipMulticastFastFwd) == 0)
                goto  pass;
				LYL_DEBUG("it is a pppoe session packet\n");
                iphdr_offset = cut_len + MAC_HEAD_LEN;
				ppphd = skb_header_pointer(skb, iphdr_offset, sizeof(pppdr), &pppdr);
				// pppoe data (IP protocol)
                if(ppphd->ppp_protol != htons(0x0021))
				    goto pass;
				
				packet_type = PPPOE_MODE;
				iphdr_offset = cut_len + MAC_HEAD_LEN + PPP_HEAD_LEN;
			} else if (eth_hdr->h_proto == htons(ETH_P_IP)) {
				LYL_DEBUG("it is a mer packet\n");
				packet_type = MER_MODE;
				iphdr_offset = cut_len + MAC_HEAD_LEN;
			} else {
				LYL_DEBUG("a packet we did not know, proto=%x\n", eth_hdr->h_proto);
				goto pass;
			}
			break;
		case RFC1483_ROUTED:
			encap_mode = cp->vcc[ch_no].rfc * 2 + cp->vcc[ch_no].framing;
			cut_len = FrameHeaderSize_1483[encap_mode];
			iphdr_offset = cut_len;
			packet_type = IPOA_MODE;
			LYL_DEBUG("it is a RFC1483_ROUTED packet, with encap_mode=%d\n", encap_mode);
			break;
		case RFC1577:
			cut_len = FrameHeaderSize_1577[0];
			iphdr_offset = cut_len;
			packet_type = RFC1577_MODE;
			LYL_DEBUG("it is a RFC1577 packet\n");
			break;
		case RFC2364:
			goto pass;
			cut_len = FrameHeaderSize_2364[cp->vcc[ch_no].framing];
			iphdr_offset = cut_len + PPP_HEAD_LEN;
			packet_type = PPPOA_MODE;
			LYL_DEBUG("it is a RFC2364 packet\n");
			break;
		default:
			goto pass;
	}
	iph = skb_header_pointer(skb, iphdr_offset, sizeof(iphdr), &iphdr);
	if (iph == NULL)
		goto pass;
	LYL_DEBUG("the srcip=%u.%u.%u.%u, dstip=%u.%u.%u.%u\n", NIPQUAD(iph->saddr), NIPQUAD(iph->daddr));
	if (iph->protocol == IPPROTO_IGMP)
		goto pass;
	if (!MULTICAST(iph->daddr)) 
		goto pass;

	//RIP protocol should pass
	udphd = skb_header_pointer(skb, iphdr_offset+sizeof(iphdr), sizeof(udpdr), &udpdr);
    if(iph->daddr == htonl(RIP_PROTO_ADDRESS) && (udphd && (udphd->dest == htons(RIP_PROTO_PORT))))
	{
        goto pass;
	}
	
	dst_dev = dev_get_by_name(&init_net, "br0");
	if (dst_dev == NULL)
		goto pass;
	
	switch (packet_type) {
		case PPPOE_MODE:
			if (!skb_make_writable(skb, iphdr_offset))
				goto pass_put_dev;
			skb_pull(skb, cut_len + PPP_HEAD_LEN);
			arp_mc_map(iph->daddr, dst_mac, dst_dev, 0);
			//safe to copy src mac 
			for (i = 6; i < 12; i++)
				*(skb->data + i) = *(eth_hdr->h_source + i - 6);
			memcpy(skb->data , dst_mac, 6);
			((u16 *)skb->data)[6] = htonl(0x0800);
			LYL_DEBUG("make the new packet\n");
			//lyl_skb_debug(skb);
			break;
		case MER_MODE:
			if (!skb_make_writable(skb, iphdr_offset))
				goto pass_put_dev;
			skb_pull(skb, cut_len);
			break;
		case IPOA_MODE:
			hh_len = LL_RESERVED_SPACE(dst_dev);
			if (skb_headroom(skb) < hh_len) {
				struct sk_buff *skb2;
				skb2 = skb_realloc_headroom(skb, LL_RESERVED_SPACE(dst_dev));
				if (skb2 == NULL) {
					kfree_skb(skb);
					goto pass_put_dev;
				}
			...
			skb_pull(skb, cut_len);
			skb_push(skb, MAC_HEAD_LEN);
			arp_mc_map(iph->daddr, dst_mac, dst_dev, 0);
			memcpy(skb->data, dst_mac, 6);
			memcpy(skb->data + 6, dst_dev->dev_addr, 6);
			((u16 *)skb->data)[6] = htonl(0x0800);
			break;
	...
	igmp_fastpath_count++;
	// this is essential, becasue, when when pass this packet to device, the skb->nh is initialized, we do this
	//in case some body refrence the skb->nh directly, such as mc_forward
	//skb->mac.raw = skb->data;
	skb->mac_header = skb->data;
	skb->network_header = skb->data + MAC_HEAD_LEN;
	dst_dev->hard_start_xmit(skb, dst_dev);
	dev_put(dst_dev);
	return 1;
pass_put_dev:
	dev_put(dst_dev);
	...
	if ((sar_mirror_flag & dir) || (atm_vcc->mirror_flag & dir))
	{
		switch (cp->vcc[ch_no].rfc)
		{
			case RFC1483_BRIDGED:
				...
				if (net_dev && NULL != net_dev->br_port)
				{
...
#ifdef CONFIG_IGMP_FASTPATH
		if (sar_igmp_fastpath(cp, ch_no, skb) == 1) {
			goto NEXT_DESC;
		}
#endif
			...
			skb->dev = cp->dev;
			// VC layer stats
			//atomic_inc(&cp->vcc[ch_no].stat.rx_netif_cnt);
			//cp->vcc[ch_no].stat.rx_netif_cnt++;
			//atomic_add(total_len, &cp->vcc[ch_no].stat.rx_byte_cnt);
			cp->vcc[ch_no].stat.rx_byte_cnt+=total_len;
			//cp->vcc[ch_no].stat.rx_cell_cnt += ((total_len%48==0) ? (total_len/48):(total_len/48+1));
			//atomic_inc(&atm_vcc->stats->rx);
			skb->stamp = xtime;
//#ifdef CONFIG_EXT_SWITCH
			
			// Kaohj, remove vlan tag
			encap_mode = cp->vcc[ch_no].rfc*2+cp->vcc[ch_no].framing;
			// 1483_br_llc && vlan tag
			//if (encap_mode==0 && *(unsigned short *)(skb->data+FrameHeaderSize_1483[encap_mode]+12) == 0x8100) {
			if (cp->vcc[ch_no].rfc == RFC1483_BRIDGED && *(unsigned short *)(skb->data+FrameHeaderSize_1483[encap_mode]+12) == 0x8100) {
				int k, copy_len;
				skb->vlan_passthrough=*(unsigned int *)(skb->data+FrameHeaderSize_1483[encap_mode]+12) ;
				copy_len = FrameHeaderSize_1483[encap_mode]+12;
				for (k=(copy_len-1); k>=0; k--)
					skb->data[k+4]=skb->data[k];
				skb_pull(skb, 4);
			}
//#endif
...
//1__SWAP
__IRAM_SYS_MIDDLE int sar_rx_bh(sar_private *cp){
	int	ch_no;
	int ret = 0;
...
//scout(']');		
	...
	//enable_irq(BSP_SAR_IRQ);	//shlee 2.6
	//Enable_IERBF(sar_dev);	
...
__IRAM_SYS_MIDDLE void _sar_rx_bh(sar_private *cp){
	while(sar_rx_bh(cp))
		{
		}
}
...
//zhh, for ppp lcp reserve
#define PPP_LCP_RESERVE 1
...
#ifdef PPP_LCP_RESERVE
	unsigned short * kppptype;
	unsigned short * klcptype;
#endif
...
#ifdef PPP_LCP_RESERVE
	if (avail_desc <= 1)//zhh : leave 1 desc for ppp lcp
#else
	if (avail_desc == 0)
#endif
	{
...
#ifdef PPP_LCP_RESERVE
	if(avail_desc<=1){//zhh : leave 1 desc for ppp lcp
	    kppptype = (unsigned short *)&skb->data[22];
	    klcptype = (unsigned short *)&skb->data[30];
	    if(*kppptype == 0x8864 && *klcptype == 0xc021){//8864 => ppp, c021=>ppp lcp
	    } else {
		goto DropPacket;
	    }
	}
...
//QCLKSEL has been removed 	Set_QoS_Ext(cp, 0);		/* external */
...
#ifdef CONFIG_RTL8676_ATM_QoS //enable 8672 new function
	Enable_IS8671G_1(cp);	
	Enable_IS8671G_0(cp);	
	Enable_IS8672_10(cp);	
#endif
	...
	cp->tasklets.func=(void (*)(unsigned long))_sar_rx_bh;
...
static int sar_open (struct atm_vcc *vcc)
{
	...
	if (!test_bit(ATM_VF_PARTIAL,&vcc->flags))
		//RTATM_VCC(vcc) = NULL;
		(vcc)->dev_data = NULL;
	ATM_OAM_DEBUG("fixme atm_find_ci in sar_open!\n");
	//error = atm_find_ci(vcc,&vpi,&vci);
	if (error)
		return error;
	vcc->vpi = vpi;
	...
	if (vcc->qos.aal != ATM_AAL5)
		return -EINVAL; /* @@@ AAL0 */
	ATM_OAM_DEBUG(DEV_LABEL "(itf %d): open %d.%d\n",vcc->dev->number,vcc->vpi,
	    vcc->vci);
	if (!test_bit(ATM_VF_PARTIAL,&vcc->flags)) {
#ifdef REMOTE_MANAGEMENT_ENABLE		
        if ((vpi==0)&&(vci==16)) {    //Remote Management Channel
...
static int do_read_igmp_fastpath(char *page, char **start, off_t off,
				 int count, int *eof, void *data)
{
	int len = 0;
	len += sprintf(page + len, "the igmp_fastpath=%d\n", atomic_read(&igmp_fastpath_enable));
	len += sprintf(page + len, "the gmp_fastpath_count = %u\n", igmp_fastpath_count);
    len += sprintf(page + len, "rbf count = %u\n", sar_rx_bh_state); 
	sar_rx_bh_state = 0;
...
static int do_write_igmp_fastpath(struct file *file, const char *buffer, 
				unsigned long count, void *data)
{
	...
	if (memcmp(buf, "query", 5) == 0) {
		printk("igmp_fastpath_count = %u\n", igmp_fastpath_count);
		printk("igmp_fastpath_enable = %d\n", atomic_read(&igmp_fastpath_enable));
		return count;
	}
	if (memcmp(buf, "clear", 5) == 0) {
		igmp_fastpath_count = 0;
		return count;
	}
	if (buf[0] == '1')
		atomic_set(&igmp_fastpath_enable, 1);
	else if (buf[0] == '0') 
		atomic_set(&igmp_fastpath_enable, 0);
	else
		printk("the command is not support\n");
	return count;
}
#endif
...
#ifdef CONFIG_IGMP_FASTPATH
    {
    	struct proc_dir_entry *igmp_fastpath;
    	igmp_fastpath = create_proc_entry("igmp_fastpath", 0, NULL);
    	if (igmp_fastpath == NULL)
    		return -ENOMEM;
    	igmp_fastpath->read_proc = do_read_igmp_fastpath;
    	igmp_fastpath->write_proc = do_write_igmp_fastpath;
    	//igmp_fastpath->owner = THIS_MODULE;
    	atomic_set(&igmp_fastpath_enable, 0);
	}
#endif
...
#if defined(CONFIG_LEDS_TBS) || defined(CONFIG_LEDS_TBS_MODULE)
/* Detect traffic for LINK(ACT) LED blinking */
static unsigned char LastState;

void ADSL_state(unsigned char state)
{
	if(state != LastState)
		{
		LastState = state;
		switch(LastState)
			...
			case C_AMSW_ACTIVATING:
				tbs_led_trigger_set(led_dsl, led_flash_2hz_trig);
				break;
				
			case C_AMSW_INITIALIZING:
				tbs_led_trigger_set(led_dsl, led_flash_4hz_trig);
				break;
				
			case C_AMSW_SHOWTIME_L0:
				tbs_led_trigger_set(led_dsl, led_on_trig);
				break;
