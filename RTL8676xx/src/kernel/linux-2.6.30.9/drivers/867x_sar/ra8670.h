...
#define 	SAR_RX_DESC_NUM	128//64 SAR_RX_NUM/8(pvc)
...
#define	SAR_RX_NUM				1024  //pvc total buffer size (SAR_RX_DESC_NUM*8 = SAR_RX_NUM)
//1/13/06' hrchen, for desc number re-assign
#define	SAR_RX_DESC_LOW_LIMIT	8		//min desc number for a PVC
#define	SAR_RX_DESC_HI_LIMIT	(SAR_RX_DESC_NUM - 2)		//max desc number for a PVC, never >=64
...
#define	RDATimer	1		//delay = RDATimer*500us
...
#ifdef CONFIG_RTL8676_SAR_DEBUG
#define	DRV_ENTER	printk(KERN_DEBUG "---->Enter %s: %d  \n",__FUNCTION__,__LINE__)
#define	DRV_LEAVE	printk(KERN_DEBUG "<----Leave %s: %d  \n",__FUNCTION__,__LINE__)
#define ATM_OAM_TRACE(fmt, arg...)      printk(fmt, arg...)
#define ATM_OAM_DEBUG(fmt, arg...)      printk(fmt, arg...)
#else
#define ATM_OAM_TRACE(fmt, arg...)      do{} while(0)
#define ATM_OAM_DEBUG(fmt, arg...)      do{} while(0)
#define	DRV_ENTER
#define	DRV_LEAVE
#endif
