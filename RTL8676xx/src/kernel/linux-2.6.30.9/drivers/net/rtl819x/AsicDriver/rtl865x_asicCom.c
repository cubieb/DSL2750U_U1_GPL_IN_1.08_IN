...
int32 rtl8651_setAsicVlan(uint16 vid, rtl865x_tblAsicDrv_vlanParam_t *vlanp)
{
	...
	rtl865xc_tblAsic_vlanTable_t tmp_entry;
	int i, select=-1;
#endif
	...
	{
		_rtl8651_readAsicEntry(TYPE_VLAN_TABLE, i, &tmp_entry);

		if (((tmp_entry.extMemberPort | tmp_entry.memberPort) == 0) && (-1 == select)) {
			select = i;
			continue;
		}
		
		if (tmp_entry.vid == vid)
			break;
	}
	if (i < RTL865XC_VLAN_NUMBER)
		vid = i;
	else if (select != -1)
		vid = select;
	else {
		//printk("error, vlan asic table is full.\n");
		return FAILED;
	}

	//printk("_rtl8651_forceAddAsicEntry add vid %d member 0x%x untag 0x%x to idx %d\n", entry.vid, entry.memberPort, entry.egressUntag, vid);
#endif
...
int32 rtl8651_delAsicVlan(uint16 vid) {
	...
	memset(&entry, 0, sizeof(entry));
	return _rtl8651_forceAddAsicEntry(TYPE_VLAN_TABLE, i, &entry);
#endif
...
int32 rtl8651_setAsicPvid(uint32 port, uint32 pvid)
{
	...
	//printk("=========>");
	//printk("%s set port %d pvid %d\n", __func__, port, pvid);
	if(port>=RTL8651_AGGREGATOR_NUMBER || pvid>=4096)
		return FAILED;;
	offset=(port*2)&(~0x3);
...
int32 rtl8651_setAsicPortDefaultPriority(uint32 port, int acl_priority)
{
	...
	if ((acl_priority < 0) || (acl_priority >= TOTAL_VLAN_PRIORITY_NUM)) 
		return FAILED;	
	...
	if((port&0x1))
	{
		regValue=  ((acl_priority &0x3) <<28) | (regValue&~0x70000000);
	}
	else
	{	
		regValue =  (acl_priority &0x3 << 12 ) | (regValue &~0x7000);
	}
	WRITE_MEM32(PVCR0+offset,regValue);
	return SUCCESS;
}
...
int32 rtl865xC_lockSWCore(void)
{
	...
	if (_rtl865xC_lockTLUCounter == 0)
	{
		...
		if(RTL865X_PHY6_DSP_BUG)
			_rtl865xC_lockTLUPHYREG[6] = READ_MEM32(PCRP6);
		...
		if(RTL865X_PHY6_DSP_BUG)
			WRITE_MEM32(PCRP6, ((READ_MEM32(PCRP6))&(~EnablePHYIf)) );
	}
#endif
...
unsigned int read_gpio_hw_setting(void)
{
    ...
    unsigned int tmp;
    int b1,b2,b3,b4;
    /* set(GP4 GP3 GP2 GP1)=(H1 H0 F3 F6)= gpio */
    REG32(PEFGHCNR_REG) = REG32(PEFGHCNR_REG)&(~(0x03<<24))&(~(0x48<<8));
    /* change to GPIO mode */
    REG32(PEFGHPTYPE_REG)=REG32(PEFGHPTYPE_REG)&(~(0x03<<24)) & (~(0x48<<8)); 
    /* 0 input, 1 output, set input */
    REG32(PEFGHDIR_REG) = REG32(PEFGHDIR_REG)&(~(0x03<<24))&(~(0x48<<8));
