...
void Setting_RTL8196C_PHY(void)
{
	...
	*/
	...
	/*
	  #=========patch for eee============================
	  #1. page4、reg24：lpi_rx_ti_timer_cnt change to f3     
	  phywb all 4 24 7-0 0xf3

	  #2. page4、reg16：rg_txqt_ps_sel change to 1    
	  phywb all 4 16 3 1  
	*/
...
/* Turn on all ethernet leds *///by xiachaoren 2011-11-28

    hw_val = (BSP_LEDS0_GPH4 |              //0:gpio H4, 1:LEDS0
    ...
    if((REG32(BSP_MISC_PINMUX) & hw_val) != hw_val)
    	{
    	REG32(BSP_MISC_PINMUX) |= hw_val;
		}
...
/*	WRITE_MEM32(MACCR,READ_MEM32(MACCR)&~(EN_FX_P4 | EN_FX_P3 | EN_FX_P2 | EN_FX_P1 | EN_FX_P0));//Disable FX mode (UTP mode)*/
	...
	WRITE_MEM32(SWTCR0,READ_MEM32(SWTCR0)|EnUkVIDtoCPU);//Enable trap unknown tagged VID (VLAN table lookup miss) packet to CPU.
...
//	#define PIN_MUX_SEL 0xb8000040
...
#if defined(CONFIG_RTL_8676HWNAT)
	if (rtl8651_tblAsicDrvPara.externalPHYProperty & RTL8676_TBLASIC_EXTPHYPROPERTY_PORT0_RTL8367B) {
		extern int RL6000_vlan_set(void);
	...
	//move RLE0412 patch here
	{
		...
		for(i=0;i<MAX_PORT_NUMBER;i++)
		{
			if (0 == i && rtl8651_tblAsicDrvPara.externalPHYProperty)
				continue;
			rtl8651_getAsicEthernetPHYReg(i, 22, &rdata);
			rtl8651_setAsicEthernetPHYReg(i, 22, 0x5bd5);
	
			//select page 1
			rtl8651_setAsicEthernetPHYReg(i, 31, 1);
			//ethernet 100Mhz output voltage to 1.0(v)
			rtl8651_setAsicEthernetPHYReg(i, 18, 0x9004);
			//adjust AOI waveform
			rtl8651_setAsicEthernetPHYReg(i, 19, 0x5400);
			//return to page 0
			rtl8651_setAsicEthernetPHYReg(i, 31, 0);
		}
...
int32 rtl8651_EnablePortRemark( enum PORTID port)
{	
	...
	/* enable L3/L4 remark */
	{		
		WRITE_MEM32( RLRC, RMLC_8021P_L4|RMLC_8021P_L3|RMLC_DSCP_L4|RMLC_DSCP_L3 );
	}

	return SUCCESS;
}

int32 rtl8651_DisablePortRemark( enum PORTID port)
{
