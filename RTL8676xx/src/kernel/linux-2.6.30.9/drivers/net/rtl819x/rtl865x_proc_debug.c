...
static struct proc_dir_entry *vlan_entry,*netif_entry,*l2_entry, *arp_entry,
		...
		*acl_entry,*acl_all_entry,*storm_control,*dump_trap2CPU_pkt,
#ifdef CONFIG_RTL_MULTI_ETH_WAN
		*sw_netif, *sw_l3,
#endif
		*sw_arp_entry,*sw_nexthop_entry,
...
extern void reset_rtl8367b_PortStat(void);
extern void show_rtl8367b_PortStat(unsigned int port);
#endif
...
static int32 vlan_write( struct file *filp, const char *buff,unsigned long len, void *data )
{
	...
	if (buff && !copy_from_user(tmpbuf, buff, len))
	{
		...
		{
			//ipv4_dump_all_dst_cache();
			//printk("\n");
			//hyking_print_busy_list();
			//printk("\n");
	#if defined(CONFIG_RTL_ETH_PRIV_SKB_DEBUG)
			unsigned long flags;
			...
			wlan_txRing_cnt = get_nic_buf_in_wireless_tx();
			rx_queue_cnt = get_buf_in_rx_skb_queue();
			poll_cnt = get_buf_in_poll();
			local_irq_restore(flags);
			
			printk("cpu completion cnt(%d)\nnic rxring cnt(%d)\nnic txring cnt(%d)\nwlan txring(%d)\nrx_queue_cnt(%d)\npoll_cnt(%d)\ntotal(%d)\n",cpu_queue_cnt,
				rxRing_cnt,txRing_cnt,wlan_txRing_cnt, rx_queue_cnt,poll_cnt,
				cpu_queue_cnt+rxRing_cnt+txRing_cnt+wlan_txRing_cnt+rx_queue_cnt+poll_cnt);
	#endif
...
#if defined(CONFIG_RTL_IPTABLES_RULE_2_ACL)
static int acl_mode_read(char *page, char **start, off_t off, 
		int count, int *eof, void *data)
{	  
      int len;	 

	  switch(rtl865x_get_acl_mode())
	  {
...
static int32 acl_mode_write( struct file *filp, const char *buff,unsigned long len, void *data )
{
	char 	tmpbuf[512];
	char		*strptr;	
	int32 retval = FAILED;
	if (buff && !copy_from_user(tmpbuf, buff, len))
	{
		...
		{
			retval = rtl865x_change_acl_mode(RTL865X_ACL_Mode_Always_Permit);
		}
		else if(strncmp(strptr, "trap",4) == 0)
		{
			retval = rtl865x_change_acl_mode(RTL865X_ACL_Mode_Always_Trap);
		}
		else if(strncmp(strptr, "normal",6) == 0)
		{
			retval = rtl865x_change_acl_mode(RTL865X_ACL_Mode_Normal);
		}
		#ifdef CONFIG_RTL8676_Dynamic_ACL
		else if(strncmp(strptr, "clean_L2",8) == 0)
		{
			retval = rtl865x_clean_filter_L2_permit();
		}
		else if(strncmp(strptr, "clean_L3",8) == 0)
		{
			retval = rtl865x_clean_filter_L34_permit();
		}
		#endif
		else
		{
...
#ifndef CONFIG_RTL8676_Dynamic_ACL
static int acl_mac_filter_mode_read(char *page, char **start, off_t off, 
		int count, int *eof, void *data)
{	  
      int len;	  
	  
	  switch(rtl865x_get_acl_mac_filter_mode())
	  {
...
static int32 acl_mac_filter_mode_write( struct file *filp, const char *buff,unsigned long len, void *data )
{
	char 	tmpbuf[512];
	char		*strptr;	
	int32 retval = FAILED;
	if (buff && !copy_from_user(tmpbuf, buff, len))
	{
		...
		{
			retval = rtl865x_change_acl_mac_filter_mode(RTL865X_ACL_MAC_FILTER_ALL_PERMIT);
		}
		else if(strncmp(strptr, "in_drop_out_permit",18) == 0)
		{
			retval = rtl865x_change_acl_mac_filter_mode(RTL865X_ACL_MAC_FILTER_IN_DROP_OUT_PERMIT);
		}
		else if(strncmp(strptr, "in_permit_out_drop",18) == 0)
		{
			retval = rtl865x_change_acl_mac_filter_mode(RTL865X_ACL_MAC_FILTER_IN_PERMIT_OUT_DROP);
		}
		else if(strncmp(strptr, "all_drop",8) == 0)
		{
			retval = rtl865x_change_acl_mac_filter_mode(RTL865X_ACL_MAC_FILTER_ALL_DROP);
		}
		else
		{
			goto errout;
	...
	for(i = 0; i < NETIF_SW_NUMBER; i++)
	{
		if (tbl[i].valid) {
			...
			if (tbl[i].enableRoute)
				len += sprintf(page+len, "ROUTE MODE.\n\n");
			else
				len += sprintf(page+len, "BRIDGE MODE.\n\n");
		}
	}
	...
	seq_printf(s, "%s\n", "ASIC ACL Table:");
		...
		for(acl_start=0; acl_start<128 ;acl_start++)	
		{
			...
			case RTL865X_ACL_SRCFILTER:
				...
				seq_printf(s, "\tsvidx: %d   svidxM: %x   sport: %d   ProtoType: %x\n",
	...
	again:
		...
		for(; acl_start<= acl_end;acl_start++)
		{
			...
			case RTL865X_ACL_SRCFILTER:
				...
				seq_printf(s, "\tsvidx: %d   svidxM: %x   sport: 0x%X   ProtoType: %x\n",
						asic_acl.srcFilterVlanIdx_, asic_acl.srcFilterVlanIdxMask_, asic_acl.srcFilterPort_,
						(asic_acl.srcFilterIgnoreL3L4_==TRUE? 2: (asic_acl.srcFilterIgnoreL4_ == 1? 1: 0))
						);
				seq_printf(s, "\tsip: %d.%d.%d.%d   sipM: %d.%d.%d.%d\n", (asic_acl.srcFilterIpAddr_>>24),
...
{			
	extern char outputQueue_Maindev[IFNAMSIZ];
	int i;	
	
	printk( "Main dev ouputQueue operating on: %s\n",strcmp(outputQueue_Maindev,"")?outputQueue_Maindev:"NULL");
...
int sw_arp_show(struct seq_file *s, void *v)
{
	...
	#ifdef HPP_Hack_ARP_SYNC
	synARP_ShowWatchList();
	#endif
	return 0;
}
	...
	for(entryIdx = 0; entryIdx < NXTHOP_ENTRY_NUM; entryIdx++)
	{
		if(NexthopTables[entryIdx].valid != 0)
		{
			...
			printk("attr:%s   netif:%s   srcIp:%u.%u.%u.%u  refCnt:%d\n"
				,(entry->flag== NEXTHOP_L3)?"For Routing":"For ACL",entry->dstNetif->name,NIPQUAD(entry->srcIp),entry->refCnt);
...
static int32 sw_l3_read( char *page, char **start, off_t off, int count, int *eof, void *data )
{
	...
	while (head)
	{
		...
		len += sprintf(page + len,"\t[%d]  %d.%d.%d.%d/%d process(%s)  is_internal:%d  netif:%s  addr:0x%x\n", idx, (head->ipAddr>>24),
			((head->ipAddr&0x00ff0000)>>16), ((head->ipAddr&0x0000ff00)>>8), (head->ipAddr&0xff), 
			mask, str[head->process], head->is_internal, head->dstNetif->name,(unsigned int)head);
...
//			len += sprintf(page+len, "CPUPort ");
		...
		len += sprintf(page+len, "Speed=%4dM | ", data0==PortStatusLinkSpeed100M? 100 :
			(data0==PortStatusLinkSpeed1000M ? 1000 :
				(data0==PortStatusLinkSpeed10M? 10 : 0)));
		data0 = regData & PortStatusNWayEnable;
		len += sprintf(page+len, "NWayMode=%s | ", data0?"Enabled":"Disabled");
		data0 = regData & PortStatusRXPAUSE;
		len += sprintf(page+len, "RXPause=%s | ", data0?"Enabled":"Disabled");
		data0 = regData & PortStatusTXPAUSE;
		len += sprintf(page+len, "TXPause=%s | ", data0?"Enabled":"Disabled");
...
int32 rtl865x_proc_debug_init(void)
{
	...
	if(rtl865x_proc_dir)
	{
		...
		/*  acl_mode  */
		{
			acl_mode = create_proc_entry("acl_mode",0,rtl865x_proc_dir);
			if(acl_mode != NULL)
			{
				acl_mode->read_proc = acl_mode_read;
				acl_mode->write_proc= acl_mode_write;

				retval = SUCCESS;				
			}
			else
			{
				rtlglue_printf("can't create proc entry for acl_mode");
				retval = FAILED;
				goto out;
			}
		}
		...
		/*  acl_mac_filter_mode  */
		{
			acl_mac_filter_mode = create_proc_entry("acl_mac_filter_mode",0,rtl865x_proc_dir);
			if(acl_mac_filter_mode != NULL)
			{
				acl_mac_filter_mode->read_proc = acl_mac_filter_mode_read;
				acl_mac_filter_mode->write_proc= acl_mac_filter_mode_write;
