...
static void synAclTimer_callback(unsigned long data);
...
static int32 _rtl865x_regist_aclChain(char *netifName, int32 priority,uint32 flag,uint8 force);
...
{
	if(_rtl865x_getNetifByName(ifName)!=NULL)
		return 1;
	else
		return 0;
}
...
int32 _rtl865x_addNetif(rtl865x_netif_t *netif)
{
    ...
    #if 0 /* 2012-11-05,xumingfeng, 禁止switch转发，防止桥之间能收到广播包 */
    entry->inAclEnd = entry->inAclStart = entry->outAclEnd = entry->outAclStart = RTL865X_ACLTBL_PERMIT_ALL; /*default permit...*/
    #else
    entry->inAclEnd  = entry->inAclStart  = RTL865X_ACLTBL_ALL_TO_CPU; /*trap to CPU...*/
    entry->outAclEnd = entry->outAclStart = RTL865X_ACLTBL_PERMIT_ALL; /*default permit...*/
    #endif/* 2012-11-05,xumingfeng */
	entry->refCnt = 1;
	entry->master = NULL;
	...
	if(entry->is_slave == 0)
	{
		...
		Netif_Asic2Sw_Mapping[asic_idx] = i ;
		...
		retval = rtl865x_regist_aclChain(netif->name, RTL865X_ACL_USER_USED, RTL865X_ACL_EGRESS,RTL865X_ACL_NOT_FORCE);

		/* Kevin, register RTL865X_ACL_PREPROCESS  */			
		retval = rtl865x_regist_aclChain(netif->name, RTL865X_ACL_PREPROCESS, RTL865X_ACL_INGRESS,RTL865X_ACL_FORCE);
		retval = rtl865x_regist_aclChain(netif->name, RTL865X_ACL_PREPROCESS, RTL865X_ACL_EGRESS,RTL865X_ACL_FORCE);		
		/* permit pkts from cpu port */
		...
		rule->srcFilterPort_ = 0x100;  /*   port id  */							
		rtl865x_add_acl(rule, netif->name, RTL865X_ACL_PREPROCESS);

		/* permit all multicast/broadcast pkts  */
		rule = kmalloc(sizeof(rtl865x_AclRule_t), GFP_KERNEL);
		if(!rule)
		{				
			printk("\n!!!!!!%s(%d): No memory freed for kmalloc!!!",__FUNCTION__,__LINE__);
		...
		/* permit all IP multicast pkts (for pkts form PPP wan whose dst. mac is unicast ) */
		rule = kmalloc(sizeof(rtl865x_AclRule_t), GFP_KERNEL);
		if(!rule)
		{				
			printk("\n!!!!!!%s(%d): No memory freed for kmalloc!!!",__FUNCTION__,__LINE__);
		...
		retval = rtl865x_regist_aclChain(netif->name, RTL865X_ACL_PREPROCESS_LOCALHOST, RTL865X_ACL_INGRESS,RTL865X_ACL_FORCE);
		retval = rtl865x_regist_aclChain(netif->name, RTL865X_ACL_PREPROCESS_LOCALHOST, RTL865X_ACL_EGRESS ,RTL865X_ACL_FORCE);		
		entry->ipv4_addr=0x00000000;	//initialized empty value
	#endif	

		/* Kevin,register  RTL865X_ACL_DEFAULT  */
		retval = rtl865x_regist_aclChain(netif->name, RTL865X_ACL_DEFAULT, RTL865X_ACL_INGRESS,RTL865X_ACL_FORCE);
		retval = rtl865x_regist_aclChain(netif->name, RTL865X_ACL_DEFAULT, RTL865X_ACL_EGRESS,RTL865X_ACL_FORCE);
		...
		rule->pktOpApp_ 	= RTL865X_ACL_ALL_LAYER;
		rule->direction_ = RTL865X_ACL_INGRESS;	
		rtl865x_add_acl(rule, netif->name, RTL865X_ACL_DEFAULT);
		kfree(rule);
		...
		rtl865x_add_acl(rule, netif->name, RTL865X_ACL_DEFAULT);
		kfree(rule);
	#endif /* CONFIG_RTL_IPTABLES_RULE_2_ACL */
		...
		retval = rtl865x_regist_aclChain(netif->name, RTL865X_ACL_QOS_USED0, RTL865X_ACL_INGRESS,RTL865X_ACL_NOT_FORCE);
		retval = rtl865x_regist_aclChain(netif->name, RTL865X_ACL_QOS_USED1, RTL865X_ACL_INGRESS,RTL865X_ACL_NOT_FORCE);
#endif
#endif //CONFIG_RTL_LAYERED_DRIVER_ACL
	}
  ...
  if(alias_name_are_eq(name,ALIASNAME_NAS0,ALIASNAME_MWNAS))
	{
		...
		if ((vid = getVidOfSmuxDev(name)) == -1) {
			printk("%s %s device not found!\n", __func__, name);
			return -1;
		}

		for(i = 0; i < NETIF_SW_NUMBER; i++)
		{
			if ((netifTbl[i].valid == 1) && !netifTbl[i].is_slave && (netifTbl[i].vid == vid))
			{
				return netifTbl[i].asicIdx;
			}
		}
		return -1;
	}
...
int8 _rtl865x_sameAclRule(rtl865x_AclRule_t *rule1, rtl865x_AclRule_t *rule2)
{
	...
	case RTL865X_ACL_802D1P:
		if(rule1->vlanTagPri_ != rule2->vlanTagPri_)
			return FALSE;
		else
			return TRUE;
...
static void _rtl865x_synAclwithAsicTbl(void)
{
	if(timer_pending(&synAclTimer))	
		return;	
	else		
		mod_timer(&synAclTimer, jiffies + HZ) ;
}
...
static void synAclTimer_callback(unsigned long data)
{
	...
	/*  Kevin,   At first , we calaulate the number of force acl rule  */
	uint32 number_force_rule=0;
	uint32 number_other_rule=0;
	...
	for(i = 0; i<NETIF_SW_NUMBER; i++)
	{
		...
		if(netif->valid)
		{
			...
			{
				if(chain->force)
					number_force_rule += chain->ruleCnt;
				/*next chain..*/
				chain = chain->nextChain;
			}
	...
	if(number_force_rule>(RTL865X_ACL_MAX_NUMBER+RTL865X_ACL_RESERVED_NUMBER))
		printk("Warning! the number of force rule is exceeding the limit !!! (%s ,line %d)\n",__func__,__LINE__);

	number_other_rule = MAX(RTL865X_ACL_MAX_NUMBER+RTL865X_ACL_MAX_NUMBER-number_force_rule , 0);	
		...
		if(netif->valid)
		{
			...
			while(chain)
			{
				...
				while(rule)
				{
					if(!chain->force)
					{						
						if(number_other_rule_used==number_other_rule)						
							break; //break while(rule)						
...
*/
int32 rtl865x_regist_aclChain(char *netifName, int32 priority, uint32 flag,uint8 force)
{
	return _rtl865x_regist_aclChain(netifName, priority, flag,force);
}
...
int32 rtl865x_init_acl(void)
{
	...
	init_timer(&synAclTimer); 
    synAclTimer.function = &synAclTimer_callback;
	_rtl865x_confReservedAcl();
	/*init acl chains*/
...
{ 
	...
	for(i=0;i<RTL8651_PORT_NUMBER;i++)
	{
		if(rtl8651_setAsicPortPatternMatch(i, 0, 0, 0x2)!=SUCCESS)
			return FAILED;
	}
...
int32 rtl865x_initNetifTable(void)
{
	...
	/*  Kevin, if there is no dynamic acl, we has to change acl rule in RTL865X_ACL_PREPROCESS_LOCALHOST when localhost's IP changes */
#if defined (CONFIG_RTL_IPTABLES_RULE_2_ACL) && !defined(CONFIG_RTL8676_Dynamic_ACL)
