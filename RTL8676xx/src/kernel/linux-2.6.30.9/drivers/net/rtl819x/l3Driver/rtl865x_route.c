...
static int32 _rtl865x_synRouteToAsic(rtl865x_route_t *rt_t)
{
	...
	if(rt_t->dstNetif->is_slave == 1)
	{
		...
		if (rt_t->dstNetif->master)
			dstNetif = _rtl865x_getNetifByName(rt_t->dstNetif->master->name);
		if(dstNetif == NULL)
...
#else
	asic_t.internal = rt_t->is_internal ? 1 : 0;
#endif
...
static int32 _rtl865x_addRoute(ipaddr_t ipAddr, ipaddr_t ipMask, ipaddr_t nextHop, int8 * ifName,ipaddr_t srcIp)
{
	...
	//printk("%s %d ipAddr %x ipMask %x nextHop %x dev %s srcIp %x\n", __func__, __LINE__,
	//		ipAddr, ipMask, nextHop, ifName, srcIp);
	/*para check*/
	if(ifName == NULL)
...
static int32 _rtl865x_delRoute( ipaddr_t ipAddr, ipaddr_t ipMask )
{
	...
	//printk("%s %d ipAddr %x ipMask %x\n", __func__, __LINE__, ipAddr, ipMask);
...
void rtl865x_refreshRouteArpExtIP(char* netifname,u32 extip)
{
	...
	while(rt)
	{
		...
		{		
			if(!strcmp(rt->dstNetif->name,netifname))
			{
				uint32 extip_idx;
