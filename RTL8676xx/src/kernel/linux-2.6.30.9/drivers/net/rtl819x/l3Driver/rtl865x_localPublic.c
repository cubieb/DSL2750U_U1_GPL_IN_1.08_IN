	...
	if(memcmp(oldMac, newMac, 6) !=0)
	{
		...
		if(memcmp(newMac, zeroMac, 6) !=0)
		{
			if(entry->hw)
			{
				...
				#if defined(CONFIG_RTL_HW_QOS_SUPPORT)
				rtl865x_regist_aclChain(entry->lpNetif, RTL865X_ACL_QOS_USED0, RTL865X_ACL_INGRESS,RTL865X_ACL_NOT_FORCE);
				rtl865x_regist_aclChain(entry->lpNetif, RTL865X_ACL_QOS_USED1, RTL865X_ACL_INGRESS,RTL865X_ACL_NOT_FORCE);
	...
	if (buffer && !copy_from_user(tmp, buffer, 128)) {
		...
		if((strcmp(cmd, "add")==0) || (strcmp(cmd, "ADD")==0))
		{
			if(totalCnt==0)
			{
				...
				#if defined(CONFIG_RTL_HW_QOS_SUPPORT)
				rtl865x_regist_aclChain("virtualNetif", RTL865X_ACL_QOS_USED0, RTL865X_ACL_INGRESS,RTL865X_ACL_NOT_FORCE);
				rtl865x_regist_aclChain("virtualNetif", RTL865X_ACL_QOS_USED1, RTL865X_ACL_INGRESS,RTL865X_ACL_NOT_FORCE);
				rtl865x_qosArrangeRuleByNetif();
				#endif
				//printk("%s:%d,ret is %d\n",__FUNCTION__,__LINE__,ret);
			}
