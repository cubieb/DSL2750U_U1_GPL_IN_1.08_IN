...
#include "../RTL867x_hwnat_API/rtl867x_hwnat_api.h"
...
int smux_pkt_recv(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (dstAddr[0] & 1)
	{
		...
		list_for_each(lh, &grp->virtual_devs)
		{
			...
			if ((skb->vlan_tci && !(vdev->priv_flags&IFF_VSMUX)) ||
				((vdev->priv_flags&IFF_VSMUX) && (!skb->vlan_tci || ((skb->vlan_tci-1)!=dev_info->vid))))
	  			continue;
		...
		case SIOCSITFGROUP:
			{
				...
				#ifdef CONFIG_RTL_8676HWNAT	
				rtl8676_update_portmapping_Multiwan_dev(dev_info->member, dev_info->vid, dev_info->proto);
				#endif
			}
					       ...
					       const char *nifname, int smux_proto, int vid, int napt)
{
	...
	if(grp && new_dev) {
        ...
        sprintf(landev_ifname, "%s%s",ALIASNAME_ETH0DOT,ORIGINATE_NUM);
...
static int smux_unregister_device(const char* vifname)
{
	...
	if(vdev) {
		...
		rtl865x_del_filter_rule_by_interface(vdev->name);
		#endif
		...
		#ifdef CONFIG_RTL_8676HWNAT 
		rtl8676_unregister_Multiwan_dev(vdev->name, dev_info->vid, dev_info->proto);
		#endif
		#ifdef UNIQUE_MAC_PER_DEV
...
#endif
int isBrgWandevPortmappingEnable(struct net_device *dev)
{
	struct smux_dev_info *dev_info;
	...
	dev_info = SMUX_DEV_INFO(dev);
	if (dev_info->member == 0xFFFFFFFF)
		return 0;
		/*
    else
    {
        printk("dev_info->member: %x",dev_info->member);
    }
    */
	return 1;
}

int isSrcDevAndDstDevInSameGroup(struct net_device *src, struct net_device *dst)
{
	struct smux_dev_info *dev_info;
	struct smux_group *grp;
	struct list_head *lh;
	struct smux_dev_info * sdev = NULL;
	unsigned int port;
	unsigned int number_bridged_wan_existed_in=0;
	...
	if (!(src->priv_flags & IFF_DOMAIN_ELAN))
		return 0;

//		if (dst->name[0] != 'n')
        if(!alias_name_are_eq(dst->name,ALIASNAME_NAS,ALIASNAME_MWNAS))
            return 0;

             grp = __find_smux_group(ALIASNAME_NAS0);
             if (!grp)           
                 return 0;
             dev_info = SMUX_DEV_INFO(dst);
             TOKEN_NUM(src->name,&port);
             port-=1;    //eth0.2-eth0.5
//        	port = src->name[5]-'1';//eth0.2-eth0.5
	...
	list_for_each(lh, &grp->virtual_devs)
	{
		...
		if( sdev->member & (1<<port) )
		{
			number_bridged_wan_existed_in ++;

			if(number_bridged_wan_existed_in>=2)
			{
				read_unlock(&smux_lock);
				return 0;
			}

			if(strcmp(dst->name, sdev->vdev->name))
			{
				read_unlock(&smux_lock);
				return 0;
			}
...
/*
	...
	printk("dst->name: %s\n",dst->name);
	printk("sdev->member: %x\n",sdev->member);
	printk("dev_info->member: %x\n",dev_info->member);
*/

	if(number_bridged_wan_existed_in==1)
		return 1;
	else
		return 0;
	...
	if (dev_info->vid != -1)
	{
		if (pvid_per_port[port] != dev_info->vid)
			return 0;
	}
	else
	{
		if (pvid_per_port[port] != RTL_LANVLANID)
			return 0;
	}
	#endif
