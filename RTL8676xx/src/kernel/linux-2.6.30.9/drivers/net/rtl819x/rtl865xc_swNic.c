...
* All rights reserved.
* 
* $Header: /usr/local/dslrepos/linux-2.6.30/drivers/net/rtl819x/rtl865xc_swNic.c,v 1.10 2011/11/04 10:44:09 kaohj Exp $
*
...
static void skb_debug(const struct sk_buff *skb, int flag)
{
...
get_next:	
	...
	{   
		...
		{
			printk("reason:0x%X (",pPkthdr->ph_reason);
			...
			printk(")\n");
			//skb_debug(pPkthdr->ph_mbuf->skb, 0);
		}
		...
		if (buf) 
		{
			...
			if(pPkthdr->ph_reason >> 9 == 1)
				((struct sk_buff*)(info->input))->trap_by_ingress_acl = 1;
			else
				((struct sk_buff*)(info->input))->trap_by_ingress_acl = 0;
