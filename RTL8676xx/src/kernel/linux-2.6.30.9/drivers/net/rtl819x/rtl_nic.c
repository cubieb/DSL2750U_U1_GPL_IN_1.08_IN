 ...
 * All rights reserved.                                                    
 * 
 * $Header: /usr/local/dslrepos/linux-2.6.30/drivers/net/rtl819x/rtl_nic.c,v 1.66 2011/11/18 09:16:13 czpinging Exp $
 *
 * $Author: czpinging $
 *
...
//static int eth_sched_weight = NUM_RX_PKTHDR_DESC;
static int eth_sched_weight = 32;
...
static unsigned int port_get_by_dev(struct net_device *dev);
#endif
...
//#define config_nic_kthread
#include <linux/kthread.h>

#ifdef config_nic_kthread

static struct task_struct *nic_taskp;
static unsigned long nic_pending;
static inline void nic_tasklet_schedule(struct tasklet_struct *t);

#define NIC_RX_BIT 1
#define NIC_TX_BIT 2
#define NIC_LINK_BIT 3
static void nic_kthread_init(void);
#endif
...
#ifdef CONFIG_RTL_MULTI_ETH_WAN
//unsigned int pvid_per_port[RTL8651_PORT_NUMBER+3]={[0 ... RTL8651_PORT_NUMBER+2]=RTL_LANVLANID};
#endif
...
/*		=====  ===   =======	===	 ===   =========   =======	====	====================================	*/
...
#else
	{ RTL_DRV_LAN_P4_NETIF_NAME,	1,	IF_ETHER,	0,		RTL_WAN_FID,		RTL_WANPORT_MASK,		RTL_WANPORT_MASK,		1500,	{ { 0x00, 0x12, 0x34, 0x56, 0x78, 0x90 } }, 0	},
#endif
	...
	{	RTL_DRV_WAN1_NETIF_NAME, 1,   IF_ETHER,    	0,		RTL_WAN_FID,    	RTL_WANPORT_MASK,         	RTL_WANPORT_MASK,     	1500, { { 0x00, 0x12, 0x34, 0x56, 0x78, 0x91 } }, 0 },
	{	RTL_DRV_WAN2_NETIF_NAME, 1,   IF_ETHER,    	0,		RTL_WAN_FID,    	RTL_WANPORT_MASK,         	RTL_WANPORT_MASK,     	1500, { { 0x00, 0x12, 0x34, 0x56, 0x78, 0x91 } }, 0 },
	{	RTL_DRV_WAN3_NETIF_NAME, 1,   IF_ETHER,    	0,		RTL_WAN_FID,    	RTL_WANPORT_MASK,         	RTL_WANPORT_MASK,     	1500, { { 0x00, 0x12, 0x34, 0x56, 0x78, 0x91 } }, 0 },
	{	RTL_DRV_WAN4_NETIF_NAME, 1,   IF_ETHER,    	0,		RTL_WAN_FID,    	RTL_WANPORT_MASK,         	RTL_WANPORT_MASK,     	1500, { { 0x00, 0x12, 0x34, 0x56, 0x78, 0x91 } }, 0 },
	{	RTL_DRV_WAN5_NETIF_NAME, 1,   IF_ETHER,    	0,		RTL_WAN_FID,    	RTL_WANPORT_MASK,         	RTL_WANPORT_MASK,     	1500, { { 0x00, 0x12, 0x34, 0x56, 0x78, 0x91 } }, 0 },
	{	RTL_DRV_WAN6_NETIF_NAME, 1,   IF_ETHER,    	0,		RTL_WAN_FID,    	RTL_WANPORT_MASK,         	RTL_WANPORT_MASK,     	1500, { { 0x00, 0x12, 0x34, 0x56, 0x78, 0x91 } }, 0 },
	/*below 5 entry reserved for pppoe session supporting. */
	{	RTL_DRV_PPP0_NETIF_NAME, 1,   IF_PPPOE,    	RTL_WANVLANID,		RTL_WAN_FID,    	RTL_WANPORT_MASK,         	RTL_WANPORT_MASK,     	1500, { { 0x00, 0x12, 0x34, 0x56, 0x78, 0x91 } }, 1 },
	{	RTL_DRV_PPP1_NETIF_NAME, 1,   IF_PPPOE,    	RTL_WANVLANID,		RTL_WAN_FID,    	RTL_WANPORT_MASK,         	RTL_WANPORT_MASK,     	1500, { { 0x00, 0x12, 0x34, 0x56, 0x78, 0x91 } }, 1 },
	{	RTL_DRV_PPP2_NETIF_NAME, 1,   IF_PPPOE,    	RTL_WANVLANID,		RTL_WAN_FID,    	RTL_WANPORT_MASK,         	RTL_WANPORT_MASK,     	1500, { { 0x00, 0x12, 0x34, 0x56, 0x78, 0x91 } }, 1 },
	{	RTL_DRV_PPP3_NETIF_NAME, 1,   IF_PPPOE,    	RTL_WANVLANID,		RTL_WAN_FID,    	RTL_WANPORT_MASK,         	RTL_WANPORT_MASK,     	1500, { { 0x00, 0x12, 0x34, 0x56, 0x78, 0x91 } }, 1 },
	{	RTL_DRV_PPP4_NETIF_NAME, 1,   IF_PPPOE,    	RTL_WANVLANID,		RTL_WAN_FID,    	RTL_WANPORT_MASK,         	RTL_WANPORT_MASK,     	1500, { { 0x00, 0x12, 0x34, 0x56, 0x78, 0x91 } }, 1 },
	{	RTL_DRV_PPP5_NETIF_NAME, 1,   IF_PPPOE,    	RTL_WANVLANID,		RTL_WAN_FID,    	RTL_WANPORT_MASK,         	RTL_WANPORT_MASK,     	1500, { { 0x00, 0x12, 0x34, 0x56, 0x78, 0x91 } }, 1 },
	{	RTL_DRV_PPP6_NETIF_NAME, 1,   IF_PPPOE,    	RTL_WANVLANID,		RTL_WAN_FID,    	RTL_WANPORT_MASK,         	RTL_WANPORT_MASK,     	1500, { { 0x00, 0x12, 0x34, 0x56, 0x78, 0x91 } }, 1 },
#endif
#endif
	RTL865X_CONFIG_END,
};
...
int  rtl_MulticastRxCheck(struct sk_buff *skb,rtl_nicRx_info *info)
{
	...
	/*  Kevin , add pppoe wan muticast checking  */
	if( (skb->data[0]==0x01 && skb->data[1]==0x00 && skb->data[2]==0x5e) || 
	  ( !(skb->data[0]&0x01) && skb->data[12]==0x88  && skb->data[13]==0x64 &&skb->data[20]==0x00 &&skb->data[21]==0x21 &&skb->data[38]>=224 )  ) 
	{
		#if defined (CONFIG_RTL_HARDWARE_MULTICAST)
		skb->srcVlanId=vid;
		skb->srcPort=pid;
			...
			if((l4Protocol==IPPROTO_UDP) || (l4Protocol==IPPROTO_TCP))
			{
				...
				if(igmpsnoopenabled && (nicIgmpModuleIndex!=0xFFFFFFFF))
				{
					...
					else
					{
						...
						{
							if(br0MCastFwdInfo.unknownMCast)
							{
								/*don't flooding unknown ipv4 multicast data*/
								vlanRelayPortMask=0;
							}
						}
					}
	...
	else if ((skb->data[0]==0x33) && (skb->data[1]==0x33) && (skb->data[2]!=0xff))
	{
		...
		#if defined(CONFIG_RTL_CUSTOM_PASSTHRU)
		if ((vid==PASSTHRU_VLAN_ID)&&(rtl_isWanDev(cp_this)==TRUE))
		{
		...
		{	
			...
			if((l4Protocol==IPPROTO_UDP) || (l4Protocol==IPPROTO_TCP))
			{
				...
				if(igmpsnoopenabled && (nicIgmpModuleIndex!=0xFFFFFFFF))
				{
					...
					if(ret==SUCCESS)
					{
						/*relay packets which are trapped by acl*/
						vlanRelayPortMask=rtl865x_getVlanPortMask(vid)& (~(1<<pid)) & nicMCastFwdInfo.fwdPortMask & ((1<<RTL8651_MAC_NUMBER)-1);
					}
					else
					{
...
static inline void rtl_processRxToProtcolStack(struct sk_buff *skb, struct dev_priv *cp_this, rtl_nicRx_info *info)
{
	...
	#else
	/* For non-ETHWAN, we have only eth0, but user program expects eth0.2-5 for IPQoS to work. - andrew */
	do {
		const char *ifnames[] = { ALIASNAME_ETH0DOT "2", ALIASNAME_ETH0DOT "3", ALIASNAME_ETH0DOT "4", ALIASNAME_ETH0DOT "5" };
		skb->vlan_member = cp_this->port_member;
		skb->src_port = IF_SWITCH;
...
static inline void rtl_processRxFrame(rtl_nicRx_info *info)
{
	...
	tbs_led_data_blinking(led_lan_green);
#endif
	/*	sanity check	*/
	#if 0
	...
	if(  ( vid==PKTHDR_EXTPORT_MAGIC || vid==PKTHDR_EXTPORT_MAGIC2) || 
		 ( !memcmp(&data[ETH_ALEN], cp_this->dev->dev_addr, ETH_ALEN) && vid!=PKTHDR_EXTPORT_MAGIC3 )   )
	{
		cp_this->net_stats.rx_dropped++;
        dev_kfree_skb_any(skb);
       	return;
	...
	/*  Kevin , add pppoe wan muticast checking  */
	if(  (data[0]&0x01) ||  (data[12]==0x88  && data[13]==0x64 &&data[20]==0x00 &&data[21]==0x21 &&data[38]>=224 )  )
	{		
		/*	multicast process	*/
		#if defined (CONFIG_RTL_IGMP_SNOOPING)
	...
	else /*	unicast pkt only	*/
	{
		/*	shortcut process	*/
		#if defined(BR_SHORTCUT)		
		int ret;
		ret = rtl_processBridgeShortCut(skb,cp_this);
		if (SUCCESS==ret){
			return;
		}
		#endif
...
static int32 interrupt_dsr_rx(unsigned long task_priv)
{
	...
	unsigned long flags;
	#endif
    int rx_count = eth_sched_weight;
	//bugid:36626 zhuomingming
	do
	{
	...
	if (!rx_count)
#ifdef config_nic_kthread
		nic_tasklet_schedule(&cp->rx_dsr_tasklet);
#else
...
/*LinkDownEventFlag is a read clear bit, so re865x_setPhyGrayCode() should be call headmost */
	...
	if(newLinkPortMask)
		{
		tbs_led_trigger_set(led_lan_green, led_blinking_trig);
		}
	else
		{
		tbs_led_trigger_set(led_lan_green, led_off_trig);
		}
#endif	
...
irqreturn_t interrupt_isr(int irq, void *dev_instance)
{
	...
	if (status & TX_ALL_DONE_IP_ALL)
	{
		...
		rtl_rxSetTxDone(FALSE);
		//nic_bh_schedule(NIC_TX_BIT);
		
		#ifdef config_nic_kthread
		nic_tasklet_schedule(&cp->tx_dsr_tasklet);
		#else
	...
	if (status & LINK_CHANGE_IP)
	{
  		#ifdef LINK_TASKLET		
		#ifdef config_nic_kthread
		nic_tasklet_schedule(&cp->link_dsr_tasklet);
		#else
...
#ifdef CONFIG_RTL8196C_GREEN_ETHERNET
int total_time_for_5_port = 15*HZ; //unit ms
...
//  if (((struct net_device *)task_priv)->name[3] == '0' ) {//
    if(alias_name_are_eq((struct net_device *)task_priv)->name,ALIASNAME_ETH0,ALIASNAME_ETH0DOT)){
		for (i=0; i<RTL8651_PHY_NUMBER; i++) {
			update_mib_counter(i);		
			calculate_led_interval(i);
			if (led_cb[i].link_status & GPIO_LINK_STATE_CHANGE) 
			{
				gpio_led_Interval_timeout(i);
			}
		}
	}	
#endif
...
static int read_eth_sched_weight(char *page, char **start, off_t off,
				 int count, int *eof, void *data)
{
	int len = 0;
	len += sprintf(page + len, "eth_sched_weight = %d\n", eth_sched_weight);
...
static int write_eth_sched_weight(struct file *file, const char *buffer, 
				unsigned long count, void *data)
{
	 ...
	 memset(buf,0,sizeof(buf));
       ...
       eth_sched_weight = (int)simple_strtoul(buf,NULL,0);
       return count;
}


void init_eth_sched_weight_proc(void)
{
	struct proc_dir_entry *eth_sched_weight;
	eth_sched_weight = create_proc_entry("eth_sched_weight", 0, NULL);
	if (eth_sched_weight == NULL)
		{
		  printk("init eth_sched_weight_proc failed !\n");
		  return;
		}
	eth_sched_weight->read_proc = read_eth_sched_weight;
	eth_sched_weight->write_proc = write_eth_sched_weight;
}
...
static int re865x_open (struct net_device *dev)
{
	...
	#if defined(CONFIG_RTL_PHY_POWER_CTRL)
	portnum = port_get_by_dev(dev);
	rtl865x_setPhyPowerOn(portnum);
	#endif
...
//    if (dev->name[3] == '0') //
if(alias_name_are_eq(dev->name,ALIASNAME_ETH0,ALIASNAME_ETH0DOT)||(alias_name_are_eq(dev->name,ALIASNAME_NAS0,ALIASNAME_MWNAS)))
	#endif
	{	
...
//	cp = netdev_priv(dev);
	...
	#if defined(CONFIG_RTL_PHY_POWER_CTRL)	
	portnum = port_get_by_dev(dev);
	DEBUG_ERR("Turn off PHY of port %d\n", portnum);
	rtl865x_setPhyPowerOff(portnum); /* Turn off the internal PHY, Add by xiachaoren */
	#endif
	rtl8186_stop_hw(dev, cp);
...
#if defined (CONFIG_RTL_IGMP_SNOOPING)
__MIPS16 int re865x_setMCastTxInfo(struct sk_buff *skb,struct net_device *dev, rtl_nicTx_info	*nicTx)
{
	int32 ret;
	 struct dev_priv *cp;
	struct iphdr *iph=NULL;
...
#if 0//QL: 2012-01-05
#ifdef CONFIG_RTL_8676HWNAT
			if((skb->src_port != IF_SWITCH) && (rtl_isWanDev(cp)!=TRUE)){
...
void nic_tx_mirror(struct sk_buff *skb)
{
	...
	rtl_direct_txInfo(0x3f, &nicTx);

	_dma_cache_wback_inv((unsigned long)skb->data, skb->len);
	while (swNic_send(skb, skb->data, skb->len, &nicTx) < 0) {
...
static int rtl865x_addAclForMldSnooping(struct rtl865x_vlanConfig* vlanConfig)
{
	...
	for(i=0; pVlanConfig[i].vid != 0; i++)
	{
		...
		{
			rtl865x_regist_aclChain(pVlanConfig[i].ifname, RTL865X_ACL_IPV6_USED, RTL865X_ACL_INGRESS,RTL865X_ACL_NOT_FORCE);
			/*ping6 issue*/
...
static DEFINE_PER_CPU(struct tasklet_head, nic_tasklet_vec);

static inline void __raise_nic_softirq_irqoff(void) {
	nic_pending = 1;
}

inline void raise_nic_softirq_irqoff(void)
{
	__raise_nic_softirq_irqoff();
	 ...
	 */
	//if (!in_interrupt()) {
	if (nic_taskp && nic_taskp->state != TASK_RUNNING)
		wake_up_process(nic_taskp);
	//}
}

static void __nic_tasklet_schedule(struct tasklet_struct *t)
{
	unsigned long flags;

	local_irq_save(flags);
	t->next = NULL;
	*__get_cpu_var(nic_tasklet_vec).tail = t;
	__get_cpu_var(nic_tasklet_vec).tail = &(t->next);	
	raise_nic_softirq_irqoff();
	local_irq_restore(flags);
}
...
static inline void nic_tasklet_schedule(struct tasklet_struct *t)
{
	if (!test_and_set_bit(TASKLET_STATE_SCHED, &t->state))
		__nic_tasklet_schedule(t);
}



static void nic_tasklet_action(void)
{
	struct tasklet_struct *list;

	local_irq_disable();
	list = __get_cpu_var(nic_tasklet_vec).head;
	__get_cpu_var(nic_tasklet_vec).head = NULL;
	__get_cpu_var(nic_tasklet_vec).tail = &__get_cpu_var(nic_tasklet_vec).head;
	local_irq_enable();
...
static void nic_kthread_init(void) {
	...
	p = kthread_create(nic_kthread, (void *)0, "nic_softirq");
	if (IS_ERR(p)) {
		printk("Failed to create nic_softirq!\n");
		return;
	}
	set_user_nice(p, 0);
...
//		sprintf(dev->name, "nas0"); //
			/*QL for fastbridge*/
			dev->priv_flags = IFF_DOMAIN_WAN;
		}
		else
		{
        sprintf(dev->name, "vport%d", ehtwan_eth_num+1);
			/*QL for fastbridge*/
			dev->priv_flags = IFF_DOMAIN_ELAN;
			ehtwan_eth_num++;
		}		
	...
	if(retVal==SUCCESS)
	{
        ...
        strcpy(devInfo.devName, RTL_DEV_NAME_NUM(ALIASNAME_ETH,*));
//		strcpy(devInfo.devName, "eth*");
	...
	#else
    	printk("Configuration LINUX to process port 0 ~ port %d for Spanning tree process\n", MAX_RE865X_STP_PORT-
    	(int)simple_strtol((const char *)ORIGINATE_NUM, NULL, 0));
...
*/
    ...
    extern uint32 rtk_vlan_portPvid_set(uint32 port, uint32 pvid, uint32 priority);
     extern void RL6000_getPortStatus(void);
#endif
int32 rtl865x_setpvid(struct rtl865x_vlanConfig pvlanconfig[])
{
	int i,j;
	...
	{
		 if( pvlanconfig[j].isWan && pvlanconfig[j].memPort != RTL_WANPORT_MASK )
		 {
		 	bridged_wan_exist = 1;		
			if(pvlanconfig[j].vid==RTL_BridgeWANVLANID)
	...
	for(i=0; i<RTL8651_PORT_NUMBER + 3; i++) 
	{
		...
		else
		{
			...
			{
				/* if the lan port only  exist in only ine bridged WAN, we can set pvid as its vid for upstream hw l2 forwarding*/
				int this_port_exist_in_bridged_wan_num=0;
				int the_first_vid_exist_in_bridged_wan=-1;			
				for(j=0; pvlanconfig[j].vid != 0; j++)			
					if ( pvlanconfig[j].isWan && pvlanconfig[j].memPort != RTL_WANPORT_MASK && (pvlanconfig[j].memPort&(1<<i)) ) 
					{
						this_port_exist_in_bridged_wan_num ++;			
						if(the_first_vid_exist_in_bridged_wan==-1)
							the_first_vid_exist_in_bridged_wan = pvlanconfig[j].vid;
					}
  ...
  if (rtl8651_tblAsicDrvPara.externalPHYProperty & RTL8676_TBLASIC_EXTPHYPROPERTY_PORT0_RTL8367B) {   
        if(i<5)        
        {
      int32  retVal=rtk_vlan_portPvid_set(i, pvid, 0);
	if(retVal!=SUCCESS)
		printk("[%s %d] rtk_vlan_portPvid_set %d failed!\n",__FUNCTION__,__LINE__,i);
...
int getVlanconfigIdxByVid(struct rtl865x_vlanConfig *vconfig, int vid)
{
	int i;	
...
int getRealVlanconfigIdx(struct rtl865x_vlanConfig *vconfig, int vid, int proto)
{
	int idx;
	
	if (vid != -1) {
		idx = getVlanconfigIdxByVid(vconfig, vid);
	}
	else {
		if (proto == SMUX_PROTO_BRIDGE)
			idx = getVlanconfigIdxByVid(vconfig, RTL_BridgeWANVLANID);
		else
			idx = getVlanconfigIdxByVid(vconfig, RTL_WANVLANID);
	}
	return idx;
}
...
int rtl_set_wanport_vlanconfig(char *ifname, int proto, int vid, int napt/*, unsigned char *addr*/)
{
	int idx=0;
	if (vid != -1) {
		idx = getVlanconfigIdxByVid(vlanconfig, vid);
		if (idx <0)
			return FAILED;
		vlanconfig[idx].vid = vid;
		...
		if (proto == SMUX_PROTO_BRIDGE)
			idx = getVlanconfigIdxByVid(vlanconfig, RTL_BridgeWANVLANID);
		else
			idx = getVlanconfigIdxByVid(vlanconfig, RTL_WANVLANID);
		if (idx <0)
			return FAILED;
		if (proto == SMUX_PROTO_BRIDGE)
			vlanconfig[idx].vid = RTL_BridgeWANVLANID;
		else
	...
	if((vlanconfig[idx].memPort!=RTL_WANPORT_MASK) || (vlanconfig[idx].untagSet!=0) || (proto != SMUX_PROTO_BRIDGE)){
		strcpy(vlanconfig[idx].ifname, ifname);
		//memcpy(vlanconfig[idx].mac.octet, addr, ETH_ALEN);
	}

	if((proto != SMUX_PROTO_BRIDGE)){
		//set napt
		vlanconfig[idx].isWan = napt;
	}
...
int rtl_set_wanport_portmapping(unsigned int member, int vid, int proto)
{
	...
	idx = getRealVlanconfigIdx(vlanconfig, vid, proto);
	if (idx <0)
		return FAILED;
...
int rtl865x_unregisterDev(char *ifname) 
{
	...
	//retval = rtl_reset_wanport_vlanconfig(vid, proto);
	//if(retval==FAILED)
	//	return FAILED;

	#ifdef CONFIG_RTL8676_Dynamic_ACL
	retval = rtl865x_del_filter_rule_by_interface(ifname);
	if(retval == FAILED)
		return FAILED;
	#endif
	retval = rtl865x_delNetif(ifname);
	return retval;
...
static int rtl865x_setPhyPowerOff(unsigned int port)
{
	unsigned int regaddr = PCRP0 + (port << 2);
	unsigned int pcr = 0;
	unsigned int regData;	
	if(port >=RTL8651_PHY_NUMBER)
	{
	...
	pcr=READ_MEM32(regaddr);
	pcr |= EnForceMode;
	pcr &= ~ForceLink;	
	WRITE_MEM32(regaddr, pcr );
	TOGGLE_BIT_IN_REG_TWICE(regaddr,EnForceMode);
		     ...
		     int count, int *eof, void *data)
{
	...
	{
		rtl8651_getAsicEthernetPHYReg(port, 0, &regData);
		len += sprintf(page+len, "port[%d] phy is power %s\n", port, (regData & POWER_DOWN) ? "off" : "on");
	}
...
static unsigned int port_get_by_dev(struct net_device *dev)
{
	...
	if(NULL != dev) {
		length = strnlen(dev->name, IFNAMSIZ);
		portnum = dev->name[length - 1] - '0';
	}
