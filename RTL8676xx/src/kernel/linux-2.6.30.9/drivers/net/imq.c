...
static void imq_nf_reinject(struct nf_queue_entry *entry, unsigned int verdict)
{
	...
	// Mason Yu. test123
	//printk("imq_nf_reinject(0), entry=%p\n", entry);
...
static int imq_dev_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	// (1) For route path. skb->nf_queue_entry is not NULL.
	if ( skb->nf_queue_entry !=NULL)
		imq_nf_reinject(skb->nf_queue_entry, NF_ACCEPT);
	// (2) For bridge path. skb->nf_queue_entry is NULL.
	else {
		skb->dev = __dev_get_by_index(dev_net(dev), skb->iif);
