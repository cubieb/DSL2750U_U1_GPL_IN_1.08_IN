...
* $Id: rtl_glue.c,v 1.1.1.1 2010/05/05 09:00:48 jiunming Exp $
*/
...
{
#if defined(RTL865X_TEST) || defined(RTL865X_MODEL_USER) || defined(RTL867X_MODEL_USER)
	return 100;
#else/*RTL865X_TEST*/
	return HZ;
#endif/*RTL865X_TEST*/
	...
	*/
	...
	if (skb->pkt_type == PACKET_AIRGO)
	{
		if (rtl865x_freeMbuf_callBack_f)
		{
			...
			(*rtl865x_freeMbuf_callBack_f)(*(struct rtl_pktHdr **)(p+1));
			return 0;
		}
	}
#endif
...
#else
	/*RTL865XC_QOS_OUTPUTQUEUE*/
	if( PH_PROCFLAG_TEST(pktHdr,PH_PROCFLAG_NEED802D1P_ACTION))
	{
		rtl8651_fwdEngineVlanTagRemark(pktHdr);
		PH_PROCFLAG_CLR(pktHdr,PH_PROCFLAG_NEED802D1P_ACTION);
	}		
#endif
...
void *rtlglue_malloc(size_t NBYTES) {
	...
	totalmem = totalmem+NBYTES;
	if(NBYTES==0) return NULL;
	ret=kmalloc(NBYTES,GFP_ATOMIC);
	printk("rtlglue_malloc(%x) size=%d / %d call by %s %d\n",(uint32)(ret),NBYTES,totalmem,__FUNCTION__,__LINE__); \
	return ret;
}
