...
#ifdef CONFIG_RTL8676_SAR
struct tasklet_struct ppsar_rx_tasklets;
#endif
#ifdef CONFIG_RTL8676_PTM
...
#ifdef CONFIG_RTL8676_PTM
#define MAX_SRAM_MAP_ID ((MAC_PTX_RING0_SIZE+MAC_PTX_RING1_SIZE+MAC_PTX_RING2_SIZE+MAC_PTX_RING3_SIZE+MAC_PTX_RING4_SIZE)  /* share pool to mac */  \
						+PTM_INTFS*PTM_PTX_RING_SIZE  /* share pool to ptm ptx */ \
						+EXT_PTX_RING0_SIZE+EXT_PTX_RING1_SIZE+EXT_PTX_RING2_SIZE  /* share pool to ext ptx  */ \
						+MAC_RX_INTFS*MAC_PRX_RING_SIZE /* mac prx */ \
						+PTM_INTFS*PTM_PRX_RING_SIZE) /* ptm prx */ \

#define MAX_SKB_ID		(MAC_PTX_RING0_SIZE+MAC_PTX_RING1_SIZE+MAC_PTX_RING2_SIZE+MAC_PTX_RING3_SIZE+MAC_PTX_RING4_SIZE \
						+PTM_INTFS*PTM_PTX_RING_SIZE \
						+EXT_PTX_RING0_SIZE+EXT_PTX_RING1_SIZE+EXT_PTX_RING2_SIZE \
						+MAC_RX_INTFS*(MAC_PRX_RING_SIZE+MAC_VRX_RING_SIZE) \
						+PTM_INTFS*(PTM_PRX_RING_SIZE+PTM_VRX_RING_SIZE) \
						+EXT_INTFS*(EXT_PRX_RING_SIZE+EXT_VRX_RING_SIZE)) \
...
#endif
#ifdef CONFIG_RTL8676_PTM
struct mac_pRx *user_ptm_prx=NULL;
#endif
...
#endif
#ifdef CONFIG_RTL8676_PTM
struct mac_pTx *user_ptm_ptx=NULL;
#endif
...
#endif
#ifdef CONFIG_RTL8676_PTM
struct sp_pRx *user_sp_to_ptm=NULL;
#endif
...
{
	return user_ptm_prx+(PTM_PRX_RING_SIZE*intf)+ring;
}
#endif
...
{
	return user_ptm_ptx+(PTM_PTX_RING_SIZE*intf)+ring;
}
#endif
...
{
	return user_sp_to_ptm+(PTM_PTX_RING_SIZE*intf)+ring;
}
#endif
...
{
	(*(volatile unsigned short*)(PTM_BASE+PTM_RCR))=0x3f;  // enable AE/AR/AB/AM/APM/AAP packet receive.	
	return 0;
}
...
//	if((((uint32)user_mac_ptx_idx(1,0))&0xff)!=0) rtlglue_printf("ERROR: init_nic() error, NIC100_TXFDP must 256 bytes alignment!\n");
	...
	WRITE_MEM32(NIC100_BASE+SMSA,(uint32)sram_buf);
...
/* ------------------------ PTM PRx---------------------------- */
#ifdef CONFIG_RTL8676_PTM
	if(user_ptm_prx==NULL) 	
	{
#ifdef SRAM_MAPPING_ENABLED
		user_ptm_prx=(struct mac_pRx *)(sram_buf+total_used_sram_size);
		total_used_sram_size+=PTM_INTFS*PTM_PRX_RING_SIZE*sizeof(struct mac_pRx);
		if(assert_sram_size(total_used_sram_size)!=SUCCESS) return FAILED;
#else
		user_ptm_prx=(struct mac_pRx *)((uint32)(rtlglue_malloc(PTM_INTFS*PTM_PRX_RING_SIZE*sizeof(struct mac_pRx)))|0xa0000000);
		memset(user_ptm_prx,0, (PTM_INTFS*PTM_PRX_RING_SIZE*sizeof(struct mac_pRx)));
#endif	
		
	}	
	if(user_ptm_prx==NULL) 	{ rtlglue_printf("Out of memory!!\n");  return FAILED; }
#endif
/* ----------------------------------------------------------------------------------------------------------- */
/* ------------------------ EXT PRx---------------------------- */
...
/* ------------------------ PTM PTx---------------------------- */
#ifdef CONFIG_RTL8676_PTM
	if(user_ptm_ptx==NULL) 	
	{
#ifdef SRAM_MAPPING_ENABLED
		user_ptm_ptx=(struct mac_pTx *)(sram_buf+total_used_sram_size);
		total_used_sram_size+=PTM_INTFS*PTM_PTX_RING_SIZE*sizeof(struct mac_pTx);
		if(assert_sram_size(total_used_sram_size)!=SUCCESS) return FAILED;
#else
		user_ptm_ptx=(struct mac_pTx *)((uint32)(rtlglue_malloc(PTM_INTFS*PTM_PTX_RING_SIZE*sizeof(struct mac_pTx)))|0xa0000000);
		memset(user_ptm_ptx, 0, (PTM_INTFS*PTM_PTX_RING_SIZE*sizeof(struct mac_pTx)));
#endif
		
	}
	if(user_ptm_ptx==NULL) 	{ rtlglue_printf("Out of memory!!\n");  return FAILED; }
#endif
...
/* ----------------------------------------------------------------------------------------------------------- */	
	...
	/* ------------------------ PTM PRx Ring---------------------------- */
#ifdef CONFIG_RTL8676_PTM
	if(user_sp_to_ptm==NULL) 	
	{
#ifdef SRAM_MAPPING_ENABLED
		user_sp_to_ptm=(struct sp_pRx *)(sram_buf+total_used_sram_size);
		total_used_sram_size+=PTM_INTFS*PTM_PTX_RING_SIZE*sizeof(struct sp_pRx);
		if(assert_sram_size(total_used_sram_size)!=SUCCESS) return FAILED;
#else
		user_sp_to_ptm=(struct sp_pRx *)((uint32)rtlglue_malloc(PTM_INTFS*PTM_PTX_RING_SIZE*sizeof(struct sp_pRx))|0xa0000000);
#endif		
	}
	if(user_sp_to_ptm==NULL) 	{ rtlglue_printf("Out of memory!!\n");  return FAILED; }
#endif
...
#elif defined(CONFIG_RTL8676_PTM) 	//ptm intf
		if(i<PTM_INTFS)
		{	
...
int rtl8672_mac_vtx_start_xmit (struct sk_buff *skb, struct net_device *dev)
{
	...
	if(vtx->own==0) /* not full */
	{
		...
		if(next_vtx->own==1)
		{
			...
			//rtl8672_dumpRing(2,intf+SAR_INTFS);
			//if((uint32)skb!=0) dev_kfree_skb(skb);
			tp->stats.tx_dropped++;			
			return FAILED;
			//return 0;
		}
...
//rtlglue_printf("intf=%d start=%d end=%d currentidx=%d\n",i,startidx%SAR_VRX_RING_SIZE,endidx%SAR_VRX_RING_SIZE,currentidx);
				...
				for(j=startidx;j<endidx;j++)
				{
					...
					if(ptmRx->own==0)
					{
						...
						skb=(struct sk_buff *)ptmRx->skb_header_addr;
						//printk("sar rx...crc:%d   icerr:%d  reason:%d\n",sarRx->crc32err,sarRx->ipcerr,sarRx->reason);
						...
						skb_reserve(skb,rxintf.RXshift);
						ptmRx->rx_buffer_addr=(uint32)skb->head;
						ptmRx->own=1;																		
