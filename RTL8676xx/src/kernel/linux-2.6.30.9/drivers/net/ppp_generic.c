...
static long ppp_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == PPPIOCGPHYDEV) {
		char pppname[IFNAMSIZ] = {0};
		char *phydevname = NULL;

		if (copy_from_user(pppname, argp, sizeof(pppname)))
			return -EFAULT;
		phydevname = getPhysicalIfName(pppname);
		if (!phydevname)
			return -EINVAL;
		if (copy_to_user(argp, phydevname, IFNAMSIZ))
			return -EFAULT;
		return 0;
	}
...
__inline int pppoe_proxy_output(struct sk_buff *skb, struct net_device *pdev, unsigned int course)
{
		...
		if (course == 2) {//downstream
			...
			if ((fpdst = br_fdb_get(br, dest)) != NULL)
			{
				...
				if (netif_running(fpdst->dst->dev)){
					...
					!alias_name_are_eq(fpdst->dst->dev->name,ALIASNAME_ETH,ALIASNAME_ETH0DOT)
					)) {
