...
static struct pppox_sock *__get_item_without_ifindex(struct pppoe_net *pn, __be16 sid,
				unsigned char *addr)
{
	int hash = hash_item(sid, addr);
	struct pppox_sock *ret;
...
#ifdef CONFIG_RTL867X_IPTABLES_FAST_PATH
static inline struct pppox_sock *get_item_without_ifindex(struct pppoe_net *pn, __be16 sid,
					unsigned char *addr)
{
	struct pppox_sock *po;

	read_lock_bh(&pn->hash_lock);
	po = __get_item_without_ifindex(pn, sid, addr );
	if (po)
		sock_hold(sk_pppox(po));
	read_unlock_bh(&pn->hash_lock);
...
static void pppoe_flush_dev(struct net_device *dev)
{
	...
	for (i = 0; i < PPPOE_HASH_SIZE; i++) {
		...
		while (po != NULL) {
			...
			delete_item(pn, po->pppoe_pa.sid, po->pppoe_pa.remote, po->pppoe_ifindex);
			po->pppoe_dev = NULL;
...
/*linux-2.6.19*/ 
//cathy, assign ppp->dev to skb->dev, and remove ppp & pppoe header then pass thru fastpath
#ifdef CONFIG_RTL867X_IPTABLES_FAST_PATH
extern int ppp_fastpath(struct ppp_channel *chan, struct sk_buff *skb);
int pppoe_fastpath(struct sk_buff *skb)
{
	...
	ph = (struct pppoe_hdr *)skb_network_header(skb);
#if 0
	/*linux-2.6.19*/
	//po = get_item(pn, (unsigned long) ph->sid, eth_hdr(skb)->h_source);
	//if(!po) return -1;
#else
	pn = net_generic(&init_net, pppoe_net_id);
	if(pn==NULL)
	{
		//printk( "<%s> ERROR pn==NULL after calling net_generic()\n",  __FUNCTION__ );
		return -1;
	}	
	po = get_item_without_ifindex(pn, (unsigned long) ph->sid, eth_hdr(skb)->h_source);
	if(po==NULL)
	{
		//printk( "<%s> FIXME call get_item_without_ifindex() done, po=%x\n",  __FUNCTION__, po );
		return -1;
	}
#endif
	sk = sk_pppox(po);
	bh_lock_sock(sk);
	if (sock_owned_by_user(sk)) return -2;	
	pch = &(po->chan);
	ret = ppp_fastpath(pch, skb);
	bh_unlock_sock(sk);
	sock_put(sk);
	return ret;
}
...

{
	...
	if (po) {
		...
		bh_lock_sock(sk);
        if (po->ppp > 0) {
                printk("<0> pppoe.c pppoe_disc_rcv() kill ppp pid=%d\n", po->ppp);
                kill_proc(po->ppp, SIGURG, 0);
                po->ppp = 0;
        }		
...
int isPPPModTypePPPoE(struct ppp_channel *pppchan)
{
    if ((pppchan->ops) &&
        (pppchan->ops->start_xmit) &&
        (pppchan->ops->start_xmit == pppoe_xmit))
        return 1;

    return 0;
}

char *getPPPoEIfName(struct ppp_channel *pppchan)
{
    struct sock *sk = (struct sock *) pppchan->private;
    struct pppox_sock *po;
    ...
    if (po->pppoe_dev)
        return po->pppoe_dev->name;

    return NULL;
}

int getPPPoESessIdAndAddr(struct ppp_channel *pppchan, unsigned char *addr)
{
    struct sock *sk = (struct sock *) pppchan->private;
    struct pppox_sock *po;
    int result;
    ...
    for (result = 0; result < 6; result++)
        addr[result] = po->pppoe_pa.remote[result];

    return po->pppoe_pa.sid;
}
...
EXPORT_SYMBOL(isPPPModTypePPPoE);
EXPORT_SYMBOL(getPPPoEIfName);
EXPORT_SYMBOL(getPPPoESessIdAndAddr);
