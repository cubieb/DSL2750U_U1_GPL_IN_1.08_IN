...
#ifdef CONFIG_RTL8676	//shlee 2.6
//#include <platform.h>
/*linux-2.6.19*/
#include <bspchip.h>
...
#endif //CONFIG_RTL8676_ETHSKB_CONTROL_POOL
	...
	if (nicLinkStatCheckCnt++ >= POLLINGTICK /* polling freq 2sec. */)
	{
		...
		for (i=0;i<SW_PORT_NUM;i++)
		{
			...
			if (((regValue>>2)&0x01) != portstatus[i]) {
				...
				brgClearTable();				
#endif //CONFIG_RTL8676_BRIDGE_FASTPATH
		...
		if(!net_smallpkt_heavytraffic)
		{
			...
			refill_pre_ethpool();
				...
				if((jiffies -NicRxPollStartStamp)>100)
				{
...
static void   insert_cputag(struct sk_buff *skb,unsigned char mask)
{
	int i;
	...
	for(i = 0 ; i<12 ; i++)
	{
		multicast_insert[i] = skb->data[i];
	}
	
	multicast_insert[12]=0x88 ;
	multicast_insert[13]=0x99 ;
	multicast_insert[14]=0x90 ;
	multicast_insert[15]= mask;
	skb_push(skb,4);
	//memcpy(skb->data, multicast_insert,16);
	for(i = 0 ; i<16 ; i++)
	{
		skb->data[i]=multicast_insert[i];
	}
}
...
#ifdef CONFIG_EXT_SWITCH
{
		...
		//{
			...
			{
#if !defined(PKT_PROCESSOR) && !defined(PRE_ALLOCATE_SKB) &&defined(CONFIG_RTL8192CD)
				dev_kfree_skb_any(skb);
#else
				dev_kfree_skb(skb);
#endif
...
#ifdef CONFIG_RTL867X_PACKET_PROCESSOR
	rc = request_irq(eth_net_dev->irq, mac_intHandler, SA_SHIRQ, eth_net_dev->name, eth_net_dev);
#endif
