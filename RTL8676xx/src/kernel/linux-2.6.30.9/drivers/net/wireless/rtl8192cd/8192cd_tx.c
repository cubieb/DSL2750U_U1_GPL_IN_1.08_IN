 ...
 *  TX handle routines
 *
 *  $Id: 8192cd_tx.c,v 1.11 2011/09/08 09:37:16 cathy Exp $
 *
...
#if !defined(CONFIG_RTL_8676HWNAT) && defined(CONFIG_RTL8676)
...
#endif
		{
			if (skb_queue_len(&pstat->dz_queue) > 2*NUM_TXPKT_QUEUE )
				return FALSE;
...
#endif		
			(*(unsigned short *)(skb->data + 12) != htons(0x888e))) {
...
static int ampdu_xmit(struct rtl8192cd_priv *priv, struct stat_info *pstat, struct net_device *dev, int tid, int from_timeout)
{
	...
	while (skb_queue_len(&pstat->ampdu_tx_que[tid]) > 0) {
		...
		__rtl8192cd_start_xmit(pskb, dev, tx_flag);
	}
	RESTORE_INT(flags);
...
static int ampdu_timer_add(struct rtl8192cd_priv *priv, struct stat_info *pstat, int tid, int from_timeout)
{
	unsigned int now, timeout, new_timer=0;
	int setup_timer;
	int current_idx, next_idx;
...
void ampdu_timeout(struct rtl8192cd_priv *priv, unsigned int current_time)
{
	struct stat_info *pstat;
...
static int ampdu_check(struct rtl8192cd_priv *priv,  struct sk_buff *skb, struct stat_info *pstat)
{
	int q_num;
	unsigned int priority;
	unsigned short protocol;
...
int mlcst2unicst(struct rtl8192cd_priv *priv, struct sk_buff *skb)
{
	...
	if( IS_IGMP_PROTO(skb->data) || isICMPv6Mng(skb) || IS_ICMPV6_PROTO(skb->data) || isMDNS(skb->data))
	{
	...
	// Yves: If we can not find device interested in this MC IP, drop it. 
	//if (iffind) {
		dev_kfree_skb_any(skb);
		return TRUE;
	//}
...
static void rtl8192cd_fill_fwinfo(struct rtl8192cd_priv *priv, struct tx_insn *txcfg, struct tx_desc *pdesc, unsigned int frag_idx)
{
	...
	if (is_MCS_rate(txcfg->tx_rate))	// HT rates
	{
		...
		if ((txcfg->aggre_en >= FG_AGGRE_MPDU) && (txcfg->aggre_en <= FG_AGGRE_MPDU_BUFFER_LAST)) {
			...
			if (!((txcfg->pstat->ADDBA_declined[TID] || !txcfg->pstat->ADDBA_ready[TID]))) {
	...
	if (txcfg->need_ack) { // unicast
		if (frag_idx == 0) {
			if ((txcfg->rts_thrshld <= get_mpdu_len(txcfg, txcfg->fr_len)) ||
				...
				{
					if(!(txcfg->pstat->is_intel_sta  && txcfg->pstat->ht_cap_len)) {
						n_protection = 1;
						pdesc->Dword4 |=set_desc(TX_RtsEn);
...
*/
		...
		flush_len[flush_num++]=get_desc(pndesc->Dword7) & TX_TxBufSizeMask; // TxBufferSize
...
int __rtl8192cd_start_xmit(struct sk_buff *skb, struct net_device *dev, int tx_flag)
{
	...
	struct net_device *wdsDev = NULL;
#if defined(CONFIG_RTL8676) || defined(TX_SHORTCUT)
	int k;
#endif
	struct tx_insn tx_insn;
	DECLARE_TXCFG(txcfg, tx_insn);
	...
	tbs_led_data_blinking(led_wlan);
#endif
...
#if defined(__KERNEL__) && defined(FILTER_WDS_TXPACKET_TO_BRPORT)
#if LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,35)
	if (!dev->base_addr &&
		dev->br_port && dev->br_port->br && dev->br_port->br->dev)
	{
		if (!memcmp(dev->br_port->br->dev->dev_addr, skb->data, MACADDRLEN))
			goto free_and_stop;
	}
#endif
#endif
		...
		(FRAGTHRSLD > 1500))
	{
			...
			if ((txcfg->aggre_en >= FG_AGGRE_MPDU) && (txcfg->aggre_en <= FG_AGGRE_MPDU_BUFFER_LAST)) {
				if (!pstat->ADDBA_ready[(int)skb->cb[1]]) {
					if (pstat->ADDBA_req_num[(int)skb->cb[1]] < 5) {
						if ((time_after(jiffies,pstat->ADDBA_Prev_Time[(int)skb->cb[1]]+msecs_to_jiffies(19))) || (pstat->ADDBA_req_num[(int)skb->cb[1]] == 0)) {
							pstat->ADDBA_req_num[(int)skb->cb[1]]++;
							issue_ADDBAreq(priv, pstat, (int)skb->cb[1]);
						}
					} else {
						DEBUG_INFO("The client maybe have NO BA!!!!!\n");
					}
				}
			}
...
static int check_txdesc(struct rtl8192cd_priv *priv, struct tx_insn* txcfg)
{
	...
	{
		pskb = ((struct sk_buff *)txcfg->pframe);
		if (pskb->len < WLAN_ETHHDR_LEN) {
			DEBUG_ERR("TX DROP: %s: pskb->len  [%d]!\n", (char *)__FUNCTION__, pskb->len );
			return FALSE;
		}
...
#endif
		{
			...
			if (txcfg->aggre_en >= FG_AGGRE_MPDU && txcfg->aggre_en <= FG_AGGRE_MPDU_BUFFER_LAST) {
				if (!pstat->ADDBA_ready[priority]) {
					if (pstat->ADDBA_req_num[priority] < 5) {
					    if ((time_after(jiffies,pstat->ADDBA_Prev_Time[priority]+msecs_to_jiffies(19))) || (pstat->ADDBA_req_num[priority] == 0)) {
...
int __rtl8192cd_firetx(struct rtl8192cd_priv *priv, struct tx_insn* txcfg)
{
	...
	unsigned int		txdesc_need = 1;
	unsigned int		ret_txdesc = 0;
#endif
