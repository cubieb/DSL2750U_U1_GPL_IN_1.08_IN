 ...
 *  Utility routines
 *
 *  $Id: 8192cd_util.c,v 1.14 2011/11/09 12:04:36 eason Exp $
 *
	...
	// any free sta info?
		for(i=0; i<NUM_STAT; i++) {
			if (priv->pshare->aidarray[i] && (priv->pshare->aidarray[i]->used == FALSE))
			{
				...
				release_stainfo(priv, pstat);	//cathy, fix infinite loop in auth_expire, all list should be deleted before init_stainfo
...
//		return 0;
		...
		if (pstat && (pstat->state & WIFI_WDS) && !(pstat->state & WIFI_ASOC_STATE)) {
			...
			return !pstat->dot11KeyMapping.keyInCam;
#else
			return 0;
#endif
		}
#endif
		{
			if (priv->pmib->dot118021xAuthEntry.dot118021xAlgrthm) {
				if (isMulticast) { // multicast
					return !priv->pmib->dot11GroupKeysTable.keyInCam;
				} else {
					return !pstat->dot11KeyMapping.keyInCam;
				}
			}
...
int should_restrict_Nrate(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
	if (OPMODE & WIFI_AP_STATE)
	{
		if (pstat->is_legacy_encrpt == 1) {
			if (priv->pmib->dot11nConfigEntry.dot11nLgyEncRstrct & BIT(1)) {
				if (!pstat->is_rtl8192s_sta || (priv->pmib->dot11nConfigEntry.dot11nLgyEncRstrct & BIT(2)))
					return 1;
			}
		}
...
void add_ps_timer(unsigned long task_priv)
{
	...
	if (set_timer)
		mod_timer(&priv->add_ps_timer, jiffies + msecs_to_jiffies(50));	// 50 ms
}
...
void add_rssi_timer(unsigned long task_priv)
{
	...
	if (set_timer)
		mod_timer(&priv->add_rssi_timer, jiffies + msecs_to_jiffies(50));	// 50 ms
}
...
void add_RATid_timer(unsigned long task_priv)
{
	...
	if (set_timer)
		mod_timer(&priv->add_RATid_timer, jiffies + msecs_to_jiffies(50));	// 50 ms
}
...
void dig_cca_dm(struct rtl8192cd_priv *priv)
{
	if ((priv->up_time % 3) == 1) {
...
/* if a>=b, return TRUE.*/
int no_less_than(unsigned long a, unsigned long b)
{
	const unsigned long MSB_LONG = 0x80000000;
	unsigned long a_MSB = a&MSB_LONG;
	unsigned long b_MSB = b&MSB_LONG;

	if(a_MSB != b_MSB) {
		return (b_MSB == MSB_LONG);
	} else {
		return (a >= b);
	}
}
...
void DetectSTAExistance(struct rtl8192cd_priv *priv, struct tx_rpt *report, struct stat_info *pstat)
{
	...
	else if( report->txfail != 0 )
	{
		...
		if(	no_less_than(priv->up_time, pstat->tx_last_good_time+TFRL_SetTime) &&
		...
		if( 	(pstat->tx_conti_fail_cnt >= maxTxFailCnt) ||
			(pstat->tx_conti_fail_cnt >= minTxFailCnt && no_less_than(priv->up_time, pstat->tx_last_good_time+txFailSecThr) )
			)
...
static const unsigned char MCS_RATE_SPEED[2][2][16] = {	// (M/10)bps
	{{0, 1, 1, 2, 3, 5, 5, 6,	1, 2, 3 , 5, 7,10,11,13},		// Long GI, 20MHz
	 {0, 1, 2, 2, 4, 5, 6, 7,	1, 2, 4, 5, 8,11,13,14}},		// Short GI, 20MHz
	{{1, 2, 4, 5, 8,10,12,13,	2, 5, 8,10,16,21,24,27},		// Long GI, 40MHz
	 {1, 3, 4, 6, 9,12,13,15,	3, 6, 9,12,18,24,27,30}}	// Short GI, 40MHz
};

/* Purpose: decrease retry limit to let FIFO packet be sent out quickly. */
void decrease_retry_limit(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
	unsigned char speed, retry_cnt;

	if (is_MCS_rate(pstat->current_tx_rate))
		speed = MCS_RATE_SPEED[(pstat->ht_current_tx_info&BIT(0))?1:0][(pstat->ht_current_tx_info&BIT(1))?1:0][pstat->current_tx_rate&0x7f];
	else
		speed = pstat->current_tx_rate/20;

	/* To delete pstat, packets should be sent faster. Set retry limit as low as possible. */
	retry_cnt = speed/3+4;		/* speed: 0~30 => retry_cnt: 4~14 */

	if(RTL_R8(RL) > retry_cnt) {
		RTL_W16(RL, (retry_cnt&SRL_Mask)<<SRL_SHIFT|(retry_cnt&LRL_Mask)<<LRL_SHIFT);
		DEBUG_WARN( "== Set RetryLimit to 0x%04X ==\n", RTL_R16(RL) );
	}

	DEBUG_WARN("decrease_retry_limit(): aid=%d, current_tx_rate=0x%X, speed=%d, retry_cnt=%d, RL=0x%04X\n", 
		pstat->aid, pstat->current_tx_rate, speed, retry_cnt, RTL_R16(RL));
}
...
/* Purpose: to enable and modify specific mac_id's retry-count of the packets	*/
/*               that are queued in Tx descriptor and owned by hardware.            	*/
void set_per_packet_retry_in_txdesc(struct rtl8192cd_priv *priv, unsigned short macid, unsigned char q_num, unsigned char retry_cnt)
{
	...
	pdescinfoH	= get_txdesc_info(priv->pshare->pdesc_info, q_num);
	if (!phdesc || !pdescinfoH)
		return;

	retry_cnt &= TX_DataRtyLmtMask;
	SAVE_INT_AND_CLI(flags);

	/* Let index i start from [H/W head], that is [S/W tail +1] in Tx descriptor ring.	*/
	end = get_txhead(phw, q_num);	/* end: [H/W tail+1] is [S/W head].		*/
	i = get_txtail(phw, q_num);		/* beginning: [H/W head] is [S/W tail +1].	*/
	...
	while (i != end) {
		...
		pdesc = phdesc + i;
		if((get_desc(pdesc->Dword0) & (TX_OWN|TX_FirstSeg)) == (TX_OWN|TX_FirstSeg)) {
			if((get_desc(pdesc->Dword1) & TX_MacIdMask) == (macid<<TX_MacIdSHIFT)) {
				pdescinfo = pdescinfoH + i;
				if(pdescinfo->pframe && (pdescinfo->type==_SKB_FRAME_TYPE_)) {
					pdesc->Dword5 &= set_desc(~(TX_DataRtyLmtMask<<TX_DataRtyLmtSHIFT));
					pdesc->Dword5 |= set_desc((retry_cnt<<TX_DataRtyLmtSHIFT) | TX_RtyLmtEn);
					rtl_cache_sync_wback(priv, pdesc, sizeof(struct tx_desc), PCI_DMA_TODEVICE);
				}
			}
		}
	}
