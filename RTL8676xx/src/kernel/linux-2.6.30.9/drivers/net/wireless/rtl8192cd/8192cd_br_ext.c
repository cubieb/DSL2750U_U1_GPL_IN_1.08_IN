 ...
 *
 *  $Id: 8192cd_br_ext.c,v 1.2 2010/10/26 09:30:21 yachang Exp $
 *
...
static __inline__ int __nat25_network_hash(unsigned char *networkAddr)
{
	...
	else
	{
		...
		x = networkAddr[0] ^ networkAddr[1] ^ networkAddr[2] ^ networkAddr[3] ^ networkAddr[4] ^
			networkAddr[5] ^ networkAddr[6] ^ networkAddr[7] ^ networkAddr[8] ^ networkAddr[9] ^
			networkAddr[10];
				...
				struct sk_buff *skb, unsigned char *networkAddr)
{
	...
	while (db != NULL)
	{
		if(!memcmp(db->networkAddr, networkAddr, 11))
		{
				...
				unsigned char *macAddr, unsigned char *networkAddr)
{
	...
	memcpy(db->networkAddr, networkAddr, 11);
	memcpy(db->macAddr, macAddr, ETH_ALEN);
		...
		for (i=0; i<NAT25_HASH_SIZE; i++)
		{
			...
			while (f != NULL)
			{
				...
				if(__nat25_has_expired(priv, f))
				{
					...
					{
#ifdef BR_EXT_DEBUG
						printk("NAT25 Expire H(%02d) M:%02x%02x%02x%02x%02x%02x N:%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x\n",
							i,
							f->macAddr[0],
							f->macAddr[1],
							f->macAddr[2],
