 ...
 *  RX handle routines
 *
 *  $Id: 8192cd_rx.c,v 1.15 2011/11/10 12:37:24 cathy Exp $
 *
...
void rtl_netif_rx(struct rtl8192cd_priv *priv, struct sk_buff *pskb, struct stat_info *pstat)
{
...
#if defined(BR_SHORTCUT)
	{
		...
		if (!(pskb->data[0] & 0x01) &&
				...
				(priv->dev->br_port) &&
#ifdef CONFIG_RTL8676
				(priv->dev->br_port->br->wlan_blocking == WLAN_BLOCK_DISABLE) &&
#endif
	...
	if (((OPMODE & WIFI_STATION_STATE) == WIFI_STATION_STATE) &&
		(*(unsigned short *)(SKB_MAC_HEADER(pskb) + MACADDRLEN * 2) == __constant_htons(0x0800)) &&
		(*(unsigned char *)(SKB_MAC_HEADER(pskb) + 23)) == 0x11) {
		CheckUDPandU2M(pskb);
	}
#endif
...
#ifdef RX_BUFFER_GATHER
				if (pfrinfo->rxbuf_shift + pfrinfo->driver_info_size + pfrinfo->pktlen + _CRCLNG_ <= (RX_BUF_LEN- sizeof(struct rx_frinfo)-32) ) {
#else
				if (pfrinfo->rxbuf_shift + pfrinfo->driver_info_size + pfrinfo->pktlen + _CRCLNG_ <= (RX_BUF_LEN- sizeof(struct rx_frinfo)-64) ) {
#endif					
					skb_reserve(pskb, (pfrinfo->rxbuf_shift + pfrinfo->driver_info_size));
					...
					if (cmd & RX_ICVERR) {
						...
						if (rtl8192cd_ICV) {
							...
							pskb->data -= (pfrinfo->rxbuf_shift + pfrinfo->driver_info_size);
							pskb->tail -= (pfrinfo->rxbuf_shift + pfrinfo->driver_info_size);
#if !defined(DELAY_REFILL_RX_BUF)//we create new_skb, we should free new_skb!
							if (new_skb != NULL)
...
//					printk("pkt in\n");
					...
					if (reuse) {
                        ...
                        if (pfrinfo->gather_flag & GATHER_FIRST){
                            ...
                            rtl_kfree_skb(priv, pskb, _SKB_RX_);
                            reuse = 0;
                            DEBUG_WARN("A-MSDU packets error, free skb\n");
                        }
#endif	
					}
				}
...
#ifdef DELAY_REFILL_RX_BUF
#ifdef RX_BUFFER_GATHER
		if (tail != phw->cur_rx_refill || pfrinfo_update)  {
#else
...
check_privacy:
	...
	if (IS_MCAST(da)) {
		...
		priv->ext_stats.rx_decache++;
		SNMP_MIB_INC(dot11FrameDuplicateCount, 1);
		DEBUG_WARN("retry:%d, pfrinfo->tid:%d, seq:%x, TupleCache: %x, pos: %x\n", GetRetry(pframe), pfrinfo->tid, GetSequence(pframe), GetTupleCache(pframe), pos);////for AR5007 IOT ISSUE
		goto free_skb_in_defrag;
	}
	else
...
static int process_datafrme(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo)
{
	...
	if (OPMODE & WIFI_AP_STATE)
	{
		...
		if (is_qos_data(pframe)) {
			...
			{
 				//because only realtek, ralink, Marvell used A-MSDU
				...
				if ( (priv->pshare->is_40m_bw && pstat->is_marvell_sta) &&
					...
					priv->pshare->Reg_RRSR_2 = RTL_R8(RRSR+2);//original reg should be have value
					priv->pshare->Reg_81b = RTL_R8(0x81b);//original reg should be have value
					RTL_W8(RRSR+2, priv->pshare->Reg_RRSR_2 | 0x60);//Wilson
					RTL_W8(0x81b, priv->pshare->Reg_81b | 0x0E); //Wilson
				}
