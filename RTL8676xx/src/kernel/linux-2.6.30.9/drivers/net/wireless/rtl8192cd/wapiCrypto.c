...
static unsigned char data_before_mic[2][70] = 
{
	{0xaa, 0xaa, 0x03, 0x00, 0x00, 0x00, 0x08, 0x06,
	0x00, 0x01, 0x08, 0x00, 0x06, 0x04, 0x00, 0x01,
	0x00, 0xe0, 0x4c, 0x72, 0x00, 0x01, 0xc0, 0xa8,
	0x01, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0xc0, 0xa8, 0x01, 0xfe},
	{0xaa, 0xaa, 0x03, 0x00, 0x00, 0x00, 0x08, 0x00,
	0x45, 0x00, 0x00, 0x3d, 0xbc, 0x7d, 0x00, 0x00,
	0x40, 0x01, 0x39, 0x74, 0xc0, 0xa8, 0x01, 0xfe,
	0xc0, 0xa8, 0x01, 0x80, 0x00, 0x00, 0xce, 0x5b,
	0x03, 0x00, 0x1a, 0x00, 0x61, 0x62, 0x63, 0x64,
...
static unsigned short data_before_mic_len[2] = {36, 69};

static unsigned char data_before_mic_key[2][16] = 
{
	{0x56, 0x65, 0xc2, 0x87, 0x04, 0x5c, 0x2b, 0x48,
	0xae, 0x42, 0xec, 0x83, 0x42, 0x48, 0x83, 0xed}, 
	{0x7e, 0xb4, 0x5d, 0xaf, 0x23, 0xe6, 0x9c, 0x76,
	0xa7, 0xb4, 0x29, 0xe8, 0x79, 0xc7, 0x6a, 0xe8}
};

static unsigned char data_before_mic_head[2][34] = 
{
	{0x88, 0x41, 0x00, 0xa9, 0x9a, 0x88, 0x89, 0x99,
	0x00, 0xe0, 0x4c, 0x72, 0x00, 0x01, 0x00, 0x00,
	0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x24},
	{0x88, 0x42, 0x00, 0xe0, 0x4c, 0x72, 0x00, 0x01,
	0x00, 0xa9, 0x9a, 0x88, 0x89, 0x99, 0x00, 0x00,
	0x00, 0xa9, 0x9a, 0x88, 0x89, 0x99, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x45}
};
static unsigned char data_before_mic_head_len[2] = {34, 34};

static unsigned char data_mic_iv[2][16] = 
{
	{0x92, 0x5c, 0x36, 0x5c, 0x36, 0x5c, 0x36, 0x5c,
	0x36, 0x5c, 0x36, 0x5c, 0x36, 0x5c, 0x36, 0x5c},
	{0x47, 0x5c, 0x36, 0x5c, 0x36, 0x5c, 0x36, 0x5c,
	0x36, 0x5c, 0x36, 0x5c, 0x36, 0x5c, 0x36, 0x5c}
};

static unsigned char data_after_mic[2][16] = 
{
	{0x30, 0x50, 0xe3, 0xc9, 0x5b, 0x7c, 0x6a, 0x9d, 
	0x1c, 0x2e, 0xd5, 0x8a, 0xc4, 0x77, 0x68, 0xba},
	{0xcd, 0x59, 0x58, 0xfe, 0x62, 0x4c, 0x36, 0xaa, 
	0x62, 0xe2, 0x1b, 0xab, 0x71, 0x5f, 0x25, 0x99}
};
...
extern void WapiSMS4CalculateMic(uint8 *Key, uint8 *IV, uint8 *Input1, uint8 Input1Length,
		uint8 *Input2, uint16 Input2Length, uint8 *Output);
...
int sha2_self_test( int verbose )
{
	...
	/*	test wapi encryption	*/
	{
		...
		buflen2 = 0;
		ptrBuf = buf+44;
		memcpy(buf, &data_before_encrypt[0][0], 44);
		WapiSMS4Encryption(&data_before_encrypt_key[0][0], &data_before_encrypt_PN[0][0],
			&data_before_encrypt[0][0]+44, data_before_encrypt_len[0]-44, ptrBuf, &buflen2);

		if (buflen2!=data_before_encrypt_len[0]-44)
			printk("WAPI Encryption length error: [%d]:[%d]\n", data_before_encrypt_len[0], buflen2);
		else
			printk("WAPI Encryption length OK: [%d]:[%d]\n", data_before_encrypt_len[0], buflen2);

		if (memcmp(buf, &data_after_encrypt[0][0], buflen2+44))
		{
			printk("***********************************************************\n");
			memDump(buf, buflen2+44, "buf");
			memDump(&data_after_encrypt[0][0], buflen2+44, "enc1");
			printk("***********************************************************\n");
	...
	for(i=0;i<2;i++)
	{
		WapiSMS4CalculateMic(&data_before_mic_key[i][0], &data_mic_iv[i][0], 
			&data_before_mic_head[i][0], data_before_mic_head_len[i], 
			&data_before_mic[i][0], data_before_mic_len[i], buf);

		if (memcmp(buf, &data_after_mic[i][0], SMS4_MIC_LEN))
		{
			printk("***********************************************************\n");
			memDump(buf, SMS4_MIC_LEN, "buf");
			memDump(&data_after_mic[i][0], SMS4_MIC_LEN, "mic");
			printk("***********************************************************\n");
			printk("WAPI MIC idx %d check failed.\n", i);
		}
		else
			printk("WAPI MIC idx %d check OK.\n", i);
	}
#endif
...
void WapiSMS4CalculateMic(muint8 *Key, muint8 *IV, muint8 *Input1, muint8 Input1Length,
                                                 muint8 *Input2, muint16 Input2Length, muint8 *Output)
{
	muint32   blockNum,i;
	muint32	remainder;
	muint8	BlockOut[16];
	...
	SMS4KeyExt((muint8 *)Key, wapiRK);
	SMS4Crypt((muint8 *)wapiBlockIn, BlockOut, wapiRK);

	for(i=0; i<blockNum; i++)
	{
	      xor_block(wapiBlockIn, Input1, BlockOut);
		SMS4Crypt((muint8 *)wapiBlockIn, BlockOut, wapiRK);
		Input1 += 16;
	}
	...
	if(remainder !=0)
	{
	      ...
	      memcpy(wapiTempBlock, Input1, remainder);
             ...
             xor_block(wapiBlockIn, wapiTempBlock, BlockOut);
        	SMS4Crypt((muint8 *)wapiBlockIn, BlockOut, wapiRK);	
      }
	...
	blockNum = Input2Length >> 4;
  	...
  	for(i=0; i<blockNum; i++)
	{
	      xor_block(wapiBlockIn, Input2, BlockOut);
		SMS4Crypt((muint8 *)wapiBlockIn, BlockOut, wapiRK);
		Input2 += 16;
	}
...
/*	uint8				pTemp[SMS4_MIC_LEN];		*/
	...
	pHeader = txcfg->phdr;	/*	txcfg->hdr_len	*/
	{
		...
		if ( !txcfg->pstat )
		{
			...
			{
				/*	MSK update	*/
				if((priv->wapiMCastKeyUpdate==1) || ((pstat)&&(pstat->wapiInfo)&&(pstat->wapiInfo->wapiUCastKeyUpdate))){
//					printk("%s(%d), during msk or usk update, do nothing! =========== \n",__FUNCTION__,__LINE__);//Added for test
			...
			if (priv->wapiMCastKeyUpdateCnt<=txcfg->fr_len)
			{
...
//						printk("%s(%d), during msk or usk update, do nothing! =========== \n",__FUNCTION__,__LINE__);//Added for test
				...
				priv->wapiMCastKeyUpdateCnt-=txcfg->fr_len;
			
			#if defined(CONFIG_RTL_HW_WAPI_SUPPORT)
			bHWEncrypt = priv->pmib->dot11GroupKeysTable.keyInCam;
			#endif
		}
		else
		{
			...
			{
				/*	USK update	*/
				if((wapiInfo->wapiUCastKeyUpdate==1) || (priv->wapiMCastKeyUpdate==1)){
//					printk("%s(%d), during usk or msk update, do nothing! =========== \n",__FUNCTION__,__LINE__);//Added for test
				}
				else{
			...
			if (wapiInfo->wapiUCastKeyUpdateCnt<=txcfg->fr_len)
			{
					...
					wapiInfo->priv->pmib->wapiInfo.wapiUpdateUCastKeyType==wapi_all_update)
				{
					/*	USK update	*/
					if((wapiInfo->wapiUCastKeyUpdate==1) || (priv->wapiMCastKeyUpdate==1)){
//						printk("%s(%d), during usk update, do nothing!KeyIdx(0x%x),wapiInfo->wapiUCastKeyId(0x%x),wapiInfo->wapiUCastKeyUpdateCnt(%u), priv->pmib->wapiInfo.wapiUpdateUCastKeyPktNum(%u),txcfg->fr_len(%d)=========== \n",
//							__FUNCTION__,__LINE__,KeyIdx,wapiInfo->wapiUCastKeyId,wapiInfo->wapiUCastKeyUpdateCnt,priv->pmib->wapiInfo.wapiUpdateUCastKeyPktNum,txcfg->fr_len);//Added for test
					}
					else{
						wapiUpdateUSK(priv, txcfg->pstat);
					}
				...
				wapiInfo->wapiUCastKeyUpdateCnt-=txcfg->fr_len;
			
			#if defined(CONFIG_RTL_HW_WAPI_SUPPORT)
			bHWEncrypt = txcfg->pstat->dot11KeyMapping.keyInCam;
			#endif
		}
...
#endif
		{
			...
			pBuf = ((struct sk_buff *)txcfg->pframe)->data-txcfg->llc;
			MicBuffer = ((struct sk_buff *)txcfg->pframe)->data+txcfg->fr_len;


			if (txcfg->llc>0)
			{
				memcpy(pBuf, pHeader+txcfg->hdr_len + txcfg->iv, txcfg->llc);
			}


			{
				SecCalculateMicSMS4(KeyIdx, pMicKey, pHeader, pBuf, txcfg->fr_len+txcfg->llc, MicBuffer);

				/* encryption for data	*/
				WapiSMS4Encryption(pDataKey, pPN, pBuf, txcfg->fr_len+txcfg->llc+SMS4_MIC_LEN, pBuf, &OutputLength);
				wapiAssert(OutputLength==txcfg->fr_len+txcfg->llc+SMS4_MIC_LEN);
			}

			//llc in wlan hdr should be sms4encrypted to deliver
...
/*	uint8				MicBuffer[SMS4_MIC_LEN];	*/
	...
	else
	{
		...
		pDataKey = wapiInfo->wapiUCastKey[KeyIdx].dataKey;
		#if defined(CONFIG_RTL_HW_WAPI_SUPPORT)
		bHWEncrypt = pstat->dot11KeyMapping.keyInCam;
		#endif
	}
