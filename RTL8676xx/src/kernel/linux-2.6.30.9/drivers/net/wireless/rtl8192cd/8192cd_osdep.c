 ...
 *
 *  $Id: 8192cd_osdep.c,v 1.24 2011/11/10 12:37:24 cathy Exp $
 *
...
/**close the "telefonica" demand**/
#undef CONFIG_CREATE_MAC_TELEFONICA  //tw add for telefonica
...
extern btn_status rtl8676_get_button_status(struct btn_dev * btn);
...
static struct button_device wps_button;
...
static int PCIE_reset_procedure(int PCIE_Port0and1_8196B_208pin, int Use_External_PCIE_CLK, int mdio_reset,unsigned long conf_addr)
{
	...
	REG32(PCI_MISC) = 0x1d400000;
	REG32(PCI_MISC) = 0x1d500000;
...
#ifdef CONFIG_RTL8676
	#ifdef HW_SUPPORT_PCIE_CLK_REQ_PIN
	// MISC_IP_SEL register can be set only when hardware pcie_clkreq pin is supported. (may implemented by gpio)
...
#endif
			{
				...
				if (priv->pshare->shift_vap_idx) {
					for (i=0; i<RTL8192CD_NUM_VWLAN; i++) {
						if (IS_DRV_OPEN(priv->pvap_priv[i]) && priv->pvap_priv[i]->vap_init_seq == priv->pshare->shift_vap_idx) {
...
static void rtl8676_wps_button_timer_handler(unsigned long data)
{
	...
	printk("wps button state = %d, current status is %d\n", state, btn->cur_status);
#endif
	...
	if(BTN_DOWN == btn->cur_status)
		{
		mod_timer(&wps_button.timer, jiffies + msecs_to_jiffies(20));
		}
}
...
__inline__ static int __rtl8192cd_interrupt(void *dev_instance)
{
	...
	status = RTL_R32(FSISR);
	status_ext = RTL_R32(GP70ISR);
	if((status & 1) || (status_ext))
		{
		RTL_W32(GP70ISR, status_ext);
		RTL_W32(FSISR, status);
#ifdef CONFIG_BUTTON_DEBUG		
		printk("wps button irq!\n");
#endif
		if(BTN_UP == wps_button.dev->cur_status)
			{
			setup_timer(&wps_button.timer, rtl8676_wps_button_timer_handler, (unsigned long)(wps_button.dev));
			mod_timer(&wps_button.timer, jiffies + msecs_to_jiffies(20));
			}
		}
#ifdef PCIE_POWER_SAVING
	...
	if (status & (HIMR_BCNDMA0| HIMR_BCNDMA1 | HIMR_BCNDMA2 | HIMR_BCNDMA3 | HIMR_BCNDMA4))
		caseBcnInt = 1;

	if (status & (HIMR_BCNDOK0| HIMR_BCNDOK1 | HIMR_BCNDOK2 | HIMR_BCNDOK3 | HIMR_BCNDOK4))
		caseBcnStatusOK = 1;
...
static int rtl8192cd_init_sw(struct rtl8192cd_priv *priv)
{
	...
	if (priv->reset_hangup &&
		...
		backup_len = ((unsigned long)&((struct rtl8192cd_priv *)0)->EE_Cached) -
				 ((unsigned long)&((struct rtl8192cd_priv *)0)->join_res);
...
#endif
	{
#if !defined(CONFIG_LEDS_TBS) && !defined(CONFIG_LEDS_TBS_MODULE)//		// zero all data members below (including) LED_Timer of share_info  ThermalValue
		offset = (unsigned long)(&((struct priv_shared_info*)0)->LED_Timer);		
#else
		// zero all data members below (including) ThermalValue of share_info  
		offset = (unsigned long)(&((struct priv_shared_info*)0)->ThermalValue);
#endif
		...
		mod_timer(&priv->pshare->meshare.mesh_test_sme_timer, jiffies + (2*HZ));
		...
		mod_timer(&priv->pshare->meshare.mesh_test_sme_timer2, jiffies + (50*HZ));
...
//------------------------- david+2007-5-31
	...
	priv->pshare->rtk_ie_buf[5] |= RTK_CAP_IE_WLAN_81xxC | RTK_CAP_IE_WLAN_8192SE;
#ifdef CLIENT_MODE
...
/*add by peteryu, 2010/10/20*/
#include <linux/kthread.h>
#include <linux/delay.h>

struct task_struct *kTPT_task;

static int kTPT_thread(void *p)
{
	...
	do {                    
		...
		if (IS_DRV_OPEN(priv) && priv->pmib->dot11RFEntry.ther > 0) {
			if (priv->pshare->rf_ft_var.tpt_period > 1){
				if ((priv->up_time % priv->pshare->rf_ft_var.tpt_period) == (priv->pshare->rf_ft_var.tpt_period-1)) {
		...
		//printk("kTPT_thread uptime: %d\n", priv->up_time);

	} while (!kthread_should_stop() );

	//printk("kTPT task exiting\n");
	return 0;
}

static int kTPT_task_init(struct rtl8192cd_priv *priv)
{
	//printk("kTPT_task_init!\n");

	kTPT_task = kthread_run(kTPT_thread, (void *)priv, "kTPTd");
	if (!IS_ERR(kTPT_task)){
//		printk("tx_power_tracking Thread create successfully!\n");
		return 0;
	}

//	printk("tx_power_tracking Thread create failed!\n");
	return -1;
}
/*end add by peteryu, 2010/10/20*/
...
static int rtl8192cd_stop_sw(struct rtl8192cd_priv *priv)
{
...
#endif
	{
		...
		if ((LED_TYPE >= LEDTYPE_SW_LINK_TXRX) && (LED_TYPE < LEDTYPE_SW_MAX))
			disable_sw_LED(priv);
#else
		tbs_led_trigger_set(led_wlan, led_off_trig);
#endif
#ifdef __KERNEL__
#ifdef DFS
...
//				if (i <tail || i >= head)
				if( (tail <= head) ? (i >= head || i <tail) :(i <tail && i >= head))
					continue;
...
//		set_fw_reg(priv, 0xf1000001 | ((priv->vap_init_seq + 1)&0xffff)<<8, 0, 0);
		...
		priv->vap_init_seq++;
		if (GET_ROOT(priv)->pmib->miscEntry.func_off && !priv->pshare->shift_vap_idx) {
			priv->pshare->shift_vap_idx = priv->vap_init_seq;
...
//		RTL_W8(_MBIDCAMCFG_, BIT(7) | BIT(6) | ((unsigned char)priv->vap_init_seq & 0x1f));
		...
		RTL_W32(RCR, RTL_R32(RCR) & ~RCR_MBID_EN);
		RTL_W32(RCR, RTL_R32(RCR) | RCR_MBID_EN);
		if (GET_ROOT(priv)->pmib->miscEntry.func_off && priv->pshare->shift_vap_idx == priv->vap_init_seq) {
			for (i=0; i<RTL8192CD_NUM_VWLAN; i++) {
				if (GET_ROOT(priv)->pvap_priv[i]->vap_init_seq > 0 && 
						IS_DRV_OPEN(GET_ROOT(priv)->pvap_priv[i]) &&
						GET_ROOT(priv)->pvap_priv[i] != priv)
				{
					priv->pshare->shift_vap_idx = GET_ROOT(priv)->pvap_priv[i]->vap_init_seq;
...
//bool tbs_bCalledByInitOne;	// for tbs_read_mac calling, 2011-05-19.
...
int rtl8192cd_open(struct net_device *dev)
{
	...
	#define CLK_MANAGE  0xb8000010
			...
			)
		{
			...
			delay_ms(10);
			PCIE_reset_procedure(0, 0, 1, priv->conf_addr);
...
//			printk("%s %d Exception Case to stop Wlan HW\n", __FUNCTION__, __LINE__);
...
//------------------- david+2006-06-30
	...
	priv->expire_to = 60;
#else
	priv->expire_to = (EXPIRETIME > 100)? (EXPIRETIME / 100) : 86400;//EXPIRETIME unit: 10ms, expire_to is time (seconds)
#endif
...
//		priv->ps_ctrl = 0x11;
...
#endif
	{
#if !defined(CONFIG_LEDS_TBS) && !defined(CONFIG_LEDS_TBS_MODULE)		// for HW/SW LED
...
#else
		tbs_led_trigger_set(led_wlan, led_blinking_trig);
#endif
#ifdef DFS
		if (!priv->pmib->dot11DFSEntry.disable_DFS &&
			(OPMODE & WIFI_AP_STATE) &&
			...
			mod_timer(&priv->DFS_timer, jiffies + (5*HZ)); // DFS activated after 5 sec; prevent switching channel due to DFS false alarm
		}
#endif
	...
	{
		if  (priv->pmib->dot11RFEntry.ther == 0){
			priv->pshare->ThermalValue = 0;//let power-tracking's thermalValue re-init
		}
	}
...
*/
	...
	if (OPMODE & WIFI_AP_STATE) {
		...
		OPMODE &= ~(WIFI_STATION_STATE|WIFI_AUTH_SUCCESS|WIFI_ASOC_STATE) ;
	}
#endif    /* WPS2DOTX   */
...
#endif
	{
...
#endif
		{
			...
			//REG32(BSP_GIMR) &= ~ BIT(irq_num);	// clear in free_irq
...
static void MDL_DEVINIT set_mib_default(struct rtl8192cd_priv *priv)
{
	...
	//memcpy(p, "\xfc\xfc\x4c\x81\x86\x86", MACADDRLEN);
	{
		int num, i; 
		unsigned char macaddr[ETH_ALEN] = {0x00, 0x1e, 0xe3, 0x55, 0x66, 0x09};
		if(tbs_read_mac(WLAN_MAC, (IS_ROOT_INTERFACE(priv) ? 0 : (priv->vap_id +1)), macaddr)==0){
#ifdef CONFIG_CREATE_MAC_TELEFONICA		
			macaddr[0] = 0xf8;
			for(num=1; num < 4; num++) {
				for(i=0; i<8 ; i++){
					if(!(macaddr[num]&(1<<i))){
						macaddr[num] = macaddr[num]|(1<<i);
						goto MacChangedDone;
					}
				}
			}
MacChangedDone: 		
#endif
			printk("Wlan MACAddress: %02x:%02x:%02x:%02x:%02x:%02x\n", macaddr[0],macaddr[1],macaddr[2],macaddr[3],macaddr[4], macaddr[5]);
		}	
		memcpy(p, macaddr, ETH_ALEN);		
	}
		...
		if (((wlan_device[wlan_index].type >> TYPE_SHIFT) & TYPE_MASK) == TYPE_PCI_BIOS) {
			...
			if (rc == 0) {
				...
				if (RTL_R8(AFE_XTAL_CTRL) & BIT(1)) {	
					...
					SAVE_INT_AND_CLI(flags);
					//tbs_bCalledByInitOne = TRUE;
					rtl8192cd_open(priv->dev);
					rtl8192cd_close(priv->dev);
					//tbs_bCalledByInitOne = FALSE;
					RESTORE_INT(flags);
				}
			}
#endif
...
#if defined(CONFIG_LEDS_TBS) || defined(CONFIG_LEDS_TBS_MODULE)
	REG32(RTL8192CD_GPIO_PIN_CTRL) |= 0x0F0F0F00;
	REG32(RTL8192CD_GPIO_LEDCFG) &= 0xfff0ffff;
#endif
	for(i = 0; rtl8676_btns[i].name != btn_end; i++)
		{
		if(btn_wps == rtl8676_btns[i].name)
			{
			wps_button.dev = &rtl8676_btns[i];
			break;
			}
		}
	wps_button.dev->ignore= 0;
	wps_button.dev->cur_status = BTN_UP;
	wps_button.dev->get_status = rtl8676_get_button_status;	
	gpioConfigCNR(wps_button.dev->gpio, 1);	/* 设置WPS_BUTTON的GPIO为输入模式 */
	gpioConfig(wps_button.dev->gpio, GPIO_FUNC_INPUT);
	gpioConfigIntr(wps_button.dev->gpio, GPIO_IRQ_EDGE_FALL); /* 开启中断, 设置指定的GPIO的中断触发模式为边沿触发 */
...
#if defined(__LINUX_2_6__) && defined(RX_TASKLET) && !defined(CONFIG_RTL8676)
	...
	for (i=0; i<sizeof(wlan_device)/sizeof(struct _device_info_); i++) {
		if (wlan_device[i].priv) {
			...
			if ((REG32(CLK_MANAGE) & BIT(11)) == 0)
			{
				...
				REG32(CLK_MANAGE) |= BIT(11);
				delay_ms(10);
				PCIE_reset_procedure(0, 0, 1, wlan_device[i].conf_addr);
				setBaseAddressRegister();
			}
#endif
			if ( check_MAC_IO_Enable(priv) ) {
