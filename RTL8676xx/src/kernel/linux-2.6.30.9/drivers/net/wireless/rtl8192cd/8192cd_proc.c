 ...
 *
 *  $Id: 8192cd_proc.c,v 1.8 2011/09/08 09:37:15 cathy Exp $
 *
...
static int rtl8192cd_proc_txdesc0_info(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
{
	return rtl8192cd_proc_txdesc_info(buf, start, offset, length, eof, data, 0);
}


static int rtl8192cd_proc_txdesc1_info(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
{
	return rtl8192cd_proc_txdesc_info(buf, start, offset, length, eof, data, 1);
}


static int rtl8192cd_proc_txdesc2_info(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
{
	return rtl8192cd_proc_txdesc_info(buf, start, offset, length, eof, data, 2);
}


static int rtl8192cd_proc_txdesc3_info(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
{
	return rtl8192cd_proc_txdesc_info(buf, start, offset, length, eof, data, 3);
}


static int rtl8192cd_proc_txdesc4_info(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
{
	return rtl8192cd_proc_txdesc_info(buf, start, offset, length, eof, data, 4);
}


static int rtl8192cd_proc_txdesc5_info(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
{
	return rtl8192cd_proc_txdesc_info(buf, start, offset, length, eof, data, 5);
}
...
#ifdef MERGE_FW
static int rtl8192cd_proc_macphy_reg(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
{
	int len;

	if (data_MACPHY_REG_end <= (data_MACPHY_REG_start + offset + length)) {
		*eof = 1;
		len = data_MACPHY_REG_end - data_MACPHY_REG_start - offset;
	}
	else
		len = length;

	memcpy(buf, &data_MACPHY_REG_start[offset], len);
	*start = buf;
	return len;
}

static int rtl8192cd_proc_phy_reg_pg(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
{
	int len;

	if (data_PHY_REG_PG_end <= (data_PHY_REG_PG_start + offset + length)) {
		*eof = 1;
		len = data_PHY_REG_PG_end - data_PHY_REG_PG_start - offset;
	}
	else
		len = length;

	memcpy(buf, &data_PHY_REG_PG_start[offset], len);
	*start = buf;
	return len;
}

#ifdef TESTCHIP_SUPPORT
static int rtl8192cd_proc_agc_tab(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
{
	int len;

	if (data_AGC_TAB_end <= (data_AGC_TAB_start + offset + length)) {
		*eof = 1;
		len = data_AGC_TAB_end - data_AGC_TAB_start - offset;
	}
	else
		len = length;

	memcpy(buf, &data_AGC_TAB_start[offset], len);
	*start = buf;
	return len;
}
static int rtl8192cd_proc_phy_reg_1t(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
{
	int len;

	if (data_PHY_REG_1T_end <= (data_PHY_REG_1T_start + offset + length)) {
		*eof = 1;
		len = data_PHY_REG_1T_end - data_PHY_REG_1T_start - offset;
	}
	else
		len = length;

	memcpy(buf, &data_PHY_REG_1T_start[offset], len);
	*start = buf;
	return len;
}


static int rtl8192cd_proc_radio_a_1t(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
{
	int len;

	if (data_radio_a_1T_end <= (data_radio_a_1T_start + offset + length)) {
		*eof = 1;
		len = data_radio_a_1T_end - data_radio_a_1T_start - offset;
	}
	else
		len = length;

	memcpy(buf, &data_radio_a_1T_start[offset], len);
	*start = buf;
	return len;
}

static int rtl8192cd_proc_radio_a(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
{
	int len;

	if (data_radio_a_2T_end <= (data_radio_a_2T_start + offset + length)) {
		*eof = 1;
		len = data_radio_a_2T_end - data_radio_a_2T_start - offset;
	}
	else
		len = length;

	memcpy(buf, &data_radio_a_2T_start[offset], len);
	*start = buf;
	return len;
}

static int rtl8192cd_proc_radio_b(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
{
	int len;

	if (data_radio_b_2T_end <= (data_radio_b_2T_start + offset + length)) {
		*eof = 1;
		len = data_radio_b_2T_end - data_radio_b_2T_start - offset;
	}
	else
		len = length;

	memcpy(buf, &data_radio_b_2T_start[offset], len);
	*start = buf;
	return len;
}

static int rtl8192cd_proc_phy_reg(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
{
	int len;

	if (data_PHY_REG_2T_end <= (data_PHY_REG_2T_start + offset + length)) {
		*eof = 1;
		len = data_PHY_REG_2T_end - data_PHY_REG_2T_start - offset;
	}
	else
		len = length;

	memcpy(buf, &data_PHY_REG_2T_start[offset], len);
	*start = buf;
	return len;
}
...
static int rtl8192cd_proc_agc_tab_n(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
{
	int len;

	if (data_AGC_TAB_n_end <= (data_AGC_TAB_n_start + offset + length)) {
		*eof = 1;
		len = data_AGC_TAB_n_end - data_AGC_TAB_n_start - offset;
	}
	else
		len = length;

	memcpy(buf, &data_AGC_TAB_n_start[offset], len);
	*start = buf;
	return len;
}


static int rtl8192cd_proc_phy_reg_2T_n(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
{
	int len;

	if (data_PHY_REG_2T_n_end <= (data_PHY_REG_2T_n_start + offset + length)) {
		*eof = 1;
		len = data_PHY_REG_2T_n_end - data_PHY_REG_2T_n_start - offset;
	}
	else
		len = length;

	memcpy(buf, &data_PHY_REG_2T_n_start[offset], len);
	*start = buf;
	return len;
}

static int rtl8192cd_proc_radio_a_2T_n(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
{
	int len;

	if (data_radio_a_2T_n_end <= (data_radio_a_2T_n_start + offset + length)) {
		*eof = 1;
		len = data_radio_a_2T_n_end - data_radio_a_2T_n_start - offset;
	}
	else
		len = length;

	memcpy(buf, &data_radio_a_2T_n_start[offset], len);
	*start = buf;
	return len;
}


static int rtl8192cd_proc_radio_b_2T_n(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
{
	int len;

	if (data_radio_b_2T_n_end <= (data_radio_b_2T_n_start + offset + length)) {
		*eof = 1;
		len = data_radio_b_2T_n_end - data_radio_b_2T_n_start - offset;
	}
	else
		len = length;

	memcpy(buf, &data_radio_b_2T_n_start[offset], len);
	*start = buf;
	return len;
}

static int rtl8192cd_proc_phy_reg_1t_n(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
{
	int len;

	if (data_PHY_REG_1T_n_end <= (data_PHY_REG_1T_n_start + offset + length)) {
		*eof = 1;
		len = data_PHY_REG_1T_n_end - data_PHY_REG_1T_n_start - offset;
	}
	else
		len = length;

	memcpy(buf, &data_PHY_REG_1T_n_start[offset], len);
	*start = buf;
	return len;
}


static int rtl8192cd_proc_radio_a_1t_n(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
{
	int len;

	if (data_radio_a_1T_n_end <= (data_radio_a_1T_n_start + offset + length)) {
		*eof = 1;
		len = data_radio_a_1T_n_end - data_radio_a_1T_n_start - offset;
	}
	else
		len = length;

	memcpy(buf, &data_radio_a_1T_n_start[offset], len);
	*start = buf;
	return len;
}
...
/*
#ifdef MP_TEST
static int rtl8192cd_proc_phy_reg_MP(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
{
	int pos = 0;
	char tmpbuf[256];
			...
			int length, int *eof, void *data)
{
	...
	size = rtl8192cd_proc_mib_rsnie(buf+len, start, offset, length, eof, data);
	CHECK_LEN;
...
static int dump_one_stainfo(int num, struct stat_info *pstat, char *buf, char **start,
			off_t offset, int length, int *eof, void *data, char istab)
{
	int pos = 0, idx = 0;
	unsigned int m, n;
	char tmp[32];
	...
	PRINT_SINGL_ARG("    dz_queue.qlen:    ", pstat->dz_queue.qlen, "%lu");
	PRINT_ONE("    rssi: ", "%s", 0);
	...
	PRINT_SINGL_ARG("    leave: ", (pstat->leave ? "yes" : "no"), "%s");
	CHECK_LEN_B;
#endif
			...
			int length, int *eof, void *data)
{
	...
	PRINT_ARRAY_ARG("    BSSID: ",priv->pmib->dot11StationConfigEntry.dot11Bssid, "%02x", 6);
			...
			int length, int *eof, void *data)
{
	...
	PRINT_SINGL_ARG("    rts_txtime_enlarge: ", priv->pshare->rf_ft_var.rts_txtime_enlarge, "%d");
...
void MDL_DEVINIT rtl8192cd_proc_init(struct net_device *dev)
{
	...
	if ( create_proc_read_entry ("mib_rsnie", 0644, rtl8192cd_proc_root,
			rtl8192cd_proc_mib_rsnie, (void *)dev) == NULL ) {
...
#ifdef AutoTestSupport
	if ((res=create_proc_read_entry("SS_Result", 0644, rtl8192cd_proc_root,
	...
	!defined(CONFIG_RTL8196C_CLIENT_ONLY) && !defined(CONFIG_RTL8196C_KLD) && !defined(AutoTestSupport)//remove proc files in AP package
		if ( create_proc_read_entry ("txdesc0", 0644, rtl8192cd_proc_root,
				rtl8192cd_proc_txdesc0_info, (void *)dev) == NULL ) {
			printk("create proc txdesc0 failed!\n");
			return;
		}
		if ( create_proc_read_entry ("txdesc1", 0644, rtl8192cd_proc_root,
				rtl8192cd_proc_txdesc1_info, (void *)dev) == NULL ) {
			printk("create proc txdesc1 failed!\n");
			return;
		}
		if ( create_proc_read_entry ("txdesc2", 0644, rtl8192cd_proc_root,
				rtl8192cd_proc_txdesc2_info, (void *)dev) == NULL ) {
			printk("create proc txdesc2 failed!\n");
			return;
		}
		if ( create_proc_read_entry ("txdesc3", 0644, rtl8192cd_proc_root,
				rtl8192cd_proc_txdesc3_info, (void *)dev) == NULL ) {
			printk("create proc txdesc3 failed!\n");
			return;
		}
		if ( create_proc_read_entry ("txdesc4", 0644, rtl8192cd_proc_root,
				rtl8192cd_proc_txdesc4_info, (void *)dev) == NULL ) {
			printk("create proc txdesc4 failed!\n");
			return;
		}
		if ( create_proc_read_entry ("txdesc5", 0644, rtl8192cd_proc_root,
				rtl8192cd_proc_txdesc5_info, (void *)dev) == NULL ) {
		...
		/*if ( create_proc_read_entry ("MACPHY_REG.txt", 0644, rtl8192cd_proc_root,
				rtl8192cd_proc_macphy_reg, (void *)dev) == NULL ) {
			printk("create proc MACPHY_REG.txt failed!\n");
			return;
		}*/
#ifdef TESTCHIP_SUPPORT
		if ( create_proc_read_entry ("AGC_TAB.txt", 0644, rtl8192cd_proc_root,
				rtl8192cd_proc_agc_tab, (void *)dev) == NULL ) {
			printk("create proc AGC_TAB.txt failed!\n");
			return;
		}
		if ( create_proc_read_entry ("PHY_REG_2T.txt", 0644, rtl8192cd_proc_root,
				rtl8192cd_proc_phy_reg, (void *)dev) == NULL ) {
			printk("create proc phy_reg_2T.txt failed!\n");
			return;
		}
		if ( create_proc_read_entry ("PHY_REG_1T.txt", 0644, rtl8192cd_proc_root,
					rtl8192cd_proc_phy_reg_1t, (void *)dev) == NULL ) {
			printk("create proc PHY_REG_1T.txt failed!\n");
			return;
		}
		if ( create_proc_read_entry ("radio_a_1T.txt", 0644, rtl8192cd_proc_root,
				rtl8192cd_proc_radio_a_1t, (void *)dev) == NULL ) {
			printk("create proc radio_a_1T.txt failed!\n");
			return;
		}
		if ( create_proc_read_entry ("radio_a_2T.txt", 0644, rtl8192cd_proc_root,
				rtl8192cd_proc_radio_a, (void *)dev) == NULL ) {
			printk("create proc radio_a.txt failed!\n");
			return;
		}
		if ( create_proc_read_entry ("radio_b_2T.txt", 0644, rtl8192cd_proc_root,
				rtl8192cd_proc_radio_b, (void *)dev) == NULL ) {
			printk("create proc radio_b_2T.txt failed!\n");
			return;
		}
		...
		if ( create_proc_read_entry ("PHY_REG_PG.txt", 0644, rtl8192cd_proc_root,
				rtl8192cd_proc_phy_reg_pg, (void *)dev) == NULL ) {
			printk("create proc phy_reg_pg.txt failed!\n");
			return;
		}

		if ( create_proc_read_entry ("AGC_TAB_n.txt", 0644, rtl8192cd_proc_root,
				rtl8192cd_proc_agc_tab_n, (void *)dev) == NULL ) {
			printk("create proc AGC_TAB_n.txt failed!\n");
			return;
		}
		if ( create_proc_read_entry ("PHY_REG_2T_n.txt", 0644, rtl8192cd_proc_root,
				rtl8192cd_proc_phy_reg_2T_n, (void *)dev) == NULL ) {
			printk("create proc phy_reg_2T_n.txt failed!\n");
			return;
		}
		if ( create_proc_read_entry ("PHY_REG_1T_n.txt", 0644, rtl8192cd_proc_root,
					rtl8192cd_proc_phy_reg_1t_n, (void *)dev) == NULL ) {
			printk("create proc PHY_REG_1T_n.txt failed!\n");
			return;
		}
		if ( create_proc_read_entry ("radio_a_2T_n.txt", 0644, rtl8192cd_proc_root,
				rtl8192cd_proc_radio_a_2T_n, (void *)dev) == NULL ) {
			printk("create proc radio_a_2T_n.txt failed!\n");
			return;
		}
		if ( create_proc_read_entry ("radio_b_2T_n.txt", 0644, rtl8192cd_proc_root,
				rtl8192cd_proc_radio_b_2T_n, (void *)dev) == NULL ) {
			printk("create proc radio_b_2T.txt failed!\n");
			return;
		}
		if ( create_proc_read_entry ("radio_a_1T_n.txt", 0644, rtl8192cd_proc_root,
				rtl8192cd_proc_radio_a_1t_n, (void *)dev) == NULL ) {
			printk("create proc radio_a_1T.txt failed!\n");
			return;
		}
	/*
#ifdef MP_TEST
		if ( create_proc_read_entry ("phy_reg_MP.txt", 0644, rtl8192cd_proc_root,
				rtl8192cd_proc_phy_reg_MP, (void *)dev) == NULL ) {
	...
	*/
		...
		res->data = (void *)dev;
#endif
#endif //!defined(CONFIG_RTL8196C_AP_ROOT) || !defined(CONFIG_RTL8196C_CLIENT_ONLY)
	}
...
void __devexit rtl8192cd_proc_remove (struct net_device *dev)
{
	...
	if (rtl8192cd_proc_root != NULL) {
		...
		remove_proc_entry( "mib_rsnie", rtl8192cd_proc_root );
		...
		{
#if !defined(CONFIG_RTL8196C_AP_ROOT) && !defined(CONFIG_RTL8198_AP_ROOT) \
	&& !defined(CONFIG_RTL8196C_CLIENT_ONLY) && !defined(CONFIG_RTL8196C_KLD) && !defined(AutoTestSupport)//remove proc file in AP package
			remove_proc_entry( "txdesc0", rtl8192cd_proc_root );
			remove_proc_entry( "txdesc1", rtl8192cd_proc_root );
			remove_proc_entry( "txdesc2", rtl8192cd_proc_root );
			remove_proc_entry( "txdesc3", rtl8192cd_proc_root );
			remove_proc_entry( "txdesc4", rtl8192cd_proc_root );
			remove_proc_entry( "txdesc5", rtl8192cd_proc_root );
...
#endif
#ifdef MERGE_FW
			remove_proc_entry( "MACPHY_REG.txt", rtl8192cd_proc_root );
			remove_proc_entry( "AGC_TAB.txt", rtl8192cd_proc_root );
			remove_proc_entry( "PHY_REG.txt", rtl8192cd_proc_root );
			remove_proc_entry( "PHY_REG_PG.txt", rtl8192cd_proc_root );
			remove_proc_entry( "radio_a.txt", rtl8192cd_proc_root );
			remove_proc_entry( "radio_b.txt", rtl8192cd_proc_root );
			remove_proc_entry( "PHY_REG_1T1R.txt", rtl8192cd_proc_root );
			remove_proc_entry( "radio_a_1T1R.txt", rtl8192cd_proc_root );
			remove_proc_entry( "radio_b_1T1R.txt", rtl8192cd_proc_root );
	/*
#ifdef MP_TEST
			remove_proc_entry( "phy_reg_MP.txt", rtl8192cd_proc_root );
#endif
	*/
#endif
...
static struct _proc_table_ proc_table[] =
{
	...
	{"mib_rsnie",			rtl8192cd_proc_mib_rsnie},
	{"mib_bssdesc",			rtl8192cd_proc_mib_bssdesc},
	{"sta_info",			rtl8192cd_proc_stainfo},
	{"sta_keyinfo",			rtl8192cd_proc_sta_keyinfo},
	{"txdesc0",				rtl8192cd_proc_txdesc0_info},
	{"txdesc1",				rtl8192cd_proc_txdesc1_info},
	{"txdesc2",				rtl8192cd_proc_txdesc2_info},
	{"txdesc3",				rtl8192cd_proc_txdesc3_info},
	{"txdesc4",				rtl8192cd_proc_txdesc4_info},
	{"txdesc5",				rtl8192cd_proc_txdesc5_info},
