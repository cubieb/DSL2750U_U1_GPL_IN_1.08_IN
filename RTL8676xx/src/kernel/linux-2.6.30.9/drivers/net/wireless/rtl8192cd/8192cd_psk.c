 ...
 *
 *  $Id: 8192cd_psk.c,v 1.4 2011/03/15 02:27:51 yachang Exp $
 *
...
#define RESEND_TIME						(HZ)//100	// in 10ms, HZ = 1 sec
...
#ifdef CLIENT_MODE
static void ClientSendEAPOL(struct rtl8192cd_priv *priv, struct stat_info *pstat, int resend);
#endif
			...
			if (pstat->wpa_sta_info->state == PSK_STATE_PTKINITDONE &&
				...
				if (priv->wpa_global_info->GKeyDoneStations==0 && !priv->wpa_global_info->GkeyReady) {
					...
					if (priv->pmib->dot1180211AuthEntry.dot11GKRekeyTime)
						mod_timer(&priv->wpa_global_info->GKRekeyTimer, jiffies + priv->pmib->dot1180211AuthEntry.dot11GKRekeyTime*HZ);
...
//				global->portSecure = TRUE;
				...
				if (pGblInfo->GKeyDoneStations==0 && !pGblInfo->GkeyReady) {
					...
					// start groupkey rekey timer
					if (priv->pmib->dot1180211AuthEntry.dot11GKRekeyTime)
						mod_timer(&pGblInfo->GKRekeyTimer, jiffies + priv->pmib->dot1180211AuthEntry.dot11GKRekeyTime*HZ);
				}
#if 0
				if (global->RSNVariable.PMKCached) {
...
void psk_indicate_evt(struct rtl8192cd_priv *priv, int id, unsigned char *mac, unsigned char *msg, int len)
{
	struct stat_info *pstat;
