...
#ifdef RTL8672_USB_HOST_TX_SHOULD_CHECK
static int ehci_rtl8672_out_64_check(struct urb *urb)
{
	int retval = 0;
...
static int ehci_rtl8672_out_1K_boundary_check(struct urb *urb, bool setup_pkt)
{
	int retval = 0;
...
#endif//RTL8672_USB_HOST_TX_SHOULD_CHECK

static int ehci_rtl8672_in_alignment_check(struct urb *urb)
{
	int retval = 0;
	unsigned char aligned;
...
static void ehci_rtl8672_dma_check(struct usb_hcd *hcd, struct urb *urb)
{
	#define RETRYCNT	5
	int retval = 0;
	...
	if(usb_pipecontrol(urb->pipe)) {
		...
		do {
			...
			retval = ehci_rtl8672_out_1K_boundary_check(urb, 1);
			if(retval < 0) {
...
parse_data_pkt:
	...
	if(usb_pipeout(urb->pipe)) {
		...
		do {
			...
			retval = ehci_rtl8672_out_64_check(urb);
			if(retval < 0) {
				printk("%s %d fail! retval=%d\n", __func__, __LINE__, retval);
				goto patch_data_pkt_fail; 
			}

			retval = ehci_rtl8672_out_1K_boundary_check(urb, 0);
			if(retval < 0) {
				printk("%s %d fail! retval=%d\n", __func__, __LINE__, retval);
				goto patch_data_pkt_fail;
			}
		...
		retval = ehci_rtl8672_in_alignment_check(urb);
...
#if 0
static void ehci_rtl8672_dma_giveback_abnormal(struct usb_hcd *hcd, struct urb *urb)
{
	if(unlikely(urb->temp_setup_buf)) {
		if(usb_pipecontrol(urb->pipe))
...
static void ehci_rtl8672_dma_giveback_normal(struct usb_hcd *hcd, struct urb *urb)
{
	if(unlikely(urb->temp_setup_buf)) {
...
void ehci_rtl8672_dma_process(struct usb_hcd *hcd, struct urb *urb, unsigned char option)
{
	switch(option) {
		case 1:
			ehci_rtl8672_dma_check(hcd, urb);
			break;
		#if 0
		case 2:
			ehci_rtl8672_dma_giveback_abnormal(hcd, urb);
			break;
		#endif
		case 3:
			ehci_rtl8672_dma_giveback_normal(hcd, urb);
			break;
		default:
			break;
	}
...
// For Port-1 
	...
	if(IS_0412) {
		...
		if(ipSel & BSP_EN_USB_PHY1) {
			ehci_phy_write(1, 0xE2, 0x9A);
			ehci_phy_write(1, 0xE5, 0x9D);
			if(!(ipSel & BSP_EN_USB_PHY0))
				ehci_phy_write(0, 0xF5, 0xD1);
			ehci_phy_write(1, 0xF5, 0xD1);
		}
		else {
