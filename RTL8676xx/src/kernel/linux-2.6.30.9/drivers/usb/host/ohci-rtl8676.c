...
static int ohci_rtl8676_out_64_check(struct urb *urb)
{
	int retval = 0;
...
static int ohci_rtl8676_out_1K_boundary_check(struct urb *urb, bool setup_pkt)
{
	int retval = 0;
...
static int ohci_rtl8676_in_alignment_check(struct urb *urb)
{
	int retval = 0;
	unsigned char aligned;
...
static void ohci_rtl8676_dma_check(struct usb_hcd *hcd, struct urb *urb)
{
	#define RETRYCNT	5
	int retval = 0;
	...
	if(usb_pipecontrol(urb->pipe)) {
		...
		do {
			...
			retval = ohci_rtl8676_out_1K_boundary_check(urb, 1);
			if(retval < 0) {
...
parse_data_pkt:
	...
	if(usb_pipeout(urb->pipe)) {
		...
		do {
			...
			retval = ohci_rtl8676_out_64_check(urb);
			if(retval < 0) {
				printk("%s %d fail! retval=%d\n", __func__, __LINE__, retval);
				goto patch_data_pkt_fail; 
			}

			retval = ohci_rtl8676_out_1K_boundary_check(urb, 0);
			if(retval < 0) {
				printk("%s %d fail! retval=%d\n", __func__, __LINE__, retval);
				goto patch_data_pkt_fail;
			}
		...
		retval = ohci_rtl8676_in_alignment_check(urb);
...
#if 0
static void ohci_rtl8676_dma_giveback_abnormal(struct usb_hcd *hcd, struct urb *urb)
{
	if(unlikely(urb->temp_setup_buf)) {
		if(usb_pipecontrol(urb->pipe))
...
static void ohci_rtl8676_dma_giveback_normal(struct usb_hcd *hcd, struct urb *urb)
{
	if(unlikely(urb->temp_setup_buf)) {
...
static void ohci_rtl8676_dma_process(struct usb_hcd *hcd, struct urb *urb, unsigned char option)
{
	switch(option) {
		case 1:
			ohci_rtl8676_dma_check(hcd, urb);
			break;
		#if 0
		case 2:
			ohci_rtl8676_dma_giveback_abnormal(hcd, urb);
			break;
		#endif
		case 3:
			ohci_rtl8676_dma_giveback_normal(hcd, urb);
			break;
		default:
			break;
	}
...
struct hc_driver ohci_hc_driver_rtl8676 = {
...
#else
	.rtl8676_dma_process = ohci_rtl8676_dma_process,
#endif	//CONFIG_USB_EHCI_HCD
#endif	//CONFIG_USB_PATCH_RTL8676
...
int ohci_usb_hcd_probe_rtl8676 (struct hc_driver *driver, struct platform_device *pdev)
{
	struct usb_hcd		*hcd;
	struct resource *res;
	int			retval; 	
...
int ohci_rtl8676_drv_probe (struct platform_device *pdev)
{
	return ohci_usb_hcd_probe_rtl8676 (&ohci_hc_driver_rtl8676, pdev);
}

static int ohci_rtl8676_drv_remove (struct platform_device *pdev)
{
	struct usb_hcd		*hcd;
 ...
 struct platform_driver platform_driver_rtl8676_ohci = {
	.probe = ohci_rtl8676_drv_probe,
	.remove = ohci_rtl8676_drv_remove,
...
#define RTL8676_OHCI_BASE	0xb8020000

static struct resource rtl8676_ohci_resources[] = {
	{
		.start	= RTL8676_OHCI_BASE,
		.end		= RTL8676_OHCI_BASE + 0x00000FFF,
...
static struct platform_device rtl8676_ohci = {
	.name			= "rtl8676-ohci",
	.id			= -1,
	.dev = {
		.release		= usb_release,
	...
	.num_resources	= ARRAY_SIZE(rtl8676_ohci_resources),
	.resource		= rtl8676_ohci_resources,
};

static int __init ohci_hcd_rtl8676_init (void) 
{
	int retval;
	...
	retval = platform_device_register(&rtl8676_ohci);
	if (retval < 0)
		return retval;
	retval = platform_driver_register(&platform_driver_rtl8676_ohci);
	if (retval < 0) {
		platform_driver_unregister(&platform_driver_rtl8676_ohci);
	}
	return retval;
}
module_init (ohci_hcd_rtl8676_init);

/*-------------------------------------------------------------------------*/

static void __exit ohci_hcd_rtl8676_cleanup (void) 
{	
	//pci_unregister_driver (&ohci_pci_driver);
	platform_driver_unregister(&platform_driver_rtl8676_ohci);
	platform_device_unregister(&rtl8676_ohci);
}
module_exit (ohci_hcd_rtl8676_cleanup);
