...
static const struct quirk_printer_struct quirk_printers[] = {
	...
	{ 0x03f0, 0x6104, USBLP_QUIRK_BIDIR }, /* HP Deskjet 5652 */
	{ 0x03f0, 0x8104, USBLP_QUIRK_BIDIR }, /* HP Deskjet 57xx */
	{ 0x03f0, 0x2b17, USBLP_QUIRK_BIDIR }, /* hP LaserJet 1020 */
	{ 0x03f0, 0x4b17, USBLP_QUIRK_BIDIR }, /* hP LaserJet P1008 */
...
#define	PRINTER_HOTPLUG
#ifdef 	PRINTER_HOTPLUG
#define	PRINTER_NAME	"usblp"
#define	BUFFER_SIZE	1024	/* should be enough memory for the env */
#define	NUM_ENVP	32	/* number of env pointers */
#define	HOTPLUG_PATH	"/sbin/hotplug" 
enum
{
	USBLP_REMOVE_ACTION,
	USBLP_ADD_ACTION	
...
static void usblp_hotplug(struct usblp *usblp, int action)
{
	int i = 0;
	char *argv [3];
	char **envp = NULL;
	...
	argv [2] = NULL;
       ...
       usblp->dev->descriptor.idVendor = le16_to_cpu(usblp->dev->descriptor.idVendor);
       usblp->dev->descriptor.idProduct = le16_to_cpu(usblp->dev->descriptor.idProduct);
	...
	envp [i++] = scratch;
	if (action==USBLP_ADD_ACTION)
		scratch += sprintf(scratch, "ACTION=%s", "add") + 1;
	else
		scratch += sprintf(scratch, "ACTION=%s", "remove") + 1;

	envp [i++] = scratch;
	scratch += sprintf (scratch, "VENDORID=%#x", usblp->dev->descriptor.idVendor) + 1;

	envp [i++] = scratch;
	scratch += sprintf (scratch, "PRODUCTID=%#x", usblp->dev->descriptor.idProduct) + 1;
...
{
#ifdef PRINTER_HOTPLUG
	usblp_hotplug(usblp, USBLP_REMOVE_ACTION);
#endif
...
static unsigned int usblp_poll(struct file *file, struct poll_table_struct *wait)
{
	...
	tbs_led_trigger_set(led_usb_green,led_on_trig);
	#endif
...
#ifdef PRINTER_HOTPLUG
	usblp_hotplug(usblp, USBLP_ADD_ACTION);	
#endif	
...
#if defined(CONFIG_LEDS_TBS) || defined(CONFIG_LEDS_TBS_MODULE)
	tbs_led_trigger_set(led_usb_green,led_off_trig);
#endif
