...
#ifdef CONFIG_USB_RTL8676_USBCTL
/* command for ioctl */
...
#endif //CONFIG_USB_RTL8676_USBCTL
...
unsigned char usb_mac_identify(unsigned char port)
{
	unsigned char is_EHCI = 0;
	...
	*/
	...
	if (IS_RTL8676) {
		...
		if (port == 0 && (ipSel & BSP_EN_USB_HOST))
			is_EHCI = 1;
		else if (port == 1 && (ipSel & BSP_EN_USB_OTG))
			is_EHCI = 0;
		else if (port == 1 && (ipSel & BSP_EN_USB_HOST))
			is_EHCI = 1;
		else
			printk("%s: incorrect ipSel = 0x%08x\n", __func__, ipSel);
	}
	else if (!IS_6085 && (!port || !(pinsr & BSP_EN_USB_OTG)))
		is_EHCI = 1;
...
static int proc_usbphy(struct dev_state *ps, void __user *arg)
{
	...
	is_EHCI = usb_mac_identify(phy.port);
...
#else
		printk("OTG driver is not configured!\n");
#endif	//CONFIG_DWC_OTG
	}
	else {
...
#else
		printk("EHCI driver is not configured!\n");
...
{
	#define EHCI_BASE	0xb8021000
	#define EHCI_PORTSC0	(EHCI_BASE + 0x54)
	#define EHCI_PORTSC1	(EHCI_BASE + 0x58)
	#define OTG_BASE	0xb8030000
	#define OTG_HPRT	(OTG_BASE + 0x440)
...
#if !defined (CONFIG_USB_EHCI_HCD) && !defined (CONFIG_USB_OTG_HOST_RTL8676)
	printk("USB test packet for host mode, please build in USB host first!\n");
	return 0;
#endif

	is_EHCI = usb_mac_identify(port);
	...
	if (!is_EHCI) {
		...
		if (!port)
			...
			return -1;
		printk("EHCI port%d is sending test packet now!\n\n", port);
	}
...
{
#ifdef CONFIG_USB_EHCI_HCD
	extern unsigned int ehci_ctrl_stage;
#endif //CONFIG_USB_EHCI_HCD
	struct usb_device *dev = ps->dev;
	...
	if (dev->bus->root_hub->speed != USB_SPEED_HIGH || dev->bus->root_hub->maxchild != 2) {
		printk("root hub is not EHCI\n");
		return -EINVAL;
	}
