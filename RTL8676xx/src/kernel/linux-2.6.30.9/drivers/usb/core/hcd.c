			   ...
			   gfp_t mem_flags)
{
	...
	if(hcd->self.uses_dma && hcd->driver->rtl8676_dma_process) {
			hcd->driver->rtl8676_dma_process(hcd, urb, 1);
	}
...
static void unmap_urb_for_dma(struct usb_hcd *hcd, struct urb *urb)
{
	...
	#ifdef CONFIG_USB_PATCH_RTL8676
	if(hcd->self.uses_dma && hcd->driver->rtl8676_dma_process)
		hcd->driver->rtl8676_dma_process(hcd, urb, 3);
	#endif	//CONFIG_USB_PATCH_RTL8676
}
...
void usb_hcd_giveback_urb(struct usb_hcd *hcd, struct urb *urb, int status)
{
	...
	if(!usb_get_root_hub_port(urb->dev,0))
		usb_act_func(LED_USB_0, USB_LED_INPROG_BLINK); 
	#endif
	#ifdef CONFIG_SW_USB_LED1 //2010-11-19 paula
	if(usb_get_root_hub_port(urb->dev,0)==1)
		usb_act_func(LED_USB_1, USB_LED_INPROG_BLINK); 
	#endif
