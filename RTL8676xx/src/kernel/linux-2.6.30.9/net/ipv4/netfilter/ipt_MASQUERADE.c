...
#ifdef CONFIG_RTL_HARDWARE_NAT  
unsigned int check_masq_if(char *ifname){
#ifdef CONFIG_RTL_MULTI_ETH_WAN
	int i;

	for(i=0; i<IP_NUMBER; i++){
		if((masq_if[i][0]!='\0') && (memcmp(masq_if[i], ifname,IFNAMSIZ)==0)){
			printk("masq_if is %s\n",masq_if[i]);
			return 1;
		}
	}
#else
	if((masq_if[0]!='\0') && (memcmp(masq_if, ifname, IFNAMSIZ)==0)){
		printk("masq_if is %s\n",masq_if);
		return 1;
	}
#endif
