...
/* 
  0, full conenat
  1, restricted conenat
  2, port restricted conenat
*/
static unsigned int conenat_type = 0;
	      ...
	      struct nf_conn *ct, enum ip_conntrack_info ctinfo)
{
	...
	/*
	   IP_CT_DIR_REPLY
	   0 - full,  *:* -> natip:natport -> lanip:lanport
	   1 - restricted, wanip:* -> natip:natport -> lanip:lanport
	   2 - port restricted, wanip:wanport -> natip:natport -> lanip:lanport
	 */
	...
	//设置期待连接的FULLCONENAT标识
	exp->flags |= NF_CT_EXPECT_FULLCONENAT;
...
/*
		...
		else
		{
			...
			minport = mr->range[0].min.udp.port == 0 ?
			    ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u.udp.
			    port : mr->range[0].min.udp.port;
			     ...
			     newport <= tmpport; newport++)
			{
				...
				tuple.src.u.udp.port =
				    ct->tuplehash[IP_CT_DIR_REPLY].tuple.src.u.
				    udp.port;
				tuple.src.l3num =
				...
				tuple.dst.protonum =
				    ...
				    protonum;
				/*fix bug could not find expect*/
				//exp = __nf_ct_expect_find(net, &tuple);
				exp = find_fullcone_exp(ct);	
				if (!exp)
				{
					pr_debug
					    ("new mapping: %pI4:%hu -> %pI4:%hu\n",
		...
		if (ret == NF_ACCEPT)
		{
			...
			if (help == NULL)
			{
				...
				rcu_assign_pointer(help->helper,
					   	&nf_conntrack_helper_cone_nat);
			}
			/*fix bug that sip alg helper replaced by fullcone nat helper*/
			#if 0
			else
			{
...
static int proc_conenat_write(struct file *filp, const char __user * buf,
			      unsigned long len, void *data)
{
	...
	if (len > 255)
	{
		printk("Error: the value must be between 0-2\n");
		return len;
	}
