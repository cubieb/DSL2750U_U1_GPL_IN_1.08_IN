			...
			)
		{
			...
			{				
				...
				if(dir == IP_CT_DIR_ORIGINAL)
					rtl865x_ADD_dynamic_acl_L34_permit(
					&ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple,&ct->tuplehash[IP_CT_DIR_REPLY].tuple,skb->dst);	
				else
					rtl865x_ADD_dynamic_acl_L34_permit(
					&ct->tuplehash[IP_CT_DIR_REPLY].tuple,&ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple,skb->dst);	
				#endif				
			...
			#ifdef CONFIG_RTL867X_IPTABLES_FAST_PATH	
			if(!test_bit(IPS_8676_IPFastPath_BIT, &ct->status) && FastPath_Enabled())
			{				
