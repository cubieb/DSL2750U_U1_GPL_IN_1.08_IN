...
#if defined(CONFIG_RTL867X_IPTABLES_FAST_PATH)
                     if (FastPath_Enabled()){
                     	//printk("%x  %x  %s  %x!!!\n",cfg->fc_dst ,inet_make_mask(cfg->fc_dst_len),(uint8 *)fi->fib_dev->name,fa->fa_type);
				rtl867x_modifyRoute(cfg->fc_dst ? cfg->fc_dst : 0, inet_make_mask(cfg->fc_dst_len),cfg->fc_gw ? cfg->fc_gw : 0, (__u8 *)fi->fib_dev->name, RT_NONE,cfg->fc_type);
                     }
#endif
...
#if defined(CONFIG_RTL867X_IPTABLES_FAST_PATH)
        //if (FastPath_Enabled())
	rtl867x_addRoute(cfg->fc_dst ? cfg->fc_dst : 0, inet_make_mask(cfg->fc_dst_len),cfg->fc_gw ? cfg->fc_gw : 0, (__u8 *)fi->fib_dev->name, RT_NONE,cfg->fc_type);
#endif
...
static int fn_hash_delete(struct fib_table *tb, struct fib_config *cfg)
{
	...
	rtl867x_delRoute(cfg->fc_dst ? cfg->fc_dst : 0, inet_make_mask(cfg->fc_dst_len));
#endif	
	...
	hlist_for_each_entry_safe(f, node, n, head, fn_hash) {
		...
		list_for_each_entry_safe(fa, fa_node, &f->fn_alias, fa_list) {
...
#if defined(CONFIG_RTL867X_IPTABLES_FAST_PATH)
				rtl867x_delRoute(f->fn_key, fz->fz_mask);
#endif	
			...
			if (tb_id==RT_TABLE_MAIN) {
				...
				ipDst =f->fn_key;
				ipMask = inet_make_mask(fz->fz_order);
