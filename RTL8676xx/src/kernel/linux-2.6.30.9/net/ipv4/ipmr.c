...
static int ipmr_mfc_add(struct net *net, struct mfcctl *mfc, int mrtsock)
{
	...
	if (c != NULL) {
		write_lock_bh(&mrt_lock);
#ifndef CONFIG_IGMP_PROXY_MULTIWAN
		c->mfc_parent = mfc->mfcc_parent;
#else 
		c->mfc_parent_other[c->mfc_parent_num]=mfc->mfcc_parent;
		c->mfc_parent_num++;
#endif
		c->mfc_parent = mfc->mfcc_parent;
	...
	c->mfc_parent_other[0]=mfc->mfcc_parent;
	c->mfc_parent_num++; 
#endif
	ipmr_update_thresholds(c, mfc->mfcc_ttls);
	if (!mrtsock)
...
#ifdef CONFIG_RTL867X_MCAST_FAST_PATH
// Modified from ipmr_queue_xmit()
/*
 *	Processing handlers for ip_mr_forward_fast
 */

static void ipmr_queue_xmit_fast(struct sk_buff *skb, struct mfc_cache *c, int vifi)
{
	struct net *net = mfc_net(c);
	const struct iphdr *iph = ip_hdr(skb);
	struct vif_device *vif = &net->ipv4.vif_table[vifi];
	...
	for (ct = cache->mfc_un.res.maxvif-1; ct >= cache->mfc_un.res.minvif; ct--) {
		if (ip_hdr(skb)->ttl > cache->mfc_un.res.ttls[ct]) {
			if (psend != -1) {
				...
				if (skb2)
					ipmr_queue_xmit_fast(skb2, cache, psend);
			}
			psend = ct;
		}
	}
	if (psend != -1) {
			ipmr_queue_xmit_fast(skb, cache, psend);
			return 1;
	}
...
#endif // of CONFIG_RTL867X_MCAST_FAST_PATH
...
static int ip_mr_forward(struct sk_buff *skb, struct mfc_cache *cache, int local)
{
	...
	for(i=0;i<cache->mfc_parent_num;i++)
      	{
          if(net->ipv4.vif_table[cache->mfc_parent_other[i]].dev == skb->dev){
		  	drop =0;
		break;	
          	}		
...
int ip_mr_input(struct sk_buff *skb)
{
    ...
    /*packet is upnp, not IGMP*/
    if(!local && ipv4_is_upnp(iph->daddr) && iph->protocol != IPPROTO_IGMP)
        local = 1;
