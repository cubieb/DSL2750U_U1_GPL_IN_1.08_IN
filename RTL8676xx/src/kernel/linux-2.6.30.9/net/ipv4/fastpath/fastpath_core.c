		...
		int type)
{
	...
	if(!CTAILQ_EMPTY(&route_list_free)) {
		...
		if( type == RTN_UNICAST && (strstr(ifname, "br") || ((ppplan!=-1) && (ppplan>=8))/*pppoe proxy lan itf*/) ) {
                     	/* For fixing the bug that system will be hang if keep changing secondary IP&netmask . */
...
#if 0//move to fp_common.c
	#ifdef CONFIG_RTL_IPTABLES_RULE_2_ACL
		rtl865x_add_filter_L34_permit(entry_napt->intIp,entry_napt->intPort,entry_napt->remIp,entry_napt->remPort
										,entry_napt->protocol,(char *)FastPath_Route(entry_napt->intIp));
		rtl865x_add_filter_L34_permit(entry_napt->remIp,entry_napt->remPort,entry_napt->extIp,entry_napt->extPort
										,entry_napt->protocol,(char *)FastPath_Route(entry_napt->remIp));
	#endif
#endif
	}
			...
			if (1) {
				...
				CTAILQ_FOREACH(entry_path, &table_path->list[hash], path_link) {
					if ((entry_path->protocol == &ep->protocol) && (entry_path->course == 1)) {
						...
						CTAILQ_INSERT_TAIL(&path_list_free, entry_path, tqe_link);
					#ifdef CONFIG_RTL_IPTABLES_RULE_2_ACL
					//QL: move to fp_common.c
					//	rtl865x_del_filter_L34_permit(napt->intIp,napt->intPort,napt->remIp,napt->remPort,napt->protocol);							
					#endif
						break;
					}
				}
				...
				CTAILQ_FOREACH(entry_path, &table_path->list[hash], path_link) {
					if ((entry_path->protocol == &ep->protocol) && (entry_path->course == 2)) {
						...
						CTAILQ_INSERT_TAIL(&path_list_free, entry_path, tqe_link);
					#ifdef CONFIG_RTL_IPTABLES_RULE_2_ACL
					//	rtl865x_del_filter_L34_permit(napt->remIp,napt->remPort,napt->extIp,napt->extPort,napt->protocol);							
					#endif
						break;
					}
				}
...
REMOVE_NAPT:
	CTAILQ_FOREACH(napt_ep, &napt_list_inuse, tqe_link) {
		...
		rtl865x_del_filter_L34_permit(napt_ep->intIp,napt_ep->intPort,napt_ep->remIp,napt_ep->remPort,napt_ep->protocol);
		rtl865x_del_filter_L34_permit(napt_ep->remIp,napt_ep->remPort,napt_ep->extIp,napt_ep->extPort,napt_ep->protocol);
		#ifdef CONFIG_RTL_MULTI_ETH_WAN
		rtl865x_del_filter_L34_redirect(napt_ep->intIp,napt_ep->intPort,napt_ep->remIp,napt_ep->remPort,napt_ep->protocol);
		#endif
#endif
...
__MIPS16 __IRAM_SYS_MIDDLE int FastPath_Process(void *pskb, struct iphdr *iph,struct net_bridge_port *br_port)
{
	...
	switch (iph->protocol) {
		case IPPROTO_TCP: {
			...
			DEBUGP_PKT("==>> [%08X] SIP: %u.%u.%u.%u:%u  -> DIP: %u.%u.%u.%u:%u <TCP>\n", 
				tcph->check,
				...
				if ((*entry_path->in_sPort == sPort) && 
					...
					#endif
					{
						...
						if (pppoe_proxy)
							...
							initSkbHdr(pskb);
							ip_finish_output3(pskb, entry_path->course);
						}
