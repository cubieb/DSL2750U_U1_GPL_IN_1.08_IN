...
int MC_FastPath_Enter(struct sk_buff *skb)	/* Ethertype = 0x0800 (IP Packet) */
{
	...
	if (mcache) {
		//printk("find 0x%x\n", iph->daddr);
		/*match the incoming interface before forward packet*/
		vif = mcache->mfc_parent;
		net = mfc_net(mcache);
		if(net == NULL)
			return 0;
		//if vif table is del the  ipv4.vif_table[vif].dev = NULL
		if((net->ipv4.vif_table[vif].dev == NULL) || (skb->dev == NULL))
			return 0;
		
		if(net->ipv4.vif_table[vif].dev->iflink == skb->dev->ifindex)
		{
