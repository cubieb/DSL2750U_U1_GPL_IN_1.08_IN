...
int arp_req_by_client_limit(void *arg)
{
    struct arpreq_by_client_limit 
    {
        ...
        char mac[ETH_ALEN];
    } *r = (struct arpreq_by_client_limit *)arg;
	struct net_device *dev = NULL;
	struct neighbour *neigh;
    
    //if ((dev = dev_get_by_name("br0")) == NULL)   
    if ((dev = dev_get_by_name(&init_net,ALIASNAME_BR0)) == NULL)
//	if ((dev = dev_get_by_name(&init_net,"br0")) == NULL)
    {
        return 0;
    }

	neigh = neigh_lookup(&arp_tbl, &(r->ip), dev);
    dev_put(dev);
	...
	if (neigh) 
    {
		read_lock_bh(&neigh->lock);
		r->arp_flags = arp_state_to_flags(neigh);
        r->confirmed = neigh->confirmed;
		memcpy(r->mac, neigh->ha, ETH_ALEN);
		read_unlock_bh(&neigh->lock);
		neigh_release(neigh);
