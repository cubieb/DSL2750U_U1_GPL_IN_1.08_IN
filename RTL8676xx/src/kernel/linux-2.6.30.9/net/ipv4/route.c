				...
				u8 tos, struct net_device *dev, int our)
{
	...
	if (!ipv4_is_local_multicast(daddr) && IN_DEV_MFORWARD(in_dev)
        || (ipv4_is_upnp(daddr) && !IN_DEV_MFORWARD(in_dev)))
			       ...
			       u8 tos, struct net_device *dev)
{
	 ...
	 */
#if 0//delete the kernel modification ,use ip rule for policy route
		/*first lookup for policy route then default route*/
#ifdef CONFIG_BRIDGE_NETFILTER
		if(NULL != skb->nf_bridge && NULL != skb->nf_bridge->physindev)
			fl.iif = skb->nf_bridge->physindev->ifindex;
#endif
#endif    
	if (skb->switch_port)
	{
		struct net_device *dev_tmp = NULL; 
		dev_tmp = dev_get_by_name(&init_net, skb->switch_port);
		if (dev_tmp != NULL)
		{
		    	fl.iif = dev_tmp->ifindex;
			dev_put(dev_tmp);
		}
	}
		   ...
		   u8 tos, struct net_device *dev)
{
	...
	if (skb->switch_port)
	{
		...
		if (dev_tmp != NULL)
		{
		    	hashiif = dev_tmp->ifindex;
			dev_put(dev_tmp);
		}
	}
	
	hash = rt_hash(daddr, saddr, hashiif, rt_genid(net));
