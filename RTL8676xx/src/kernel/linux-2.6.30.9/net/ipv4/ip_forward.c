...
#ifdef CONFIG_BRIDGE_ISOLATE
    if (skb->dev && skb->dst && skb->dst->dev){
        if(strstr(skb->dev->name, "br") != NULL
            && strstr(skb->dst->dev->name, "br") != NULL){
            if(strcmp(skb->dev->name, skb->dst->dev->name) != 0){
                goto drop;
            }
        }
    }
