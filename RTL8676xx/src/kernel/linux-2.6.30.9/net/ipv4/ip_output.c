...
{    
	...
	if (dev->br_port == 0 && !(dev->priv_flags & IFF_EBRIDGE)) {
...
static void ip_copy_metadata(struct sk_buff *to, struct sk_buff *from)
{
	...
	// Kaohj --- restore vlan_member (been kept at skbuff.c::pskb_copy())
	to->vlan_member = from->vlan_member;
	// restore from_dev (been kept at skbuff.c::pskb_copy())
...
#endif
//port from linux-2.6.19
	to->isVlanPack=from->isVlanPack;

#ifdef CONFIG_NEW_PORTMAPPING
	to->fgroup = from->fgroup;
#endif
	...
	to->islanPPP = from->islanPPP;
#endif
#ifdef CONFIG_PPPOE_PROXY
	to->source_port = from->source_port;
#endif
