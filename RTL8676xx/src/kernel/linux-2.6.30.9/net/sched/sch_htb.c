...
/* add by wangjiaming, 2012-7-9, adsl line rate limit is not accurate */
/* for Bridge, LLC/SNAP, Eth header 14, ATM LLC 3, ATM SNAP 5, ATM pad 2, ATM AAL5 SAR 8, total 32*/
/* ajust the para to add 8,  total 40, 40 is a proper value to promise the percent deviation in 5% whatever packet length, */
/* you can adjust it by '/proc/net/htb_overhead' */
/* add by zhoumingming 2012 08 24*/
static int overhead = 40; 
...
static int htb_enqueue(struct sk_buff *skb, struct Qdisc *sch)
{
	...
	if (cl == HTB_DIRECT) {
		...
		cl->bstats.bytes += (qdisc_pkt_len(skb) + overhead);
		htb_activate(q, cl);
	}

	sch->q.qlen++;
	sch->bstats.packets += skb_is_gso(skb)?skb_shinfo(skb)->gso_segs:1;
	sch->bstats.bytes += (qdisc_pkt_len(skb) + overhead);
...
{
	int bytes = (qdisc_pkt_len(skb) + overhead);
...
next:
	...
	if (likely(skb != NULL)) {
		cl->un.leaf.deficit[level] -= (qdisc_pkt_len(skb) + overhead);
	...
	for (i = 0; i < dev->num_tx_queues; i++)
	{
		...
		list_for_each_entry(q, &txq_root->list, list)
		{
			if (q->parent)
			{
				...
				{
					if(queueNum>=RTL8651_OUTPUTQUEUE_SIZE) 
					{
						printk("\033[1m\033[40;33m (%s line %d) Warning!! 0412 only support %d outputqueues.\033[0m\n"
							,__func__,__LINE__,RTL8651_OUTPUTQUEUE_SIZE);
						printk("\033[1m\033[40;33m The following htb class would not be written into 0412 switch : \033[0m");
						printk("\033[1m\033[40;33m ==>  classid:0x%08X       rate:  %d(bits)   ceil:%d(bits)\033[0m\n",cl->common.classid,cl->rate->rate.rate<<3,cl->ceil->rate.rate<<3);
						continue;
					}
...
//		int32		totalGbandwidth, totalRbandwidth, calcRbandwidth;
		...
		rtl865x_qosProcessQueue_htb(dev->name, queueInfo);
	}
...
static int htb_getClassIDByClassify(__u32 classify, __u32 *classID, struct Qdisc *sch)
{		
	struct htb_class *cl;
	if ((cl = htb_find(classify, sch)) != NULL && cl->level == 0)
	{
		*classID = classify;
		return SUCCESS;
	}
...
static const struct Qdisc_class_ops htb_class_ops = {
	...
	.getHandleByMark	=	htb_getClassIDByMark,
	.getHandleByClassify = htb_getClassIDByClassify,
#endif
};
...
static int read_htb_overhead(char *page, char **start, off_t off,
...
static int write_htb_overhead(struct file *file, const char *buffer, 
				unsigned long count, void *data)
{
    ...
    {
        printk("arg value must be in (0, 128)\n");
    }
...
void init_htb_overhead(void)
{
    struct proc_dir_entry *htb_overhead;
    htb_overhead = create_proc_entry("htb_overhead", 0, init_net.proc_net);
    if (htb_overhead == NULL)
    {
        printk("init htb_overhead failed !\n");
        return;
    }
    htb_overhead->read_proc = read_htb_overhead;
    htb_overhead->write_proc = write_htb_overhead;
}
