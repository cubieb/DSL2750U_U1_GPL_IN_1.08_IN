...
char outputQueue_Maindev[IFNAMSIZ];
#endif
...
static int tc_sync_hardware(struct net_device *dev)
{
	...
	{
		if( !strncmp(dev->name,"imq",3) || rtl865x_netif_is_wan(dev->name) )
		{
			strcpy(outputQueue_Maindev,dev->name);		
			printk("(%s)======= outputQueue_Maindev = %s===============\n",__func__,outputQueue_Maindev);
		}
		else
			return 0;
	}
	...
	{
		if(strcmp(outputQueue_Maindev,dev->name))
			return 0;
	}
	...
	if(!main_dev)
		return 0;	
	if(!strcmp(netdev_get_tx_queue(main_dev,0)->qdisc_sleeping->ops->id,"htb") 
		|| !strcmp(netdev_get_tx_queue(main_dev,0)->qdisc_sleeping->ops->id,"prio"))
	{		
		cops = netdev_get_tx_queue(main_dev,0)->qdisc_sleeping->ops->cl_ops;
		if (cops && cops->syncHwQueue)			
			return cops->syncHwQueue(main_dev);	
	}
	
	/* reset IPQos rule and output queue */		
	rtl865x_qosFlushBandwidth(outputQueue_Maindev);
	ret = rtl865x_closeQos(outputQueue_Maindev);
	strcpy(outputQueue_Maindev,"");
	printk("(%s)======= outputQueue_Maindev = %s===============\n",__func__,"NULL");
	return ret;
