...
struct proc_dir_entry *proc_mac_port;
static int macp_proc_read(char *page, char **start, off_t off,
			  int count, int *eof, void *data)
{
    ...
    out +=sprintf(out,"dev  \tmac\n");
	read_lock(&dev_base_lock);
	for (dev = dev_base; dev != NULL; dev = dev->next) {
		if (dev->priv_flags & 0x2){
            br = netdev_priv(dev);
            for (i = 0; i < BR_HASH_SIZE; i++) {
			     ...
			     if(!f->is_local) 
			     {
                    out +=sprintf(out, "%s\t"NMACQUAD_FMT"\n",
                        f->dst->dev->name, NMACQUAD(f->addr.addr));
			  ...
			  int count, int *eof, void *data)
{
	...
	out += sprintf(out, "mc=%d\n", br_mcast_to_unicast);
	out += sprintf(out, "snooping=%d\n", snooping);
	len = out - page;
	len -= off;
	if (len < count) {
		*eof = 1;
...
static int debug_proc_write( struct file *filp, const char __user *buf,unsigned long len, void *data )
{
    ...
    else if (strcmp(action, "snooping") == 0)
    {
        snooping = val;
    }
...
void get_lan_ip_mask(void)
{
	...
	struct in_ifaddr *ifap = NULL;
      	...
      	if ((landev = __dev_get_by_name(&init_net,ALIASNAME_BR0)) != NULL){in_dev=(struct in_device *)(landev->ip_ptr);
		if (in_dev != NULL) {
			for (ifap=in_dev->ifa_list; ifap != NULL; ifap=ifap->ifa_next) {
				//if (strcmp(ALIASNAME_BR0, ifap->ifa_label) == 0){
		...
		hlist_for_each_entry_safe(f, h, n, &bridge0->hash[i], hlist) {
			if(MULTICAST_MAC(f->addr.addr) )
			{
				  len += sprintf(page+len,"[%d][%d]mCastMac:0x%x:%x:%x:%x:%x:%x,use_count is %d,ageing_timer is %lu\n",
					i,j,f->addr.addr[0],f->addr.addr[1],f->addr.addr[2],f->addr.addr[3],f->addr.addr[4],f->addr.addr[5],atomic_read(&f->use_count),(jiffies<f->ageing_timer) ?0:(jiffies-f->ageing_timer));
...
static int __init br_init(void)
{
	...
	proc_mac_port= create_proc_entry( "macport", 0644, NULL);
	proc_mac_port->read_proc = macp_proc_read;
#if defined(CONFIG_IGMP_SNOOPING)
    /* 创建debug接口 */
    proc_debug = create_proc_entry( "debug", 0644, NULL);
    proc_debug->read_proc  = debug_proc_read;
    proc_debug->write_proc = debug_proc_write;
#endif
