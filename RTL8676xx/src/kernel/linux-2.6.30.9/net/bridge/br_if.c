...
//Chris: stp+mesh
...
#if defined (CONFIG_RTL_STP)
	{
		...
		#endif
		{
			retval = rtl865x_setMulticastSpanningTreePortState(Port , BR_STATE_DISABLED);

			retval = rtl865x_setSpanningTreePortState(Port, BR_STATE_DISABLED);
		}
...
//				return NULL;
		...
		if(0==retval)
			printk("RTL865x Port %d state=DISABLED********%d\n", Port, index);
		else
			printk("RTL865x port %d set state error=%d", Port, retval);
	}
	...
	if(alias_name_is_eq(CMD_CMP,p->dev->name,ALIASNAME_ETH0))
	{
	        retval=rtl865x_getNetifVid(ALIASNAME_BR0, &vid);
...
#ifdef CONFIG_NEW_PORTMAPPING
    if(alias_name_is_eq(CMD_NCMP,dev->name,ALIASNAME_VC))
    //if(dev->name[0] == 'v')//
		p->fgroup = br2684_get_fgroup(dev);
...
//august: 20110930
    ...
    //if(dev->name[0] == 'n')//
	{
		...
		p->fgroup = &(dev_info->member);
	}
#endif //end CONFIG_RTL_MULTI_ETH_WAN
#endif // end NEW_PORTMAPPING
