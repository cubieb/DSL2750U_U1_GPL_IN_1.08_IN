...
//Added by penggenyao, check switch port based on sk_buff->switch_port
static inline int ebt_dev_check_switch_port(char* entry, char* port)
{
    int i = 0;
    if (*entry == '\0')
		return 0;
	if (!port)//not set sk_buff->switch_port
		return 1;
	/* 1 is the wildcard token */
	while (entry[i] != '\0' && entry[i] != 1 && entry[i] == port[i])
		i++;
	return (port[i] != entry[i] && entry[i] != 1);
}
#endif
...
static inline int ebt_basic_match(struct ebt_entry *e, struct sk_buff *skb, 
   const struct net_device *in, const struct net_device *out,struct sk_buff** pskb)
#else
static inline int ebt_basic_match(struct ebt_entry *e, struct sk_buff *skb,
...
#ifdef CONFIG_RE8306_USE_CPUTAG
else if (!((*pskb)->isVlanPack)&&!(e->bitmask & EBT_NOPROTO) &&
	   FWINV2(e->ethproto != h->h_proto, EBT_IPROTO))
		return 1;
#else

	else if (!(e->bitmask & EBT_NOPROTO)) {
#if defined(CONFIG_BRIDGE_EBT_TBSVLAN) || defined(CONFIG_BRIDGE_EBT_TBSVLAN_MODULE)
		if (e->ethproto == 0x8100 && skb->vlan_tci!= 0)
			goto Next;
#endif  
...
Next:
#endif
    //modified by penggenyao, check switch port first,
    //if not matched, check the standard bridge port
#ifdef CONFIG_RE8306_USE_CPUTAG

#ifdef CONFIG_RTL_ALIASNAME
    char ETH0[16]=""
    sprintf(ETH0,"%s_sw",ALIASNAME_ETH0);
    if(!strncmp(e->in, ETH0, strlen(ETH0)))//switch port
#else
	if(!strncmp(e->in, "eth0_sw", 6))//switch port
#endif	
	{
	    if(FWINV2(ebt_dev_check_switch_port(e->in,(*pskb)->switch_port), EBT_IIN))
		return 1;
	}else
#endif
	{
   ...
   struct ebt_table *table)
{
	...
	while (i < nentries) {
#ifdef CONFIG_RE8306_USE_CPUTAG
		if (ebt_basic_match(point, skb, in, out,(struct sk_buff**)&skb))
#else
