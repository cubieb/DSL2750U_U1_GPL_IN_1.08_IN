   ...
   const void *data, unsigned int datalen)
{
    ...
    //printk("markset 0x%08x, mark 0x%x, nfmark 0x%x \n", info->markset, info->mark, (*pskb)->nfmark);
	if(prio) {
		(*pskb)->nfmark &= ~(PRIO_LOC_NFMASK << info->markpos);		
		(*pskb)->nfmark |= (prio << info->markpos);
//		(*pskb)->nfcache |= NFC_ALTERED;
		//printk("mark 0x%x, nfmark 0x%x\n",( prio << info->markpos), (*pskb)->nfmark);			
	}
