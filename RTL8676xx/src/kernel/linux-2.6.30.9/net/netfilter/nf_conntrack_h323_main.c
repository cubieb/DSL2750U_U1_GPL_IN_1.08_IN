...
#define PROCFS_NAME "algonoff_h323"
ALGONOFF_INIT(h323);
#endif

#define MAX_H323_BUFFER 4096
...
/*
		tpkt = skb_header_pointer(skb, tcpdataoff, tcpdatalen,
					  h323_buffer);
*/	
	if (tcpdatalen < MAX_H323_BUFFER)
		tpkt = skb_header_pointer(skb, tcpdataoff, tcpdatalen,
					  h323_buffer);
	else
	{
		pr_err("nf_ct_h323: the skb datalen %d > MAX_H323_BUFFER(%d),Need adjust MAX_H323_BUFFER\n",*datalen,MAX_H323_BUFFER);
		tpkt = skb_header_pointer(skb, tcpdataoff, MAX_H323_BUFFER,
					  h323_buffer);
	}
		BUG_ON(tpkt == NULL);
...
*/	
	if (*datalen < MAX_H323_BUFFER)
		return skb_header_pointer(skb, dataoff, *datalen, h323_buffer);
	else
	{
		pr_err("nf_ct_h323: the skb datalen %d > MAX_H323_BUFFER(%d),Need adjust MAX_H323_BUFFER\n",*datalen,MAX_H323_BUFFER);
		return skb_header_pointer(skb, dataoff, MAX_H323_BUFFER, h323_buffer);
	}
}
...
*/	
	h323_buffer = kmalloc(MAX_H323_BUFFER, GFP_KERNEL);
	if (!h323_buffer)
		return -ENOMEM;
