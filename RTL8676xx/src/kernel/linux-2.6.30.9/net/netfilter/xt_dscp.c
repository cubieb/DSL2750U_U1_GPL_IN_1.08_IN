			  ...
			  unsigned int *invflags)
{
	...
	if(ip == NULL || info == NULL || rule == NULL)
		return 1;
...
static int tos_match2acl(const char *tablename,
			  const void *ip,
			  const struct xt_match *match,
			  void *matchinfo,
			  rtl865x_AclRule_t *acl_rule,
...
tos_mt_v0(const struct sk_buff *skb, const struct xt_match_param *par)
{
	...
	return ((ip_hdr(skb)->tos & info->mask) == (info->tos & info->mask)) ^ info->invert;
