...
#define MAX_FTP_BUFFER 4096

// Mason Yu. alg_onoff_20110509
#ifdef  CONFIG_IP_NF_ALG_ONOFF
#define PROCFS_NAME "algonoff_ftp"
ALGONOFF_INIT(ftp);
#endif
...
*/	
	if (datalen < MAX_FTP_BUFFER)
		fb_ptr = skb_header_pointer(skb, dataoff, datalen, ftp_buffer);
	else
	{
		pr_err("ftp: the skb datalen %d > MAX_FTP_BUFFER(%d),Need adjust MAX_FTP_BUFFER\n",datalen,MAX_FTP_BUFFER);
		fb_ptr = skb_header_pointer(skb, dataoff, MAX_FTP_BUFFER, ftp_buffer);
	}
	BUG_ON(fb_ptr == NULL);
...
*/	
	ftp_buffer = kmalloc(MAX_FTP_BUFFER, GFP_KERNEL);
	if (!ftp_buffer)
		return -ENOMEM;
