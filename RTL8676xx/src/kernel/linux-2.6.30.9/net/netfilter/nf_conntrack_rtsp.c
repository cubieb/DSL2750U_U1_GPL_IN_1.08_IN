...
/******************************************************************
hxy add, 20111230. adjust tcp sequence after deal whole package.
*/
void adjust_tcp_sequence_rtsp(struct sk_buff *skb,
			 struct nf_conn *ct,
			 enum ip_conntrack_info ctinfo,
			 int firstsizediff,
			 int othersizefiff
...

{
	...
	set_bit(IPS_SEQ_ADJUST_BIT, &ct->status);	
        	 ...
        	 * retransmit */
        	if(this_way->correction_pos== seq && othersizefiff!=0)  {
        		   this_way->correction_pos = seq;                   
     //   		   this_way->offset_before = this_way->offset_before-firstsizediff;                   
        		   this_way->offset_after += othersizefiff;
        	}
		...
		nf_conntrack_tcp_update(skb, ip_hdrlen(skb),
					ct, CTINFO2DIR(ctinfo),othersizefiff);

		nf_conntrack_event_cache(IPCT_NATSEQADJ, ct);
 }

/*  判断是否为重传的setup 包*/
static int is_retransmit_setup(struct sk_buff *skb, 
                         struct nf_conn *ct,    
			 enum ip_conntrack_info ctinfo)
{
  	struct iphdr *iph;
	...
	tcph = (void *)iph + iph->ihl*4;
        ...
        if(this_way->correction_pos == tcph->seq)
            return 1;
        else 
            return 0;  
}
			       ...
			       __be16 rtpport, int *delta)
{
	...
	rtp_exp->derived_timeout = 0xFFFFFFFF;
 #endif
				...
				char dash, int *delta)
{
	...
	rtcp_exp->derived_timeout = 0xFFFFFFFF;
#endif    
		...
		struct nf_conn *ct, enum ip_conntrack_info ctinfo)
{
	...
	while(get_next_message(tcpdata, tcpdatalen, &msgoff, &msglen,
			       &msghdrlen)) {
		/*判断是否为重传包*/	       
		flag_retransmit =is_retransmit_setup(skb,ct,ctinfo);
        
		/* Messages from LAN side through MASQUERADED connections */
		if (memcmp(&ct->tuplehash[dir].tuple.src.u3,
			   &ct->tuplehash[!dir].tuple.dst.u3,
			   sizeof(ct->tuplehash[dir].tuple.src.u3)) != 0) {
			if(memcmp(tcpdata+msgoff, "PAUSE ", 6) == 0 ||memcmp(tcpdata+msgoff, "GET_PARAMETER ", 14) == 0) {
        ...
        if(!flag_retransmit && (othersizefiff !=0))
             adjust_tcp_sequence_rtsp(skb, ct, ctinfo, firstsizediff, othersizefiff);
