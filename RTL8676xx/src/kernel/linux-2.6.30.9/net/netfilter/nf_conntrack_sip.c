...
/*add by zhoufeng2 用于register*/
int (*nf_nat_register_hook)(struct sk_buff *skb,
				 enum ip_conntrack_info ctinfo,
				 struct nf_conntrack_expect *exp);
EXPORT_SYMBOL_GPL(nf_nat_register_hook);
...
str               指定查找的字符串指针
str_len           指定查找的字符串长度
case_sensitiver   是否大小写敏感

*begin保存查找到的字符串尾部指针，比如
查找UDP
   XXXXXUDP welcome
            ^ 
            |
...
/*
在*begin起始end结束的字符串中间查找并分析ct_sip_hdrs[field]指定的消息头
*/
int find_field(char **begin, char *end, int field)
{
	...
	while (p <= q) {
		/*比较消息包消息头是否和hnfo指向的消息头相同*/
		if (hnfo->lname == NULL ||/*hnfo->lname为空或和p指向的消息头相同*/
		    (strncmp(p, hnfo->lname, hnfo->lnlen) == 0)) {
		    	/*已经匹配到了消息头，指针指向消息内容*/
		    	p += hnfo->lnlen;
		} else {/*消息头未匹配，再查找是不是用了消息头的缩写*/
			if (hnfo->sname != NULL &&/*缩写匹配，比如Via缩写为v*/
...
int parse_sip_uri(char **begin, char *end, struct nf_conntrack_man *addr)
{
	...
	/* We found user part */
	if (*p == '@')
		p0 = ++p;/*p0指向@后一个字符，同时p也后移一个字符*/
	/* No user part */
	else 
		p = p0 = *begin;/*@未找到，查询指针恢复到*begin位置*/

	/* Address 
	查找 192.168.3.4:12345类型地址
	parse_addr()取出p字符串中的ip地址，保存在addr->u3.ip中
	返回:被转换为地址的字符串长度*/
	...
	if (p[len] == ':') {
		...
		if (port < 1 || port > 65535)
			return 0;
		addr->u.all = htons((unsigned short)port);/*保存端口号*/
	} else {
		addr->u.all = 0;
	}

	*begin = p0;/*XXX@YYY类型地址p0才不等于*begin*/
	return p + len - p0;
}
			 ...
			 char **dptr, int *dlen, struct nf_conntrack_man *addr)
{
	...
	*addr = a;
	...
	/* We care only LAN->WAN situations 无nat网络*/
	if (!memcmp(&ct->tuplehash[dir].tuple.src.u3,
		    &ct->tuplehash[!dir].tuple.dst.u3,
		    sizeof(ct->tuplehash[dir].tuple.src.u3)))
		goto end;

	/* LAN->WAN. Change the LAN IP to WAN. 有nat网络*/
			 ...
			 char **dptr, int *dlen, struct nf_conntrack_man *addr)
{
	...
	/*这个expect的五元组是
	L3 protocol = ct中的协议代码
	sip = 0.0.0.0
	dip = wan ip(实际是nat lan ip后的结果)
	sport = 0
	dport = (m=audio带port)*/
		       ...
		       char **dptr, int *dlen)
{
	...
	例子:
		Via: SIP/2.0/UDP 192.168.1.2:18968;branch=z9hG4bK-d87543-db04f969df0b596f-1--d87543-;rport
	当数据包进过NAT后,Via中的数据和tuple应该是不同的*/
	/*查找sip信令包中的VIA消息包头，并使p指向UDP 后的位置*/
	while (find_field(&p, *dptr + *dlen, POS_VIA)) {
		/*查找到Via消息头，p已经指向例子中192.168....的位置*/
		if ((len = parse_sip_uri(&p, *dptr + *dlen, &addr)) == 0)
			continue;
		/*查找到地址并且保存在addr中，p指向192.168...，带有@的地址部分被p跳过*/
		pr_debug("nf_conntrack_sip: Via=%.*s\n", len, p);

		/* Different SIP port than this one */
		/********非常重要****
		Via取出的addr表示当前方向的源ip和源port
		如:	addr.u3.ip=192.168.1.64   addr.u.all=18968,当前的源ip和源port
		*******************/
		/*当via包含的地址与tuple[dir]一致，而port不同且不为0时，创建expect，
		即进过NAT后源port发生了转换的情况
		如:
		192.168.1.64:18968->172.24.242.151:5060
		NAT转换后(wan=10.88.138.50)变成
		10.88.138.50:xxxxx->172.24.242.151:5060
		*/
		if (!memcmp(&addr.u3, &ct->tuplehash[dir].tuple.src.u3,
		...
		/* addr的IP地址和ct的同方向源ip相同 ，addr端口号和ct同方向port相同，
		说明该连接进过了nat，但是源port没有改变
		所以必须对via中的ip地址进行nat转换*/ 
			 ...
			 (nf_nat_addr = rcu_dereference(nf_nat_addr_hook))) {
			/* nf_nat_addr_hook在nf_nat_sip.c中被初始化，指向函数nf_nat_sip() */

			/*将addr(取自Via消息头)的ip和port换成conntrack中反方向的ip和port*/			
			addr.u3 = ct->tuplehash[!dir].tuple.dst.u3;
			addr.u.all = ct->tuplehash[!dir].tuple.dst.u.all;
			/*
				nf_nat_addr_hook=nf_nat_addr
            */
			ret = nf_nat_addr(skb, protoff, ct, ctinfo, dptr,
					  dlen, &p, &len, &addr);
		...
		/* addr的IP地址和ct的同方向目的ip相同 ，addr端口号和ct目的port相同，
		说明该连接进过了nat，但是是方向的数据包，也可以判定为WAN->LAN方向
		所以必须对via中的ip地址进行nat转换*/ 
		else if (!memcmp(&addr.u3, &ct->tuplehash[dir].tuple.dst.u3,
				 sizeof(addr.u3)) &&
			 addr.u.udp.port ==
...
static int process_register(struct sk_buff *skb, struct nf_conn *ct, 
							enum ip_conntrack_info ctinfo)
{

	/*在REGISTER，创建一个expect用于处理UAS在返回信令时使用其他源端口
	如: 
	192.168.2.1:12345   -> 172.24.242.251:5060
	172.24.242.251:506x -> 192.168.2.1:12345
	UAS在信令阶段会改变到发送到UAC的报文的源端口可能是5060-5064之间的
	任何一个端口
	*/
	struct nf_conntrack_tuple *tuple;
	struct nf_conntrack_expect *exp;
	typeof(nf_nat_register_hook) nf_nat_sip_register;
	int ret = NF_ACCEPT;
	...
	if ((exp = nf_ct_expect_alloc(ct)) == NULL)
		return ret;
	/*dir肯定是org方向，取reply方向tuple用于创建expect*/
	tuple = &ct->tuplehash[IP_CT_DIR_REPLY].tuple;

	nf_ct_expect_init(exp, SIP_EXPECT_CLASS_SIGNALLING,
				  nf_ct_l3num(ct),				/*协议簇，ipv4 or ipv6*/
				  &tuple->src.u3, 				/*源ip*/
				  &tuple->dst.u3, 				/*目的ip*/
				  IPPROTO_UDP,  				/*L4协议*/
				  NULL,  						/*源port=NULL，表示不关心*/
				  &tuple->dst.u.udp.port); 		/*目的port*/
	exp->timeout.expires = sip_timeout * HZ;
	exp->helper = nfct_help(ct)->helper;
	exp->flags = NF_CT_EXPECT_PERMANENT;

	/*指向函数nf_nat_register()*/
	nf_nat_sip_register = rcu_dereference(nf_nat_register_hook);
	
	if (nf_nat_sip_register && ct->status & IPS_NAT_MASK)
		ret = nf_nat_sip_register(skb, ctinfo, exp);
...
/*专门用来刷新信令阶段的expect*/
static inline int refresh_expect_timer(struct nf_conntrack_expect *i)
{
...
/*	p = &master_help->helper->expect_policy[i->class];*/
	i->timeout.expires = jiffies + SIP_TIMEOUT*HZ;
	add_timer(&i->timeout);
	return 1;
}
...
/*
   ...
   |               |           iphdr_len          |
   --------------------------------------------------
skb->data   skb->network_header                l4_hdr_ptr
   ************************************************
所以 protoff = skb->network_header - skb->data + iphdr_len
即*号部分长度
*/
		    ...
		    enum ip_conntrack_info ctinfo)
{
	...
	if (!skb_is_nonlinear(skb))
		dptr = skb->data + dataoff;/*指向udp的payload位置，即sip协议起始*/
	else {
	...
	datalen = skb->len - dataoff;/*sip协议长度*/
	...
	*/
	if (memcmp(dptr, "REGISTER", sizeof("REGISTER") - 1) == 0 )
	{
		...
		ret=process_register(skb,ct,ctinfo);
	}
	
	/*若是宿主连接则，刷新期待连接*/
	if (!(ct->status & IPS_EXPECTED))
	{
		struct nf_conntrack_expect *h;
		struct net *n;
		n=nf_ct_net(ct);/*取net指针，因为下个函数需要这个参数*/
		h=nf_ct_expect_find_get(n, &ct->tuplehash[IP_CT_DIR_REPLY].tuple);/*查找是否有匹配的expect*/
		if(h)/*若有匹配的expect则刷新expect*/
		{
			refresh_expect_timer(h);
		}
  ...
  /*
  Via:字段指示返回到本机的应答的地址
  Contact：字段指示发送到本机的请求地址
  绝大多数情况下这两个字段包含的地址信息是一样的
  */
	/* Process Via field */
	...
	/*Contact 头域提供了一个 URI，这个 URI 的含义取决于是在请求还是在应答中.
    Contact 头域包含了一个显示的名字，一个包含参数的 URI，还有 header 参数
    组成。*/
	...
	if (memcmp(dptr, "INVITE", sizeof("INVITE") - 1) == 0 ||
		...
		/*分析消息正文o=字段，addr保存ip地址和L3协议代码，过nat的网络将lan
		ip替换为wan的ip*/
		ret = process_owner(skb, protoff, ct, ctinfo, &dptr,
				    &datalen, &addr);
		if (ret != NF_ACCEPT)
			goto out;
		/*分析消息正文c=字段，和o=字段结果一样*/
		...
		/*分析消息正文m=audio字段，这个字段表示本机接收的rtp流所用端口号，
		即，对方发音频流的目的port为这个字段指定的port
		这里的addr保留了上个o=或c=(若c=存在的话)的ip地址和L3协议代码*/
...
static int __init nf_conntrack_sip_init(void)
{
	...
	for (i = 0; i < ports_c; i++) {
		...
		/*helper匹配的方向是reply方向的五元组，所以sip的初始化关心域
		应该是: tuple的目的协议代码和源port，其他不关心的域都为0，
		表示不关心该域
		匹配发生在创建conntrack时的resolve_normal_ct()->init_conntrack()
		->__nf_ct_try_assign_helper()*/
		for (j = 0; j < 2; j++) {
			sip[i][j].tuple.dst.protonum = IPPROTO_UDP;
			sip[i][j].tuple.src.u.udp.port = htons(ports[i]);
			sip[i][j].me = THIS_MODULE;
			/*在helper模块被匹配时被调用，在NF_INET_POST_ROUTING的
			ipv4_confirm()中调用help函数，(又变了????)*/
			...
			/*向全局hash表nf_ct_helper_hash冲突链中加入helper模块*/
