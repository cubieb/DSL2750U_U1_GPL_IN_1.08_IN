...
static int packet_rcv_spkt(struct sk_buff *skb, struct net_device *dev,  struct packet_type *pt, struct net_device *orig_dev)
{
	...
	//ramen-20080805
	//set the file's fromport for pppoe proxy
	//rame-20090105-fix the potential risj
	if (sk->sk_socket&&sk->sk_socket->file)
		sk->sk_socket->file->fromport = skb->fw_mark; 
#endif
...
/*linux-2.6.19*/
	...
	if(sock_flag(sk, SOCK_SENDETHPRIO)) /*wt-146, ethprio*/
		skb->mark=sk->sk_ethprio;
...
packet_setsockopt(struct socket *sock, int level, int optname, char __user *optval, int optlen)
{
	...
	/*linux-2.6.19*/
	case PACKET_SENDETHPRIO: /*wt-146, ethprio*/
	{
		int val;
