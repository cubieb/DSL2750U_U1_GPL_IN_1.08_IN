...
int dev_getipaddr(struct net *net, __be32 osipaddr,__be32 rsipaddr)
{
	struct net_device *dev;
	...
	for_each_netdev(net, dev)
	{
		...
		if(in_dev)
			...
			{
				//printk("ifap->ifa_address %x\n",ifap->ifa_local);
				if((osipaddr == ifap->ifa_local) || (rsipaddr == ifap->ifa_local))
					return 1;
			}
	}
	return 0;
...
#ifdef CONFIG_BRIDGE_EBT_AROUTE
int (*after_route_hook) (struct sk_buff *skb);
#endif
			...
			struct netdev_queue *txq)
{
	...
	rhook = rcu_dereference(after_route_hook);
	if (rhook != NULL) {
        rhook(skb);
    }
#endif
...
/*TBS_TAG: call the mirror function of dev add by xiachaoren*/
#if (defined(CONFIG_WAN_MIRROR) || defined(CONFIG_WAN_MIRROR_MODULE))
		if((NULL != dev->mirror_tx) && ((dev->direct_flags & 0x01) != 0))
			{
			dev->mirror_tx(skb, dev->mirror_dev, 0x01);
			}
#endif
/*TBS_TAG_END 2011-11-22*/
		
#if defined (CONFIG_RTL_LOCAL_PUBLIC)
	if(	(skb->srcLocalPublicIp!=0) && (rtl865x_curOpMode==GATEWAY_MODE) &&
...
/*linux-2.6.19*/ 
// Kaohj -- backlog drop count
int netdev_backlog_drop;
...
int rtl865x_attainDevType(unsigned char *devName, unsigned int *isLanDev, unsigned int *isWanDev)
{
	...
	*isWanDev=0;
    ...
    //if(strncmp(devName, "eth1", 4) ==0)//
    if(alias_name_is_eq(CMD_NCMP,devName,ALIASNAME_ETH1))
#endif
		{
			*isWanDev=1;
		}
    //else if((strncmp(devName, "br0", 3) ==0)||(strncmp(devName, "eth", 3) ==0) || (strncmp(devName, "wlan", 4) ==0))//
    else if(alias_name_is_eq(CMD_NCMP,devName,ALIASNAME_BR0)||alias_name_are_eq(devName,ALIASNAME_ETH,ALIASNAME_ETH0DOT)||alias_name_is_eq(CMD_NCMP,devName,ALIASNAME_WLAN))
		{
			*isLanDev=1;
		}
		else
	...
	{
	//if(strncmp(devName, "wlan", 4)==0)//
	if(alias_name_is_eq(CMD_NCMP,devName,ALIASNAME_WLAN))
		{
			*isWanDev=1;
		}

else if(alias_name_is_eq(CMD_NCMP,devName,ALIASNAME_BR0)||alias_name_are_eq(devName,ALIASNAME_ETH,ALIASNAME_ETH0DOT))
//    else if((strncmp(devName, "br0", 3) ==0) ||(strncmp(devName, "eth", 3) ==0))//
		{
			*isLanDev=1;
		}
		else
...
//else if((strncmp(devName, "br0", 3) ==0)||(strncmp(devName, "eth", 3) ==0) || (strncmp(devName, "wlan", 4) ==0))//    
	...
	//	if(	(strncmp(devName, "br0", 3) ==0) ||(strncmp(devName, "eth", 4) ==0) ||(strncmp(devName, "wlan", 4) ==0))
		{
			*isLanDev=0;
		}
		else
...
struct net_device *rtl865x_getLanDev(void )
{
	struct net_device * lanDev=NULL;
 	...
 	lanDev=dev_get_by_name(&init_net,ALIASNAME_BR0);
//	lanDev=dev_get_by_name(&init_net,"br0");//
...
struct net_device *rtl865x_getWanDev(void )
{
	...
	if(rtl865x_curOpMode==GATEWAY_MODE)
	{	
		wanDev=dev_get_by_name(&init_net,ALIASNAME_ETH1);	    
//		wanDev=dev_get_by_name(&init_net,"eth1");//
	}
	else if(rtl865x_curOpMode==WISP_MODE)
	{
		wanDev=dev_get_by_name(&init_net,ALIASNAME_WLAN0);
//		wanDev=dev_get_by_name(&init_net,"wlan0");//
	}
	else if(rtl865x_curOpMode==BRIDGE_MODE)
	{
		 wanDev=dev_get_by_name(&init_net,ALIASNAME_BR0);
//		wanDev=dev_get_by_name(&init_net,"br0");//
	}
...
#ifdef CONFIG_VNET
int (*vnet_handle)(struct sk_buff *skb, struct net_device *dev) = NULL;
#endif
...
/*TBS_TAG: call the mirror function of dev add by xiachaoren*/
	...
	if(NULL != skb->dev)
		{
		if((NULL != skb->dev->mirror_tx) && ((skb->dev->direct_flags & 0x02) != 0))
			{
			skb->dev->mirror_tx(skb, skb->dev->mirror_dev, 0x02);
			}
		}
#endif
/*TBS_TAG_END 2011-11-22*/
...
//				if (skb->dev->name[0] == 'n') {
                if(alias_name_are_eq(skb->dev->name,ALIASNAME_NAS,ALIASNAME_MWNAS)){
					//orig_dev = skb->dev;
					if (getSmuxDev(skb)==NULL)
						goto FP_EXIT;
				}
#endif
...
FP_EXIT:
#endif
	//printk("%s %s Try fast bridge.\n", __func__, skb->dev->name);
#endif //CONFIG_RTL867X_IPTABLES_UPSTREAM_FAST_PATH
...
/*  Bridge fastpath  */
	...
	//if (unlikely(br->wlan_blocking != WLAN_BLOCK_DISABLE)) {
	//goto SKIP_FBRIDGE;
	//}
	//#endif
	//TW添加为支持无线多SSID之间隔离，
	//当开启多SSID之间隔离时，不要让包走shortcut。
	if (unlikely(wlan_mbssid_block==1)) { 
		goto SKIP_FBRIDGE;
	}
	
    //printk("(%s)Try Bridge fastpath.\n",__func__);
...
SKIP_FBRIDGE:
#endif
/* Kevin, if pkt is to extPort and the pkt's src_mac has been changed to br0's GMAC 
                (it means that L234 header has been converted by 0412 NAPT engine )
			use br_dev_xmit() instead of br_handle_frame()
*/
#if defined(CONFIG_RTL_HARDWARE_NAT)
	if(skb->acl_forward_to_extPort==1)
	{
		extern struct net_bridge *bridge0;
		
		skb_push(skb, ETH_HLEN);		
		if(bridge0 && !compare_ether_addr(bridge0->dev->dev_addr,&skb->data[6])) 
	    {
			if (netif_running(bridge0->dev))
			{
				skb->dev = bridge0->dev;
				#if defined(CONFIG_COMPAT_NET_DEV_OPS)
				bridge0->dev->hard_start_xmit(skb, skb->dev);				
				#else
				bridge0->dev->netdev_ops->ndo_start_xmit(skb,skb->dev);
				#endif				
				return NET_RX_SUCCESS;
			}
					    ...
					    struct net_device *orig_dev)
{
	...
	if ((skb->pkt_type != PACKET_LOOPBACK)&&((port = rcu_dereference(skb->dev->br_port)) != NULL)){
		...
		// If bridged PPPoE enabled, only PPPoE packets
		// with hw addr other than that of this
		// interface should go bridging. --- Kaohj
		// Kaohj
		
		if ((!skb->dev->brpppoe&&(!(skb->dev->priv_flags & IFF_DOMAIN_WAN) || memcmp(eth_hdr(skb)->h_dest, skb->dev->dev_addr, ETH_ALEN)))||((eth_hdr(skb)->h_proto == 0x8863 ||
			eth_hdr(skb)->h_proto == 0x8864) &&memcmp(eth_hdr(skb)->h_dest, skb->dev->dev_addr, ETH_ALEN))){
...
int (*pioneer_hook_func) (struct sk_buff  *skb) = NULL;
#endif
...
/*TBS_TAG_END 2011-11-22*/
...
#if defined(CONFIG_RTL_FAST_BRIDGE)
	//int retval = 0;
	//retval = rtl_fast_br_forwarding(skb);
	//if(retval == RTL_FAST_BR_SUCCESS)
	//only forwarding pkt from bridge port
	if(skb->dev->br_port)
		if(rtl_fast_br_forwarding(skb) == RTL_FAST_BR_SUCCESS)	
			return NET_RX_SUCCESS;
	//printk("====%s(%d),fast br forwarding failed!!,retval(%d)\n",__FUNCTION__,__LINE__,retval);
#endif
	...
	rcu_read_lock();
	/*
	 * TBS_TAG by baiyonghui 2011-3-21
	 * Desc: supported vnet and proute
	 */
#ifdef CONFIG_BRIDGE_EBT_PROUTE
	if (pioneer_hook_func &&
			skb->pkt_type != PACKET_LOOPBACK &&
			skb->iif == skb->dev->ifindex)
	{
		int kret;
		kret = pioneer_hook_func(skb);
		if (!kret)
		{
			kfree_skb(skb);
			ret = NET_RX_DROP;
...
ncls:
	...
	if(orig_dev->priv_flags & IFF_RSMUX) {
		...
		atomic_inc(&skb->users);
		ret = smux_pkt_recv(skb, orig_dev);
	}
	else {
#endif
		/*linux-2.6.19*/
		...
		if (skb->protocol == htons(ETH_P_8021Q)) {
			...
			skb_reset_network_header(skb); //skb->nh.raw += VLAN_HLEN;
		}
#endif
...
int dev_change_flags(struct net_device *dev, unsigned flags)
{
	...
	/*linux-2.6.19*/ 
	// Kaohj, enable blindly set broadcast and pointopoint flags
	m_flags=(old_flags^flags)&(IFF_BROADCAST | IFF_POINTOPOINT);
	if (m_flags) /* Bits is different  ? */
	{
		dev->flags ^= m_flags;
	}
...
#ifdef CONFIG_VNET
EXPORT_SYMBOL(vnet_handle);
#endif
#ifdef CONFIG_BRIDGE_EBT_PROUTE
EXPORT_SYMBOL(pioneer_hook_func);
#endif
#ifdef CONFIG_BRIDGE_EBT_AROUTE
EXPORT_SYMBOL(after_route_hook);
#endif
