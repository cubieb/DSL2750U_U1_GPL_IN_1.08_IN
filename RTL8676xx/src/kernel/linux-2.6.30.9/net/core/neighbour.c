...
/*2007-12-19*/
static int syn_asic_arp(struct neighbour *n, char *fun, int add)
{
#ifdef CONFIG_RTL_LAYERED_DRIVER_L3
...
#if defined(CONFIG_RTL867X_IPTABLES_FAST_PATH)
					if(FastPath_Enabled()) {
					rtl867x_delArp(*(u32*)n->primary_key);
					}
#endif
...
#if defined(CONFIG_RTL_HARDWARE_NAT) && defined(CONFIG_RTL_LAYERED_DRIVER) && defined(CONFIG_RTL_LAYERED_DRIVER_L3)
	/*delete asic arp entry*/
	syn_asic_arp( neigh, __FUNCTION__, 0);
#endif
...
/*2007-12-19*/
	if (neigh->nud_state & NUD_REACHABLE)
		syn_asic_arp(neigh, __FUNCTION__, 1);
	#if 0
...
#if defined(CONFIG_RTL_HARDWARE_NAT) && defined(CONFIG_RTL_LAYERED_DRIVER) && defined(CONFIG_RTL_LAYERED_DRIVER_L3)
				if (state & NUD_VALID) {
					int tval = rtl865x_arpSync(htonl(*((u32 *)n->primary_key)), 0);
					if (tval > 0)
					{
						n->used = jiffies;
						n->dead=0;
