...
#define NUM2PRINT 50
	...
	printk(KERN_NOTICE "rt2684#skb: %s\n", buf);
}
#else
...
{
return (struct rt2684_dev *)netdev_priv(net_dev);
}
...
static int rt2684_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
    ...
    //printk(KERN_NOTICE "**TX raw data:**\n");
	skb_debug(skb);
	if (rtvcc->encap) { /*llc封装*/
		void *here;

		here = skb_push(skb,RFC1483LLC_LEN);
		memcpy(here,llc_oui,sizeof(llc_oui)); /*添加llc封装头部*/
		((u16 *) here)[3] = skb->protocol;
	}
	
    //printk(KERN_NOTICE "**TX encaped data:**\n");
    skb_debug(skb);
...
//	skb->__unused=FROM_WAN;
#endif
    //printk(KERN_NOTICE "##RX raw data:##\n");
	skb_debug(skb);
	...
	skb->mark = 0x100000UL; /*for Qos used, added by xuxiaodong*/
	ATM_SKB(skb)->vcc = atmvcc;
	/*
	 * TBS_TAG by baiyonghui 2011-3-15
	 * Desc: mac redfined 
	 */
	...
	if (!rtvcc->encap || skb->len < RFC1483LLC_LEN || memcmp(skb->data,
		...
		skb_pull(skb,RFC1483LLC_LEN); /*去掉llc封装头*/
	}
	DPRINTK("received packet's protocol: %x\n", ntohs(skb->protocol));
	//printk(KERN_NOTICE "##RX unencaped data:##\n");
	skb_debug(skb);
	rtvcc->last_use = jiffies;
	if (!(rtdev->net_dev->flags & IFF_UP)) { /* sigh, interface is down */
		//rtdev->stats.rx_dropped++;
        rtdev->net_dev->stats.rx_dropped++;
        dev_kfree_skb(skb);
		return;
	}
	//rtdev->stats.rx_packets++;
	//rtdev->stats.rx_bytes += skb->len;
	rtdev->net_dev->stats.rx_packets++;
	rtdev->net_dev->stats.rx_bytes += skb->len;
...
//	struct sk_buff_head copy;
	...
	if (copy_from_user(&be, (void *)arg, sizeof(struct atm_backend_rt2684))) {
		return -EFAULT;
	}
	write_lock_irq(&devs_lock);
    ...
    if (atmvcc->dev->ops->ioctl) {
		...
		cfg.vci = atmvcc->vci;
		switch(be.encaps) {
			case RT2684_ENCAPS_NULL:
				cfg.rfc = RFC1483_ROUTED;
				cfg.framing = VC_MUX;
				break;
			case RT2684_ENCAPS_LLC:
				cfg.rfc = RFC1483_ROUTED;
...
static const struct net_device_ops rt2684_netdev_ops = {
        .ndo_start_xmit         = rt2684_start_xmit,
...
static void rt2684_setup(struct net_device *netdev)
{
	struct rt2684_dev *rtdev = RTPRIV(netdev);

	rtdev->net_dev = netdev;

	netdev->netdev_ops = &rt2684_netdev_ops;
	netdev->get_stats = rt2684_get_stats;
	netdev->type = ARPHRD_PPP; /* We are not using atmarp, so set to PPP */
	netdev->hard_header_len = RFC1483LLC_LEN;
	netdev->mtu = RFC1626_MTU;
	netdev->addr_len = 0;
	netdev->tx_queue_len = 100; /* "normal" queue (packets) */
	    /* When using a "real" qdisc, the qdisc determines the queue */
	    /* length. tx_queue_len is only used for the default case, */
	    /* without any more elaborate queuing. 100 is a reasonable */
	    /* compromise between decent burst-tolerance and protection */
