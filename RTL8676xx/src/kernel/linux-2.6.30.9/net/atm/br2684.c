...
#include "common.h"
#ifdef	CONFIG_RTL867X_MCAST_FAST_PATH
#include "../ipv4/fastpath/mc_fastpath_core.h"
#include <linux/in.h>
#endif
 ...
 * otherwise false
 */
static int br2684_xmit_vcc(struct sk_buff *skb, struct net_device *dev, struct br2684_vcc *brvcc)
{
...
#if defined(CONFIG_RTL867X_MCAST_FAST_PATH) //&& defined(CONFIG_RTL_IPTABLES_FAST_PATH)
	/*linux-2.6.19*/
	//skb->nh.iph=(struct iphdr *)skb->data;
	skb_reset_network_header(skb);
	//if (skb->protocol == ETH_P_IP && MULTICAST(skb->nh.iph->daddr))	  // Only IP packet for multicast fast path
	if (skb->protocol == ETH_P_IP 
		&& MULTICAST(ip_hdr(skb)->daddr) 
		&& !RIP_MCAST(ip_hdr(skb)->daddr))	  // Only IP packet for multicast fast path, except RIP protocol message
	{
		if (MC_FastPath_Enter(skb)==1) {
			//printk("in mcast fast path!\n");
			return;
		}
	}
#endif
...
static int br2684_setbrpppoe(struct atm_vcc *atmvcc, void __user *arg)
{
...
unsigned short *br2684_get_fgroup(struct net_device *dev)
{
	...
	list_for_each(lhd, &br2684_devs)
    {
		...
		if (0 == strcmp(net_dev->name, dev->name))
        {
    	    ...
    	    brvcc = list_entry_brvcc(brdev->brvccs.next); /* 1 vcc/dev right now */
            fgroup = (unsigned short *)(&(brvcc->atmvcc->fgroup));
			...
			unsigned long arg)
{
	...
	case ATM_SETBRIDGEPPPOE:
		return br2684_setbrpppoe(atmvcc, argp);
#if 1 //def CONFIG_EXT_SWITCH
	case ATM_SETVLAN:
