...
extern atomic_t igmp_fastpath_enable;
extern unsigned int igmp_fastpath_count;
#define PPP_HEADER_LEN 2
#define MAC_HEAD_LEN 14
int handle_igmp_fastpath(struct sk_buff *skb)
{
	int hh_len = 0;
	...
	if (atomic_read(&igmp_fastpath_enable) == 0) 
		goto pass;
	iph = skb_header_pointer(skb, PPP_HEADER_LEN, sizeof(iphdr), &iphdr);
	if (iph == NULL)
		goto pass;
...
static void pppoatm_push(struct atm_vcc *atmvcc, struct sk_buff *skb)
{
    ...
    /*for Qos used.
    * 解决pppoa下行速率低的问题。原因是下行
    * 忘记打qos mark，导致报文走上行的qos队列
    */
    skb->mark = 0x100000UL; 
    
#ifdef CONFIG_IGMP_FASTPATH
	if (handle_igmp_fastpath(skb) == 1)
		return;
#endif
	...
	unsigned long arg)
{
	...
	// Kaohj --- attach net_device into atm_vcc
	case PPPIOCATTACH:
		if (chan->private) {
			((struct pppoatm_vcc *)chan->private)->atmvcc->net_dev = (void *)arg;
			return 0;
		}
		else
			return -EFAULT;
...
int isPPPModTypePPPoA(struct ppp_channel *pppchan)
{
    if ((pppchan->ops) &&
        (pppchan->ops->start_xmit) &&
        (pppchan->ops->start_xmit == pppoatm_send))
        return 1;
...
EXPORT_SYMBOL(isPPPModTypePPPoA);
