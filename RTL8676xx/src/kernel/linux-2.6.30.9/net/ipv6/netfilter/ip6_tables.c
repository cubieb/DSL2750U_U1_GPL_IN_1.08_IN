	      ...
	      struct xt_table *table)
{
	...
	struct xt_match_param reasmmtpar;
	struct xt_target_param tgpar;
	struct sk_buff * reasmskb = NULL;


      /*ping 1500 的包ping不通wan侧ipv6地址问题
	  * 如果是分片包，使用原包检验不过，
	  * 再使用组装好的包做ip6tables 检查
	  *目前只对localin做，forward连默认是accept
	  * 不需要做处理
	  */
#if defined(CONFIG_NF_CONNTRACK) || defined(CONFIG_NF_CONNTRACK_MODULE)
      if(hook == NF_INET_LOCAL_IN && skb->nfct_reasm)
      {
		reasmskb = skb->nfct_reasm;
      }
#endif
	...
	do {
		...
		if (ip6_packet_match(skb, indev, outdev, &e->ipv6,
			...
			(reasmskb && ip6_packet_match(reasmskb, indev, outdev, &e->ipv6,
			&reasmmtpar.thoff, &reasmmtpar.fragoff, &hotdrop)) ) {
			struct ip6t_entry_target *t;
				 
			if (IP6T_MATCH_ITERATE(e, do_match, skb, &mtpar) != 0 )
			{
			    if(!reasmskb || IP6T_MATCH_ITERATE(e, do_match, reasmskb, &reasmmtpar) != 0 )
				   goto no_match;
			}
