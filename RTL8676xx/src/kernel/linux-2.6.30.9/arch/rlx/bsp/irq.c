...
static const char dispatch_table[32] = {
	...
	BSP_PCIE_IRQ,	// 14
	BSP_SW_IRQ,	// 15
	BSP_GPIO_ABCD_IRQ,	// 16
	BSP_GPIO_EFGH_IRQ,	// 17
	BSP_PKT_SAR_IRQ,	// 18
	BSP_PCM_IRQ,	// 19
	BSP_SECURITY_IRQ,	// 20
	BSP_GDMA_IRQ,	// 21
	BSP_PKT_NIC100MII_IRQ,	// 22
	BSP_PKT_NIC100_IRQ,	// 23
	BSP_LBCTMOm2_IRQ, //DMT_IRQ,	// 24
	BSP_SAR_IRQ,	// 25
	BSP_NIC100_IRQ,	// 26
	BSP_SPI_IRQ,	// 27
	BSP_PKT_IRQ,	// 28
...
__IRAM_SYS_MIDDLE void bsp_irq_dispatch(void)
	{
...
#else
		static volatile unsigned int extint_ip;
		extint_ip |= (REG32(BSP_GIMR) & REG32(BSP_GISR));
		int tmp;
		...
		for (tmp = IRQ_LO_BOUND; tmp <= IRQ_HI_BOUND; tmp++) {
			...
			if ((extint_ip & (1<<tmp)) && (dispatch_table[tmp] >= 0)) {
				extint_ip &= (~(1<<tmp));
				do_IRQ(dispatch_table[tmp]);
			}
		}
...
{
	if(!test_and_clear_bit(0, (volatile unsigned long *)(&activeFlag)))
		{
	    return;
		}
...
{
	if(test_and_set_bit(0, (volatile unsigned long *)(&activeFlag)))
		{
		return;
		}
	#ifdef CONFIG_RTL8676NIC
