...
var G_PreferFailure    = ''; // 允许资源冲突时不开启映射
...
`
	<?mget :InternetGatewayDevice.WANDevice.1.WANConnectionDevice.<?echo $var:FstIndex?>.WAN<?echo $var:WanType?>Connection.<?echo $var:SecIndex?>.X_TWSZ-COM_PCP.<?echo $var:ThdIndex?>.Map.<?echo $var:FourthIndex?>. "Protocol InternalPort ExternalAddr6 ExternalPort Lifetime ThirdPartyAddr6 PreferFailure"
	`
		G_Protocol        = "$01";
		G_InternalPort    = "$02";
		G_ExternalAddr6   = "$03";
...
function uiOnload(){
	...
	setJSONValue({
		...
		'INPUT_PreferFailure'    : G_PreferFailure, // 允许资源冲突时不开启映射
	});
...
function uiSubmit() {
	...
	if('-' == '<?echo $var:FourthIndex?>'){
		$H({
			'add-obj' : '<?echo $var:wanEditable?>' + '.X_TWSZ-COM_PCP.<?echo $var:ThdIndex?>.Map.',
			...
			':PreferFailure'     : Form.Checkbox('INPUT_PreferFailure'),
			'obj-action' : 'add-set'
		},true);
	} else {
		$H({
			':InternetGatewayDevice.WANDevice.1.WANConnectionDevice.<?echo $var:FstIndex?>.WAN<?echo $var:WanType?>Connection.<?echo $var:SecIndex?>.X_TWSZ-COM_PCP.<?echo $var:ThdIndex?>.Map.<?echo $var:FourthIndex?>.Protocol'     : $('SELECT_Protocol').value,
			':InternetGatewayDevice.WANDevice.1.WANConnectionDevice.<?echo $var:FstIndex?>.WAN<?echo $var:WanType?>Connection.<?echo $var:SecIndex?>.X_TWSZ-COM_PCP.<?echo $var:ThdIndex?>.Map.<?echo $var:FourthIndex?>.InternalPort'     : $('INPUT_InternalPort').value,
			':InternetGatewayDevice.WANDevice.1.WANConnectionDevice.<?echo $var:FstIndex?>.WAN<?echo $var:WanType?>Connection.<?echo $var:SecIndex?>.X_TWSZ-COM_PCP.<?echo $var:ThdIndex?>.Map.<?echo $var:FourthIndex?>.ExternalAddr6'     : $('INPUT_ExternalAddr6').value,
			':InternetGatewayDevice.WANDevice.1.WANConnectionDevice.<?echo $var:FstIndex?>.WAN<?echo $var:WanType?>Connection.<?echo $var:SecIndex?>.X_TWSZ-COM_PCP.<?echo $var:ThdIndex?>.Map.<?echo $var:FourthIndex?>.ExternalPort'     : $('INPUT_ExternalPort').value,
			':InternetGatewayDevice.WANDevice.1.WANConnectionDevice.<?echo $var:FstIndex?>.WAN<?echo $var:WanType?>Connection.<?echo $var:SecIndex?>.X_TWSZ-COM_PCP.<?echo $var:ThdIndex?>.Map.<?echo $var:FourthIndex?>.Lifetime'     : $('INPUT_Lifetime').value,
			':InternetGatewayDevice.WANDevice.1.WANConnectionDevice.<?echo $var:FstIndex?>.WAN<?echo $var:WanType?>Connection.<?echo $var:SecIndex?>.X_TWSZ-COM_PCP.<?echo $var:ThdIndex?>.Map.<?echo $var:FourthIndex?>.ThirdPartyAddr6'     : $('INPUT_ThirdPartyAddr6').value,
			':InternetGatewayDevice.WANDevice.1.WANConnectionDevice.<?echo $var:FstIndex?>.WAN<?echo $var:WanType?>Connection.<?echo $var:SecIndex?>.X_TWSZ-COM_PCP.<?echo $var:ThdIndex?>.Map.<?echo $var:FourthIndex?>.PreferFailure'     : Form.Checkbox('INPUT_PreferFailure'),
			'obj-action':'set'
		},true);		
	}
	$H({
...
function uiPageRefresh(){
	document.location.href = '/cgi-bin/webproc?getpage=html/index.html&var:menu=advanced&var:page=pcp_list&var:subpage=pcp_map_config&var:WanType=<?echo $var:WanType?>&var:FstIndex=<?echo $var:FstIndex?>&var:SecIndex=<?echo $var:SecIndex?>&var:ThdIndex=<?echo $var:ThdIndex?>&var:FourthIndex=<?echo $var:FourthIndex?>';
}
...
function dealWithError(){
	...
	arrayHint['PreferFailure']   = 'INPUT_PreferFailure';
