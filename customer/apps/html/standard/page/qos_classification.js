...
var m = 0;
<?objget :InternetGatewayDevice.QueueManagement.Classification. "ClassificationEnable SourceVendorClassID ClassInterface SourceVendorClassIDExclude SourceMACAddress SourceMACMask DestMACAddress DestMACMask X_TWSZ-COM_IpProtocolType EthernetPriorityCheck EthernetPriorityMark VLANIDCheck SourceIP SourceVendorClassIDExclude"
`	G_Qos_Classifition[m] = [];
	G_Qos_Classifition[m][0] = "<?echo $11?>"; //ClassificationEnable
	G_Qos_Classifition[m][1] = ("<?echo $13?>".indexOf('LAN') >= 0) ? 'UP' : 'DOWN';  // ClassInterface contains 'LAN' means Upstream, otherwise means Downstream, BLANK means Downstream also
	if ("<?echo $13?>".indexOf('Local') >= 0)
		G_Qos_Classifition[m][1] = 'UP';  // ClassInterface contains 'Local' means Upstream
	G_Qos_Classifition[m][2] = "<?echo $13?>"; //ClassInterface
	G_Qos_Classifition[m][3] = "<?echo $14?>"; //SourceVendorClassIDExclude
	...
	G_Qos_Classifition[m][13] = "<?echo $1e?>"; //SourceVendorClassIDExclude
	G_Qos_Classifition[m][14] = "<?get :InternetGatewayDevice.QueueManagement.Classification.<?echo $20?>.SourceMask?>"; //SourceMask
	G_Qos_Classifition[m][15] = "<?get :InternetGatewayDevice.QueueManagement.Classification.<?echo $20?>.DestIP?>"; //DestIP
	G_Qos_Classifition[m][16] = "<?get :InternetGatewayDevice.QueueManagement.Classification.<?echo $20?>.SourceVendorClassIDMode?>"; //SourceVendorClassIDMode
	G_Qos_Classifition[m][17] = "<?get :InternetGatewayDevice.QueueManagement.Classification.<?echo $20?>.DestMask?>"; //DestMask
	G_Qos_Classifition[m][18] = "<?get :InternetGatewayDevice.QueueManagement.Classification.<?echo $20?>.IPLengthMin?>"; //IPLengthMin
	G_Qos_Classifition[m][19] = "<?get :InternetGatewayDevice.QueueManagement.Classification.<?echo $20?>.IPLengthMax?>"; //IPLengthMax
	G_Qos_Classifition[m][20] = "<?get :InternetGatewayDevice.QueueManagement.Classification.<?echo $20?>.DSCPCheck?>"; //DSCPCheck
	G_Qos_Classifition[m][21] = "<?get :InternetGatewayDevice.QueueManagement.Classification.<?echo $20?>.DestVendorClassID?>";//DestVendorClassID
	G_Qos_Classifition[m][22] = "<?get :InternetGatewayDevice.QueueManagement.Classification.<?echo $20?>.DSCPMark?>"; //DSCPMark
	G_Qos_Classifition[m][23] = "<?get :InternetGatewayDevice.QueueManagement.Classification.<?echo $20?>.Protocol?>";//Protocol
	G_Qos_Classifition[m][24] = "<?get :InternetGatewayDevice.QueueManagement.Classification.<?echo $20?>.SourcePort?>";//SourcePort
	G_Qos_Classifition[m][25] = "<?get :InternetGatewayDevice.QueueManagement.Classification.<?echo $20?>.SourcePortRangeMax?>";//SourcePortRangeMax
	G_Qos_Classifition[m][26] = "<?get :InternetGatewayDevice.QueueManagement.Classification.<?echo $20?>.DestPort?>";//DestPort
	G_Qos_Classifition[m][27] = "<?get :InternetGatewayDevice.QueueManagement.Classification.<?echo $20?>.DestPortRangeMax?>";//DestPortRangeMax
	G_Qos_Classifition[m][28] = "<?get :InternetGatewayDevice.QueueManagement.Classification.<?echo $20?>.ClassQueue?>";//ClassQueue
	G_Qos_Classifition[m][29] = "InternetGatewayDevice.QueueManagement.Classification.<?echo $10?>.";//Path
	m++;
`?>
...
function uiOnload(){
	...
	$('lang_classifyMode').style.display = 'none';
	$('SELECT_ClassifyModel').style.display = 'none';   // Classification forever
...
function BusinessTable(){
	...
	for(var i = 0, _len = G_Qos_Business.length; i < _len; i++){
		value_array[i] = [i+1,//序号
						  ...
						  G_Qos_Business[i][1] == '-1' ? '' : 'UP_Q_' + G_Qos_Business[i][1],//分类队列
...
function fluxTable(){
	...
	for(var i = 0, _len = G_Qos_Classifition.length; i < _len; i++){
		value_array[i] = [i+1,//序号
						  ...
						  displayTag(i),//标记
						  G_Qos_Classifition[i][28] == '-1' ? '' : G_Qos_Classifition[i][1] + '_Q_' + (G_Qos_Classifition[i][28]),//分类队列
...
function ClassifyContent(Id){
	var _str = '';
		_str = data_language.dynamic.lang_srcDstMacAddress + G_Qos_Classifition[Id][4] + '<br>' +
		   data_language.dynamic.lang_ethernetType + G_Qos_Classifition[Id][8] + '<br>' +
		   data_language.dynamic.lang_interface + G_Qos_Classifition[Id][2] + '<br>' +
...
function getDSCPMark(typeValue){
	...
	//如果是不存在的DSCP值，也显示
	if(typeValue != '' && value_options.indexOf(typeValue*4) < 0){
		dscpMark = typeValue;
	}
...
function uiEdit(){
	...
	$H({
		...
		'var:menu'     :'advanced',
		'var:page'     :'qos_config',
		'var:subpage'  :Index[2] == 'App' ? 'qos_queue_edit' : 'qos_classification_edit'  // 2009-03-05 modify
	},true);
	$('uiPostForm').submit();
}
...
function uiAddConfig(){
	$H({
		...
		'var:menu'   :'advanced',
		'var:page'   :'qos_config',
		'var:subpage':$('SELECT_ClassifyModel').value == 0 ? 'qos_queue_edit' : 'qos_classification_edit'
	},true);
	$('uiPostForm').submit();
}
...
function uiDelete(){
	...
	$H({
		...
		'var:config' : $('SELECT_ClassifyModel').value,		
		'var:errorpage' :'qos_classification' 
	},true);
	$('uiPostForm').submit();
}
