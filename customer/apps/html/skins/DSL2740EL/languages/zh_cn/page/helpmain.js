var data_language = {
	...
	'lang_upgrade_help' : '<dl><dd>在此页面，您可以升级最新版本固件，以完善设备功能，提高性能。<p>请参照以下步骤进行固件升级：</p><ol><li>点击 <span class=button_ref>浏览</span> 按钮，在您的计算机上选择升级文件。<li>选择正确的升级文件后，点击下面的“升级固件”按钮，开始升级固件。此操作过程需要一分钟或更长时间。<li>路由器重启。此操作过程需要一分钟或更长时间。</ol><dd>注意：一些固件升级操作可能导致配置恢复到出厂默认值。在进行升级操作时，请确保您已经在<a href="/cgi-bin/webproc?getpage=html/index.html&var:menu=maintenance&var:page=system">管理 -> 系统管理</a>页面，选择“备份设置”。</dd></dl>',
	'lang_access_subtitle' : '访问控制',
	'lang_access_help'   : '<dl><dt>用户密码<dd><p>帐户密码用于设置登陆Web管理页面的密码。强烈建议您修改密码，以确保您的路由器的安全。</p><p>您可以通过“admin”帐号登陆路由器，进行管理。</p><dl><dt>admin密码<dd>输入admin的密码。使用“admin”登陆路由器，可以对Web界面进行全面管理。</dd></dl></dd></dl><dl><dt>服务<dd>在此页面，您可以通过FTP、HTTP、ICMP、TFTP和DNS访问路由器。<dl><dt>本地接口<dd>从局域网访问路由器。<dt>远程接口<dd>从ISP访问路由器。</dd></dl></dd></dl><dl><dt>IP过滤<dd>选择开启访问控制模式，只有在访问控制表中的IP地址才可以访问路由器。此功能与<b>服务功能</b>同时使用。</dd></dl>',
	'lang_dgn_subtitle' : '诊断',
	'lang_dng_help' : '<dl><dt>DSL测试<dd><p>在此页面，您可以运行DSL诊断测试，以检查您的路由器连接状态。</p><dt>路由跟踪<dd>路由跟踪功能通过发送数据包确定信息在Internet上所经过的所有路由器。',
	'lang_log_subtitle' : '系统日志',
	'lang_log_help' : '<p>在此页面，您可以选择开启日志功能，系统会开始记录关键事件。</p><p>如果在“模式”栏选择“远程”或“两者”，记录的事件将会发送给远程系统日志服务器特定的IP地址和UDP端口。如果在“模式”栏选择“本地”或“两者”，事件将会记录在本地记忆里。根据实际情况，选择相应的模式，点击“应用”按钮，以设置系统日志。</p>点击查看系统日志按钮，您可以查看设置的系统日志。</dd></dl>'
	}
}