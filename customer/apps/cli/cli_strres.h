...
#define NODE_FIRMWARE_VERSION "InternetGatewayDevice.DeviceInfo.ModemFirmwareVersion"

/* SOC，adsl，flash及memory的品牌及型号 */
#define DEVICE_SOC_BRAND           "REALTEK"
#define DEVICE_SOC_MODEL          "RTL8672-VE-GR"
#define DEVICE_ADSL_BRAND         "REALTEK"
#define DEVICE_ADSL_MODEL         "RTL8271B-VR-GR"
#define DEVICE_RAM_BRAND          "Hynix"
#define DEVICE_RAM_MODEL          "HY5DU561622ETP-5"
#define DEVICE_FLASH_BRAND      "MXIC"
#define DEVICE_FLASH_MODEL      "MX25L6445EM2I-10G"
...
#define CMDHELP_GLB_NATP                HELP_FUN "Show or config the natp(Port Forwarding) module" \
                                        ...
                                        HELP_EXP "natp add http-server PVC:0/32 tcp 80 80 8080 192.168.1.5 schedule Always" 
...
#ifdef CONFIG_TELEFONICA_CHILE
#define CMDHELP_GLB_WAN                 HELP_FUN "Show or config the wan connection" \
                                        HELP_FMT "wan add <protocol> <vpi> <vci> <encap> [nat (enable|disable)] [username <string>] [password <string>] [dialup <AlwaysOn|OnDemand|Manual>] [pppidletimeout <seconds>] [pppmru <num>] [service <string>] [ipaddr <ip>] [netmask <mask>] [remoteip <ip>]" \
                                        HELP_FMT "wan modify <pvc> <protocol> <vpi> <vci> <encap> [nat (enable|disable)] [passthrough (enable|disable)][username <string>] [password <string>] [dialup <AlwaysOn|OnDemand|Manual>] [pppidletimeout <seconds>] [pppmru <num>] [service <string>] [ipaddr <ip>] [netmask <mask>] [remoteip <ip>]" \
										...
										HELP_OPT "passthrough : Choose enable or disable passthrough" \
                                        ...
                                        HELP_EXP "wan add bridge 0 32 LLC" \
                                        HELP_EXP "wan add pppoe 0 32 LLC nat enable username test password test dialup AlwaysOn" \
                                        HELP_EXP "wan add mer 0 32 LLC nat enable ipaddr 10.10.10.10 netmask 255.255.255.0 remoteip 10.10.10.1" \
                                        HELP_EXP "wan modify PVC:0/32 mer 0 32 LLC nat enable passthrough enable username user password user dialup AlwaysOn ipaddr 10.10.10.10 netmask 255.255.255.0 remoteip 10.10.10.1" \
                                        HELP_EXP "wan del PVC:0/32" \
                                        HELP_EXP "wan show"
#else
#define CMDHELP_GLB_WAN                 HELP_FUN "Show or config the wan connection" \
                                        HELP_FMT "wan add <protocol> <vpi> <vci> <encap> [nat (enable|disable)] [username <string>] [password <string>] [dialup <AlwaysOn|OnDemand|Manual>] [pppidletimeout <seconds>] [pppmru <num>] [service <string>] [ipaddr <ip>] [netmask <mask>] [remoteip <ip>]" \
                                        HELP_FMT "wan modify <pvc> <protocol> <vpi> <vci> <encap> [nat (enable|disable)][username <string>] [password <string>] [dialup <AlwaysOn|OnDemand|Manual>] [pppidletimeout <seconds>] [pppmru <num>] [service <string>] [ipaddr <ip>] [netmask <mask>] [remoteip <ip>]" \
										HELP_FMT "wan del <pvc name>" \
                                        HELP_FMT "wan show [conn <pvc name>]" \
                                        HELP_OPT "protocol : The connection type. It can be one of bridge, pppoe, pppoa, mer, ipoa" \
                                        HELP_SPC "vpi      : The range is 0~255" \
                                        ...
                                        HELP_EXP "wan add pppoe 0 32 LLC nat enable username test password test dialup AlwaysOn" \
                                        HELP_EXP "wan add mer 0 32 LLC nat enable ipaddr 10.10.10.10 netmask 255.255.255.0 remoteip 10.10.10.1" \
										HELP_EXP "wan modify PVC:0/32 mer 0 32 LLC nat enable ipaddr 10.10.10.10 netmask 255.255.255.0 remoteip 10.10.10.1" \
                                        HELP_EXP "wan del PVC:0/32" \
                                        HELP_EXP "wan show"
#endif

#ifdef CONFIG_RTL8673
#define CMDHELP_GLB_DHCPSERVER	HELP_FUN "Show or config or enable or disable the DHCPServer" \
                			    ...
                			    HELP_FMT "dhcpserver config <start-IP-address(p)> <end-IP-address(p)> <lease-time(u0~4294967295)> <dnsserver(p)><secdnsserver(p)>" \
                			    ...
                			    HELP_FMT "dhcpserver show" \
	                                ...
	                                HELP_SPC "secdnsserver: Second dns server IP" \
	                                HELP_EXP "dhcpserver config 192.168.0.2 192.168.0.100 86400 192.168.0.1 192.168.2.1" \
	                                HELP_EXP "dhcpserver port lan enable" \
...
#define CMDHELP_GLB_LAN_IPINTERFACE	HELP_FUN "Show or config or enabl or|disable the lan IPInterface" \
					   ...
					   HELP_FMT "lan showmac : Show the MAC table of the devices connected to the modem's LAN" \
					   HELP_FMT "lan show <interface(u1~2)/all>" \
					   HELP_FMT "lan showv6" \
					   HELP_FMT "lan configv6 <ip(s)>" \
...
#define CMDHELP_GLB_WLAN	HELP_FUN "Show or config or enable or disable the wlan and the wlan encrpytion" \
	                              ...
	                              HELP_FUN "Show ssid status(visible/invisible) "\
	                              HELP_FMT "wlan show <interface(u1~4)> ssidstatus" \
	                              HELP_FMT "wlan visible <interface(u1~4)>" \
	                              HELP_FMT "wlan invisible <interface(u1~4)>" \
	                              HELP_FMT "wlan enable [interface<value(u2~4)>]" \
	                              HELP_FMT "wlan disable [interface<value(u2~4)>]" \
	                              HELP_FMT "wlan config <interface(u1~4)> basic [ssid <ssid-v(s256)>] [channel <channel-v(u0~13)>] [wps <wps-v((u0~1)])>" \
	                              HELP_FMT "wlan wps enable [u0~1]"\
	                              ...
	                              HELP_FMT "wlan config <interface(u1~4)> encryption wpa <wpa-version(1|2|auto)> <wpa-encryption(TKIP|AES|TKIPAES)> psk <interval(u0~1000)> <pre-shared-key(s63)>" \
	                              HELP_FMT "wlan config <interface(u1~4)> encryption wpa <wpa-version(1|2|auto)> <wpa-encryption(TKIP|AES|TKIPAES)> ent <interval(u0~1000)> <server-ip(p)> <port(u1025~65535)> <secret(s64)>" \
...
#define CMDHELP_GLB_ADMIN            HELP_FUN "config the username and the password" \
                                     ...
                                     HELP_FMT "password telnet <oldpassword> <newpassword>"
#else
#define CMDHELP_GLB_ADMIN            HELP_FUN "config the username and the password" \
                                     HELP_FMT "password change <userlevel(admin|user|support)> <oldpassword> <newusername> <newpassword>"
#endif
...
#define CMDHELP_GLB_TR069           HELP_FUN "config or show or set connection request the Tr069 list" \
                                    HELP_FMT "tr069 config <Inform-Interval> <URL> <Username> <Password>"\
                                    HELP_FMT "tr069 show"\
                                    HELP_FMT "tr069 request <Enable-value(enable|disable)> [username <RequestUsername>] [password <RequestPassword>"\
                                    HELP_EXP "tr069 config 360 http://asc.o2online.de admin 123"\
                                    HELP_EXP "tr069 request enable username admin password 1234"\
                                    HELP_EXP "tr069 request disable"

//3G help
#define CMDHELP_GLB_3G         HELP_FUN "display the brand,design,firmware dongle connect status, information of 3G..." \
                                    HELP_OPT "show dongstatus:    display the status of 3G dongle"\
                                    HELP_OPT "show connstatus:    display the status of 3G"\
                                    HELP_OPT "show attenuation:   display the attenuation of the 3G link"\
                                    HELP_OPT "show connmode:    display the connection mode and the 3G sub-connection"\
                                    HELP_OPT "show detail:  display the information of 3G.such as: APN,Auth,use,pwd,IMSI..."

#define CMDHELP_GLB_FIRMWARE_UPDATE         HELP_FUN "download image via network using TFTP,FTP or HTTP server and save it to flash" \
											HELP_FMT  "firmware update ftp  server <serverip(p)> file <filename(s)> [username <name(s)>] [password <password-v(s)>] [port <port-v(u1~65535)>] config (clear|keep)"\
											HELP_FMT  "firmware update http server <serverip(p)> file <filename(s)> [username <name(s)>] [password <password-v(s)>] [port <port-v(u1~65535)>] config (clear|keep)"\
											HELP_FMT  "firmware update tftp server <serverip(p)> file <filename(s)> [port <port-v(u1~65535)>] config (clear|keep)"\
											HELP_EXP  "firmware update ftp  server 192.168.1.12 file cc/image.img username 123 password 123 port 21 config clear"\
											HELP_EXP  "firmware update http  server 192.168.1.12 file image.bin port 80 config keep"\
											HELP_EXP  "firmware update tftp  server 192.168.1.12 file abc.image config clear"
